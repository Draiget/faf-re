...
namespace Wm3
{
   ...
   */
  void TRational64::EliminatePowersOfTwo()
  {
    ...
    if (numerFirstNonZero > 127 || numerFirstNonZero == -1)
    {
      TInteger64 one{};
      std::memset(&one, 0, sizeof(one));
      int oneValue = 1;
      System::Memcpy(&one, 4u, &oneValue, 4u);
      System::Memcpy(&m_kDenom, sizeof(TInteger64), &one, sizeof(TInteger64));
      return;
    }
