...
namespace Wm3
{
  ...
  template <class T> struct Quaternion
  {
    ...
    constexpr Quaternion(T _w, T _x, T _y, T _z) :
        w(_w),
        x(_x),
        y(_y),
        z(_z)
    ...
    static Quaternion Inverse(const Quaternion& q) noexcept
    {
      ...
      if (lsq == T(0))
        return Quaternion(); // fallback to identity
      const T inv = T(1) / lsq;
    ...
    void ToAxisAngle(Vector3<T>& axisOut, T& angleOut, T eps = T(1e-6)) const noexcept
    {
    ...
    static Quaternion Slerp(const Quaternion& a, const Quaternion& b, T t, T eps = T(1e-5)) noexcept
    {
      ...
      Quaternion out(q1.w * w1 + q2.w * w2, q1.x * w1 + q2.x * w2, q1.y * w1 + q2.y * w2, q1.z * w1 + q2.z * w2);
    ...
    static Quaternion Nlerp(const Quaternion& a, const Quaternion& b, T t) noexcept
    {
      ...
      Quaternion out(
        q1.w + (q2.w - q1.w) * t, q1.x + (q2.x - q1.x) * t, q1.y + (q2.y - q1.y) * t, q1.z + (q2.z - q1.z) * t
      );
      return out.Normalized();
    }
    ...
    {
      return std::fabs(w - rhs.w) <= eps && std::fabs(x - rhs.x) <= eps && std::fabs(y - rhs.y) <= eps &&
             std::fabs(z - rhs.z) <= eps;
    }

    /** Convert from different underlying scalar type. */
    template <class U> static Quaternion From(const Quaternion<U>& q)
    {
  ...
  template <class T> inline Vector3<T> operator*(const Quaternion<T>& q, const Vector3<T>& v) noexcept
  {
    return q.Rotate(v);
  }
