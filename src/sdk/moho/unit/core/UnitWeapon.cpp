...
namespace
{
  ...
  ) noexcept
  {
    ...
    storage = msvc8::vector<moho::SBlackListInfo>{};
    if (elementCount == 0u) {
      return true;
    }

    if (elementCount > msvc8::vector<moho::SBlackListInfo>::max_size()) {
      return false;
    }
   ...
   * occupant from its owner chain through UnlinkBlacklistWeakEntityRange,
   * because SBlackListInfo's destructor is trivial and the container will
