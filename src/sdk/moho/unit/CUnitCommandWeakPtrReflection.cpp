...
namespace
{
  ...
  void LoadWeakPtrCUnitCommandVector(gpg::ReadArchive* archive, int objectPtr, int, gpg::RRef* ownerRef)
  {
    ...
    // `clear()`: each weak node's destructor unlinks it from its owner chain
    // (`_Destroy_range` 0x005A2270, cited on Vector.h).
    storage->clear();

    // The binary grows the destination with reserve(count) (FUN_006EB1D0,
    // msvc8::vector<WeakPtr<CUnitCommand>>::reserve) and then fills the
    // reserved run in place with empty weak links: with the capacity already
    // there, `insert(end(), count, T())` takes the in-place `_Ufill` arm
    // (0x006EC5B0) rather than reallocating.
    const std::size_t targetCount = static_cast<std::size_t>(count);
    if (targetCount > 0u) {
      storage->reserve(targetCount);
      storage->insert(storage->end(), targetCount, moho::WeakPtr<moho::CUnitCommand>{});
    }
...
namespace gpg
{
  ...
  void RVectorType<moho::WeakPtr<moho::CUnitCommand>>::SetCount(void* obj, int count) const
  {
    ...
    // `resize(count)`: shrinking erases the tail (each weak node's destructor
    // unlinks it, `_Destroy_range` 0x005A2270); growing fill-constructs
    // value-initialised links (0x006EA710). Both are cited on Vector.h.
