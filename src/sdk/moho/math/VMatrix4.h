...
namespace moho
{
  ...
  struct VMatrix4
  {
    ...
    [[nodiscard]]
    Vector4f GetTranslationRow() const
    {
      return r[3];
    }
    ...
    {
#if MOHO_USE_SSE2
      const __m128 a = _mm_loadu_ps(&vRow.x);
      const __m128 m0 = _mm_loadu_ps(&M.r[0].x);
      const __m128 m1 = _mm_loadu_ps(&M.r[1].x);
      const __m128 m2 = _mm_loadu_ps(&M.r[2].x);
      const __m128 m3 = _mm_loadu_ps(&M.r[3].x);
    ...
    static VMatrix4 Multiply(const VMatrix4& A, const VMatrix4& B)
    {
      VMatrix4 r{};
#if MOHO_USE_SSE2
      const __m128 b0 = _mm_loadu_ps(&B.r[0].x);
      const __m128 b1 = _mm_loadu_ps(&B.r[1].x);
      const __m128 b2 = _mm_loadu_ps(&B.r[2].x);
      const __m128 b3 = _mm_loadu_ps(&B.r[3].x);

      for (int i = 0; i < 4; ++i) {
        const __m128 ai = _mm_loadu_ps(&A.r[i].x);
    ...
    static VMatrix4 Transpose(const VMatrix4& M)
    {
      VMatrix4 T{};
#if MOHO_USE_SSE2
      __m128 row0 = _mm_loadu_ps(&M.r[0].x);
      __m128 row1 = _mm_loadu_ps(&M.r[1].x);
      __m128 row2 = _mm_loadu_ps(&M.r[2].x);
      __m128 row3 = _mm_loadu_ps(&M.r[3].x);
      _MM_TRANSPOSE4_PS(row0, row1, row2, row3);
      _mm_storeu_ps(&T.r[0].x, row0);
      _mm_storeu_ps(&T.r[1].x, row1);
      _mm_storeu_ps(&T.r[2].x, row2);
      _mm_storeu_ps(&T.r[3].x, row3);
#else
