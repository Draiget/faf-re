...
namespace moho
{
  ...
  public:
     ...
     * Calls helper Address: 0x006FDB00 (FUN_006FDB00), which then calls Address: 0x006FD570 (FUN_006FD570).
     */
    virtual ~SimArmy();

    // The army interface. SimArmy introduces this vtable; IArmy is a
    // non-polymorphic data base at +0x08 (see IArmy.h). Slot order is the
    // binary's and must not change.
  ...
  // vfptr (+0x00), four bytes of alignment padding, then the 8-byte-aligned
  // IArmy payload (+0x08..+0x1E8). Together with alignof(IArmy) == 8 this pins
  // the base at +0x08, which is what every IArmy accessor call site assumes.
  static_assert(sizeof(SimArmy) == 0x1E8, "SimArmy size must be 0x1E8");
} // namespace moho
