...
/**
 * Address: 0x00661330 (FUN_00661330)
 * Mangled: ??0WCurveEditor@Moho@@QAE@@Z
 *
 * What it does:
 * Creates the control over `parent` (wxControl through its inline copy at
 * 0x00660B10, named wxControlNameStr), sets the view to
 * (0, viewValueMin) - (viewTimeMax, viewValueMax), rescales the empty curve to
 * [0, viewTimeMax] and gives it one key halfway along, selected, then shows
 * itself.
 */
...
/**
 * Address: 0x00662680 (FUN_00662680)
 * Mangled: ??0WCurveEditorPanel@Moho@@QAE@@Z
 *
 * What it does:
 * Creates the panel (wxPanel through its inline copy at 0x004FB890), then the
 * editor, and lays them out: a row of five label + number fields (Tick,
 * Value, Range, Window Min:, Max: - ids 622-626, each seeded with its value
 * formatted "%f") above the editor, which takes the rest.
 */
  ...
  , mFieldsLive(0)
{
  ...
  const auto addField = [this, fieldSizer](const std::int32_t id, const wxChar* const label, const float value) {
    // Label style 0x110: the binary passes wxALIGN_CENTRE_HORIZONTAL | wxLEFT.
    fieldSizer->Add(
      new wxStaticText(this, -1, label, wxDefaultPosition, wxDefaultSize, wxALIGN_CENTRE_HORIZONTAL | wxLEFT),
      0,
      wxALL,
      3
    );
  ...
  mKeyTimeText = addField(622, wxT("Tick"), selectedKeyValue(0));
  mKeyValueText = addField(623, wxT("Value"), selectedKeyValue(1));
  mKeyTangentText = addField(624, wxT("Range"), selectedKeyValue(2));
  mViewValueMinText = addField(625, wxT("Window Min:"), mCurveEditor->mViewValueMin);
  mViewValueMaxText = addField(626, wxT("Max:"), mCurveEditor->mViewValueMax);
...
BEGIN_EVENT_TABLE(moho::WCurveEditorPanel, wxPanel)
  EVT_TEXT_ENTER(622, moho::WCurveEditorPanel::OnCurveFieldCommitted)
  EVT_TEXT_ENTER(623, moho::WCurveEditorPanel::OnCurveFieldCommitted)
  EVT_TEXT_ENTER(624, moho::WCurveEditorPanel::OnCurveFieldCommitted)
  EVT_TEXT_ENTER(625, moho::WCurveEditorPanel::OnCurveFieldCommitted)
  EVT_TEXT_ENTER(626, moho::WCurveEditorPanel::OnCurveFieldCommitted)
END_EVENT_TABLE()
