...
namespace moho
{
   ...
   * water surface.
   */
  void MediumFidelityTerrain::DrawWaterAlbedoDecals(const std::int32_t gameTick, const float deltaSeconds)
  {
    if (!ren_Decals) {
      return;
    }
    ...
    for (const TerrainDecalDrawCommand& command : mDecalDrawCommands) {
      ...
      BindDecalTexture(shaderVars.decalAlbedoTexture, decal, 0, deltaSeconds, gameTick);
   ...
   */
  void MediumFidelityTerrain::DrawWaterTerrain(
   ...
   *
   * What it does:
   * Per-frame render-context update; see the header declaration for the full
   * behavioral summary.
   */
  void MediumFidelityTerrain::UpdateRenderContext(
    const std::int32_t gameTick,
    const float deltaSeconds,
    ...
    const std::int32_t /*forceRegenerate*/)
  {
    ...
    if (!dirty) {
      dirty = mCamera->tranform.Compare(mOverlayTransform);
    }

    if (!minimapPass || ren_ForceUpdateMinimapTerrain || dirty) {
      mOverlayTransform = mCamera->tranform;
      ...
      if (ren_GenerateMesh && (dirty || decalsDirty)) {
        ...
        if (!minimapPass && ren_Decals) {
          ...
          for (UserEntity* const entity : visibleDecals) {
            ...
            // Unlike the low-fidelity class, fidelity 0 always passes: the
            // LOD-area threshold only gates decals with fidelity != 0.
            ...
            if (isFlat) {
              ...
              if (command.indexCount > 0 && (command.startIndex + command.indexCount) < kSkirtMaxIndexCount) {
                mDecalDrawCommands.PushBack(command);
      ...
      if (!minimapPass && ren_Splats) {
        ...
        for (UserEntity* const entity : visibleSplats) {
          ...
          splat->UpdateBatchTexture(sMediumFidelityTextureBatcher);
