...
moho::ScrDebugWindow::ScrDebugWindow()
{
  ...
  // 0x004BDA9E allocates the table, hands it to the frame and drops its own
  // reference immediately - the frame holds the only one that matters. Both
  // calls now go through the wx bridge, the only TU here that can name
  // `wxAcceleratorTable`.
  void* const accelTable = moho::scrdebug::ConstructWxAcceleratorTable(8, accelerators);
  SetAcceleratorTable(accelTable);
  moho::scrdebug::DestroyWxAcceleratorTable(accelTable);
