...
namespace
{
  ...
  void LoadFastVectorCountedPtrCParticleTexture(gpg::ReadArchive* archive, int objectPtr, int, gpg::RRef* ownerRef)
  {
    ...
    // `fastvector<CountedPtr<CParticleTexture>>::resize` (0x00657900, cited on
    // FastVector.h): released on shrink, retained on grow, through CountedPtr's
    // own destructor and copy constructor.
    const moho::CountedPtr_CParticleTexture fill{};
...
namespace gpg
{
  ...
  void RFastVectorType<moho::CountedPtr_CParticleTexture>::SetCount(void* obj, const int count) const
  {
    ...
    auto& vec = *static_cast<gpg::fastvector<moho::CountedPtr_CParticleTexture>*>(obj);
    // `fastvector<CountedPtr<CParticleTexture>>::resize` (0x00657900, cited on FastVector.h).
    const moho::CountedPtr_CParticleTexture fill{};
    vec.resize(static_cast<std::size_t>(count), fill);
  }
} // namespace gpg
