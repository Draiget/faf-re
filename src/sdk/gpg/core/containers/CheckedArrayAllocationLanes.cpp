...
namespace gpg::core::legacy
{
  ...
  /**
   * Address: 0x007D97A0 (FUN_007D97A0, sub_7D97A0)
   *
   * What it does:
   * Allocates a contiguous lane of 28-byte elements and applies the legacy
   * 32-bit overflow guard before forwarding to global `operator new`.
   */
  void* AllocateChecked28ByteLane(const std::uint32_t elementCount)
  {
    return AllocateCheckedElements(elementCount, 28u);
  }
   ...
   * 32-bit overflow guard before forwarding to global `operator new`.
   */
  void* AllocateChecked64ByteLane(const std::uint32_t elementCount)
  {
    return AllocateCheckedElements(elementCount, 64u);
  }
   ...
   * 32-bit overflow guard before forwarding to global `operator new`.
   */
  void* AllocateChecked96ByteLane(const std::uint32_t elementCount)
  {
    return AllocateCheckedElements(elementCount, 96u);
  }
   ...
   * 32-bit overflow guard before forwarding to global `operator new`.
   */
  void* AllocateChecked144ByteLane(const std::uint32_t elementCount)
  {
    return AllocateCheckedElements(elementCount, 144u);
  }
   ...
   * 32-bit overflow guard before forwarding to global `operator new`.
   */
  void* AllocateChecked44ByteLane(const std::uint32_t elementCount)
  {
    return AllocateCheckedElements(elementCount, 44u);
  }
   ...
   *
   * What it does:
   * Allocates a contiguous lane of 784-byte elements and applies the legacy
   * 32-bit overflow guard before forwarding to global `operator new`.
   */
  void* AllocateChecked784ByteLane(const std::uint32_t elementCount)
  {
    return AllocateCheckedElements(elementCount, 784u);
  }
   ...
   *
   * What it does:
   * Allocates a contiguous lane of 3280-byte elements and applies the legacy
   * 32-bit overflow guard before forwarding to global `operator new`.
   */
  void* AllocateChecked3280ByteLane(const std::uint32_t elementCount)
  {
    return AllocateCheckedElements(elementCount, 3280u);
  }
   ...
   *
   * What it does:
   * Allocates a contiguous lane of 56-byte elements and applies the legacy
   * 32-bit overflow guard before forwarding to global `operator new`.
   */
  void* AllocateChecked56ByteLane(const std::uint32_t elementCount)
  {
    return AllocateCheckedElements(elementCount, 56u);
  }
