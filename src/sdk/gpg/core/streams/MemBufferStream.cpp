...
namespace
{
    ...
    constexpr const char* kWriteWindowAssertSource = "c:\\work\\rts\\main\\code\\src\\libs\\gpgcore\\streams\\MemBufferStream.cpp";
 ...
 */
MemBuffer<char> gpg::AllocMemBuffer(const std::size_t size)
{
 ...
 */
MemBuffer<const char> gpg::CopyMemBuffer(const void* const source, const std::size_t size)
{
 ...
 */
MemBuffer<const char> gpg::LoadFileToMemBuffer(const char* const path)
{
 ...
 */
MemBufferStream::MemBufferStream(const unsigned int size)
    : Stream(),
 ...
 */
MemBufferStream::MemBufferStream(const MemBuffer<char>& input, const unsigned int initialLength)
    : Stream(),
      mInput(input),
      mOutput(input)
{
 ...
 */
MemBufferStream::MemBufferStream(const MemBuffer<const char>& output, unsigned int initialLength)
    : Stream(),
      mInput(),
      mOutput(output)
{
 ...
 */
size_t MemBufferStream::VirtRead(char* const buf, const size_t len)
{
    SyncReadEndWithWriteHead();

    const std::size_t available = (mReadEnd != nullptr && mReadHead != nullptr)
        ? static_cast<std::size_t>(mReadEnd - mReadHead)
        : 0u;
 ...
 */
void MemBufferStream::VirtUnGetByte(const int value)
{
    (void)value;
 ...
 */
void MemBufferStream::Resize(const std::uint64_t size)
{
    ...
    const std::size_t writeOffset = (mWriteHead != nullptr && mWriteStart != nullptr)
        ? static_cast<std::size_t>(mWriteHead - mWriteStart)
        : 0u;
    const std::size_t readOffset = (mReadHead != nullptr && mReadStart != nullptr)
        ? static_cast<std::size_t>(mReadHead - mReadStart)
        : 0u;
    const std::size_t usedBytes = (mWriteHead != nullptr && mWriteHead > mReadEnd)
        ? writeOffset
        : ((mReadEnd != nullptr && mReadStart != nullptr)
            ? static_cast<std::size_t>(mReadEnd - mReadStart)
            : 0u);
...
std::uint64_t MemBufferStream::VirtSeek(const Mode mode, const SeekOrigin origin, const std::int64_t pos)
{
    ...
    const std::int64_t currentReadOffset = (mReadHead != nullptr && mReadStart != nullptr)
        ...
        : 0;
    const std::int64_t currentLength = (mReadEnd != nullptr && mReadStart != nullptr)
        ? static_cast<std::int64_t>(mReadEnd - mReadStart)
        : 0;
    ...
    if ((mode & ModeSend) != 0) {
        ...
        const std::int64_t currentWriteOffset = (mWriteHead != nullptr && mWriteStart != nullptr)
            ? static_cast<std::int64_t>(mWriteHead - mWriteStart)
            : 0;
        if (origin == OriginBegin) {
            writePos = offset;
        } else if (origin == OriginCurr) {
 ...
 */
void MemBufferStream::VirtWrite(const char* const data, const size_t size)
{
    if (mInput.begin() == nullptr) {
    ...
    if ((mWriteHead + size) > mWriteEnd) {
        const std::uint64_t needed = static_cast<std::uint64_t>(mWriteHead - mWriteStart) + static_cast<std::uint64_t>(size);
        Resize(needed);
    }
