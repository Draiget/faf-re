...
namespace gpg
{
  namespace
  {
    constexpr unsigned int kHashMixMultiplier = 275604541u;

    [[nodiscard]] unsigned int MixHashStep(
      const unsigned int accumulator,
      const unsigned int lane
    ) noexcept
    {
      const unsigned int mixed = kHashMixMultiplier * (accumulator ^ lane);
      return mixed ^ (mixed >> 13u);
    }
  } // namespace
   ...
   */
  static inline void load_block_le(
    const void* src,
    uint32_t (&M)[16]
  )
  {
  ...
  bool MD5Digest::operator!=(
    MD5Digest* rhs
  ) const
  {
    // Keep exact pointer-parameter semantics as in the original binary.
    return rhs->vals[0] != vals[0] || rhs->vals[1] != vals[1] || rhs->vals[2] != vals[2] || rhs->vals[3] != vals[3];
  }
  ...
  /**
   * Address: 0x008E5870 (FUN_008E5870, gpg::MD5Context::Update)
   *
   * What it does:
   * Extends the running digest with one byte span, consuming complete
   * 64-byte blocks immediately and caching the trailing partial lane.
   */
  void MD5Context::Update(
    const void* ptr,
    size_t len
   ...
   *
   * What it does:
   * Finalizes one MD5 digest copy using RFC1321 padding while preserving
   * the source context state.
   */
  ...
  /**
   * Address: 0x008E3EA0 (FUN_008E3EA0, gpg::HashDword)
   *
   * What it does:
   * Mixes one 32-bit lane into the salted hash accumulator.
   */
  unsigned int HashDword(
    const unsigned int dat,
    const unsigned int salt
  )
  {
    return MixHashStep(salt, dat);
  }

  /**
   * Address: 0x008E3EC0 (FUN_008E3EC0, gpg::HashBytes)
   *
   * What it does:
   * Hashes one raw byte span using 4-byte little-endian chunks followed by
   * tail-byte mixing.
   */
  ...
  {
    const auto* bytes = static_cast<const std::uint8_t*>(dat);
    unsigned int result = salt;

    while (size >= 4u) {
      const unsigned int packed = static_cast<unsigned int>(bytes[0]) | (static_cast<unsigned int>(bytes[1]) << 8u) |
        (static_cast<unsigned int>(bytes[2]) << 16u) | (static_cast<unsigned int>(bytes[3]) << 24u);
      result = MixHashStep(result, packed);
      bytes += 4;
      size -= 4u;
    }

    while (size != 0u) {
      result = MixHashStep(result, static_cast<unsigned int>(*bytes));
      ++bytes;
      --size;
    }
  ...
  /**
   * Address: 0x008E3F40 (FUN_008E3F40, gpg::Hash)
   *
   * What it does:
   * Hashes one `std::string` payload using `HashBytes`.
   */
  unsigned int Hash(
  ...
  {
    return HashBytes(dat.data(), dat.size(), salt);
  }
} // namespace gpg
