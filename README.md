...
- **`blocked`: `2,094` (3.12%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `2,518`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `2,518`*
...
### Not-yet-recovered (2,518 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `416` | 16.52% |
| Vtable-anchored (recover with the owning class) | `203` | 8.06% |
| Framework dispatch (wx/EH/Lua/reflection) | `71` | 2.82% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `841` | 33.40% |
| No indexed callsite evidence (needs investigation/evidence) | `983` | 39.04% |
