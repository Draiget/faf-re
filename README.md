...
- Total FAF functions: `67,167`
  - *IDA index, exported*
- Progress coverage:  **`95.92%`**
  - *Consists of `recovered` + `skip` + `external_dependency` ÷ exported*
  - *Total amount of completed tokens: `64,425`*

Progress DB status breakdown:

- `recovered`: `52,772` (81.91%)
- `skip`: `6,230` (9.67%) — proven ICF aliases / thunks / EH or static-init glue with no distinct source body
- `external_dependency`: `5,423` (8.42%) — proven third-party/import-boundary code
...
- **`blocked`: `2,404` (3.58%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `2,812`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `2,812`*
...
### Recovered (52,772 functions) — binary caller context

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `16,307` | 30.90% |
| Vtable-anchored (virtual override of a recovered class) | `5,880` | 11.14% |
| Framework dispatch (wx event, EH handler, Lua binding, reflection table, …) | `5,557` | 10.53% |
| No recovered caller token yet (orphan risk) | `1,865` | 3.53% |
| No callsite evidence (no recorded code/data caller in the index) | `22,943` | 43.48% |
| Unclassified data xref (manual review) | `216` | 0.41% |
| RTTI-only | `4` | 0.01% |

### Not-yet-recovered (2,812 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `379` | 13.48% |
| Vtable-anchored (recover with the owning class) | `144` | 5.12% |
| Framework dispatch (wx/EH/Lua/reflection) | `13` | 0.46% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `1,035` | 36.81% |
| No indexed callsite evidence (needs investigation/evidence) | `1,238` | 44.03% |
