...
- Progress coverage:  **`97.16%`**
  ...
  - *Total amount of completed tokens: `65,258`*

Progress DB status breakdown:

- `recovered`: `53,380` (81.80%)
...
- **`blocked`: `1,941` (2.89%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `1,945`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `1,945`*
...
### Recovered (53,380 functions) — wiring health

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Confirmed caller** (recovered binary caller wired by name) | `15,465` | 28.97% |
...
### Not-yet-recovered (1,945 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Recoverable now** (`OK_RECOVERED_CALLER` — recovered caller exists; recover next) | `302` | 15.53% |
| Vtable-anchored (recover with the owning class) | `186` | 9.56% |
| Framework dispatch (wx/EH/Lua/reflection) | `34` | 1.75% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `987` | 50.75% |
| No callsite evidence (likely external/dead — candidates for `external_dependency`) | `434` | 22.31% |
