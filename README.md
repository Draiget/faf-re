...
- Progress coverage:  **`96.74%`**
  ...
  - *Total amount of completed tokens: `64,975`*

Progress DB status breakdown:

- `recovered`: `53,094` (81.71%)
...
- **`blocked`: `2,219` (3.30%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `2,222`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `2,222`*
...
### Recovered (53,094 functions) — wiring health

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Confirmed caller** (recovered binary caller wired by name) | `14,977` | 28.21% |
| Vtable-anchored (virtual override of a recovered class) | `5,817` | 10.96% |
| Framework dispatch (wx event, EH handler, Lua binding, reflection table, …) | `5,496` | 10.35% |
| Caller still blocked (orphan-helper risk — caller awaits recovery) | `2,840` | 5.35% |
| No callsite evidence (no recorded code/data caller in the index) | `23,745` | 44.72% |
| Unclassified data xref (manual review) | `215` | 0.40% |
| RTTI-only | `4` | 0.01% |

### Not-yet-recovered (2,222 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Recoverable now** (`OK_RECOVERED_CALLER` — recovered caller exists; recover next) | `476` | 21.42% |
| Vtable-anchored (recover with the owning class) | `211` | 9.50% |
| Framework dispatch (wx/EH/Lua/reflection) | `35` | 1.58% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `1,057` | 47.57% |
| No callsite evidence (likely external/dead — candidates for `external_dependency`) | `441` | 19.85% |
