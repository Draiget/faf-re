...
- Total FAF functions: `67,167`
  - *IDA index, exported*
- Progress coverage:  **`96.52%`**
  - *Consists of `recovered` + `skip` + `external_dependency` ÷ exported*
  - *Total amount of completed tokens: `64,832`*

Progress DB status breakdown:

- `recovered`: `53,205` (82.07%)
...
- **`blocked`: `1,987` (2.96%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `2,405`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `2,405`*
...
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `16,222` | 30.49% |
...
### Not-yet-recovered (2,405 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `372` | 15.47% |
| Vtable-anchored (recover with the owning class) | `186` | 7.73% |
| Framework dispatch (wx/EH/Lua/reflection) | `45` | 1.87% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `815` | 33.89% |
| No indexed callsite evidence (needs investigation/evidence) | `983` | 40.87% |
