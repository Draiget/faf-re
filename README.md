...
- Progress coverage:  **`97.10%`**
  ...
  - *Total amount of completed tokens: `65,221`*

Progress DB status breakdown:

- `recovered`: `53,343` (81.79%)
...
- **`blocked`: `1,978` (2.94%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `1,982`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `1,982`*
...
### Recovered (53,343 functions) — wiring health

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Confirmed caller** (recovered binary caller wired by name) | `15,422` | 28.91% |
| Vtable-anchored (virtual override of a recovered class) | `5,826` | 10.92% |
...
### Not-yet-recovered (1,982 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Recoverable now** (`OK_RECOVERED_CALLER` — recovered caller exists; recover next) | `307` | 15.49% |
| Vtable-anchored (recover with the owning class) | `197` | 9.94% |
| Framework dispatch (wx/EH/Lua/reflection) | `35` | 1.77% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `1,006` | 50.76% |
| No callsite evidence (likely external/dead — candidates for `external_dependency`) | `435` | 21.95% |
