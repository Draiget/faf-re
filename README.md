...
- Progress coverage:  **`97.56%`**
  ...
  - *Total amount of completed tokens: `65,529`*
...
- **`blocked`: `1,665` (2.48%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `1,699`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `1,699`*
...
### Not-yet-recovered (1,699 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `182` | 10.71% |
| Vtable-anchored (recover with the owning class) | `129` | 7.59% |
| Framework dispatch (wx/EH/Lua/reflection) | `8` | 0.47% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `780` | 45.91% |
| No indexed callsite evidence (needs investigation/evidence) | `599` | 35.26% |
