...
- **`blocked`: `1,065` (1.59%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `1,098`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `1,098`*
...
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `16,781` | 31.60% |
...
### Not-yet-recovered (1,098 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `50` | 4.55% |
| Vtable-anchored (recover with the owning class) | `124` | 11.29% |
| Framework dispatch (wx/EH/Lua/reflection) | `8` | 0.73% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `549` | 50.00% |
| No indexed callsite evidence (needs investigation/evidence) | `366` | 33.33% |
