...
- **`blocked`: `1,080` (1.61%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `1,113`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `1,113`*
...
### Recovered (53,083 functions) — binary caller context

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `16,759` | 31.57% |
...
| No recovered caller token yet (orphan risk) | `1,779` | 3.35% |
| No callsite evidence (no recorded code/data caller in the index) | `22,875` | 43.09% |
| Unclassified data xref (manual review) | `216` | 0.41% |
| RTTI-only | `4` | 0.01% |

### Not-yet-recovered (1,113 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `53` | 4.76% |
| Vtable-anchored (recover with the owning class) | `124` | 11.14% |
| Framework dispatch (wx/EH/Lua/reflection) | `8` | 0.72% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `561` | 50.40% |
| No indexed callsite evidence (needs investigation/evidence) | `366` | 32.88% |
