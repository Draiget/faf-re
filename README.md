...
- Total FAF functions: `67,167`
  - *IDA index, exported*
- Progress coverage:  **`96.10%`**
  - *Consists of `recovered` + `skip` + `external_dependency` ÷ exported*
  - *Total amount of completed tokens: `64,550`*

Progress DB status breakdown:

- `recovered`: `52,686` (81.62%)
- `skip`: `6,407` (9.93%) — proven ICF aliases / thunks / EH or static-init glue with no distinct source body
- `external_dependency`: `5,457` (8.45%) — proven third-party/import-boundary code
...
- `in_progress`: `113` (0.17%)
- **`blocked`: `2,168` (3.23%)**
  - *strict circular/dep-blocked (in-DB literal `status == "blocked"`)*  
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `2,575`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `2,575`*
...
### Recovered (52,686 functions) — binary caller context

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `16,332` | 31.00% |
| Vtable-anchored (virtual override of a recovered class) | `5,879` | 11.16% |
| Framework dispatch (wx event, EH handler, Lua binding, reflection table, …) | `5,556` | 10.55% |
| No recovered caller token yet (orphan risk) | `1,843` | 3.50% |
| No callsite evidence (no recorded code/data caller in the index) | `22,856` | 43.38% |
| Unclassified data xref (manual review) | `216` | 0.41% |
| RTTI-only | `4` | 0.01% |

### Not-yet-recovered (2,575 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `308` | 11.96% |
| Vtable-anchored (recover with the owning class) | `140` | 5.44% |
| Framework dispatch (wx/EH/Lua/reflection) | `11` | 0.43% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `972` | 37.75% |
| No indexed callsite evidence (needs investigation/evidence) | `1,142` | 44.35% |
