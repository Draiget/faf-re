...
- Progress coverage:  **`97.29%`**
  ...
  - *Total amount of completed tokens: `65,350`*

Progress DB status breakdown:

- `recovered`: `53,746` (82.24%)
- `skip`: `6,117` (9.36%) — proven ICF aliases / thunks / EH or static-init glue with no distinct source body
...
- **`blocked`: `1,808` (2.69%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `1,874`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `1,874`*
...
### Not-yet-recovered (1,874 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `309` | 16.49% |
| Vtable-anchored (recover with the owning class) | `178` | 9.50% |
| Framework dispatch (wx/EH/Lua/reflection) | `54` | 2.88% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `869` | 46.37% |
| No indexed callsite evidence (needs investigation/evidence) | `460` | 24.55% |
