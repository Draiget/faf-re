...
- **`blocked`: `1,827` (2.72%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `1,830`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `1,830`*
...
### Recovered (53,538 functions) — wiring health
...
| Framework dispatch (wx event, EH handler, Lua binding, reflection table, …) | `5,516` | 10.30% |
| Caller still blocked (orphan-helper risk — caller awaits recovery) | `2,535` | 4.73% |
| No callsite evidence (no recorded code/data caller in the index) | `23,723` | 44.31% |
| Unclassified data xref (manual review) | `217` | 0.41% |
| RTTI-only | `4` | 0.01% |

### Not-yet-recovered (1,830 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Recoverable now** (`OK_RECOVERED_CALLER` — recovered caller exists; recover next) | `280` | 15.30% |
| Vtable-anchored (recover with the owning class) | `174` | 9.51% |
| Framework dispatch (wx/EH/Lua/reflection) | `10` | 0.55% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `930` | 50.82% |
| No callsite evidence (likely external/dead — candidates for `external_dependency`) | `434` | 23.72% |
