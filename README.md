...
## Recovery Coverage (`18/07/2026`, `fa_full_2026_03_26`)

Progress snapshot:

- Total FAF functions: `67,167`
  - *IDA index, exported*
- Progress coverage:  **`97.19%`**
  - *Consists of `recovered` + `skip` + `external_dependency` ÷ exported*
  - *Total amount of completed tokens: `65,280`*

Progress DB status breakdown:

- `recovered`: `53,402` (81.80%)
- `skip`: `6,111` (9.36%) — CRT-internal / compiler-generated / orphan template instantiations / static-init glue
- `external_dependency`: `5,767` (8.83%) — third-party libs
...
- **`blocked`: `1,919` (2.86%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `1,923`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `1,923`*
...
### Recovered (53,402 functions) — wiring health

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Confirmed caller** (recovered binary caller wired by name) | `15,492` | 29.01% |
| Vtable-anchored (virtual override of a recovered class) | `5,839` | 10.93% |
| Framework dispatch (wx event, EH handler, Lua binding, reflection table, …) | `5,499` | 10.30% |
| Caller still blocked (orphan-helper risk — caller awaits recovery) | `2,601` | 4.87% |
| No callsite evidence (no recorded code/data caller in the index) | `23,752` | 44.48% |
| Unclassified data xref (manual review) | `215` | 0.40% |
| RTTI-only | `4` | 0.01% |

### Not-yet-recovered (1,923 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Recoverable now** (`OK_RECOVERED_CALLER` — recovered caller exists; recover next) | `296` | 15.39% |
| Vtable-anchored (recover with the owning class) | `184` | 9.57% |
| Framework dispatch (wx/EH/Lua/reflection) | `34` | 1.77% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `973` | 50.60% |
| No callsite evidence (likely external/dead — candidates for `external_dependency`) | `434` | 22.57% |
