...
- Progress coverage:  **`97.18%`**
  ...
  - *Total amount of completed tokens: `65,275`*

Progress DB status breakdown:

- `recovered`: `53,658` (82.20%)
...
### Recovered (53,658 functions) — binary caller context

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `15,974` | 29.77% |
...
| No recovered caller token yet (orphan risk) | `2,417` | 4.50% |
...
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `313` | 16.06% |
| Vtable-anchored (recover with the owning class) | `193` | 9.90% |
| Framework dispatch (wx/EH/Lua/reflection) | `56` | 2.87% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `922` | 47.31% |
| No indexed callsite evidence (needs investigation/evidence) | `461` | 23.65% |
