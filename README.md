...
- Progress coverage:  **`97.06%`**
  ...
  - *Total amount of completed tokens: `65,189`*

Progress DB status breakdown:

- `recovered`: `53,654` (82.31%)
...
- **`blocked`: `1,640` (2.44%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `2,048`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `2,048`*
...
### Recovered (53,654 functions) — binary caller context

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `16,588` | 30.92% |
| Vtable-anchored (virtual override of a recovered class) | `5,909` | 11.01% |
| Framework dispatch (wx event, EH handler, Lua binding, reflection table, …) | `5,568` | 10.38% |
| No recovered caller token yet (orphan risk) | `2,101` | 3.92% |
| No callsite evidence (no recorded code/data caller in the index) | `23,267` | 43.36% |
| Unclassified data xref (manual review) | `217` | 0.40% |
| RTTI-only | `4` | 0.01% |

### Not-yet-recovered (2,048 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `279` | 13.62% |
| Vtable-anchored (recover with the owning class) | `113` | 5.52% |
| Framework dispatch (wx/EH/Lua/reflection) | `1` | 0.05% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `742` | 36.23% |
| No indexed callsite evidence (needs investigation/evidence) | `911` | 44.48% |
