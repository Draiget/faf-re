...
- Progress coverage:  **`96.90%`**
  ...
  - *Total amount of completed tokens: `65,088`*

Progress DB status breakdown:

- `recovered`: `53,547` (82.27%)
...
### Recovered (53,547 functions) — binary caller context

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `16,570` | 30.94% |
...
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `303` | 14.10% |
| Vtable-anchored (recover with the owning class) | `112` | 5.21% |
| Framework dispatch (wx/EH/Lua/reflection) | `1` | 0.05% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `746` | 34.71% |
| No indexed callsite evidence (needs investigation/evidence) | `985` | 45.84% |
