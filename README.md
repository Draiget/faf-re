...
- Progress coverage:  **`97.54%`**
  ...
  - *Total amount of completed tokens: `65,514`*
...
- `external_dependency`: `5,531` (8.44%) — proven third-party/import-boundary code
...
- **`blocked`: `1,680` (2.50%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `1,714`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `1,714`*
...
### Not-yet-recovered (1,714 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `188` | 10.97% |
| Vtable-anchored (recover with the owning class) | `130` | 7.58% |
| Framework dispatch (wx/EH/Lua/reflection) | `8` | 0.47% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `787` | 45.92% |
| No indexed callsite evidence (needs investigation/evidence) | `600` | 35.01% |
