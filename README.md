...
- Progress coverage:  **`97.35%`**
  ...
  - *Total amount of completed tokens: `65,390`*

Progress DB status breakdown:

- `recovered`: `52,799` (80.74%)
- `skip`: `7,085` (10.83%) — proven ICF aliases / thunks / EH or static-init glue with no distinct source body
...
- **`blocked`: `1,802` (2.68%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `1,836`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `1,836`*
...
### Recovered (52,799 functions) — binary caller context

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `16,412` | 31.08% |
...
| No callsite evidence (no recorded code/data caller in the index) | `22,861` | 43.30% |
| Unclassified data xref (manual review) | `216` | 0.41% |
| RTTI-only | `4` | 0.01% |

### Not-yet-recovered (1,836 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `237` | 12.91% |
| Vtable-anchored (recover with the owning class) | `131` | 7.14% |
| Framework dispatch (wx/EH/Lua/reflection) | `8` | 0.44% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `850` | 46.30% |
| No indexed callsite evidence (needs investigation/evidence) | `608` | 33.12% |
