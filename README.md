...
- Progress coverage:  **`97.13%`**
  ...
  - *Total amount of completed tokens: `65,239`*

Progress DB status breakdown:

- `recovered`: `53,361` (81.79%)
...
### Recovered (53,361 functions) — wiring health

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Confirmed caller** (recovered binary caller wired by name) | `15,440` | 28.93% |
...
| Caller still blocked (orphan-helper risk — caller awaits recovery) | `2,617` | 4.90% |
...
|---|---:|---:|
| **Recoverable now** (`OK_RECOVERED_CALLER` — recovered caller exists; recover next) | `308` | 15.68% |
| Vtable-anchored (recover with the owning class) | `189` | 9.62% |
| Framework dispatch (wx/EH/Lua/reflection) | `34` | 1.73% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `997` | 50.76% |
| No callsite evidence (likely external/dead — candidates for `external_dependency`) | `434` | 22.10% |
