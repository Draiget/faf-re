...
- Progress coverage:  **`96.81%`**
  ...
  - *Total amount of completed tokens: `65,025`*

Progress DB status breakdown:

- `recovered`: `53,144` (81.73%)
...
- **`blocked`: `2,171` (3.23%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `2,174`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `2,174`*
...
### Recovered (53,144 functions) — wiring health

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Confirmed caller** (recovered binary caller wired by name) | `15,050` | 28.32% |
...
| Caller still blocked (orphan-helper risk — caller awaits recovery) | `2,816` | 5.30% |
| No callsite evidence (no recorded code/data caller in the index) | `23,747` | 44.68% |
| Unclassified data xref (manual review) | `215` | 0.40% |
| RTTI-only | `4` | 0.01% |

### Not-yet-recovered (2,174 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Recoverable now** (`OK_RECOVERED_CALLER` — recovered caller exists; recover next) | `435` | 20.01% |
| Vtable-anchored (recover with the owning class) | `210` | 9.66% |
| Framework dispatch (wx/EH/Lua/reflection) | `35` | 1.61% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `1,053` | 48.44% |
| No callsite evidence (likely external/dead — candidates for `external_dependency`) | `439` | 20.19% |
