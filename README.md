...
- **`blocked`: `693` (1.03%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `696`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `696`*
...
### Recovered (53,319 functions) — binary caller context
...
| No callsite evidence (no recorded code/data caller in the index) | `22,860` | 42.87% |
| Unclassified data xref (manual review) | `216` | 0.41% |
| RTTI-only | `4` | 0.01% |

### Not-yet-recovered (696 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `11` | 1.58% |
| Vtable-anchored (recover with the owning class) | `109` | 15.66% |
| Framework dispatch (wx/EH/Lua/reflection) | `0` | 0.00% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `264` | 37.93% |
| No indexed callsite evidence (needs investigation/evidence) | `311` | 44.68% |
