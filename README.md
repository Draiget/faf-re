...
- Progress coverage:  **`97.00%`**
  ...
  - *Total amount of completed tokens: `65,150`*

Progress DB status breakdown:

- `recovered`: `53,595` (82.26%)
...
- **`blocked`: `1,679` (2.50%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `2,087`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `2,087`*
...
### Recovered (53,595 functions) — binary caller context

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `16,533` | 30.85% |
...
### Not-yet-recovered (2,087 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `283` | 13.56% |
| Vtable-anchored (recover with the owning class) | `117` | 5.61% |
| Framework dispatch (wx/EH/Lua/reflection) | `1` | 0.05% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `772` | 36.99% |
| No indexed callsite evidence (needs investigation/evidence) | `912` | 43.70% |
