...
- Total FAF functions: `67,167`
  - *IDA index, exported*
- Progress coverage:  **`97.29%`**
  - *Consists of `recovered` + `skip` + `external_dependency` ÷ exported*
  - *Total amount of completed tokens: `65,345`*

Progress DB status breakdown:

- `recovered`: `53,746` (82.25%)
- `skip`: `6,112` (9.35%) — proven ICF aliases / thunks / EH or static-init glue with no distinct source body
...
- **`blocked`: `1,813` (2.70%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `1,879`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `1,879`*
...
### Recovered (53,746 functions) — binary caller context

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `16,110` | 29.97% |
...
| No recovered caller token yet (orphan risk) | `2,352` | 4.38% |
| No callsite evidence (no recorded code/data caller in the index) | `23,719` | 44.13% |
| Unclassified data xref (manual review) | `216` | 0.40% |
| RTTI-only | `4` | 0.01% |

### Not-yet-recovered (1,879 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `314` | 16.71% |
| Vtable-anchored (recover with the owning class) | `178` | 9.47% |
| Framework dispatch (wx/EH/Lua/reflection) | `54` | 2.87% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `869` | 46.25% |
| No indexed callsite evidence (needs investigation/evidence) | `460` | 24.48% |
