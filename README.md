...
## Recovery Coverage (`02/09/2026`, `fa_full_2026_03_26`)

Progress snapshot:

- Total FAF functions: `67,170`
  - *IDA index, exported*
- Progress coverage:  **`99.43%`**
  - *Consists of `recovered` + `skip` + `external_dependency` ÷ exported*
  - *Total amount of completed tokens: `66,785`*

Progress DB status breakdown:

- `recovered`: `52,721` (78.94%)
- `skip`: `8,324` (12.46%) — proven ICF aliases / thunks / EH or static-init glue with no distinct source body
- `external_dependency`: `5,740` (8.59%) — proven third-party/import-boundary code
...
- **`blocked`: `495` (0.74%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `498`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `498`*
...
### Recovered (52,721 functions) — binary caller context

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `17,174` | 32.58% |
| Vtable-anchored (virtual override of a recovered class) | `5,892` | 11.18% |
| Framework dispatch (wx event, EH handler, Lua binding, reflection table, …) | `5,617` | 10.65% |
| No recovered caller token yet (orphan risk) | `1,649` | 3.13% |
| No callsite evidence (no recorded code/data caller in the index) | `22,169` | 42.05% |
| Unclassified data xref (manual review) | `216` | 0.41% |
| RTTI-only | `4` | 0.01% |

### Not-yet-recovered (498 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `6` | 1.20% |
| Vtable-anchored (recover with the owning class) | `82` | 16.47% |
| Framework dispatch (wx/EH/Lua/reflection) | `0` | 0.00% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `132` | 26.51% |
| No indexed callsite evidence (needs investigation/evidence) | `278` | 55.82% |
