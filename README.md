...
- Total FAF functions: `67,170`
  - *IDA index, exported*
- Progress coverage:  **`99.68%`**
  - *Consists of `recovered` + `skip` + `external_dependency` ÷ exported*
  - *Total amount of completed tokens: `66,954`*

Progress DB status breakdown:

- `recovered`: `52,810` (78.88%)
- `skip`: `8,346` (12.47%) — proven ICF aliases / thunks / EH or static-init glue with no distinct source body
- `external_dependency`: `5,798` (8.66%) — proven third-party/import-boundary code
...
- **`blocked`: `326` (0.49%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `329`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `329`*
...
### Recovered (52,810 functions) — binary caller context

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `17,202` | 32.57% |
| Vtable-anchored (virtual override of a recovered class) | `5,892` | 11.16% |
| Framework dispatch (wx event, EH handler, Lua binding, reflection table, …) | `5,618` | 10.64% |
| No recovered caller token yet (orphan risk) | `1,627` | 3.08% |
| No callsite evidence (no recorded code/data caller in the index) | `22,251` | 42.13% |
| Unclassified data xref (manual review) | `216` | 0.41% |
| RTTI-only | `4` | 0.01% |

### Not-yet-recovered (329 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `6` | 1.82% |
| Vtable-anchored (recover with the owning class) | `82` | 24.92% |
| Framework dispatch (wx/EH/Lua/reflection) | `0` | 0.00% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `130` | 39.51% |
| No indexed callsite evidence (needs investigation/evidence) | `111` | 33.74% |
