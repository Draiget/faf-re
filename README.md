...
- Total FAF functions: `67,167`
  - *IDA index, exported*
- Progress coverage:  **`96.92%`**
  - *Consists of `recovered` + `skip` + `external_dependency` ÷ exported*
  - *Total amount of completed tokens: `65,099`*

Progress DB status breakdown:

- `recovered`: `53,546` (82.25%)
- `skip`: `6,118` (9.40%) — proven ICF aliases / thunks / EH or static-init glue with no distinct source body
- `external_dependency`: `5,435` (8.35%) — proven third-party/import-boundary code
...
- **`blocked`: `1,730` (2.58%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `2,138`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `2,138`*
...
### Recovered (53,546 functions) — binary caller context

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `16,484` | 30.78% |
...
| No callsite evidence (no recorded code/data caller in the index) | `23,265` | 43.45% |
| Unclassified data xref (manual review) | `217` | 0.41% |
| RTTI-only | `4` | 0.01% |

### Not-yet-recovered (2,138 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `328` | 15.34% |
| Vtable-anchored (recover with the owning class) | `117` | 5.47% |
| Framework dispatch (wx/EH/Lua/reflection) | `1` | 0.05% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `778` | 36.39% |
| No indexed callsite evidence (needs investigation/evidence) | `912` | 42.66% |
