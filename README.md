...
- **`blocked`: `2,218` (3.30%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `2,221`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `2,221`*
...
### Recovered (53,095 functions) — wiring health
...
| No callsite evidence (no recorded code/data caller in the index) | `23,746` | 44.72% |
| Unclassified data xref (manual review) | `215` | 0.40% |
| RTTI-only | `4` | 0.01% |

### Not-yet-recovered (2,221 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Recoverable now** (`OK_RECOVERED_CALLER` — recovered caller exists; recover next) | `476` | 21.43% |
| Vtable-anchored (recover with the owning class) | `211` | 9.50% |
| Framework dispatch (wx/EH/Lua/reflection) | `35` | 1.58% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `1,057` | 47.59% |
| No callsite evidence (likely external/dead — candidates for `external_dependency`) | `440` | 19.81% |
