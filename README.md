...
- Progress coverage:  **`97.71%`**
  ...
  - *Total amount of completed tokens: `65,628`*

Progress DB status breakdown:

- `recovered`: `52,967` (80.71%)
- `skip`: `7,114` (10.84%) — proven ICF aliases / thunks / EH or static-init glue with no distinct source body
...
- **`blocked`: `1,567` (2.33%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `1,601`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `1,601`*
...
### Not-yet-recovered (1,601 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `134` | 8.37% |
| Vtable-anchored (recover with the owning class) | `129` | 8.06% |
| Framework dispatch (wx/EH/Lua/reflection) | `8` | 0.50% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `730` | 45.60% |
| No indexed callsite evidence (needs investigation/evidence) | `599` | 37.41% |
