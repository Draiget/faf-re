...
- Total FAF functions: `67,167`
  - *IDA index, exported*
- Progress coverage:  **`97.83%`**
  - *Consists of `recovered` + `skip` + `external_dependency` ÷ exported*
  - *Total amount of completed tokens: `65,710`*

Progress DB status breakdown:

- `recovered`: `53,046` (80.73%)
- `skip`: `7,117` (10.83%) — proven ICF aliases / thunks / EH or static-init glue with no distinct source body
- `external_dependency`: `5,547` (8.44%) — proven third-party/import-boundary code
...
### Recovered (53,046 functions) — binary caller context

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `16,700` | 31.48% |
...
| No recovered caller token yet (orphan risk) | `1,801` | 3.40% |
| No callsite evidence (no recorded code/data caller in the index) | `22,875` | 43.12% |
...
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `70` | 4.61% |
| Vtable-anchored (recover with the owning class) | `129` | 8.49% |
| Framework dispatch (wx/EH/Lua/reflection) | `8` | 0.53% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `715` | 47.04% |
| No indexed callsite evidence (needs investigation/evidence) | `597` | 39.28% |
