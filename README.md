...
- Progress coverage:  **`96.89%`**
  ...
  - *Total amount of completed tokens: `65,080`*

Progress DB status breakdown:

- `recovered`: `53,487` (82.19%)
...
- **`blocked`: `1,748` (2.60%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `2,157`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `2,157`*
...
### Recovered (53,487 functions) — binary caller context

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `16,396` | 30.65% |
| Vtable-anchored (virtual override of a recovered class) | `5,894` | 11.02% |
| Framework dispatch (wx event, EH handler, Lua binding, reflection table, …) | `5,567` | 10.41% |
| No recovered caller token yet (orphan risk) | `2,149` | 4.02% |
| No callsite evidence (no recorded code/data caller in the index) | `23,261` | 43.49% |
| Unclassified data xref (manual review) | `216` | 0.40% |
| RTTI-only | `4` | 0.01% |

### Not-yet-recovered (2,157 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `346` | 16.04% |
| Vtable-anchored (recover with the owning class) | `122` | 5.66% |
| Framework dispatch (wx/EH/Lua/reflection) | `2` | 0.09% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `769` | 35.65% |
| No indexed callsite evidence (needs investigation/evidence) | `915` | 42.42% |
