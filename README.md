...
- Progress coverage:  **`99.29%`**
  ...
  - *Total amount of completed tokens: `66,694`*
...
- **`blocked`: `583` (0.87%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `586`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `586`*
...
### Not-yet-recovered (586 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `9` | 1.54% |
| Vtable-anchored (recover with the owning class) | `106` | 18.09% |
| Framework dispatch (wx/EH/Lua/reflection) | `0` | 0.00% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `165` | 28.16% |
| No indexed callsite evidence (needs investigation/evidence) | `305` | 52.05% |
