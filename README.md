...
- Total FAF functions: `67,170`
  - *IDA index, exported*
- Progress coverage:  **`99.34%`**
  - *Consists of `recovered` + `skip` + `external_dependency` ÷ exported*
  - *Total amount of completed tokens: `66,726`*

Progress DB status breakdown:

- `recovered`: `52,749` (79.05%)
- `skip`: `8,232` (12.34%) — proven ICF aliases / thunks / EH or static-init glue with no distinct source body
- `external_dependency`: `5,745` (8.61%) — proven third-party/import-boundary code
...
- **`blocked`: `551` (0.82%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `554`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `554`*
...
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `17,169` | 32.55% |
...
### Not-yet-recovered (554 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `9` | 1.62% |
| Vtable-anchored (recover with the owning class) | `101` | 18.23% |
| Framework dispatch (wx/EH/Lua/reflection) | `0` | 0.00% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `154` | 27.80% |
| No indexed callsite evidence (needs investigation/evidence) | `289` | 52.17% |
| Unclassified data xref (manual review) | `1` | 0.18% |
| RTTI-only | `0` | 0.00% |
