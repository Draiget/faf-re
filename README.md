...
- Progress coverage:  **`98.76%`**
  ...
  - *Total amount of completed tokens: `66,336`*
...
- **`blocked`: `888` (1.32%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `895`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `895`*
...
### Not-yet-recovered (895 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `31` | 3.46% |
| Vtable-anchored (recover with the owning class) | `109` | 12.18% |
| Framework dispatch (wx/EH/Lua/reflection) | `0` | 0.00% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `394` | 44.02% |
| No indexed callsite evidence (needs investigation/evidence) | `360` | 40.22% |
