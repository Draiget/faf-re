...
### Recovered (53,532 functions) — binary caller context

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `16,463` | 30.75% |
...
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `336` | 15.64% |
| Vtable-anchored (recover with the owning class) | `117` | 5.45% |
| Framework dispatch (wx/EH/Lua/reflection) | `1` | 0.05% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `780` | 36.31% |
| No indexed callsite evidence (needs investigation/evidence) | `912` | 42.46% |
