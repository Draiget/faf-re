...
- Progress coverage:  **`97.39%`**
  ...
  - *Total amount of completed tokens: `65,411`*

Progress DB status breakdown:

- `recovered`: `53,804` (82.26%)
- `skip`: `6,118` (9.35%) — proven ICF aliases / thunks / EH or static-init glue with no distinct source body
...
- `needs_evidence`: `64` (0.10%)
- `in_progress`: `14` (0.02%)
- **`blocked`: `1,749` (2.60%)**
  - *strict circular/dep-blocked (in-DB literal `status == "blocked"`)*  
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `1,813`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `1,813`*
...
### Recovered (53,804 functions) — binary caller context

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `16,201` | 30.11% |
| Vtable-anchored (virtual override of a recovered class) | `5,856` | 10.88% |
| Framework dispatch (wx event, EH handler, Lua binding, reflection table, …) | `5,504` | 10.23% |
| No recovered caller token yet (orphan risk) | `2,303` | 4.28% |
| No callsite evidence (no recorded code/data caller in the index) | `23,720` | 44.09% |
| Unclassified data xref (manual review) | `216` | 0.40% |
| RTTI-only | `4` | 0.01% |

### Not-yet-recovered (1,813 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `296` | 16.33% |
| Vtable-anchored (recover with the owning class) | `162` | 8.94% |
| Framework dispatch (wx/EH/Lua/reflection) | `54` | 2.98% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `839` | 46.28% |
| No indexed callsite evidence (needs investigation/evidence) | `458` | 25.26% |
| Unclassified data xref (manual review) | `3` | 0.17% |
| RTTI-only | `1` | 0.06% |
