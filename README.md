...
- Progress coverage:  **`99.30%`**
  ...
  - *Total amount of completed tokens: `66,698`*
...
- **`blocked`: `579` (0.86%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `582`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `582`*
...
### Not-yet-recovered (582 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `9` | 1.55% |
| Vtable-anchored (recover with the owning class) | `106` | 18.21% |
| Framework dispatch (wx/EH/Lua/reflection) | `0` | 0.00% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `161` | 27.66% |
| No indexed callsite evidence (needs investigation/evidence) | `305` | 52.41% |
