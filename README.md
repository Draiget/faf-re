...
- Progress coverage:  **`97.24%`**
  ...
  - *Total amount of completed tokens: `65,310`*

Progress DB status breakdown:

- `recovered`: `53,435` (81.82%)
...
- **`blocked`: `1,891` (2.82%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `1,894`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `1,894`*
...
### Recovered (53,435 functions) — wiring health
...
### Not-yet-recovered (1,894 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Recoverable now** (`OK_RECOVERED_CALLER` — recovered caller exists; recover next) | `285` | 15.05% |
| Vtable-anchored (recover with the owning class) | `183` | 9.66% |
| Framework dispatch (wx/EH/Lua/reflection) | `34` | 1.80% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `956` | 50.48% |
| No callsite evidence (likely external/dead — candidates for `external_dependency`) | `434` | 22.91% |
