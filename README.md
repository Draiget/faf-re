...
- **`blocked`: `1,042` (1.55%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `1,062`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `1,062`*
...
### Recovered (53,101 functions) — binary caller context

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `16,807` | 31.65% |
...
| No recovered caller token yet (orphan risk) | `1,751` | 3.30% |
| No callsite evidence (no recorded code/data caller in the index) | `22,873` | 43.07% |
| Unclassified data xref (manual review) | `216` | 0.41% |
| RTTI-only | `4` | 0.01% |

### Not-yet-recovered (1,062 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `40` | 3.77% |
| Vtable-anchored (recover with the owning class) | `120` | 11.30% |
| Framework dispatch (wx/EH/Lua/reflection) | `8` | 0.75% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `527` | 49.62% |
| No indexed callsite evidence (needs investigation/evidence) | `366` | 34.46% |
