...
- Progress coverage:  **`96.73%`**
  ...
  - *Total amount of completed tokens: `64,970`*

Progress DB status breakdown:

- `recovered`: `52,742` (81.18%)
...
- **`blocked`: `1,846` (2.75%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `2,252`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `2,252`*
...
### Recovered (52,742 functions) — binary caller context

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `16,365` | 31.03% |
...
### Not-yet-recovered (2,252 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `242` | 10.75% |
| Vtable-anchored (recover with the owning class) | `136` | 6.04% |
| Framework dispatch (wx/EH/Lua/reflection) | `10` | 0.44% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `877` | 38.94% |
| No indexed callsite evidence (needs investigation/evidence) | `985` | 43.74% |
