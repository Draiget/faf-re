...
- Progress coverage:  **`99.69%`**
  ...
  - *Total amount of completed tokens: `66,960`*

Progress DB status breakdown:

- `recovered`: `52,821` (78.88%)
- `skip`: `8,346` (12.46%) — proven ICF aliases / thunks / EH or static-init glue with no distinct source body
- `external_dependency`: `5,793` (8.65%) — proven third-party/import-boundary code
...
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `17,206` | 32.57% |
...
| No recovered caller token yet (orphan risk) | `1,629` | 3.08% |
| No callsite evidence (no recorded code/data caller in the index) | `22,256` | 42.13% |
