...
- Progress coverage:  **`98.86%`**
  ...
  - *Total amount of completed tokens: `66,400`*

Progress DB status breakdown:

- `recovered`: `53,231` (80.17%)
- `skip`: `7,395` (11.14%) — proven ICF aliases / thunks / EH or static-init glue with no distinct source body
- `external_dependency`: `5,774` (8.70%) — proven third-party/import-boundary code
...
- **`blocked`: `827` (1.23%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `834`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `834`*
...
### Recovered (53,231 functions) — binary caller context

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `16,957` | 31.86% |
| Vtable-anchored (virtual override of a recovered class) | `5,885` | 11.06% |
| Framework dispatch (wx event, EH handler, Lua binding, reflection table, …) | `5,565` | 10.45% |
| No recovered caller token yet (orphan risk) | `1,715` | 3.22% |
| No callsite evidence (no recorded code/data caller in the index) | `22,889` | 43.00% |
| Unclassified data xref (manual review) | `216` | 0.41% |
| RTTI-only | `4` | 0.01% |

### Not-yet-recovered (834 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `27` | 3.24% |
| Vtable-anchored (recover with the owning class) | `109` | 13.07% |
| Framework dispatch (wx/EH/Lua/reflection) | `0` | 0.00% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `360` | 43.17% |
| No indexed callsite evidence (needs investigation/evidence) | `337` | 40.41% |
