...
- Progress coverage:  **`97.21%`**
  ...
  - *Total amount of completed tokens: `65,295`*

Progress DB status breakdown:

- `recovered`: `53,688` (82.22%)
...
### Recovered (53,688 functions) — binary caller context
...
| Framework dispatch (wx event, EH handler, Lua binding, reflection table, …) | `5,503` | 10.25% |
...
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `310` | 16.07% |
| Vtable-anchored (recover with the owning class) | `189` | 9.80% |
| Framework dispatch (wx/EH/Lua/reflection) | `55` | 2.85% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `911` | 47.23% |
| No indexed callsite evidence (needs investigation/evidence) | `460` | 23.85% |
