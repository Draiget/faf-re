...
- **`blocked`: `530` (0.79%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `533`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `533`*
...
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `17,171` | 32.55% |
...
### Not-yet-recovered (533 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `10` | 1.88% |
| Vtable-anchored (recover with the owning class) | `89` | 16.70% |
| Framework dispatch (wx/EH/Lua/reflection) | `0` | 0.00% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `152` | 28.52% |
| No indexed callsite evidence (needs investigation/evidence) | `282` | 52.91% |
