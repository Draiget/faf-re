...
- **`blocked`: `298` (0.44%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `301`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `301`*
...
### Recovered (47,470 functions) — binary caller context

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `17,324` | 36.49% |
| Vtable-anchored (virtual override of a recovered class) | `5,900` | 12.43% |
| Framework dispatch (wx event, EH handler, Lua binding, reflection table, …) | `5,618` | 11.83% |
| No recovered caller token yet (orphan risk) | `1,573` | 3.31% |
| No callsite evidence (no recorded code/data caller in the index) | `16,835` | 35.46% |
| Unclassified data xref (manual review) | `216` | 0.46% |
| RTTI-only | `4` | 0.01% |

### Not-yet-recovered (301 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `3` | 1.00% |
| Vtable-anchored (recover with the owning class) | `75` | 24.92% |
| Framework dispatch (wx/EH/Lua/reflection) | `0` | 0.00% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `114` | 37.87% |
| No indexed callsite evidence (needs investigation/evidence) | `109` | 36.21% |
