...
- Total FAF functions: `67,167`
  - *IDA index, exported*
- Progress coverage:  **`97.00%`**
  - *Consists of `recovered` + `skip` + `external_dependency` ÷ exported*
  - *Total amount of completed tokens: `65,154`*

Progress DB status breakdown:

- `recovered`: `53,599` (82.27%)
...
- **`blocked`: `1,675` (2.49%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `2,083`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `2,083`*
...
### Recovered (53,599 functions) — binary caller context

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `16,537` | 30.85% |
...
### Not-yet-recovered (2,083 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `279` | 13.39% |
| Vtable-anchored (recover with the owning class) | `117` | 5.62% |
| Framework dispatch (wx/EH/Lua/reflection) | `1` | 0.05% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `772` | 37.06% |
| No indexed callsite evidence (needs investigation/evidence) | `912` | 43.78% |
