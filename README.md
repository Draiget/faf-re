...
- **`blocked`: `1,539` (2.29%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `1,963`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `1,963`*
...
### Recovered (53,645 functions) — binary caller context

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `16,128` | 30.06% |
| Vtable-anchored (virtual override of a recovered class) | `5,839` | 10.88% |
| Framework dispatch (wx event, EH handler, Lua binding, reflection table, …) | `5,494` | 10.24% |
| No recovered caller token yet (orphan risk) | `2,265` | 4.22% |
| No callsite evidence (no recorded code/data caller in the index) | `23,699` | 44.18% |
| Unclassified data xref (manual review) | `216` | 0.40% |
| RTTI-only | `4` | 0.01% |

### Not-yet-recovered (1,963 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `401` | 20.43% |
| Vtable-anchored (recover with the owning class) | `179` | 9.12% |
| Framework dispatch (wx/EH/Lua/reflection) | `64` | 3.26% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `850` | 43.30% |
| No indexed callsite evidence (needs investigation/evidence) | `465` | 23.69% |
