...
- Total FAF functions: `67,167`
  - *IDA index, exported*
- Progress coverage:  **`98.79%`**
  - *Consists of `recovered` + `skip` + `external_dependency` ÷ exported*
  - *Total amount of completed tokens: `66,354`*

Progress DB status breakdown:

- `recovered`: `53,145` (80.09%)
- `skip`: `7,384` (11.13%) — proven ICF aliases / thunks / EH or static-init glue with no distinct source body
...
- **`blocked`: `870` (1.30%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `877`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `877`*
...
### Recovered (53,145 functions) — binary caller context

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `16,854` | 31.71% |
...
| No recovered caller token yet (orphan risk) | `1,745` | 3.28% |
| No callsite evidence (no recorded code/data caller in the index) | `22,876` | 43.04% |
| Unclassified data xref (manual review) | `216` | 0.41% |
| RTTI-only | `4` | 0.01% |

### Not-yet-recovered (877 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `31` | 3.53% |
| Vtable-anchored (recover with the owning class) | `109` | 12.43% |
| Framework dispatch (wx/EH/Lua/reflection) | `0` | 0.00% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `376` | 42.87% |
| No indexed callsite evidence (needs investigation/evidence) | `360` | 41.05% |
| Unclassified data xref (manual review) | `1` | 0.11% |
| RTTI-only | `0` | 0.00% |
