...
- Progress coverage:  **`97.11%`**
  ...
  - *Total amount of completed tokens: `65,229`*

Progress DB status breakdown:

- `recovered`: `53,351` (81.79%)
...
- **`blocked`: `1,970` (2.93%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `1,974`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `1,974`*
...
### Recovered (53,351 functions) — wiring health
...
### Not-yet-recovered (1,974 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Recoverable now** (`OK_RECOVERED_CALLER` — recovered caller exists; recover next) | `307` | 15.55% |
| Vtable-anchored (recover with the owning class) | `193` | 9.78% |
| Framework dispatch (wx/EH/Lua/reflection) | `35` | 1.77% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `1,002` | 50.76% |
| No callsite evidence (likely external/dead — candidates for `external_dependency`) | `435` | 22.04% |
