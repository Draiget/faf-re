...
- Total FAF functions: `67,167`
  - *IDA index, exported*
- Progress coverage:  **`99.07%`**
  - *Consists of `recovered` + `skip` + `external_dependency` ÷ exported*
  - *Total amount of completed tokens: `66,540`*

Progress DB status breakdown:

- `recovered`: `53,158` (79.89%)
- `skip`: `7,615` (11.44%) — proven ICF aliases / thunks / EH or static-init glue with no distinct source body
- `external_dependency`: `5,767` (8.67%) — proven third-party/import-boundary code
...
- **`blocked`: `692` (1.03%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `695`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `695`*
...
### Recovered (53,158 functions) — binary caller context

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `17,093` | 32.16% |
| Vtable-anchored (virtual override of a recovered class) | `5,882` | 11.07% |
| Framework dispatch (wx event, EH handler, Lua binding, reflection table, …) | `5,577` | 10.49% |
| No recovered caller token yet (orphan risk) | `1,697` | 3.19% |
| No callsite evidence (no recorded code/data caller in the index) | `22,689` | 42.68% |
| Unclassified data xref (manual review) | `216` | 0.41% |
| RTTI-only | `4` | 0.01% |

### Not-yet-recovered (695 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `11` | 1.58% |
| Vtable-anchored (recover with the owning class) | `109` | 15.68% |
| Framework dispatch (wx/EH/Lua/reflection) | `0` | 0.00% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `263` | 37.84% |
| No indexed callsite evidence (needs investigation/evidence) | `311` | 44.75% |
