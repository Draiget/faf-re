...
- Progress coverage:  **`95.93%`**
  ...
  - *Total amount of completed tokens: `64,433`*

Progress DB status breakdown:

- `recovered`: `52,747` (81.86%)
- `skip`: `6,256` (9.71%) — proven ICF aliases / thunks / EH or static-init glue with no distinct source body
...
- **`blocked`: `2,397` (3.57%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `2,805`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `2,805`*
...
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `16,331` | 30.96% |
...
| No callsite evidence (no recorded code/data caller in the index) | `22,906` | 43.43% |
| Unclassified data xref (manual review) | `216` | 0.41% |
| RTTI-only | `4` | 0.01% |

### Not-yet-recovered (2,805 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `363` | 12.94% |
| Vtable-anchored (recover with the owning class) | `144` | 5.13% |
| Framework dispatch (wx/EH/Lua/reflection) | `13` | 0.46% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `1,034` | 36.86% |
| No indexed callsite evidence (needs investigation/evidence) | `1,248` | 44.49% |
