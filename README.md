...
- Progress coverage:  **`97.90%`**
  ...
  - *Total amount of completed tokens: `65,758`*
...
- `external_dependency`: `5,581` (8.49%) — proven third-party/import-boundary code
...
- **`blocked`: `1,439` (2.14%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `1,472`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `1,472`*
...
### Not-yet-recovered (1,472 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `63` | 4.28% |
| Vtable-anchored (recover with the owning class) | `126` | 8.56% |
| Framework dispatch (wx/EH/Lua/reflection) | `8` | 0.54% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `680` | 46.20% |
| No indexed callsite evidence (needs investigation/evidence) | `594` | 40.35% |
