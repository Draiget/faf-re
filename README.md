...
- **`blocked`: `2,430` (3.62%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `2,838`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `2,838`*
...
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `16,282` | 30.87% |
...
| No callsite evidence (no recorded code/data caller in the index) | `22,943` | 43.50% |
| Unclassified data xref (manual review) | `216` | 0.41% |
| RTTI-only | `4` | 0.01% |

### Not-yet-recovered (2,838 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `405` | 14.27% |
| Vtable-anchored (recover with the owning class) | `144` | 5.07% |
| Framework dispatch (wx/EH/Lua/reflection) | `13` | 0.46% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `1,035` | 36.47% |
| No indexed callsite evidence (needs investigation/evidence) | `1,238` | 43.62% |
