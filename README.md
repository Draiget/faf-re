...
- **`blocked`: `2,226` (3.31%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `2,229`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `2,229`*
...
### Recovered (53,087 functions) — wiring health

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Confirmed caller** (recovered binary caller wired by name) | `14,970` | 28.20% |
...
### Not-yet-recovered (2,229 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Recoverable now** (`OK_RECOVERED_CALLER` — recovered caller exists; recover next) | `480` | 21.53% |
| Vtable-anchored (recover with the owning class) | `211` | 9.47% |
| Framework dispatch (wx/EH/Lua/reflection) | `35` | 1.57% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `1,060` | 47.55% |
| No callsite evidence (likely external/dead — candidates for `external_dependency`) | `441` | 19.78% |
