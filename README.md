...
- Progress coverage:  **`97.36%`**
  ...
  - *Total amount of completed tokens: `65,397`*

Progress DB status breakdown:

- `recovered`: `53,763` (82.21%)
...
### Recovered (53,763 functions) — binary caller context

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `16,036` | 29.83% |
...
| No recovered caller token yet (orphan risk) | `2,400` | 4.46% |
...
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `276` | 14.99% |
| Vtable-anchored (recover with the owning class) | `174` | 9.45% |
| Framework dispatch (wx/EH/Lua/reflection) | `1` | 0.05% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `926` | 50.30% |
| No indexed callsite evidence (needs investigation/evidence) | `461` | 25.04% |
