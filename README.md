...
- Progress coverage:  **`97.21%`**
  ...
  - *Total amount of completed tokens: `65,294`*

Progress DB status breakdown:

- `recovered`: `53,687` (82.22%)
...
- **`blocked`: `1,864` (2.78%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `1,930`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `1,930`*
...
### Recovered (53,687 functions) — binary caller context
...
### Not-yet-recovered (1,930 blocked + needs_evidence) — backlog readiness
...
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `911` | 47.20% |
| No indexed callsite evidence (needs investigation/evidence) | `460` | 23.83% |
