...
- Progress coverage:  **`96.91%`**
  ...
  - *Total amount of completed tokens: `65,094`*

Progress DB status breakdown:

- `recovered`: `53,537` (82.25%)
...
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `16,471` | 30.77% |
...
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `332` | 15.49% |
| Vtable-anchored (recover with the owning class) | `117` | 5.46% |
| Framework dispatch (wx/EH/Lua/reflection) | `1` | 0.05% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `779` | 36.35% |
| No indexed callsite evidence (needs investigation/evidence) | `912` | 42.56% |
