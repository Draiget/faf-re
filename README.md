...
- Total FAF functions: `67,167`
  - *IDA index, exported*
- Progress coverage:  **`98.94%`**
  - *Consists of `recovered` + `skip` + `external_dependency` ÷ exported*
  - *Total amount of completed tokens: `66,455`*

Progress DB status breakdown:

- `recovered`: `53,288` (80.19%)
- `skip`: `7,399` (11.13%) — proven ICF aliases / thunks / EH or static-init glue with no distinct source body
- `external_dependency`: `5,768` (8.68%) — proven third-party/import-boundary code
...
- **`blocked`: `781` (1.16%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `784`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `784`*
...
### Recovered (53,288 functions) — binary caller context

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `17,004` | 31.91% |
...
| No recovered caller token yet (orphan risk) | `1,707` | 3.20% |
| No callsite evidence (no recorded code/data caller in the index) | `22,907` | 42.99% |
| Unclassified data xref (manual review) | `216` | 0.41% |
| RTTI-only | `4` | 0.01% |

### Not-yet-recovered (784 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `24` | 3.06% |
| Vtable-anchored (recover with the owning class) | `109` | 13.90% |
| Framework dispatch (wx/EH/Lua/reflection) | `0` | 0.00% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `332` | 42.35% |
| No indexed callsite evidence (needs investigation/evidence) | `318` | 40.56% |
