...
- Progress coverage:  **`97.87%`**
  ...
  - *Total amount of completed tokens: `65,733`*

Progress DB status breakdown:

- `recovered`: `53,048` (80.70%)
- `skip`: `7,120` (10.83%) — proven ICF aliases / thunks / EH or static-init glue with no distinct source body
- `external_dependency`: `5,565` (8.47%) — proven third-party/import-boundary code
...
- **`blocked`: `1,464` (2.18%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `1,497`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `1,497`*
...
### Recovered (53,048 functions) — binary caller context

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `16,702` | 31.48% |
...
### Not-yet-recovered (1,497 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `66` | 4.41% |
| Vtable-anchored (recover with the owning class) | `126` | 8.42% |
| Framework dispatch (wx/EH/Lua/reflection) | `8` | 0.53% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `701` | 46.83% |
| No indexed callsite evidence (needs investigation/evidence) | `595` | 39.75% |
