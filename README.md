...
- Progress coverage:  **`96.86%`**
  ...
  - *Total amount of completed tokens: `65,057`*
...
- **`blocked`: `1,772` (2.64%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `2,180`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `2,180`*
...
### Not-yet-recovered (2,180 blocked + needs_evidence) — backlog readiness
...
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `771` | 35.37% |
| No indexed callsite evidence (needs investigation/evidence) | `919` | 42.16% |
