...
- Progress coverage:  **`99.05%`**
  ...
  - *Total amount of completed tokens: `66,531`*
...
- **`blocked`: `705` (1.05%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `708`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `708`*
...
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `17,076` | 32.00% |
...
| No recovered caller token yet (orphan risk) | `1,699` | 3.18% |
| No callsite evidence (no recorded code/data caller in the index) | `22,917` | 42.94% |
| Unclassified data xref (manual review) | `216` | 0.40% |
| RTTI-only | `4` | 0.01% |

### Not-yet-recovered (708 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `18` | 2.54% |
| Vtable-anchored (recover with the owning class) | `109` | 15.40% |
| Framework dispatch (wx/EH/Lua/reflection) | `0` | 0.00% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `267` | 37.71% |
| No indexed callsite evidence (needs investigation/evidence) | `313` | 44.21% |
