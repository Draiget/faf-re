...
- Total FAF functions: `67,167`
  - *IDA index, exported*
- Progress coverage:  **`97.30%`**
  - *Consists of `recovered` + `skip` + `external_dependency` ÷ exported*
  - *Total amount of completed tokens: `65,351`*

Progress DB status breakdown:

- `recovered`: `53,747` (82.24%)
...
- **`blocked`: `1,807` (2.69%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `1,873`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `1,873`*
...
| No callsite evidence (no recorded code/data caller in the index) | `23,720` | 44.13% |
| Unclassified data xref (manual review) | `216` | 0.40% |
| RTTI-only | `4` | 0.01% |

### Not-yet-recovered (1,873 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `309` | 16.50% |
| Vtable-anchored (recover with the owning class) | `178` | 9.50% |
| Framework dispatch (wx/EH/Lua/reflection) | `54` | 2.88% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `869` | 46.40% |
| No indexed callsite evidence (needs investigation/evidence) | `459` | 24.51% |
