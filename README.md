...
- `recovered`: `52,780` (80.73%)
- `skip`: `7,090` (10.84%) — proven ICF aliases / thunks / EH or static-init glue with no distinct source body
- `external_dependency`: `5,507` (8.42%) — proven third-party/import-boundary code
...
### Recovered (52,780 functions) — binary caller context
...
| No callsite evidence (no recorded code/data caller in the index) | `22,859` | 43.31% |
...
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `237` | 12.82% |
| Vtable-anchored (recover with the owning class) | `132` | 7.14% |
| Framework dispatch (wx/EH/Lua/reflection) | `8` | 0.43% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `862` | 46.62% |
| No indexed callsite evidence (needs investigation/evidence) | `608` | 32.88% |
