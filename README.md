...
- Total FAF functions: `67,167`
  - *IDA index, exported*
- Progress coverage:  **`96.89%`**
  - *Consists of `recovered` + `skip` + `external_dependency` ÷ exported*
  - *Total amount of completed tokens: `65,079`*

Progress DB status breakdown:

- `recovered`: `53,485` (82.18%)
- `skip`: `6,120` (9.40%) — proven ICF aliases / thunks / EH or static-init glue with no distinct source body
- `external_dependency`: `5,474` (8.41%) — proven third-party/import-boundary code
...
- **`blocked`: `1,749` (2.60%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `2,158`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `2,158`*
...
### Recovered (53,485 functions) — binary caller context

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `16,395` | 30.65% |
| Vtable-anchored (virtual override of a recovered class) | `5,893` | 11.02% |
| Framework dispatch (wx event, EH handler, Lua binding, reflection table, …) | `5,567` | 10.41% |
| No recovered caller token yet (orphan risk) | `2,150` | 4.02% |
| No callsite evidence (no recorded code/data caller in the index) | `23,260` | 43.49% |
| Unclassified data xref (manual review) | `216` | 0.40% |
| RTTI-only | `4` | 0.01% |

### Not-yet-recovered (2,158 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `346` | 16.03% |
| Vtable-anchored (recover with the owning class) | `123` | 5.70% |
| Framework dispatch (wx/EH/Lua/reflection) | `2` | 0.09% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `769` | 35.63% |
| No indexed callsite evidence (needs investigation/evidence) | `915` | 42.40% |
