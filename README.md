...
- Total FAF functions: `67,167`
  - *IDA index, exported*
- Progress coverage:  **`97.64%`**
  - *Consists of `recovered` + `skip` + `external_dependency` ÷ exported*
  - *Total amount of completed tokens: `65,580`*

Progress DB status breakdown:

- `recovered`: `54,044` (82.41%)
...
- **`blocked`: `1,249` (1.86%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `1,657`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `1,657`*
...
### Recovered (54,044 functions) — binary caller context

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `16,645` | 30.80% |
| Vtable-anchored (virtual override of a recovered class) | `5,912` | 10.94% |
| Framework dispatch (wx event, EH handler, Lua binding, reflection table, …) | `5,569` | 10.30% |
| No recovered caller token yet (orphan risk) | `2,054` | 3.80% |
| No callsite evidence (no recorded code/data caller in the index) | `23,643` | 43.75% |
| Unclassified data xref (manual review) | `217` | 0.40% |
| RTTI-only | `4` | 0.01% |

### Not-yet-recovered (1,657 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `292` | 17.62% |
| Vtable-anchored (recover with the owning class) | `111` | 6.70% |
| Framework dispatch (wx/EH/Lua/reflection) | `1` | 0.06% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `716` | 43.21% |
| No indexed callsite evidence (needs investigation/evidence) | `535` | 32.29% |
