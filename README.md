...
- Total FAF functions: `67,167`
  - *IDA index, exported*
- Progress coverage:  **`97.04%`**
  - *Consists of `recovered` + `skip` + `external_dependency` ÷ exported*
  - *Total amount of completed tokens: `65,177`*

Progress DB status breakdown:

- `recovered`: `53,631` (82.29%)
...
- **`blocked`: `1,652` (2.46%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `2,060`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `2,060`*
...
### Recovered (53,631 functions) — binary caller context

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `16,563` | 30.88% |
| Vtable-anchored (virtual override of a recovered class) | `5,896` | 10.99% |
| Framework dispatch (wx event, EH handler, Lua binding, reflection table, …) | `5,568` | 10.38% |
| No recovered caller token yet (orphan risk) | `2,118` | 3.95% |
| No callsite evidence (no recorded code/data caller in the index) | `23,265` | 43.38% |
| Unclassified data xref (manual review) | `217` | 0.40% |
| RTTI-only | `4` | 0.01% |

### Not-yet-recovered (2,060 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `275` | 13.35% |
| Vtable-anchored (recover with the owning class) | `116` | 5.63% |
| Framework dispatch (wx/EH/Lua/reflection) | `1` | 0.05% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `754` | 36.60% |
| No indexed callsite evidence (needs investigation/evidence) | `912` | 44.27% |
