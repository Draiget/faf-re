...
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `16,464` | 30.75% |
...
| No recovered caller token yet (orphan risk) | `2,119` | 3.96% |
...
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `335` | 15.60% |
| Vtable-anchored (recover with the owning class) | `117` | 5.45% |
| Framework dispatch (wx/EH/Lua/reflection) | `1` | 0.05% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `780` | 36.33% |
| No indexed callsite evidence (needs investigation/evidence) | `912` | 42.48% |
