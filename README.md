...
- Progress coverage:  **`96.53%`**
  ...
  - *Total amount of completed tokens: `64,833`*

Progress DB status breakdown:

- `recovered`: `53,206` (82.07%)
- `skip`: `6,130` (9.46%) — proven ICF aliases / thunks / EH or static-init glue with no distinct source body
- `external_dependency`: `5,497` (8.48%) — proven third-party/import-boundary code
...
- **`blocked`: `1,986` (2.96%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `2,404`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `2,404`*
...
### Recovered (53,206 functions) — binary caller context
...
| No recovered caller token yet (orphan risk) | `2,232` | 4.20% |
| No callsite evidence (no recorded code/data caller in the index) | `23,185` | 43.58% |
| Unclassified data xref (manual review) | `216` | 0.41% |
| RTTI-only | `4` | 0.01% |

### Not-yet-recovered (2,404 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `372` | 15.47% |
| Vtable-anchored (recover with the owning class) | `186` | 7.74% |
| Framework dispatch (wx/EH/Lua/reflection) | `45` | 1.87% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `814` | 33.86% |
| No indexed callsite evidence (needs investigation/evidence) | `983` | 40.89% |
