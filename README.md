...
- Progress coverage:  **`96.54%`**
  ...
  - *Total amount of completed tokens: `64,845`*

Progress DB status breakdown:

- `recovered`: `53,210` (82.06%)
- `skip`: `6,130` (9.45%) — proven ICF aliases / thunks / EH or static-init glue with no distinct source body
- `external_dependency`: `5,505` (8.49%) — proven third-party/import-boundary code
  - *libpng, zlib, wxWidgets, LuaPlus/Lua, external Boost internals, WildMagic/Wm3, CRI Sofdec/ADX, undname, bugsplat, CRT imports*
- `needs_evidence`: `414` (0.62%)
- `in_progress`: `0` (0.00%)
- **`blocked`: `1,978` (2.94%)**
  - *strict circular/dep-blocked (in-DB literal `status == "blocked"`)*  
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `2,392`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `2,392`*
...
### Recovered (53,210 functions) — binary caller context

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `16,230` | 30.50% |
...
| No recovered caller token yet (orphan risk) | `2,229` | 4.19% |
| No callsite evidence (no recorded code/data caller in the index) | `23,185` | 43.57% |
| Unclassified data xref (manual review) | `216` | 0.41% |
| RTTI-only | `4` | 0.01% |

### Not-yet-recovered (2,392 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `360` | 15.05% |
| Vtable-anchored (recover with the owning class) | `186` | 7.78% |
| Framework dispatch (wx/EH/Lua/reflection) | `45` | 1.88% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `814` | 34.03% |
| No indexed callsite evidence (needs investigation/evidence) | `983` | 41.10% |
