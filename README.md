...
- Progress coverage:  **`97.67%`**
  ...
  - *Total amount of completed tokens: `65,600`*

Progress DB status breakdown:

- `recovered`: `52,955` (80.72%)
- `skip`: `7,099` (10.82%) — proven ICF aliases / thunks / EH or static-init glue with no distinct source body
- `external_dependency`: `5,546` (8.45%) — proven third-party/import-boundary code
...
- **`blocked`: `1,595` (2.37%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `1,629`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `1,629`*
...
### Recovered (52,955 functions) — binary caller context

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `16,581` | 31.31% |
...
| No recovered caller token yet (orphan risk) | `1,832` | 3.46% |
| No callsite evidence (no recorded code/data caller in the index) | `22,873` | 43.19% |
| Unclassified data xref (manual review) | `216` | 0.41% |
| RTTI-only | `4` | 0.01% |

### Not-yet-recovered (1,629 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `147` | 9.02% |
| Vtable-anchored (recover with the owning class) | `129` | 7.92% |
| Framework dispatch (wx/EH/Lua/reflection) | `8` | 0.49% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `745` | 45.73% |
| No indexed callsite evidence (needs investigation/evidence) | `599` | 36.77% |
