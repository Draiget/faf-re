...
- **`blocked`: `855` (1.27%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `862`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `862`*
...
### Recovered (53,181 functions) — binary caller context

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `16,904` | 31.79% |
...
| No recovered caller token yet (orphan risk) | `1,731` | 3.25% |
| No callsite evidence (no recorded code/data caller in the index) | `22,876` | 43.02% |
| Unclassified data xref (manual review) | `216` | 0.41% |
| RTTI-only | `4` | 0.01% |

### Not-yet-recovered (862 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `27` | 3.13% |
| Vtable-anchored (recover with the owning class) | `109` | 12.65% |
| Framework dispatch (wx/EH/Lua/reflection) | `0` | 0.00% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `365` | 42.34% |
| No indexed callsite evidence (needs investigation/evidence) | `360` | 41.76% |
