...
- Progress coverage:  **`97.14%`**
  ...
  - *Total amount of completed tokens: `65,247`*

Progress DB status breakdown:

- `recovered`: `53,369` (81.80%)
...
- **`blocked`: `1,952` (2.91%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `1,956`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `1,956`*
...
### Recovered (53,369 functions) — wiring health

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Confirmed caller** (recovered binary caller wired by name) | `15,451` | 28.95% |
...
### Not-yet-recovered (1,956 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Recoverable now** (`OK_RECOVERED_CALLER` — recovered caller exists; recover next) | `306` | 15.64% |
| Vtable-anchored (recover with the owning class) | `188` | 9.61% |
| Framework dispatch (wx/EH/Lua/reflection) | `34` | 1.74% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `992` | 50.72% |
| No callsite evidence (likely external/dead — candidates for `external_dependency`) | `434` | 22.19% |
