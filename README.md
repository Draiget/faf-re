...
- Progress coverage:  **`97.18%`**
  ...
  - *Total amount of completed tokens: `65,276`*

Progress DB status breakdown:

- `recovered`: `53,659` (82.20%)
...
### Recovered (53,659 functions) — binary caller context

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `15,975` | 29.77% |
...
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `313` | 16.07% |
| Vtable-anchored (recover with the owning class) | `193` | 9.91% |
| Framework dispatch (wx/EH/Lua/reflection) | `56` | 2.87% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `921` | 47.28% |
| No indexed callsite evidence (needs investigation/evidence) | `461` | 23.67% |
