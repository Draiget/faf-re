...
- Progress coverage:  **`96.77%`**
  ...
  - *Total amount of completed tokens: `64,996`*

Progress DB status breakdown:

- `recovered`: `53,115` (81.72%)
...
- **`blocked`: `2,200` (3.28%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `2,203`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `2,203`*
...
### Recovered (53,115 functions) — wiring health

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Confirmed caller** (recovered binary caller wired by name) | `15,005` | 28.25% |
...
| Caller still blocked (orphan-helper risk — caller awaits recovery) | `2,833` | 5.33% |
| No callsite evidence (no recorded code/data caller in the index) | `23,746` | 44.71% |
| Unclassified data xref (manual review) | `215` | 0.40% |
| RTTI-only | `4` | 0.01% |

### Not-yet-recovered (2,203 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Recoverable now** (`OK_RECOVERED_CALLER` — recovered caller exists; recover next) | `460` | 20.88% |
| Vtable-anchored (recover with the owning class) | `211` | 9.58% |
| Framework dispatch (wx/EH/Lua/reflection) | `35` | 1.59% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `1,055` | 47.89% |
| No callsite evidence (likely external/dead — candidates for `external_dependency`) | `440` | 19.97% |
