...
## Recovery Coverage (`17/07/2026`, `fa_full_2026_03_26`)

Progress snapshot:

- Total FAF functions: `67,167`
  - *IDA index, exported*
- Progress coverage:  **`97.14%`**
  - *Consists of `recovered` + `skip` + `external_dependency` ÷ exported*
  - *Total amount of completed tokens: `65,248`*

Progress DB status breakdown:

- `recovered`: `53,370` (81.80%)
...
- **`blocked`: `1,951` (2.90%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `1,955`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `1,955`*
...
### Recovered (53,370 functions) — wiring health

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Confirmed caller** (recovered binary caller wired by name) | `15,452` | 28.95% |
| Vtable-anchored (virtual override of a recovered class) | `5,835` | 10.93% |
| Framework dispatch (wx event, EH handler, Lua binding, reflection table, …) | `5,499` | 10.30% |
| Caller still blocked (orphan-helper risk — caller awaits recovery) | `2,613` | 4.90% |
| No callsite evidence (no recorded code/data caller in the index) | `23,752` | 44.50% |
| Unclassified data xref (manual review) | `215` | 0.40% |
| RTTI-only | `4` | 0.01% |

### Not-yet-recovered (1,955 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Recoverable now** (`OK_RECOVERED_CALLER` — recovered caller exists; recover next) | `305` | 15.60% |
| Vtable-anchored (recover with the owning class) | `188` | 9.62% |
| Framework dispatch (wx/EH/Lua/reflection) | `34` | 1.74% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `992` | 50.74% |
| No callsite evidence (likely external/dead — candidates for `external_dependency`) | `434` | 22.20% |
