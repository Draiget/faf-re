...
- Total FAF functions: `67,167`
  - *IDA index, exported*
- Progress coverage:  **`95.89%`**
  - *Consists of `recovered` + `skip` + `external_dependency` ÷ exported*
  - *Total amount of completed tokens: `64,405`*

Progress DB status breakdown:

- `recovered`: `52,752` (81.91%)
...
- **`blocked`: `2,424` (3.61%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `2,832`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `2,832`*
...
### Recovered (52,752 functions) — binary caller context

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `16,287` | 30.87% |
...
| No recovered caller token yet (orphan risk) | `1,865` | 3.54% |
| No callsite evidence (no recorded code/data caller in the index) | `22,943` | 43.49% |
| Unclassified data xref (manual review) | `216` | 0.41% |
| RTTI-only | `4` | 0.01% |

### Not-yet-recovered (2,832 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `399` | 14.09% |
| Vtable-anchored (recover with the owning class) | `144` | 5.08% |
| Framework dispatch (wx/EH/Lua/reflection) | `13` | 0.46% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `1,035` | 36.55% |
| No indexed callsite evidence (needs investigation/evidence) | `1,238` | 43.71% |
