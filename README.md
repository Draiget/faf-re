...
- Progress coverage:  **`97.68%`**
  ...
  - *Total amount of completed tokens: `65,612`*

Progress DB status breakdown:

- `recovered`: `52,966` (80.73%)
- `skip`: `7,100` (10.82%) — proven ICF aliases / thunks / EH or static-init glue with no distinct source body
...
- **`blocked`: `1,583` (2.36%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `1,617`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `1,617`*
...
### Recovered (52,966 functions) — binary caller context

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `16,606` | 31.35% |
...
| No recovered caller token yet (orphan risk) | `1,818` | 3.43% |
| No callsite evidence (no recorded code/data caller in the index) | `22,873` | 43.18% |
| Unclassified data xref (manual review) | `216` | 0.41% |
| RTTI-only | `4` | 0.01% |

### Not-yet-recovered (1,617 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `144` | 8.91% |
| Vtable-anchored (recover with the owning class) | `129` | 7.98% |
| Framework dispatch (wx/EH/Lua/reflection) | `8` | 0.49% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `736` | 45.52% |
| No indexed callsite evidence (needs investigation/evidence) | `599` | 37.04% |
