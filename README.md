...
- Progress coverage:  **`98.94%`**
  ...
  - *Total amount of completed tokens: `66,452`*

Progress DB status breakdown:

- `recovered`: `53,288` (80.19%)
- `skip`: `7,396` (11.13%) — proven ICF aliases / thunks / EH or static-init glue with no distinct source body
...
- **`blocked`: `784` (1.17%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `787`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `787`*
...
### Not-yet-recovered (787 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `24` | 3.05% |
| Vtable-anchored (recover with the owning class) | `109` | 13.85% |
| Framework dispatch (wx/EH/Lua/reflection) | `0` | 0.00% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `334` | 42.44% |
| No indexed callsite evidence (needs investigation/evidence) | `319` | 40.53% |
