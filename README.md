...
- `recovered`: `53,084` (79.78%)
- `skip`: `7,690` (11.56%) — proven ICF aliases / thunks / EH or static-init glue with no distinct source body
...
### Recovered (53,084 functions) — binary caller context

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `17,093` | 32.20% |
| Vtable-anchored (virtual override of a recovered class) | `5,882` | 11.08% |
| Framework dispatch (wx event, EH handler, Lua binding, reflection table, …) | `5,578` | 10.51% |
| No recovered caller token yet (orphan risk) | `1,697` | 3.20% |
| No callsite evidence (no recorded code/data caller in the index) | `22,614` | 42.60% |
