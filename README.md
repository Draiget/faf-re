...
- Total FAF functions: `67,167`
  - *IDA index, exported*
- Progress coverage:  **`98.90%`**
  - *Consists of `recovered` + `skip` + `external_dependency` ÷ exported*
  - *Total amount of completed tokens: `66,426`*

Progress DB status breakdown:

- `recovered`: `53,258` (80.18%)
- `skip`: `7,391` (11.13%) — proven ICF aliases / thunks / EH or static-init glue with no distinct source body
- `external_dependency`: `5,777` (8.70%) — proven third-party/import-boundary code
...
- **`blocked`: `805` (1.20%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `808`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `808`*
...
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `16,979` | 31.88% |
...
| No recovered caller token yet (orphan risk) | `1,712` | 3.21% |
| No callsite evidence (no recorded code/data caller in the index) | `22,897` | 42.99% |
| Unclassified data xref (manual review) | `216` | 0.41% |
| RTTI-only | `4` | 0.01% |

### Not-yet-recovered (808 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `27` | 3.34% |
| Vtable-anchored (recover with the owning class) | `109` | 13.49% |
| Framework dispatch (wx/EH/Lua/reflection) | `0` | 0.00% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `343` | 42.45% |
| No indexed callsite evidence (needs investigation/evidence) | `328` | 40.59% |
