...
- Total FAF functions: `67,167`
  - *IDA index, exported*
- Progress coverage:  **`97.41%`**
  - *Consists of `recovered` + `skip` + `external_dependency` ÷ exported*
  - *Total amount of completed tokens: `65,427`*

Progress DB status breakdown:

- `recovered`: `53,806` (82.24%)
...
### Recovered (53,806 functions) — binary caller context

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `16,207` | 30.12% |
...
| No recovered caller token yet (orphan risk) | `2,299` | 4.27% |
| No callsite evidence (no recorded code/data caller in the index) | `23,720` | 44.08% |
| Unclassified data xref (manual review) | `216` | 0.40% |
| RTTI-only | `4` | 0.01% |

### Not-yet-recovered (1,797 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `295` | 16.42% |
| Vtable-anchored (recover with the owning class) | `162` | 9.02% |
| Framework dispatch (wx/EH/Lua/reflection) | `54` | 3.01% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `838` | 46.63% |
| No indexed callsite evidence (needs investigation/evidence) | `444` | 24.71% |
