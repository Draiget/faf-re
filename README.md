...
- `recovered`: `53,235` (80.00%)
- `skip`: `7,535` (11.32%) — proven ICF aliases / thunks / EH or static-init glue with no distinct source body
...
### Recovered (53,235 functions) — binary caller context

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `17,093` | 32.11% |
| Vtable-anchored (virtual override of a recovered class) | `5,882` | 11.05% |
| Framework dispatch (wx event, EH handler, Lua binding, reflection table, …) | `5,572` | 10.47% |
| No recovered caller token yet (orphan risk) | `1,697` | 3.19% |
| No callsite evidence (no recorded code/data caller in the index) | `22,771` | 42.77% |
