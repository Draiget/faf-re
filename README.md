...
- Progress coverage:  **`97.42%`**
  ...
  - *Total amount of completed tokens: `65,437`*

Progress DB status breakdown:

- `recovered`: `53,816` (82.24%)
...
- **`blocked`: `1,363` (2.03%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `1,787`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `1,787`*
...
### Recovered (53,816 functions) — binary caller context

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `16,227` | 30.15% |
...
| No recovered caller token yet (orphan risk) | `2,289` | 4.25% |
| No callsite evidence (no recorded code/data caller in the index) | `23,720` | 44.08% |
| Unclassified data xref (manual review) | `216` | 0.40% |
| RTTI-only | `4` | 0.01% |

### Not-yet-recovered (1,787 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `286` | 16.00% |
| Vtable-anchored (recover with the owning class) | `162` | 9.07% |
| Framework dispatch (wx/EH/Lua/reflection) | `54` | 3.02% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `837` | 46.84% |
| No indexed callsite evidence (needs investigation/evidence) | `444` | 24.85% |
