...
- Progress coverage:  **`97.21%`**
  ...
  - *Total amount of completed tokens: `65,296`*

Progress DB status breakdown:

- `recovered`: `53,689` (82.22%)
...
- **`blocked`: `1,862` (2.77%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `1,928`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `1,928`*
...
### Recovered (53,689 functions) — binary caller context

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `16,013` | 29.83% |
...
| No recovered caller token yet (orphan risk) | `2,401` | 4.47% |
| No callsite evidence (no recorded code/data caller in the index) | `23,719` | 44.18% |
| Unclassified data xref (manual review) | `216` | 0.40% |
| RTTI-only | `4` | 0.01% |

### Not-yet-recovered (1,928 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `310` | 16.08% |
| Vtable-anchored (recover with the owning class) | `189` | 9.80% |
| Framework dispatch (wx/EH/Lua/reflection) | `54` | 2.80% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `911` | 47.25% |
| No indexed callsite evidence (needs investigation/evidence) | `460` | 23.86% |
