...
- Progress coverage:  **`96.90%`**
  ...
  - *Total amount of completed tokens: `65,082`*

Progress DB status breakdown:

- `recovered`: `53,203` (81.75%)
...
- **`blocked`: `2,114` (3.15%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `2,117`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `2,117`*
...
### Recovered (53,203 functions) — wiring health

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Confirmed caller** (recovered binary caller wired by name) | `15,208` | 28.58% |
...
| Caller still blocked (orphan-helper risk — caller awaits recovery) | `2,717` | 5.11% |
| No callsite evidence (no recorded code/data caller in the index) | `23,747` | 44.63% |
| Unclassified data xref (manual review) | `215` | 0.40% |
| RTTI-only | `4` | 0.01% |

### Not-yet-recovered (2,117 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Recoverable now** (`OK_RECOVERED_CALLER` — recovered caller exists; recover next) | `385` | 18.19% |
| Vtable-anchored (recover with the owning class) | `210` | 9.92% |
| Framework dispatch (wx/EH/Lua/reflection) | `35` | 1.65% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `1,046` | 49.41% |
| No callsite evidence (likely external/dead — candidates for `external_dependency`) | `439` | 20.74% |
