...
- **`blocked`: `2,207` (3.29%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `2,210`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `2,210`*
...
### Recovered (53,107 functions) — wiring health

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Confirmed caller** (recovered binary caller wired by name) | `14,995` | 28.24% |
...
### Not-yet-recovered (2,210 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Recoverable now** (`OK_RECOVERED_CALLER` — recovered caller exists; recover next) | `467` | 21.13% |
| Vtable-anchored (recover with the owning class) | `211` | 9.55% |
| Framework dispatch (wx/EH/Lua/reflection) | `35` | 1.58% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `1,055` | 47.74% |
| No callsite evidence (likely external/dead — candidates for `external_dependency`) | `440` | 19.91% |
