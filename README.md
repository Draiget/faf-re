...
- Progress coverage:  **`96.70%`**
  ...
  - *Total amount of completed tokens: `64,952`*

Progress DB status breakdown:

- `recovered`: `53,348` (82.13%)
...
- **`blocked`: `1,876` (2.79%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `2,285`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `2,285`*
...
### Recovered (53,348 functions) — binary caller context

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `16,359` | 30.66% |
| Vtable-anchored (virtual override of a recovered class) | `5,866` | 11.00% |
| Framework dispatch (wx event, EH handler, Lua binding, reflection table, …) | `5,519` | 10.35% |
| No recovered caller token yet (orphan risk) | `2,169` | 4.07% |
| No callsite evidence (no recorded code/data caller in the index) | `23,215` | 43.52% |
| Unclassified data xref (manual review) | `216` | 0.40% |
| RTTI-only | `4` | 0.01% |

### Not-yet-recovered (2,285 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `352` | 15.40% |
| Vtable-anchored (recover with the owning class) | `151` | 6.61% |
| Framework dispatch (wx/EH/Lua/reflection) | `43` | 1.88% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `782` | 34.22% |
| No indexed callsite evidence (needs investigation/evidence) | `954` | 41.75% |
