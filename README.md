...
- Progress coverage:  **`97.04%`**
  ...
  - *Total amount of completed tokens: `65,181`*

Progress DB status breakdown:

- `recovered`: `53,635` (82.29%)
...
- **`blocked`: `1,648` (2.45%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `2,056`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `2,056`*
...
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `16,564` | 30.88% |
| Vtable-anchored (virtual override of a recovered class) | `5,897` | 10.99% |
| Framework dispatch (wx event, EH handler, Lua binding, reflection table, …) | `5,568` | 10.38% |
| No recovered caller token yet (orphan risk) | `2,119` | 3.95% |
| No callsite evidence (no recorded code/data caller in the index) | `23,266` | 43.38% |
| Unclassified data xref (manual review) | `217` | 0.40% |
| RTTI-only | `4` | 0.01% |

### Not-yet-recovered (2,056 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `277` | 13.47% |
| Vtable-anchored (recover with the owning class) | `115` | 5.59% |
| Framework dispatch (wx/EH/Lua/reflection) | `1` | 0.05% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `750` | 36.48% |
| No indexed callsite evidence (needs investigation/evidence) | `911` | 44.31% |
