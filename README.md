...
- Total FAF functions: `67,167`
  - *IDA index, exported*
- Progress coverage:  **`97.98%`**
  - *Consists of `recovered` + `skip` + `external_dependency` ÷ exported*
  - *Total amount of completed tokens: `65,807`*

Progress DB status breakdown:

- `recovered`: `53,059` (80.63%)
- `skip`: `7,128` (10.83%) — proven ICF aliases / thunks / EH or static-init glue with no distinct source body
- `external_dependency`: `5,620` (8.54%) — proven third-party/import-boundary code
...
- **`blocked`: `1,391` (2.07%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `1,424`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `1,424`*
...
### Not-yet-recovered (1,424 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `59` | 4.14% |
| Vtable-anchored (recover with the owning class) | `126` | 8.85% |
| Framework dispatch (wx/EH/Lua/reflection) | `8` | 0.56% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `650` | 45.65% |
| No indexed callsite evidence (needs investigation/evidence) | `580` | 40.73% |
