...
- **`blocked`: `1,538` (2.29%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `1,962`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `1,962`*
...
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `16,129` | 30.07% |
...
### Not-yet-recovered (1,962 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `400` | 20.39% |
| Vtable-anchored (recover with the owning class) | `179` | 9.12% |
| Framework dispatch (wx/EH/Lua/reflection) | `64` | 3.26% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `850` | 43.32% |
| No indexed callsite evidence (needs investigation/evidence) | `465` | 23.70% |
