...
- **`blocked`: `1,877` (2.79%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `1,944`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `1,944`*
...
### Recovered (53,664 functions) — binary caller context

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `15,989` | 29.79% |
...
### Not-yet-recovered (1,944 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `317` | 16.31% |
| Vtable-anchored (recover with the owning class) | `193` | 9.93% |
| Framework dispatch (wx/EH/Lua/reflection) | `56` | 2.88% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `913` | 46.97% |
| No indexed callsite evidence (needs investigation/evidence) | `461` | 23.71% |
