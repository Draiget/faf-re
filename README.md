...
- Progress coverage:  **`96.93%`**
  ...
  - *Total amount of completed tokens: `65,105`*

Progress DB status breakdown:

- `recovered`: `53,552` (82.25%)
...
- **`blocked`: `1,724` (2.57%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `2,132`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `2,132`*
...
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `16,492` | 30.80% |
...
| No callsite evidence (no recorded code/data caller in the index) | `23,265` | 43.44% |
| Unclassified data xref (manual review) | `217` | 0.41% |
| RTTI-only | `4` | 0.01% |

### Not-yet-recovered (2,132 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `325` | 15.24% |
| Vtable-anchored (recover with the owning class) | `117` | 5.49% |
| Framework dispatch (wx/EH/Lua/reflection) | `1` | 0.05% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `775` | 36.35% |
| No indexed callsite evidence (needs investigation/evidence) | `912` | 42.78% |
