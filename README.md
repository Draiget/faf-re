...
- Progress coverage:  **`96.75%`**
  ...
  - *Total amount of completed tokens: `64,981`*

Progress DB status breakdown:

- `recovered`: `52,755` (81.19%)
...
- **`blocked`: `1,840` (2.74%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `2,245`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `2,245`*
...
### Recovered (52,755 functions) — binary caller context

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `16,376` | 31.04% |
...
| No recovered caller token yet (orphan risk) | `1,869` | 3.54% |
| No callsite evidence (no recorded code/data caller in the index) | `22,852` | 43.32% |
| Unclassified data xref (manual review) | `216` | 0.41% |
| RTTI-only | `4` | 0.01% |

### Not-yet-recovered (2,245 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `239` | 10.65% |
| Vtable-anchored (recover with the owning class) | `133` | 5.92% |
| Framework dispatch (wx/EH/Lua/reflection) | `10` | 0.45% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `876` | 39.02% |
| No indexed callsite evidence (needs investigation/evidence) | `985` | 43.88% |
