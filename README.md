...
- `recovered`: `53,635` (82.19%)
- `skip`: `6,132` (9.40%) — proven ICF aliases / thunks / EH or static-init glue with no distinct source body
- `external_dependency`: `5,489` (8.41%) — proven third-party/import-boundary code
...
- **`blocked`: `1,544` (2.30%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `1,968`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `1,968`*
...
### Recovered (53,635 functions) — binary caller context

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `16,108` | 30.03% |
| Vtable-anchored (virtual override of a recovered class) | `5,839` | 10.89% |
| Framework dispatch (wx event, EH handler, Lua binding, reflection table, …) | `5,494` | 10.24% |
| No recovered caller token yet (orphan risk) | `2,275` | 4.24% |
| No callsite evidence (no recorded code/data caller in the index) | `23,699` | 44.19% |
| Unclassified data xref (manual review) | `216` | 0.40% |
| RTTI-only | `4` | 0.01% |

### Not-yet-recovered (1,968 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `404` | 20.53% |
| Vtable-anchored (recover with the owning class) | `179` | 9.10% |
| Framework dispatch (wx/EH/Lua/reflection) | `64` | 3.25% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `852` | 43.29% |
| No indexed callsite evidence (needs investigation/evidence) | `465` | 23.63% |
