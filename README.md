...
- Total FAF functions: `67,167`
  - *IDA index, exported*
- Progress coverage:  **`96.83%`**
  - *Consists of `recovered` + `skip` + `external_dependency` ÷ exported*
  - *Total amount of completed tokens: `65,040`*

Progress DB status breakdown:

- `recovered`: `53,433` (82.15%)
- `skip`: `6,131` (9.43%) — proven ICF aliases / thunks / EH or static-init glue with no distinct source body
- `external_dependency`: `5,476` (8.42%) — proven third-party/import-boundary code
...
- **`blocked`: `1,789` (2.66%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `2,197`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `2,197`*
...
### Recovered (53,433 functions) — binary caller context

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `16,382` | 30.66% |
| Vtable-anchored (virtual override of a recovered class) | `5,886` | 11.02% |
| Framework dispatch (wx event, EH handler, Lua binding, reflection table, …) | `5,540` | 10.37% |
| No recovered caller token yet (orphan risk) | `2,156` | 4.03% |
| No callsite evidence (no recorded code/data caller in the index) | `23,249` | 43.51% |
| Unclassified data xref (manual review) | `216` | 0.40% |
| RTTI-only | `4` | 0.01% |

### Not-yet-recovered (2,197 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `349` | 15.89% |
| Vtable-anchored (recover with the owning class) | `131` | 5.96% |
| Framework dispatch (wx/EH/Lua/reflection) | `22` | 1.00% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `772` | 35.14% |
| No indexed callsite evidence (needs investigation/evidence) | `920` | 41.88% |
