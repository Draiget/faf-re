...
- **`blocked`: `1,759` (2.62%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `2,167`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `2,167`*
...
### Recovered (53,535 functions) — binary caller context
...
### Not-yet-recovered (2,167 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `306` | 14.12% |
| Vtable-anchored (recover with the owning class) | `113` | 5.21% |
| Framework dispatch (wx/EH/Lua/reflection) | `1` | 0.05% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `761` | 35.12% |
| No indexed callsite evidence (needs investigation/evidence) | `984` | 45.41% |
