...
- Progress coverage:  **`96.38%`**
  ...
  - *Total amount of completed tokens: `64,738`*

Progress DB status breakdown:

- `recovered`: `53,123` (82.06%)
...
- **`blocked`: `2,077` (3.09%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `2,501`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `2,501`*
...
| Framework dispatch (wx event, EH handler, Lua binding, reflection table, …) | `5,501` | 10.36% |
...
### Not-yet-recovered (2,501 blocked + needs_evidence) — backlog readiness
...
| Framework dispatch (wx/EH/Lua/reflection) | `58` | 2.32% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `834` | 33.35% |
| No indexed callsite evidence (needs investigation/evidence) | `987` | 39.46% |
