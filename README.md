...
- Progress coverage:  **`97.60%`**
  ...
  - *Total amount of completed tokens: `65,557`*

Progress DB status breakdown:

- `recovered`: `52,941` (80.76%)
...
- **`blocked`: `1,638` (2.44%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `1,672`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `1,672`*
...
### Recovered (52,941 functions) — binary caller context

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `16,563` | 31.29% |
...
| No recovered caller token yet (orphan risk) | `1,836` | 3.47% |
| No callsite evidence (no recorded code/data caller in the index) | `22,873` | 43.20% |
| Unclassified data xref (manual review) | `216` | 0.41% |
| RTTI-only | `4` | 0.01% |

### Not-yet-recovered (1,672 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `162` | 9.69% |
| Vtable-anchored (recover with the owning class) | `129` | 7.72% |
| Framework dispatch (wx/EH/Lua/reflection) | `8` | 0.48% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `773` | 46.23% |
| No indexed callsite evidence (needs investigation/evidence) | `599` | 35.83% |
