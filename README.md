...
- Progress coverage:  **`95.77%`**
  ...
  - *Total amount of completed tokens: `64,326`*

Progress DB status breakdown:

- `recovered`: `52,675` (81.89%)
...
- **`blocked`: `2,503` (3.73%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `2,911`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `2,911`*
...
### Recovered (52,675 functions) — binary caller context

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `16,201` | 30.76% |
...
### Not-yet-recovered (2,911 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `472` | 16.21% |
| Vtable-anchored (recover with the owning class) | `144` | 4.95% |
| Framework dispatch (wx/EH/Lua/reflection) | `13` | 0.45% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `1,039` | 35.69% |
| No indexed callsite evidence (needs investigation/evidence) | `1,240` | 42.60% |
