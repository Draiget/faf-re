...
## Recovery Coverage (`08/07/2026`, `fa_full_2026_03_26`)
...
- **`blocked`: `2,130` (3.17%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `2,133`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `2,133`*
...
### Recovered (53,187 functions) — wiring health

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Confirmed caller** (recovered binary caller wired by name) | `15,168` | 28.52% |
| Vtable-anchored (virtual override of a recovered class) | `5,816` | 10.94% |
| Framework dispatch (wx event, EH handler, Lua binding, reflection table, …) | `5,496` | 10.33% |
| Caller still blocked (orphan-helper risk — caller awaits recovery) | `2,741` | 5.15% |
| No callsite evidence (no recorded code/data caller in the index) | `23,747` | 44.65% |
| Unclassified data xref (manual review) | `215` | 0.40% |
| RTTI-only | `4` | 0.01% |

### Not-yet-recovered (2,133 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Recoverable now** (`OK_RECOVERED_CALLER` — recovered caller exists; recover next) | `399` | 18.71% |
| Vtable-anchored (recover with the owning class) | `210` | 9.85% |
| Framework dispatch (wx/EH/Lua/reflection) | `35` | 1.64% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `1,048` | 49.13% |
| No callsite evidence (likely external/dead — candidates for `external_dependency`) | `439` | 20.58% |
