...
- Progress coverage:  **`96.55%`**
  ...
  - *Total amount of completed tokens: `64,847`*

Progress DB status breakdown:

- `recovered`: `53,210` (82.05%)
- `skip`: `6,130` (9.45%) — proven ICF aliases / thunks / EH or static-init glue with no distinct source body
- `external_dependency`: `5,507` (8.49%) — proven third-party/import-boundary code
...
- **`blocked`: `1,978` (2.94%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `2,390`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `2,390`*
...
### Not-yet-recovered (2,390 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `358` | 14.98% |
| Vtable-anchored (recover with the owning class) | `186` | 7.78% |
| Framework dispatch (wx/EH/Lua/reflection) | `45` | 1.88% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `814` | 34.06% |
| No indexed callsite evidence (needs investigation/evidence) | `983` | 41.13% |
