...
- Progress coverage:  **`97.08%`**
  ...
  - *Total amount of completed tokens: `65,205`*

Progress DB status breakdown:

- `recovered`: `53,327` (81.78%)
...
- **`blocked`: `1,993` (2.97%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `1,997`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `1,997`*
...
### Recovered (53,327 functions) — wiring health

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Confirmed caller** (recovered binary caller wired by name) | `15,415` | 28.91% |
...
| Caller still blocked (orphan-helper risk — caller awaits recovery) | `2,630` | 4.93% |
| No callsite evidence (no recorded code/data caller in the index) | `23,750` | 44.54% |
| Unclassified data xref (manual review) | `215` | 0.40% |
| RTTI-only | `4` | 0.01% |

### Not-yet-recovered (1,997 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Recoverable now** (`OK_RECOVERED_CALLER` — recovered caller exists; recover next) | `305` | 15.27% |
| Vtable-anchored (recover with the owning class) | `209` | 10.47% |
| Framework dispatch (wx/EH/Lua/reflection) | `35` | 1.75% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `1,011` | 50.63% |
| No callsite evidence (likely external/dead — candidates for `external_dependency`) | `435` | 21.78% |
