...
## Recovery Coverage (`16/07/2026`, `fa_full_2026_03_26`)

Progress snapshot:

- Total FAF functions: `67,167`
  - *IDA index, exported*
- Progress coverage:  **`97.09%`**
  - *Consists of `recovered` + `skip` + `external_dependency` ÷ exported*
  - *Total amount of completed tokens: `65,211`*

Progress DB status breakdown:

- `recovered`: `53,333` (81.79%)
...
- **`blocked`: `1,988` (2.96%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `1,992`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `1,992`*
...
### Recovered (53,333 functions) — wiring health

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Confirmed caller** (recovered binary caller wired by name) | `15,421` | 28.91% |
| Vtable-anchored (virtual override of a recovered class) | `5,817` | 10.91% |
| Framework dispatch (wx event, EH handler, Lua binding, reflection table, …) | `5,498` | 10.31% |
| Caller still blocked (orphan-helper risk — caller awaits recovery) | `2,627` | 4.93% |
| No callsite evidence (no recorded code/data caller in the index) | `23,751` | 44.53% |
| Unclassified data xref (manual review) | `215` | 0.40% |
| RTTI-only | `4` | 0.01% |

### Not-yet-recovered (1,992 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Recoverable now** (`OK_RECOVERED_CALLER` — recovered caller exists; recover next) | `303` | 15.21% |
| Vtable-anchored (recover with the owning class) | `207` | 10.39% |
| Framework dispatch (wx/EH/Lua/reflection) | `35` | 1.76% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `1,010` | 50.70% |
| No callsite evidence (likely external/dead — candidates for `external_dependency`) | `435` | 21.84% |
