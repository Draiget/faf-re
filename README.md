...
- **`blocked`: `1,635` (2.43%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `1,669`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `1,669`*
...
### Recovered (52,942 functions) — binary caller context

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `16,565` | 31.29% |
...
### Not-yet-recovered (1,669 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `159` | 9.53% |
| Vtable-anchored (recover with the owning class) | `129` | 7.73% |
| Framework dispatch (wx/EH/Lua/reflection) | `8` | 0.48% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `773` | 46.32% |
| No indexed callsite evidence (needs investigation/evidence) | `599` | 35.89% |
