...
- Total FAF functions: `67,167`
  - *IDA index, exported*
- Progress coverage:  **`96.95%`**
  - *Consists of `recovered` + `skip` + `external_dependency` ÷ exported*
  - *Total amount of completed tokens: `65,116`*

Progress DB status breakdown:

- `recovered`: `53,579` (82.28%)
...
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `16,625` | 31.03% |
...
| No recovered caller token yet (orphan risk) | `2,060` | 3.84% |
...
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `299` | 14.10% |
| Vtable-anchored (recover with the owning class) | `111` | 5.23% |
| Framework dispatch (wx/EH/Lua/reflection) | `1` | 0.05% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `722` | 34.04% |
| No indexed callsite evidence (needs investigation/evidence) | `986` | 46.49% |
