...
- Total FAF functions: `67,170`
  - *IDA index, exported*
- Progress coverage:  **`99.38%`**
  - *Consists of `recovered` + `skip` + `external_dependency` ÷ exported*
  - *Total amount of completed tokens: `66,756`*

Progress DB status breakdown:

- `recovered`: `52,772` (79.05%)
- `skip`: `8,252` (12.36%) — proven ICF aliases / thunks / EH or static-init glue with no distinct source body
...
- **`blocked`: `521` (0.78%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `524`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `524`*
...
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `17,195` | 32.58% |
...
| No recovered caller token yet (orphan risk) | `1,675` | 3.17% |
| No callsite evidence (no recorded code/data caller in the index) | `22,193` | 42.05% |
| Unclassified data xref (manual review) | `216` | 0.41% |
| RTTI-only | `4` | 0.01% |

### Not-yet-recovered (524 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `9` | 1.72% |
| Vtable-anchored (recover with the owning class) | `89` | 16.98% |
| Framework dispatch (wx/EH/Lua/reflection) | `0` | 0.00% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `144` | 27.48% |
| No indexed callsite evidence (needs investigation/evidence) | `282` | 53.82% |
