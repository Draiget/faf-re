...
- Progress coverage:  **`96.75%`**
  ...
  - *Total amount of completed tokens: `64,987`*

Progress DB status breakdown:

- `recovered`: `53,106` (81.72%)
...
### Recovered (53,106 functions) — wiring health

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Confirmed caller** (recovered binary caller wired by name) | `14,993` | 28.23% |
...
| Caller still blocked (orphan-helper risk — caller awaits recovery) | `2,835` | 5.34% |
...
|---|---:|---:|
| **Recoverable now** (`OK_RECOVERED_CALLER` — recovered caller exists; recover next) | `468` | 21.17% |
| Vtable-anchored (recover with the owning class) | `211` | 9.54% |
| Framework dispatch (wx/EH/Lua/reflection) | `35` | 1.58% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `1,055` | 47.72% |
| No callsite evidence (likely external/dead — candidates for `external_dependency`) | `440` | 19.90% |
