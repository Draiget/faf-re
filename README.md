...
- Progress coverage:  **`97.14%`**
  ...
  - *Total amount of completed tokens: `65,244`*

Progress DB status breakdown:

- `recovered`: `53,366` (81.79%)
...
- **`blocked`: `1,955` (2.91%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `1,959`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `1,959`*
...
### Recovered (53,366 functions) — wiring health

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Confirmed caller** (recovered binary caller wired by name) | `15,446` | 28.94% |
| Vtable-anchored (virtual override of a recovered class) | `5,834` | 10.93% |
| Framework dispatch (wx event, EH handler, Lua binding, reflection table, …) | `5,499` | 10.30% |
| Caller still blocked (orphan-helper risk — caller awaits recovery) | `2,616` | 4.90% |
| No callsite evidence (no recorded code/data caller in the index) | `23,752` | 44.51% |
| Unclassified data xref (manual review) | `215` | 0.40% |
| RTTI-only | `4` | 0.01% |

### Not-yet-recovered (1,959 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Recoverable now** (`OK_RECOVERED_CALLER` — recovered caller exists; recover next) | `306` | 15.62% |
| Vtable-anchored (recover with the owning class) | `189` | 9.65% |
| Framework dispatch (wx/EH/Lua/reflection) | `34` | 1.74% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `994` | 50.74% |
| No callsite evidence (likely external/dead — candidates for `external_dependency`) | `434` | 22.15% |
