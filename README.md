...
- Total FAF functions: `67,167`
  - *IDA index, exported*
- Progress coverage:  **`96.69%`**
  - *Consists of `recovered` + `skip` + `external_dependency` ÷ exported*
  - *Total amount of completed tokens: `64,943`*

Progress DB status breakdown:

- `recovered`: `53,337` (82.13%)
- `skip`: `6,128` (9.44%) — proven ICF aliases / thunks / EH or static-init glue with no distinct source body
- `external_dependency`: `5,478` (8.44%) — proven third-party/import-boundary code
...
- **`blocked`: `1,885` (2.81%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `2,294`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `2,294`*
...
### Recovered (53,337 functions) — binary caller context

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `16,340` | 30.64% |
| Vtable-anchored (virtual override of a recovered class) | `5,866` | 11.00% |
| Framework dispatch (wx event, EH handler, Lua binding, reflection table, …) | `5,516` | 10.34% |
| No recovered caller token yet (orphan risk) | `2,184` | 4.09% |
| No callsite evidence (no recorded code/data caller in the index) | `23,211` | 43.52% |
| Unclassified data xref (manual review) | `216` | 0.40% |
| RTTI-only | `4` | 0.01% |

### Not-yet-recovered (2,294 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `352` | 15.34% |
| Vtable-anchored (recover with the owning class) | `151` | 6.58% |
| Framework dispatch (wx/EH/Lua/reflection) | `44` | 1.92% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `785` | 34.22% |
| No indexed callsite evidence (needs investigation/evidence) | `959` | 41.80% |
