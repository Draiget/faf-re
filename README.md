...
- Progress coverage:  **`96.54%`**
  ...
  - *Total amount of completed tokens: `64,841`*

Progress DB status breakdown:

- `recovered`: `52,954` (81.67%)
...
### Recovered (52,954 functions) — wiring health

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Confirmed caller** (recovered binary caller wired by name) | `14,840` | 28.02% |
| Vtable-anchored (virtual override of a recovered class) | `5,793` | 10.94% |
| Framework dispatch (wx event, EH handler, Lua binding, reflection table, …) | `5,493` | 10.37% |
| Caller still blocked (orphan-helper risk — caller awaits recovery) | `2,877` | 5.43% |
| No callsite evidence (no recorded code/data caller in the index) | `23,732` | 44.82% |
...
|---|---:|---:|
| **Recoverable now** (`OK_RECOVERED_CALLER` — recovered caller exists; recover next) | `554` | 23.45% |
| Vtable-anchored (recover with the owning class) | `235` | 9.95% |
| Framework dispatch (wx/EH/Lua/reflection) | `36` | 1.52% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `1,085` | 45.94% |
| No callsite evidence (likely external/dead — candidates for `external_dependency`) | `450` | 19.05% |
