...
- Progress coverage:  **`97.74%`**
  ...
  - *Total amount of completed tokens: `65,647`*

Progress DB status breakdown:

- `recovered`: `52,987` (80.72%)
...
- **`blocked`: `1,548` (2.30%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `1,582`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `1,582`*
...
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `16,640` | 31.40% |
...
### Not-yet-recovered (1,582 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `120` | 7.59% |
| Vtable-anchored (recover with the owning class) | `129` | 8.15% |
| Framework dispatch (wx/EH/Lua/reflection) | `8` | 0.51% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `726` | 45.89% |
| No indexed callsite evidence (needs investigation/evidence) | `598` | 37.80% |
