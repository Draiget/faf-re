...
- Progress coverage:  **`96.76%`**
  ...
  - *Total amount of completed tokens: `64,993`*

Progress DB status breakdown:

- `recovered`: `52,767` (81.19%)
...
- **`blocked`: `1,828` (2.72%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `2,233`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `2,233`*
...
### Recovered (52,767 functions) — binary caller context
...
| No recovered caller token yet (orphan risk) | `1,863` | 3.53% |
| No callsite evidence (no recorded code/data caller in the index) | `22,856` | 43.31% |
| Unclassified data xref (manual review) | `216` | 0.41% |
| RTTI-only | `4` | 0.01% |

### Not-yet-recovered (2,233 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `239` | 10.70% |
| Vtable-anchored (recover with the owning class) | `132` | 5.91% |
| Framework dispatch (wx/EH/Lua/reflection) | `10` | 0.45% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `869` | 38.92% |
| No indexed callsite evidence (needs investigation/evidence) | `981` | 43.93% |
