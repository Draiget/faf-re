...
- Total FAF functions: `67,167`
  - *IDA index, exported*
- Progress coverage:  **`97.79%`**
  - *Consists of `recovered` + `skip` + `external_dependency` ÷ exported*
  - *Total amount of completed tokens: `65,680`*

Progress DB status breakdown:

- `recovered`: `53,018` (80.72%)
...
- **`blocked`: `1,515` (2.26%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `1,549`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `1,549`*
...
### Recovered (53,018 functions) — binary caller context

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `16,675` | 31.45% |
| Vtable-anchored (virtual override of a recovered class) | `5,885` | 11.10% |
| Framework dispatch (wx event, EH handler, Lua binding, reflection table, …) | `5,564` | 10.49% |
| No recovered caller token yet (orphan risk) | `1,801` | 3.40% |
| No callsite evidence (no recorded code/data caller in the index) | `22,873` | 43.14% |
| Unclassified data xref (manual review) | `216` | 0.41% |
| RTTI-only | `4` | 0.01% |

### Not-yet-recovered (1,549 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `95` | 6.13% |
| Vtable-anchored (recover with the owning class) | `129` | 8.33% |
| Framework dispatch (wx/EH/Lua/reflection) | `8` | 0.52% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `718` | 46.35% |
| No indexed callsite evidence (needs investigation/evidence) | `598` | 38.61% |
