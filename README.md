...
- Progress coverage:  **`98.95%`**
  ...
  - *Total amount of completed tokens: `66,460`*

Progress DB status breakdown:

- `recovered`: `53,290` (80.18%)
...
- **`blocked`: `776` (1.16%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `779`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `779`*
...
### Recovered (53,290 functions) — binary caller context

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `17,006` | 31.91% |
...
### Not-yet-recovered (779 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `24` | 3.08% |
| Vtable-anchored (recover with the owning class) | `109` | 13.99% |
| Framework dispatch (wx/EH/Lua/reflection) | `0` | 0.00% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `327` | 41.98% |
| No indexed callsite evidence (needs investigation/evidence) | `318` | 40.82% |
