...
- Progress coverage:  **`95.74%`**
  ...
  - *Total amount of completed tokens: `64,309`*

Progress DB status breakdown:

- `recovered`: `52,658` (81.88%)
...
- **`blocked`: `2,520` (3.75%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `2,928`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `2,928`*
...
### Recovered (52,658 functions) — binary caller context

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `16,184` | 30.73% |
...
### Not-yet-recovered (2,928 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `489` | 16.70% |
| Vtable-anchored (recover with the owning class) | `144` | 4.92% |
| Framework dispatch (wx/EH/Lua/reflection) | `13` | 0.44% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `1,039` | 35.48% |
| No indexed callsite evidence (needs investigation/evidence) | `1,240` | 42.35% |
