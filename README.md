...
- Progress coverage:  **`97.18%`**
  ...
  - *Total amount of completed tokens: `65,271`*

Progress DB status breakdown:

- `recovered`: `53,393` (81.80%)
...
- **`blocked`: `1,928` (2.87%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `1,932`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `1,932`*
...
### Recovered (53,393 functions) — wiring health

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Confirmed caller** (recovered binary caller wired by name) | `15,482` | 29.00% |
...
| Caller still blocked (orphan-helper risk — caller awaits recovery) | `2,604` | 4.88% |
| No callsite evidence (no recorded code/data caller in the index) | `23,752` | 44.49% |
| Unclassified data xref (manual review) | `215` | 0.40% |
| RTTI-only | `4` | 0.01% |

### Not-yet-recovered (1,932 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Recoverable now** (`OK_RECOVERED_CALLER` — recovered caller exists; recover next) | `297` | 15.37% |
| Vtable-anchored (recover with the owning class) | `186` | 9.63% |
| Framework dispatch (wx/EH/Lua/reflection) | `34` | 1.76% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `979` | 50.67% |
| No callsite evidence (likely external/dead — candidates for `external_dependency`) | `434` | 22.46% |
