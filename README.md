...
### Recovered (53,489 functions) — binary caller context

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `16,400` | 30.66% |
...
| No callsite evidence (no recorded code/data caller in the index) | `23,262` | 43.49% |
...
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `346` | 16.06% |
| Vtable-anchored (recover with the owning class) | `122` | 5.66% |
| Framework dispatch (wx/EH/Lua/reflection) | `1` | 0.05% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `769` | 35.68% |
| No indexed callsite evidence (needs investigation/evidence) | `914` | 42.41% |
