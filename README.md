...
- Progress coverage:  **`96.63%`**
  ...
  - *Total amount of completed tokens: `64,906`*

Progress DB status breakdown:

- `recovered`: `53,295` (82.11%)
...
- **`blocked`: `1,919` (2.86%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `2,329`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `2,329`*
...
### Recovered (53,295 functions) — binary caller context

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `16,315` | 30.61% |
| Vtable-anchored (virtual override of a recovered class) | `5,854` | 10.98% |
...
### Not-yet-recovered (2,329 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `350` | 15.03% |
| Vtable-anchored (recover with the owning class) | `163` | 7.00% |
| Framework dispatch (wx/EH/Lua/reflection) | `45` | 1.93% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `795` | 34.13% |
| No indexed callsite evidence (needs investigation/evidence) | `973` | 41.78% |
