...
- Progress coverage:  **`99.07%`**
  ...
  - *Total amount of completed tokens: `66,542`*

Progress DB status breakdown:

- `recovered`: `53,012` (79.67%)
- `skip`: `7,764` (11.67%) — proven ICF aliases / thunks / EH or static-init glue with no distinct source body
- `external_dependency`: `5,766` (8.67%) — proven third-party/import-boundary code
...
### Recovered (53,012 functions) — binary caller context

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `17,093` | 32.24% |
| Vtable-anchored (virtual override of a recovered class) | `5,884` | 11.10% |
| Framework dispatch (wx event, EH handler, Lua binding, reflection table, …) | `5,578` | 10.52% |
| No recovered caller token yet (orphan risk) | `1,697` | 3.20% |
| No callsite evidence (no recorded code/data caller in the index) | `22,540` | 42.52% |
