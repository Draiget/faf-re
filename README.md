...
## Recovery Coverage (`04/06/2026`, `fa_full_2026_03_26`)
...
- Progress coverage:  **`96.62%`**
  ...
  - *Total amount of completed tokens: `64,894`*

Progress DB status breakdown:

- `recovered`: `53,007` (81.68%)
...
- **`blocked`: `2,305` (3.43%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `2,308`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `2,308`*
...
### Recovered (53,007 functions) — wiring health

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Confirmed caller** (recovered binary caller wired by name) | `14,918` | 28.14% |
| Vtable-anchored (virtual override of a recovered class) | `5,796` | 10.93% |
| Framework dispatch (wx event, EH handler, Lua binding, reflection table, …) | `5,494` | 10.36% |
| Caller still blocked (orphan-helper risk — caller awaits recovery) | `2,847` | 5.37% |
| No callsite evidence (no recorded code/data caller in the index) | `23,733` | 44.77% |
| Unclassified data xref (manual review) | `215` | 0.41% |
| RTTI-only | `4` | 0.01% |

### Not-yet-recovered (2,308 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Recoverable now** (`OK_RECOVERED_CALLER` — recovered caller exists; recover next) | `513` | 22.23% |
| Vtable-anchored (recover with the owning class) | `233` | 10.10% |
| Framework dispatch (wx/EH/Lua/reflection) | `35` | 1.52% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `1,076` | 46.62% |
| No callsite evidence (likely external/dead — candidates for `external_dependency`) | `449` | 19.45% |
