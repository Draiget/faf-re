...
### Recovered (53,536 functions) — binary caller context

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `16,470` | 30.76% |
...
| No recovered caller token yet (orphan risk) | `2,116` | 3.95% |
...
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `333` | 15.53% |
| Vtable-anchored (recover with the owning class) | `117` | 5.46% |
| Framework dispatch (wx/EH/Lua/reflection) | `1` | 0.05% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `779` | 36.33% |
| No indexed callsite evidence (needs investigation/evidence) | `912` | 42.54% |
