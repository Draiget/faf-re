...
- Progress coverage:  **`99.25%`**
  ...
  - *Total amount of completed tokens: `66,669`*

Progress DB status breakdown:

- `recovered`: `52,744` (79.11%)
- `skip`: `8,193` (12.29%) — proven ICF aliases / thunks / EH or static-init glue with no distinct source body
...
- **`blocked`: `608` (0.91%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `611`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `611`*
...
### Not-yet-recovered (611 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `9` | 1.47% |
| Vtable-anchored (recover with the owning class) | `106` | 17.35% |
| Framework dispatch (wx/EH/Lua/reflection) | `0` | 0.00% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `190` | 31.10% |
| No indexed callsite evidence (needs investigation/evidence) | `305` | 49.92% |
