...
- Progress coverage:  **`97.11%`**
  ...
  - *Total amount of completed tokens: `65,228`*

Progress DB status breakdown:

- `recovered`: `53,350` (81.79%)
...
- **`blocked`: `1,971` (2.93%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `1,975`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `1,975`*
...
### Recovered (53,350 functions) — wiring health

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Confirmed caller** (recovered binary caller wired by name) | `15,433` | 28.93% |
| Vtable-anchored (virtual override of a recovered class) | `5,829` | 10.93% |
...
### Not-yet-recovered (1,975 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Recoverable now** (`OK_RECOVERED_CALLER` — recovered caller exists; recover next) | `307` | 15.54% |
| Vtable-anchored (recover with the owning class) | `194` | 9.82% |
| Framework dispatch (wx/EH/Lua/reflection) | `35` | 1.77% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `1,002` | 50.73% |
| No callsite evidence (likely external/dead — candidates for `external_dependency`) | `435` | 22.03% |
