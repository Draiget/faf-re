...
- `recovered`: `52,944` (80.73%)
- `skip`: `7,092` (10.81%) — proven ICF aliases / thunks / EH or static-init glue with no distinct source body
- `external_dependency`: `5,544` (8.45%) — proven third-party/import-boundary code
...
- **`blocked`: `1,615` (2.40%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `1,649`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `1,649`*
...
### Recovered (52,944 functions) — binary caller context

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `16,567` | 31.29% |
...
| No recovered caller token yet (orphan risk) | `1,835` | 3.47% |
| No callsite evidence (no recorded code/data caller in the index) | `22,873` | 43.20% |
| Unclassified data xref (manual review) | `216` | 0.41% |
| RTTI-only | `4` | 0.01% |

### Not-yet-recovered (1,649 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `156` | 9.46% |
| Vtable-anchored (recover with the owning class) | `129` | 7.82% |
| Framework dispatch (wx/EH/Lua/reflection) | `8` | 0.49% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `756` | 45.85% |
| No indexed callsite evidence (needs investigation/evidence) | `599` | 36.33% |
