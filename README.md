...
- Progress coverage:  **`96.45%`**
  ...
  - *Total amount of completed tokens: `64,780`*

Progress DB status breakdown:

- `recovered`: `53,156` (82.06%)
...
- **`blocked`: `2,040` (3.04%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `2,458`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `2,458`*
...
### Recovered (53,156 functions) — binary caller context

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `16,189` | 30.46% |
...
| No callsite evidence (no recorded code/data caller in the index) | `23,185` | 43.62% |
| Unclassified data xref (manual review) | `216` | 0.41% |
| RTTI-only | `4` | 0.01% |

### Not-yet-recovered (2,458 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `386` | 15.70% |
| Vtable-anchored (recover with the owning class) | `203` | 8.26% |
| Framework dispatch (wx/EH/Lua/reflection) | `55` | 2.24% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `827` | 33.65% |
| No indexed callsite evidence (needs investigation/evidence) | `983` | 39.99% |
