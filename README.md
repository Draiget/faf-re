...
- Total FAF functions: `67,167`
  - *IDA index, exported*
- Progress coverage:  **`97.71%`**
  - *Consists of `recovered` + `skip` + `external_dependency` ÷ exported*
  - *Total amount of completed tokens: `65,627`*
...
- **`blocked`: `1,568` (2.33%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `1,602`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `1,602`*
...
### Not-yet-recovered (1,602 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `135` | 8.43% |
| Vtable-anchored (recover with the owning class) | `129` | 8.05% |
| Framework dispatch (wx/EH/Lua/reflection) | `8` | 0.50% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `730` | 45.57% |
| No indexed callsite evidence (needs investigation/evidence) | `599` | 37.39% |
