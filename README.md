...
- Progress coverage:  **`96.67%`**
  ...
  - *Total amount of completed tokens: `64,929`*

Progress DB status breakdown:

- `recovered`: `53,323` (82.13%)
...
- **`blocked`: `1,897` (2.82%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `2,307`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `2,307`*
...
### Recovered (53,323 functions) — binary caller context

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `16,327` | 30.62% |
| Vtable-anchored (virtual override of a recovered class) | `5,863` | 11.00% |
| Framework dispatch (wx event, EH handler, Lua binding, reflection table, …) | `5,515` | 10.34% |
| No recovered caller token yet (orphan risk) | `2,194` | 4.11% |
| No callsite evidence (no recorded code/data caller in the index) | `23,204` | 43.52% |
| Unclassified data xref (manual review) | `216` | 0.41% |
| RTTI-only | `4` | 0.01% |

### Not-yet-recovered (2,307 blocked + needs_evidence) — backlog readiness
...
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `787` | 34.11% |
| No indexed callsite evidence (needs investigation/evidence) | `966` | 41.87% |
