...
- Progress coverage:  **`96.67%`**
  ...
  - *Total amount of completed tokens: `64,931`*

Progress DB status breakdown:

- `recovered`: `53,325` (82.13%)
...
- **`blocked`: `1,896` (2.82%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `2,306`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `2,306`*
...
### Recovered (53,325 functions) — binary caller context

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `16,330` | 30.62% |
| Vtable-anchored (virtual override of a recovered class) | `5,863` | 10.99% |
| Framework dispatch (wx event, EH handler, Lua binding, reflection table, …) | `5,515` | 10.34% |
| No recovered caller token yet (orphan risk) | `2,192` | 4.11% |
| No callsite evidence (no recorded code/data caller in the index) | `23,205` | 43.52% |
| Unclassified data xref (manual review) | `216` | 0.41% |
| RTTI-only | `4` | 0.01% |

### Not-yet-recovered (2,306 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `352` | 15.26% |
| Vtable-anchored (recover with the owning class) | `154` | 6.68% |
| Framework dispatch (wx/EH/Lua/reflection) | `45` | 1.95% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `787` | 34.13% |
| No indexed callsite evidence (needs investigation/evidence) | `965` | 41.85% |
