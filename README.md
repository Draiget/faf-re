...
- **`blocked`: `689` (1.03%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `692`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `692`*
...
### Not-yet-recovered (692 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `10` | 1.45% |
| Vtable-anchored (recover with the owning class) | `108` | 15.61% |
| Framework dispatch (wx/EH/Lua/reflection) | `0` | 0.00% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `262` | 37.86% |
| No indexed callsite evidence (needs investigation/evidence) | `311` | 44.94% |
