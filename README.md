...
- Progress coverage:  **`96.65%`**
  ...
  - *Total amount of completed tokens: `64,920`*

Progress DB status breakdown:

- `recovered`: `53,311` (82.12%)
...
- **`blocked`: `1,906` (2.84%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `2,316`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `2,316`*
...
### Recovered (53,311 functions) — binary caller context

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `16,319` | 30.61% |
...
| No recovered caller token yet (orphan risk) | `2,199` | 4.12% |
| No callsite evidence (no recorded code/data caller in the index) | `23,196` | 43.51% |
| Unclassified data xref (manual review) | `216` | 0.41% |
| RTTI-only | `4` | 0.01% |

### Not-yet-recovered (2,316 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `352` | 15.20% |
| Vtable-anchored (recover with the owning class) | `154` | 6.65% |
| Framework dispatch (wx/EH/Lua/reflection) | `45` | 1.94% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `789` | 34.07% |
| No indexed callsite evidence (needs investigation/evidence) | `973` | 42.01% |
