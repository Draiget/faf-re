...
- Progress coverage:  **`99.29%`**
  ...
  - *Total amount of completed tokens: `66,691`*

Progress DB status breakdown:

- `recovered`: `52,746` (79.09%)
...
- **`blocked`: `586` (0.87%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `589`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `589`*
...
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `17,166` | 32.54% |
...
### Not-yet-recovered (589 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `9` | 1.53% |
| Vtable-anchored (recover with the owning class) | `106` | 18.00% |
| Framework dispatch (wx/EH/Lua/reflection) | `0` | 0.00% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `168` | 28.52% |
| No indexed callsite evidence (needs investigation/evidence) | `305` | 51.78% |
