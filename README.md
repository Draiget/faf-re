...
- Progress coverage:  **`96.58%`**
  ...
  - *Total amount of completed tokens: `64,868`*

Progress DB status breakdown:

- `recovered`: `52,981` (81.68%)
...
### Recovered (52,981 functions) — wiring health

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Confirmed caller** (recovered binary caller wired by name) | `14,879` | 28.08% |
...
| Caller still blocked (orphan-helper risk — caller awaits recovery) | `2,860` | 5.40% |
...
|---|---:|---:|
| **Recoverable now** (`OK_RECOVERED_CALLER` — recovered caller exists; recover next) | `534` | 22.87% |
| Vtable-anchored (recover with the owning class) | `233` | 9.98% |
| Framework dispatch (wx/EH/Lua/reflection) | `35` | 1.50% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `1,082` | 46.34% |
| No callsite evidence (likely external/dead — candidates for `external_dependency`) | `449` | 19.23% |
