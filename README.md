...
- Total FAF functions: `67,167`
  - *IDA index, exported*
- Progress coverage:  **`98.40%`**
  - *Consists of `recovered` + `skip` + `external_dependency` ÷ exported*
  - *Total amount of completed tokens: `66,091`*

Progress DB status breakdown:

- `recovered`: `53,066` (80.29%)
- `skip`: `7,373` (11.16%) — proven ICF aliases / thunks / EH or static-init glue with no distinct source body
- `external_dependency`: `5,652` (8.55%) — proven third-party/import-boundary code
...
- **`blocked`: `1,107` (1.65%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `1,140`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `1,140`*
...
### Recovered (53,066 functions) — binary caller context

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `16,732` | 31.53% |
...
| No callsite evidence (no recorded code/data caller in the index) | `22,874` | 43.10% |
| Unclassified data xref (manual review) | `216` | 0.41% |
| RTTI-only | `4` | 0.01% |

### Not-yet-recovered (1,140 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `53` | 4.65% |
| Vtable-anchored (recover with the owning class) | `124` | 10.88% |
| Framework dispatch (wx/EH/Lua/reflection) | `8` | 0.70% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `585` | 51.32% |
| No indexed callsite evidence (needs investigation/evidence) | `369` | 32.37% |
