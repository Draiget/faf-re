...
- Total FAF functions: `67,167`
  - *IDA index, exported*
- Progress coverage:  **`98.91%`**
  - *Consists of `recovered` + `skip` + `external_dependency` ÷ exported*
  - *Total amount of completed tokens: `66,434`*

Progress DB status breakdown:

- `recovered`: `53,278` (80.20%)
- `skip`: `7,389` (11.12%) — proven ICF aliases / thunks / EH or static-init glue with no distinct source body
- `external_dependency`: `5,767` (8.68%) — proven third-party/import-boundary code
...
- **`blocked`: `797` (1.19%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `800`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `800`*
...
### Recovered (53,278 functions) — binary caller context

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `16,988` | 31.89% |
...
| No callsite evidence (no recorded code/data caller in the index) | `22,905` | 42.99% |
| Unclassified data xref (manual review) | `216` | 0.41% |
| RTTI-only | `4` | 0.01% |

### Not-yet-recovered (800 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `25` | 3.12% |
| Vtable-anchored (recover with the owning class) | `109` | 13.63% |
| Framework dispatch (wx/EH/Lua/reflection) | `0` | 0.00% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `342` | 42.75% |
| No indexed callsite evidence (needs investigation/evidence) | `323` | 40.38% |
