...
- Progress coverage:  **`97.57%`**
  ...
  - *Total amount of completed tokens: `65,532`*

Progress DB status breakdown:

- `recovered`: `52,918` (80.75%)
- `skip`: `7,092` (10.82%) — proven ICF aliases / thunks / EH or static-init glue with no distinct source body
- `external_dependency`: `5,522` (8.43%) — proven third-party/import-boundary code
...
- **`blocked`: `1,663` (2.48%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `1,697`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `1,697`*
...
### Recovered (52,918 functions) — binary caller context

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `16,537` | 31.25% |
...
| No recovered caller token yet (orphan risk) | `1,839` | 3.48% |
| No callsite evidence (no recorded code/data caller in the index) | `22,873` | 43.22% |
| Unclassified data xref (manual review) | `216` | 0.41% |
| RTTI-only | `4` | 0.01% |

### Not-yet-recovered (1,697 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `180` | 10.61% |
| Vtable-anchored (recover with the owning class) | `129` | 7.60% |
| Framework dispatch (wx/EH/Lua/reflection) | `8` | 0.47% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `780` | 45.96% |
| No indexed callsite evidence (needs investigation/evidence) | `599` | 35.30% |
