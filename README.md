...
- Progress coverage:  **`99.39%`**
  ...
  - *Total amount of completed tokens: `66,763`*
...
- `external_dependency`: `5,737` (8.59%) — proven third-party/import-boundary code
...
- **`blocked`: `517` (0.77%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `520`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `520`*
...
### Not-yet-recovered (520 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `9` | 1.73% |
| Vtable-anchored (recover with the owning class) | `89` | 17.12% |
| Framework dispatch (wx/EH/Lua/reflection) | `0` | 0.00% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `143` | 27.50% |
| No indexed callsite evidence (needs investigation/evidence) | `279` | 53.65% |
