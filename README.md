...
- **`blocked`: `2,234` (3.33%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `2,237`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `2,237`*
...
### Recovered (53,079 functions) — wiring health

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Confirmed caller** (recovered binary caller wired by name) | `14,962` | 28.19% |
...
### Not-yet-recovered (2,237 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Recoverable now** (`OK_RECOVERED_CALLER` — recovered caller exists; recover next) | `484` | 21.64% |
| Vtable-anchored (recover with the owning class) | `211` | 9.43% |
| Framework dispatch (wx/EH/Lua/reflection) | `35` | 1.56% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `1,064` | 47.56% |
| No callsite evidence (likely external/dead — candidates for `external_dependency`) | `441` | 19.71% |
