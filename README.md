...
- Progress coverage:  **`98.79%`**
  ...
  - *Total amount of completed tokens: `66,357`*

Progress DB status breakdown:

- `recovered`: `53,148` (80.09%)
- `skip`: `7,383` (11.13%) — proven ICF aliases / thunks / EH or static-init glue with no distinct source body
- `external_dependency`: `5,826` (8.78%) — proven third-party/import-boundary code
...
- **`blocked`: `867` (1.29%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `874`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `874`*
...
### Recovered (53,148 functions) — binary caller context

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `16,860` | 31.72% |
...
| No recovered caller token yet (orphan risk) | `1,742` | 3.28% |
| No callsite evidence (no recorded code/data caller in the index) | `22,876` | 43.04% |
| Unclassified data xref (manual review) | `216` | 0.41% |
| RTTI-only | `4` | 0.01% |

### Not-yet-recovered (874 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `31` | 3.55% |
| Vtable-anchored (recover with the owning class) | `109` | 12.47% |
| Framework dispatch (wx/EH/Lua/reflection) | `0` | 0.00% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `373` | 42.68% |
| No indexed callsite evidence (needs investigation/evidence) | `360` | 41.19% |
