...
- Progress coverage:  **`97.15%`**
  ...
  - *Total amount of completed tokens: `65,255`*

Progress DB status breakdown:

- `recovered`: `53,377` (81.80%)
...
- **`blocked`: `1,944` (2.89%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `1,948`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `1,948`*
...
### Recovered (53,377 functions) — wiring health

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Confirmed caller** (recovered binary caller wired by name) | `15,459` | 28.96% |
...
### Not-yet-recovered (1,948 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Recoverable now** (`OK_RECOVERED_CALLER` — recovered caller exists; recover next) | `302` | 15.50% |
| Vtable-anchored (recover with the owning class) | `186` | 9.55% |
| Framework dispatch (wx/EH/Lua/reflection) | `34` | 1.75% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `990` | 50.82% |
| No callsite evidence (likely external/dead — candidates for `external_dependency`) | `434` | 22.28% |
