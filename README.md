...
- Total FAF functions: `67,170`
  - *IDA index, exported*
- Progress coverage:  **`99.32%`**
  - *Consists of `recovered` + `skip` + `external_dependency` ÷ exported*
  - *Total amount of completed tokens: `66,713`*

Progress DB status breakdown:

- `recovered`: `52,749` (79.07%)
- `skip`: `8,220` (12.32%) — proven ICF aliases / thunks / EH or static-init glue with no distinct source body
...
- **`blocked`: `564` (0.84%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `567`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `567`*
...
### Not-yet-recovered (567 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `9` | 1.59% |
| Vtable-anchored (recover with the owning class) | `105` | 18.52% |
| Framework dispatch (wx/EH/Lua/reflection) | `0` | 0.00% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `156` | 27.51% |
| No indexed callsite evidence (needs investigation/evidence) | `296` | 52.20% |
