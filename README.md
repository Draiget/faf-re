...
- Progress coverage:  **`96.40%`**
  ...
  - *Total amount of completed tokens: `64,750`*

Progress DB status breakdown:

- `recovered`: `53,130` (82.05%)
...
- `needs_evidence`: `419` (0.62%)
- `in_progress`: `0` (0.00%)
- **`blocked`: `2,069` (3.08%)**
  - *strict circular/dep-blocked (in-DB literal `status == "blocked"`)*  
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `2,488`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `2,488`*
...
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `16,144` | 30.39% |
...
| No recovered caller token yet (orphan risk) | `2,265` | 4.26% |
| No callsite evidence (no recorded code/data caller in the index) | `23,181` | 43.63% |
| Unclassified data xref (manual review) | `216` | 0.41% |
| RTTI-only | `4` | 0.01% |

### Not-yet-recovered (2,488 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `408` | 16.40% |
| Vtable-anchored (recover with the owning class) | `203` | 8.16% |
| Framework dispatch (wx/EH/Lua/reflection) | `55` | 2.21% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `831` | 33.40% |
| No indexed callsite evidence (needs investigation/evidence) | `987` | 39.67% |
