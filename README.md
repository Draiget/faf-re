...
## Recovery Coverage (`30/07/2026`, `fa_full_2026_03_26`)

Progress snapshot:

- Total FAF functions: `67,167`
  - *IDA index, exported*
- Progress coverage:  **`97.33%`**
  - *Consists of `recovered` + `skip` + `external_dependency` ÷ exported*
  - *Total amount of completed tokens: `65,374`*

Progress DB status breakdown:

- `recovered`: `53,640` (82.05%)
- `skip`: `6,133` (9.38%) — proven ICF aliases / thunks / EH or static-init glue with no distinct source body
- `external_dependency`: `5,601` (8.57%) — proven third-party/import-boundary code
...
- **`blocked`: `1,791` (2.67%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `1,859`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `1,859`*
...
### Recovered (53,640 functions) — binary caller context

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `15,868` | 29.58% |
| Vtable-anchored (virtual override of a recovered class) | `5,843` | 10.89% |
| Framework dispatch (wx event, EH handler, Lua binding, reflection table, …) | `5,542` | 10.33% |
| No recovered caller token yet (orphan risk) | `2,453` | 4.57% |
| No callsite evidence (no recorded code/data caller in the index) | `23,714` | 44.21% |
| Unclassified data xref (manual review) | `216` | 0.40% |
| RTTI-only | `4` | 0.01% |

### Not-yet-recovered (1,859 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `279` | 15.01% |
| Vtable-anchored (recover with the owning class) | `179` | 9.63% |
| Framework dispatch (wx/EH/Lua/reflection) | `1` | 0.05% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `936` | 50.35% |
| No indexed callsite evidence (needs investigation/evidence) | `461` | 24.80% |
