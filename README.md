...
- Total FAF functions: `67,167`
  - *IDA index, exported*
- Progress coverage:  **`97.69%`**
  - *Consists of `recovered` + `skip` + `external_dependency` ÷ exported*
  - *Total amount of completed tokens: `65,617`*

Progress DB status breakdown:

- `recovered`: `52,970` (80.73%)
...
- **`blocked`: `1,578` (2.35%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `1,612`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `1,612`*
...
### Recovered (52,970 functions) — binary caller context
...
| No recovered caller token yet (orphan risk) | `1,816` | 3.43% |
| No callsite evidence (no recorded code/data caller in the index) | `22,873` | 43.18% |
| Unclassified data xref (manual review) | `216` | 0.41% |
| RTTI-only | `4` | 0.01% |

### Not-yet-recovered (1,612 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `142` | 8.81% |
| Vtable-anchored (recover with the owning class) | `129` | 8.00% |
| Framework dispatch (wx/EH/Lua/reflection) | `8` | 0.50% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `733` | 45.47% |
| No indexed callsite evidence (needs investigation/evidence) | `599` | 37.16% |
