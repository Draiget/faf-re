...
- Total FAF functions: `67,167`
  - *IDA index, exported*
- Progress coverage:  **`96.72%`**
  - *Consists of `recovered` + `skip` + `external_dependency` ÷ exported*
  - *Total amount of completed tokens: `64,966`*

Progress DB status breakdown:

- `recovered`: `53,361` (82.14%)
...
- **`blocked`: `1,863` (2.77%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `2,271`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `2,271`*
...
### Recovered (53,361 functions) — binary caller context

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `16,369` | 30.68% |
| Vtable-anchored (virtual override of a recovered class) | `5,867` | 10.99% |
| Framework dispatch (wx event, EH handler, Lua binding, reflection table, …) | `5,521` | 10.35% |
| No recovered caller token yet (orphan risk) | `2,167` | 4.06% |
| No callsite evidence (no recorded code/data caller in the index) | `23,217` | 43.51% |
| Unclassified data xref (manual review) | `216` | 0.40% |
| RTTI-only | `4` | 0.01% |

### Not-yet-recovered (2,271 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `351` | 15.46% |
| Vtable-anchored (recover with the owning class) | `150` | 6.61% |
| Framework dispatch (wx/EH/Lua/reflection) | `41` | 1.81% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `774` | 34.08% |
| No indexed callsite evidence (needs investigation/evidence) | `952` | 41.92% |
