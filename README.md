...
- `recovered`: `52,830` (79.10%)
- `skip`: `8,215` (12.30%) — proven ICF aliases / thunks / EH or static-init glue with no distinct source body
...
### Recovered (52,830 functions) — binary caller context

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `17,174` | 32.51% |
| Vtable-anchored (virtual override of a recovered class) | `5,892` | 11.15% |
| Framework dispatch (wx event, EH handler, Lua binding, reflection table, …) | `5,633` | 10.66% |
| No recovered caller token yet (orphan risk) | `1,649` | 3.12% |
| No callsite evidence (no recorded code/data caller in the index) | `22,262` | 42.14% |
