...
- Progress coverage:  **`96.83%`**
  ...
  - *Total amount of completed tokens: `65,036`*

Progress DB status breakdown:

- `recovered`: `53,431` (82.16%)
- `skip`: `6,129` (9.42%) — proven ICF aliases / thunks / EH or static-init glue with no distinct source body
...
- **`blocked`: `1,793` (2.67%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `2,201`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `2,201`*
...
### Recovered (53,431 functions) — binary caller context
...
| Framework dispatch (wx event, EH handler, Lua binding, reflection table, …) | `5,538` | 10.36% |
...
### Not-yet-recovered (2,201 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `350` | 15.90% |
| Vtable-anchored (recover with the owning class) | `131` | 5.95% |
| Framework dispatch (wx/EH/Lua/reflection) | `24` | 1.09% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `773` | 35.12% |
| No indexed callsite evidence (needs investigation/evidence) | `920` | 41.80% |
