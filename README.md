...
## Recovery Coverage (`14/08/2026`, `fa_full_2026_03_26`)
...
- Progress coverage:  **`97.19%`**
  ...
  - *Total amount of completed tokens: `65,277`*

Progress DB status breakdown:

- `recovered`: `53,660` (82.20%)
...
### Recovered (53,660 functions) — binary caller context

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `15,982` | 29.78% |
| Vtable-anchored (virtual override of a recovered class) | `5,829` | 10.86% |
| Framework dispatch (wx event, EH handler, Lua binding, reflection table, …) | `5,501` | 10.25% |
| No recovered caller token yet (orphan risk) | `2,411` | 4.49% |
| No callsite evidence (no recorded code/data caller in the index) | `23,717` | 44.20% |
...
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `313` | 16.08% |
| Vtable-anchored (recover with the owning class) | `193` | 9.91% |
| Framework dispatch (wx/EH/Lua/reflection) | `56` | 2.88% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `920` | 47.25% |
| No indexed callsite evidence (needs investigation/evidence) | `461` | 23.68% |
