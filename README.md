...
- Progress coverage:  **`96.38%`**
  ...
  - *Total amount of completed tokens: `64,734`*

Progress DB status breakdown:

- `recovered`: `53,119` (82.06%)
...
- **`blocked`: `2,081` (3.10%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `2,505`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `2,505`*
...
### Recovered (53,119 functions) — binary caller context
...
| Framework dispatch (wx event, EH handler, Lua binding, reflection table, …) | `5,497` | 10.35% |
...
### Not-yet-recovered (2,505 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `415` | 16.57% |
| Vtable-anchored (recover with the owning class) | `203` | 8.10% |
| Framework dispatch (wx/EH/Lua/reflection) | `62` | 2.48% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `834` | 33.29% |
| No indexed callsite evidence (needs investigation/evidence) | `987` | 39.40% |
