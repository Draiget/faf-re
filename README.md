...
- Total FAF functions: `67,167`
  - *IDA index, exported*
- Progress coverage:  **`96.46%`**
  - *Consists of `recovered` + `skip` + `external_dependency` ÷ exported*
  - *Total amount of completed tokens: `64,786`*

Progress DB status breakdown:

- `recovered`: `53,162` (82.06%)
...
- **`blocked`: `2,033` (3.03%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `2,451`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `2,451`*
...
### Recovered (53,162 functions) — binary caller context

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `16,197` | 30.47% |
...
| No recovered caller token yet (orphan risk) | `2,241` | 4.22% |
| No callsite evidence (no recorded code/data caller in the index) | `23,185` | 43.61% |
| Unclassified data xref (manual review) | `216` | 0.41% |
| RTTI-only | `4` | 0.01% |

### Not-yet-recovered (2,451 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `380` | 15.50% |
| Vtable-anchored (recover with the owning class) | `203` | 8.28% |
| Framework dispatch (wx/EH/Lua/reflection) | `55` | 2.24% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `826` | 33.70% |
| No indexed callsite evidence (needs investigation/evidence) | `983` | 40.11% |
