...
- Progress coverage:  **`96.71%`**
  ...
  - *Total amount of completed tokens: `64,958`*

Progress DB status breakdown:

- `recovered`: `53,077` (81.71%)
...
- **`blocked`: `2,236` (3.33%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `2,239`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `2,239`*
...
### Recovered (53,077 functions) — wiring health

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Confirmed caller** (recovered binary caller wired by name) | `14,960` | 28.19% |
...
| No callsite evidence (no recorded code/data caller in the index) | `23,745` | 44.74% |
| Unclassified data xref (manual review) | `215` | 0.41% |
| RTTI-only | `4` | 0.01% |

### Not-yet-recovered (2,239 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Recoverable now** (`OK_RECOVERED_CALLER` — recovered caller exists; recover next) | `485` | 21.66% |
| Vtable-anchored (recover with the owning class) | `211` | 9.42% |
| Framework dispatch (wx/EH/Lua/reflection) | `35` | 1.56% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `1,065` | 47.57% |
| No callsite evidence (likely external/dead — candidates for `external_dependency`) | `441` | 19.70% |
