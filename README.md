...
- Progress coverage:  **`96.69%`**
  ...
  - *Total amount of completed tokens: `64,945`*

Progress DB status breakdown:

- `recovered`: `53,339` (82.13%)
- `skip`: `6,128` (9.44%) — proven ICF aliases / thunks / EH or static-init glue with no distinct source body
- `external_dependency`: `5,478` (8.43%) — proven third-party/import-boundary code
...
- **`blocked`: `1,883` (2.80%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `2,292`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `2,292`*
...
### Recovered (53,339 functions) — binary caller context

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `16,340` | 30.63% |
...
| No callsite evidence (no recorded code/data caller in the index) | `23,213` | 43.52% |
| Unclassified data xref (manual review) | `216` | 0.40% |
| RTTI-only | `4` | 0.01% |

### Not-yet-recovered (2,292 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `352` | 15.36% |
| Vtable-anchored (recover with the owning class) | `151` | 6.59% |
| Framework dispatch (wx/EH/Lua/reflection) | `44` | 1.92% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `785` | 34.25% |
| No indexed callsite evidence (needs investigation/evidence) | `957` | 41.75% |
