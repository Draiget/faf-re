...
- Progress coverage:  **`97.34%`**
  ...
  - *Total amount of completed tokens: `65,379`*

Progress DB status breakdown:

- `recovered`: `53,772` (82.25%)
...
- **`blocked`: `1,780` (2.65%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `1,845`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `1,845`*
...
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `16,142` | 30.02% |
...
| No recovered caller token yet (orphan risk) | `2,334` | 4.34% |
| No callsite evidence (no recorded code/data caller in the index) | `23,720` | 44.11% |
| Unclassified data xref (manual review) | `216` | 0.40% |
| RTTI-only | `4` | 0.01% |

### Not-yet-recovered (1,845 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `307` | 16.64% |
| Vtable-anchored (recover with the owning class) | `166` | 9.00% |
| Framework dispatch (wx/EH/Lua/reflection) | `54` | 2.93% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `856` | 46.40% |
| No indexed callsite evidence (needs investigation/evidence) | `458` | 24.82% |
