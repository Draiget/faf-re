...
- **`blocked`: `2,170` (3.23%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `2,578`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `2,578`*
...
### Recovered (52,710 functions) — binary caller context

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `16,325` | 30.97% |
...
### Not-yet-recovered (2,578 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `310` | 12.02% |
| Vtable-anchored (recover with the owning class) | `140` | 5.43% |
| Framework dispatch (wx/EH/Lua/reflection) | `11` | 0.43% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `972` | 37.70% |
| No indexed callsite evidence (needs investigation/evidence) | `1,143` | 44.34% |
