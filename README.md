...
- Total FAF functions: `67,167`
  - *IDA index, exported*
- Progress coverage:  **`97.72%`**
  - *Consists of `recovered` + `skip` + `external_dependency` ÷ exported*
  - *Total amount of completed tokens: `65,638`*

Progress DB status breakdown:

- `recovered`: `52,977` (80.71%)
...
- **`blocked`: `1,557` (2.32%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `1,591`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `1,591`*
...
### Recovered (52,977 functions) — binary caller context

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `16,626` | 31.38% |
...
| No recovered caller token yet (orphan risk) | `1,809` | 3.41% |
| No callsite evidence (no recorded code/data caller in the index) | `22,873` | 43.18% |
| Unclassified data xref (manual review) | `216` | 0.41% |
| RTTI-only | `4` | 0.01% |

### Not-yet-recovered (1,591 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `124` | 7.79% |
| Vtable-anchored (recover with the owning class) | `129` | 8.11% |
| Framework dispatch (wx/EH/Lua/reflection) | `8` | 0.50% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `730` | 45.88% |
| No indexed callsite evidence (needs investigation/evidence) | `599` | 37.65% |
