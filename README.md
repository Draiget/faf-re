...
- Total FAF functions: `67,167`
  - *IDA index, exported*
- Progress coverage:  **`96.23%`**
  - *Consists of `recovered` + `skip` + `external_dependency` ÷ exported*
  - *Total amount of completed tokens: `64,634`*

Progress DB status breakdown:

- `recovered`: `52,692` (81.52%)
...
### Recovered (52,692 functions) — binary caller context

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `16,337` | 31.00% |
...
| No recovered caller token yet (orphan risk) | `1,844` | 3.50% |
