...
- Total FAF functions: `67,167`
  - *IDA index, exported*
- Progress coverage:  **`95.77%`**
  - *Consists of `recovered` + `skip` + `external_dependency` ÷ exported*
  - *Total amount of completed tokens: `64,324`*

Progress DB status breakdown:

- `recovered`: `52,673` (81.89%)
...
- **`blocked`: `2,505` (3.73%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `2,913`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `2,913`*
...
### Recovered (52,673 functions) — binary caller context

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `16,199` | 30.75% |
| Vtable-anchored (virtual override of a recovered class) | `5,880` | 11.16% |
...
| No callsite evidence (no recorded code/data caller in the index) | `22,941` | 43.55% |
| Unclassified data xref (manual review) | `216` | 0.41% |
| RTTI-only | `4` | 0.01% |

### Not-yet-recovered (2,913 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `474` | 16.27% |
| Vtable-anchored (recover with the owning class) | `144` | 4.94% |
| Framework dispatch (wx/EH/Lua/reflection) | `13` | 0.45% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `1,039` | 35.67% |
| No indexed callsite evidence (needs investigation/evidence) | `1,240` | 42.57% |
