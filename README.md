...
- Progress coverage:  **`96.83%`**
  ...
  - *Total amount of completed tokens: `65,038`*

Progress DB status breakdown:

- `recovered`: `53,432` (82.16%)
- `skip`: `6,130` (9.43%) — proven ICF aliases / thunks / EH or static-init glue with no distinct source body
...
- **`blocked`: `1,791` (2.67%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `2,199`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `2,199`*
...
### Recovered (53,432 functions) — binary caller context

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `16,381` | 30.66% |
| Vtable-anchored (virtual override of a recovered class) | `5,886` | 11.02% |
| Framework dispatch (wx event, EH handler, Lua binding, reflection table, …) | `5,539` | 10.37% |
| No recovered caller token yet (orphan risk) | `2,157` | 4.04% |
| No callsite evidence (no recorded code/data caller in the index) | `23,249` | 43.51% |
| Unclassified data xref (manual review) | `216` | 0.40% |
| RTTI-only | `4` | 0.01% |

### Not-yet-recovered (2,199 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `350` | 15.92% |
| Vtable-anchored (recover with the owning class) | `131` | 5.96% |
| Framework dispatch (wx/EH/Lua/reflection) | `23` | 1.05% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `772` | 35.11% |
| No indexed callsite evidence (needs investigation/evidence) | `920` | 41.84% |
