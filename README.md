...
- Progress coverage:  **`97.82%`**
  ...
  - *Total amount of completed tokens: `65,701`*

Progress DB status breakdown:

- `recovered`: `53,035` (80.72%)
...
- **`blocked`: `1,495` (2.23%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `1,529`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `1,529`*
...
### Recovered (53,035 functions) — binary caller context

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `16,689` | 31.47% |
...
### Not-yet-recovered (1,529 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `78` | 5.10% |
| Vtable-anchored (recover with the owning class) | `129` | 8.44% |
| Framework dispatch (wx/EH/Lua/reflection) | `8` | 0.52% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `716` | 46.83% |
| No indexed callsite evidence (needs investigation/evidence) | `597` | 39.05% |
