...
- Total FAF functions: `67,167`
  - *IDA index, exported*
- Progress coverage:  **`98.77%`**
  - *Consists of `recovered` + `skip` + `external_dependency` ÷ exported*
  - *Total amount of completed tokens: `66,339`*

Progress DB status breakdown:

- `recovered`: `53,128` (80.09%)
...
- **`blocked`: `885` (1.32%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `892`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `892`*
...
### Recovered (53,128 functions) — binary caller context

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `16,841` | 31.70% |
...
| No recovered caller token yet (orphan risk) | `1,741` | 3.28% |
| No callsite evidence (no recorded code/data caller in the index) | `22,876` | 43.06% |
| Unclassified data xref (manual review) | `216` | 0.41% |
| RTTI-only | `4` | 0.01% |

### Not-yet-recovered (892 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `30` | 3.36% |
| Vtable-anchored (recover with the owning class) | `109` | 12.22% |
| Framework dispatch (wx/EH/Lua/reflection) | `0` | 0.00% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `392` | 43.95% |
| No indexed callsite evidence (needs investigation/evidence) | `360` | 40.36% |
