...
- Progress coverage:  **`97.42%`**
  ...
  - *Total amount of completed tokens: `65,436`*

Progress DB status breakdown:

- `recovered`: `53,815` (82.24%)
...
- **`blocked`: `1,364` (2.03%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `1,788`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `1,788`*
...
### Recovered (53,815 functions) — binary caller context

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `16,226` | 30.15% |
...
### Not-yet-recovered (1,788 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `287` | 16.05% |
| Vtable-anchored (recover with the owning class) | `162` | 9.06% |
| Framework dispatch (wx/EH/Lua/reflection) | `54` | 3.02% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `837` | 46.81% |
| No indexed callsite evidence (needs investigation/evidence) | `444` | 24.83% |
