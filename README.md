...
- Progress coverage:  **`97.03%`**
  ...
  - *Total amount of completed tokens: `65,172`*

Progress DB status breakdown:

- `recovered`: `53,290` (81.77%)
...
- **`blocked`: `2,026` (3.02%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `2,030`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `2,030`*
...
### Recovered (53,290 functions) — wiring health

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Confirmed caller** (recovered binary caller wired by name) | `15,353` | 28.81% |
...
### Not-yet-recovered (2,030 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Recoverable now** (`OK_RECOVERED_CALLER` — recovered caller exists; recover next) | `329` | 16.21% |
| Vtable-anchored (recover with the owning class) | `209` | 10.30% |
| Framework dispatch (wx/EH/Lua/reflection) | `35` | 1.72% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `1,020` | 50.25% |
| No callsite evidence (likely external/dead — candidates for `external_dependency`) | `435` | 21.43% |
