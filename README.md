...
- Total FAF functions: `67,167`
  - *IDA index, exported*
- Progress coverage:  **`97.49%`**
  - *Consists of `recovered` + `skip` + `external_dependency` ÷ exported*
  - *Total amount of completed tokens: `65,483`*

Progress DB status breakdown:

- `recovered`: `52,895` (80.78%)
- `skip`: `7,078` (10.81%) — proven ICF aliases / thunks / EH or static-init glue with no distinct source body
- `external_dependency`: `5,510` (8.41%) — proven third-party/import-boundary code
...
- **`blocked`: `1,710` (2.55%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `1,744`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `1,744`*
...
### Recovered (52,895 functions) — binary caller context

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `16,499` | 31.19% |
| Vtable-anchored (virtual override of a recovered class) | `5,885` | 11.13% |
| Framework dispatch (wx event, EH handler, Lua binding, reflection table, …) | `5,564` | 10.52% |
| No recovered caller token yet (orphan risk) | `1,854` | 3.51% |
| No callsite evidence (no recorded code/data caller in the index) | `22,873` | 43.24% |
| Unclassified data xref (manual review) | `216` | 0.41% |
| RTTI-only | `4` | 0.01% |

### Not-yet-recovered (1,744 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `190` | 10.89% |
| Vtable-anchored (recover with the owning class) | `130` | 7.45% |
| Framework dispatch (wx/EH/Lua/reflection) | `8` | 0.46% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `814` | 46.67% |
| No indexed callsite evidence (needs investigation/evidence) | `601` | 34.46% |
