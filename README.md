...
- Progress coverage:  **`95.94%`**
  ...
  - *Total amount of completed tokens: `64,440`*

Progress DB status breakdown:

- `recovered`: `52,740` (81.84%)
- `skip`: `6,268` (9.73%) — proven ICF aliases / thunks / EH or static-init glue with no distinct source body
...
- **`blocked`: `2,390` (3.56%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `2,798`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `2,798`*
...
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `16,338` | 30.98% |
...
### Not-yet-recovered (2,798 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `360` | 12.87% |
| Vtable-anchored (recover with the owning class) | `144` | 5.15% |
| Framework dispatch (wx/EH/Lua/reflection) | `13` | 0.46% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `1,033` | 36.92% |
| No indexed callsite evidence (needs investigation/evidence) | `1,245` | 44.50% |
