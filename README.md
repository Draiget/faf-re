...
- Progress coverage:  **`96.84%`**
  ...
  - *Total amount of completed tokens: `65,042`*

Progress DB status breakdown:

- `recovered`: `53,435` (82.15%)
...
- **`blocked`: `1,787` (2.66%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `2,195`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `2,195`*
...
### Recovered (53,435 functions) — binary caller context
...
### Not-yet-recovered (2,195 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `348` | 15.85% |
| Vtable-anchored (recover with the owning class) | `130` | 5.92% |
| Framework dispatch (wx/EH/Lua/reflection) | `22` | 1.00% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `772` | 35.17% |
| No indexed callsite evidence (needs investigation/evidence) | `920` | 41.91% |
