...
- Total FAF functions: `67,167`
  - *IDA index, exported*
- Progress coverage:  **`96.03%`**
  - *Consists of `recovered` + `skip` + `external_dependency` ÷ exported*
  - *Total amount of completed tokens: `64,501`*

Progress DB status breakdown:

- `recovered`: `52,716` (81.73%)
- `skip`: `6,345` (9.84%) — proven ICF aliases / thunks / EH or static-init glue with no distinct source body
- `external_dependency`: `5,440` (8.43%) — proven third-party/import-boundary code
...
- `in_progress`: `158` (0.24%)
- **`blocked`: `2,171` (3.23%)**
  - *strict circular/dep-blocked (in-DB literal `status == "blocked"`)*  
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `2,579`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `2,579`*
...
### Recovered (52,716 functions) — binary caller context

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `16,326` | 30.97% |
...
| No recovered caller token yet (orphan risk) | `1,846` | 3.50% |
| No callsite evidence (no recorded code/data caller in the index) | `22,889` | 43.42% |
| Unclassified data xref (manual review) | `216` | 0.41% |
| RTTI-only | `4` | 0.01% |

### Not-yet-recovered (2,579 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `311` | 12.06% |
| Vtable-anchored (recover with the owning class) | `140` | 5.43% |
| Framework dispatch (wx/EH/Lua/reflection) | `11` | 0.43% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `972` | 37.69% |
| No indexed callsite evidence (needs investigation/evidence) | `1,143` | 44.32% |
