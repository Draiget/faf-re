...
- Progress coverage:  **`96.08%`**
  ...
  - *Total amount of completed tokens: `64,534`*

Progress DB status breakdown:

- `recovered`: `52,711` (81.68%)
...
- **`blocked`: `2,170` (3.23%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `2,577`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `2,577`*
...
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `16,328` | 30.98% |
...
### Not-yet-recovered (2,577 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `309` | 11.99% |
| Vtable-anchored (recover with the owning class) | `140` | 5.43% |
| Framework dispatch (wx/EH/Lua/reflection) | `11` | 0.43% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `972` | 37.72% |
| No indexed callsite evidence (needs investigation/evidence) | `1,143` | 44.35% |
