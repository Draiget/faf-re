...
- Total FAF functions: `67,167`
  - *IDA index, exported*
- Progress coverage:  **`96.50%`**
  - *Consists of `recovered` + `skip` + `external_dependency` ÷ exported*
  - *Total amount of completed tokens: `64,818`*

Progress DB status breakdown:

- `recovered`: `53,194` (82.07%)
...
- **`blocked`: `2,001` (2.98%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `2,419`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `2,419`*
...
### Recovered (53,194 functions) — binary caller context

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `16,206` | 30.47% |
| Vtable-anchored (virtual override of a recovered class) | `5,833` | 10.97% |
| Framework dispatch (wx event, EH handler, Lua binding, reflection table, …) | `5,513` | 10.36% |
| No recovered caller token yet (orphan risk) | `2,237` | 4.21% |
| No callsite evidence (no recorded code/data caller in the index) | `23,185` | 43.59% |
| Unclassified data xref (manual review) | `216` | 0.41% |
| RTTI-only | `4` | 0.01% |

### Not-yet-recovered (2,419 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `379` | 15.67% |
| Vtable-anchored (recover with the owning class) | `186` | 7.69% |
| Framework dispatch (wx/EH/Lua/reflection) | `45` | 1.86% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `822` | 33.98% |
| No indexed callsite evidence (needs investigation/evidence) | `983` | 40.64% |
