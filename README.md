...
- Progress coverage:  **`96.58%`**
  ...
  - *Total amount of completed tokens: `64,870`*

Progress DB status breakdown:

- `recovered`: `52,983` (81.68%)
...
- **`blocked`: `2,330` (3.47%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `2,333`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `2,333`*
...
### Recovered (52,983 functions) — wiring health

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Confirmed caller** (recovered binary caller wired by name) | `14,882` | 28.09% |
...
| Caller still blocked (orphan-helper risk — caller awaits recovery) | `2,859` | 5.40% |
| No callsite evidence (no recorded code/data caller in the index) | `23,733` | 44.79% |
| Unclassified data xref (manual review) | `215` | 0.41% |
| RTTI-only | `4` | 0.01% |

### Not-yet-recovered (2,333 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Recoverable now** (`OK_RECOVERED_CALLER` — recovered caller exists; recover next) | `532` | 22.80% |
| Vtable-anchored (recover with the owning class) | `233` | 9.99% |
| Framework dispatch (wx/EH/Lua/reflection) | `35` | 1.50% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `1,082` | 46.38% |
| No callsite evidence (likely external/dead — candidates for `external_dependency`) | `449` | 19.25% |
