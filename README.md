...
- Total FAF functions: `67,167`
  - *IDA index, exported*
- Progress coverage:  **`96.81%`**
  - *Consists of `recovered` + `skip` + `external_dependency` ÷ exported*
  - *Total amount of completed tokens: `65,026`*

Progress DB status breakdown:

- `recovered`: `53,421` (82.15%)
...
- **`blocked`: `1,803` (2.68%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `2,211`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `2,211`*
...
### Recovered (53,421 functions) — binary caller context

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `16,381` | 30.66% |
| Vtable-anchored (virtual override of a recovered class) | `5,885` | 11.02% |
| Framework dispatch (wx event, EH handler, Lua binding, reflection table, …) | `5,529` | 10.35% |
| No recovered caller token yet (orphan risk) | `2,157` | 4.04% |
| No callsite evidence (no recorded code/data caller in the index) | `23,249` | 43.52% |
| Unclassified data xref (manual review) | `216` | 0.40% |
| RTTI-only | `4` | 0.01% |

### Not-yet-recovered (2,211 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `350` | 15.83% |
| Vtable-anchored (recover with the owning class) | `132` | 5.97% |
| Framework dispatch (wx/EH/Lua/reflection) | `33` | 1.49% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `773` | 34.96% |
| No indexed callsite evidence (needs investigation/evidence) | `920` | 41.61% |
