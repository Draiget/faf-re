...
- Progress coverage:  **`98.40%`**
  ...
  - *Total amount of completed tokens: `66,094`*

Progress DB status breakdown:

- `recovered`: `53,066` (80.29%)
- `skip`: `7,376` (11.16%) — proven ICF aliases / thunks / EH or static-init glue with no distinct source body
...
- **`blocked`: `1,104` (1.64%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `1,137`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `1,137`*
...
### Not-yet-recovered (1,137 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `53` | 4.66% |
| Vtable-anchored (recover with the owning class) | `124` | 10.91% |
| Framework dispatch (wx/EH/Lua/reflection) | `8` | 0.70% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `582` | 51.19% |
| No indexed callsite evidence (needs investigation/evidence) | `369` | 32.45% |
