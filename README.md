...
- `recovered`: `53,446` (82.16%)
- `skip`: `6,131` (9.42%) — proven ICF aliases / thunks / EH or static-init glue with no distinct source body
...
### Recovered (53,446 functions) — binary caller context
...
| Framework dispatch (wx event, EH handler, Lua binding, reflection table, …) | `5,546` | 10.38% |
...
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `347` | 15.89% |
| Vtable-anchored (recover with the owning class) | `127` | 5.82% |
| Framework dispatch (wx/EH/Lua/reflection) | `16` | 0.73% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `771` | 35.30% |
| No indexed callsite evidence (needs investigation/evidence) | `920` | 42.12% |
