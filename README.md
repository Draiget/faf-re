...
- Total FAF functions: `67,170`
  - *IDA index, exported*
- Progress coverage:  **`99.71%`**
  - *Consists of `recovered` + `skip` + `external_dependency` ÷ exported*
  - *Total amount of completed tokens: `66,972`*

Progress DB status breakdown:

- `recovered`: `47,456` (70.86%)
...
- **`blocked`: `314` (0.47%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `317`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `317`*
...
### Recovered (47,456 functions) — binary caller context

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `17,313` | 36.48% |
...
| No callsite evidence (no recorded code/data caller in the index) | `16,834` | 35.47% |
| Unclassified data xref (manual review) | `216` | 0.46% |
| RTTI-only | `4` | 0.01% |

### Not-yet-recovered (317 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `3` | 0.95% |
| Vtable-anchored (recover with the owning class) | `81` | 25.55% |
| Framework dispatch (wx/EH/Lua/reflection) | `0` | 0.00% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `123` | 38.80% |
| No indexed callsite evidence (needs investigation/evidence) | `110` | 34.70% |
