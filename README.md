...
- Total FAF functions: `67,167`
  - *IDA index, exported*
- Progress coverage:  **`97.54%`**
  - *Consists of `recovered` + `skip` + `external_dependency` ÷ exported*
  - *Total amount of completed tokens: `65,516`*
...
- `external_dependency`: `5,533` (8.45%) — proven third-party/import-boundary code
...
- **`blocked`: `1,678` (2.50%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `1,712`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `1,712`*
...
### Not-yet-recovered (1,712 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `188` | 10.98% |
| Vtable-anchored (recover with the owning class) | `130` | 7.59% |
| Framework dispatch (wx/EH/Lua/reflection) | `8` | 0.47% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `785` | 45.85% |
| No indexed callsite evidence (needs investigation/evidence) | `600` | 35.05% |
