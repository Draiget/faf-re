...
- **`blocked`: `2,232` (3.32%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `2,235`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `2,235`*
...
### Recovered (53,081 functions) — wiring health

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Confirmed caller** (recovered binary caller wired by name) | `14,964` | 28.19% |
...
### Not-yet-recovered (2,235 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Recoverable now** (`OK_RECOVERED_CALLER` — recovered caller exists; recover next) | `483` | 21.61% |
| Vtable-anchored (recover with the owning class) | `211` | 9.44% |
| Framework dispatch (wx/EH/Lua/reflection) | `35` | 1.57% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `1,063` | 47.56% |
| No callsite evidence (likely external/dead — candidates for `external_dependency`) | `441` | 19.73% |
