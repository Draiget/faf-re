...
- Progress coverage:  **`95.98%`**
  ...
  - *Total amount of completed tokens: `64,465`*

Progress DB status breakdown:

- `recovered`: `52,721` (81.78%)
- `skip`: `6,307` (9.78%) — proven ICF aliases / thunks / EH or static-init glue with no distinct source body
...
- **`blocked`: `2,365` (3.52%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `2,773`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `2,773`*
...
| No callsite evidence (no recorded code/data caller in the index) | `22,890` | 43.42% |
| Unclassified data xref (manual review) | `216` | 0.41% |
| RTTI-only | `4` | 0.01% |

### Not-yet-recovered (2,773 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `349` | 12.59% |
| Vtable-anchored (recover with the owning class) | `144` | 5.19% |
| Framework dispatch (wx/EH/Lua/reflection) | `13` | 0.47% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `1,022` | 36.86% |
| No indexed callsite evidence (needs investigation/evidence) | `1,242` | 44.79% |
