...
- Progress coverage:  **`97.84%`**
  ...
  - *Total amount of completed tokens: `65,715`*
...
- **`blocked`: `1,481` (2.20%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `1,515`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `1,515`*
...
### Not-yet-recovered (1,515 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `67` | 4.42% |
| Vtable-anchored (recover with the owning class) | `129` | 8.51% |
| Framework dispatch (wx/EH/Lua/reflection) | `8` | 0.53% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `714` | 47.13% |
| No indexed callsite evidence (needs investigation/evidence) | `596` | 39.34% |
