...
- Progress coverage:  **`97.84%`**
  ...
  - *Total amount of completed tokens: `65,718`*

Progress DB status breakdown:

- `recovered`: `53,048` (80.72%)
- `skip`: `7,120` (10.83%) — proven ICF aliases / thunks / EH or static-init glue with no distinct source body
- `external_dependency`: `5,550` (8.45%) — proven third-party/import-boundary code
...
- **`blocked`: `1,478` (2.20%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `1,512`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `1,512`*
...
### Not-yet-recovered (1,512 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `67` | 4.43% |
| Vtable-anchored (recover with the owning class) | `129` | 8.53% |
| Framework dispatch (wx/EH/Lua/reflection) | `8` | 0.53% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `711` | 47.02% |
| No indexed callsite evidence (needs investigation/evidence) | `596` | 39.42% |
