...
## Recovery Coverage (`14/07/2026`, `fa_full_2026_03_26`)
...
- **`blocked`: `2,019` (3.01%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `2,023`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `2,023`*
...
### Recovered (53,299 functions) — wiring health

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Confirmed caller** (recovered binary caller wired by name) | `15,362` | 28.82% |
...
| No callsite evidence (no recorded code/data caller in the index) | `23,750` | 44.56% |
| Unclassified data xref (manual review) | `215` | 0.40% |
| RTTI-only | `4` | 0.01% |

### Not-yet-recovered (2,023 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Recoverable now** (`OK_RECOVERED_CALLER` — recovered caller exists; recover next) | `326` | 16.11% |
| Vtable-anchored (recover with the owning class) | `209` | 10.33% |
| Framework dispatch (wx/EH/Lua/reflection) | `35` | 1.73% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `1,016` | 50.22% |
| No callsite evidence (likely external/dead — candidates for `external_dependency`) | `435` | 21.50% |
