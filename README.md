...
- Progress coverage:  **`97.38%`**
  ...
  - *Total amount of completed tokens: `65,407`*

Progress DB status breakdown:

- `recovered`: `53,774` (82.21%)
...
- **`blocked`: `1,749` (2.60%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `1,817`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `1,817`*
...
### Recovered (53,774 functions) — binary caller context

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `16,020` | 29.79% |
...
### Not-yet-recovered (1,817 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `275` | 15.13% |
| Vtable-anchored (recover with the owning class) | `173` | 9.52% |
| Framework dispatch (wx/EH/Lua/reflection) | `1` | 0.06% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `907` | 49.92% |
| No indexed callsite evidence (needs investigation/evidence) | `458` | 25.21% |
