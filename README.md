...
- Progress coverage:  **`97.64%`**
  ...
  - *Total amount of completed tokens: `65,581`*
...
- `external_dependency`: `5,545` (8.46%) — proven third-party/import-boundary code
...
- **`blocked`: `1,614` (2.40%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `1,648`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `1,648`*
...
### Not-yet-recovered (1,648 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `155` | 9.41% |
| Vtable-anchored (recover with the owning class) | `129` | 7.83% |
| Framework dispatch (wx/EH/Lua/reflection) | `8` | 0.49% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `756` | 45.87% |
| No indexed callsite evidence (needs investigation/evidence) | `599` | 36.35% |
