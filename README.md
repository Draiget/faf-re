...
- Total FAF functions: `67,167`
  - *IDA index, exported*
- Progress coverage:  **`97.15%`**
  - *Consists of `recovered` + `skip` + `external_dependency` ÷ exported*
  - *Total amount of completed tokens: `65,254`*

Progress DB status breakdown:

- `recovered`: `53,376` (81.80%)
...
- **`blocked`: `1,945` (2.90%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `1,949`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `1,949`*
...
### Recovered (53,376 functions) — wiring health

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Confirmed caller** (recovered binary caller wired by name) | `15,457` | 28.96% |
...
| Caller still blocked (orphan-helper risk — caller awaits recovery) | `2,612` | 4.89% |
| No callsite evidence (no recorded code/data caller in the index) | `23,752` | 44.50% |
| Unclassified data xref (manual review) | `215` | 0.40% |
| RTTI-only | `4` | 0.01% |

### Not-yet-recovered (1,949 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Recoverable now** (`OK_RECOVERED_CALLER` — recovered caller exists; recover next) | `303` | 15.55% |
| Vtable-anchored (recover with the owning class) | `186` | 9.54% |
| Framework dispatch (wx/EH/Lua/reflection) | `34` | 1.74% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `990` | 50.80% |
| No callsite evidence (likely external/dead — candidates for `external_dependency`) | `434` | 22.27% |
