...
- Progress coverage:  **`96.76%`**
  ...
  - *Total amount of completed tokens: `64,989`*

Progress DB status breakdown:

- `recovered`: `53,108` (81.72%)
...
### Recovered (53,108 functions) — wiring health

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Confirmed caller** (recovered binary caller wired by name) | `14,996` | 28.24% |
...
|---|---:|---:|
| **Recoverable now** (`OK_RECOVERED_CALLER` — recovered caller exists; recover next) | `466` | 21.10% |
| Vtable-anchored (recover with the owning class) | `211` | 9.55% |
| Framework dispatch (wx/EH/Lua/reflection) | `35` | 1.58% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `1,055` | 47.76% |
| No callsite evidence (likely external/dead — candidates for `external_dependency`) | `440` | 19.92% |
