...
- Progress coverage:  **`96.91%`**
  ...
  - *Total amount of completed tokens: `65,090`*

Progress DB status breakdown:

- `recovered`: `53,499` (82.19%)
- `skip`: `6,118` (9.40%) — proven ICF aliases / thunks / EH or static-init glue with no distinct source body
- `external_dependency`: `5,473` (8.41%) — proven third-party/import-boundary code
...
- **`blocked`: `1,738` (2.59%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `2,147`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `2,147`*
...
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `16,405` | 30.66% |
...
| No recovered caller token yet (orphan risk) | `2,146` | 4.01% |
| No callsite evidence (no recorded code/data caller in the index) | `23,264` | 43.48% |
| Unclassified data xref (manual review) | `216` | 0.40% |
| RTTI-only | `4` | 0.01% |

### Not-yet-recovered (2,147 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `345` | 16.07% |
| Vtable-anchored (recover with the owning class) | `118` | 5.50% |
| Framework dispatch (wx/EH/Lua/reflection) | `1` | 0.05% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `767` | 35.72% |
| No indexed callsite evidence (needs investigation/evidence) | `913` | 42.52% |
