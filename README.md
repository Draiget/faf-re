...
- **`blocked`: `585` (0.87%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `588`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `588`*
...
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `17,168` | 32.55% |
...
### Not-yet-recovered (588 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `9` | 1.53% |
| Vtable-anchored (recover with the owning class) | `106` | 18.03% |
| Framework dispatch (wx/EH/Lua/reflection) | `0` | 0.00% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `167` | 28.40% |
| No indexed callsite evidence (needs investigation/evidence) | `305` | 51.87% |
