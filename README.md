...
- **`blocked`: `1,773` (2.64%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `2,181`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `2,181`*
...
### Recovered (53,451 functions) — binary caller context
...
| Framework dispatch (wx event, EH handler, Lua binding, reflection table, …) | `5,550` | 10.38% |
...
### Not-yet-recovered (2,181 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `347` | 15.91% |
| Vtable-anchored (recover with the owning class) | `127` | 5.82% |
| Framework dispatch (wx/EH/Lua/reflection) | `14` | 0.64% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `771` | 35.35% |
| No indexed callsite evidence (needs investigation/evidence) | `919` | 42.14% |
