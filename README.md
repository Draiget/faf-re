...
- Progress coverage:  **`96.77%`**
  ...
  - *Total amount of completed tokens: `64,999`*

Progress DB status breakdown:

- `recovered`: `52,773` (81.19%)
...
- **`blocked`: `1,822` (2.71%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `2,227`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `2,227`*
...
### Recovered (52,773 functions) — binary caller context

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `16,392` | 31.06% |
...
### Not-yet-recovered (2,227 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `238` | 10.69% |
| Vtable-anchored (recover with the owning class) | `132` | 5.93% |
| Framework dispatch (wx/EH/Lua/reflection) | `8` | 0.36% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `866` | 38.89% |
| No indexed callsite evidence (needs investigation/evidence) | `981` | 44.05% |
