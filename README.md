...
- Total FAF functions: `67,167`
  - *IDA index, exported*
- Progress coverage:  **`96.85%`**
  - *Consists of `recovered` + `skip` + `external_dependency` ÷ exported*
  - *Total amount of completed tokens: `65,048`*

Progress DB status breakdown:

- `recovered`: `53,441` (82.16%)
...
- **`blocked`: `1,781` (2.65%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `2,189`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `2,189`*
...
### Recovered (53,441 functions) — binary caller context

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `16,384` | 30.66% |
...
### Not-yet-recovered (2,189 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `347` | 15.85% |
| Vtable-anchored (recover with the owning class) | `130` | 5.94% |
| Framework dispatch (wx/EH/Lua/reflection) | `17` | 0.78% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `772` | 35.27% |
| No indexed callsite evidence (needs investigation/evidence) | `920` | 42.03% |
