...
- Progress coverage:  **`98.44%`**
  ...
  - *Total amount of completed tokens: `66,122`*

Progress DB status breakdown:

- `recovered`: `53,087` (80.29%)
...
- **`blocked`: `1,076` (1.60%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `1,109`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `1,109`*
...
### Recovered (53,087 functions) — binary caller context

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `16,769` | 31.59% |
...
| No recovered caller token yet (orphan risk) | `1,773` | 3.34% |
| No callsite evidence (no recorded code/data caller in the index) | `22,875` | 43.09% |
| Unclassified data xref (manual review) | `216` | 0.41% |
| RTTI-only | `4` | 0.01% |

### Not-yet-recovered (1,109 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `52` | 4.69% |
| Vtable-anchored (recover with the owning class) | `124` | 11.18% |
| Framework dispatch (wx/EH/Lua/reflection) | `8` | 0.72% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `558` | 50.32% |
| No indexed callsite evidence (needs investigation/evidence) | `366` | 33.00% |
