...
## Recovery Coverage (`26/07/2026`, `fa_full_2026_03_26`)

Progress snapshot:

- Total FAF functions: `67,167`
  - *IDA index, exported*
- Progress coverage:  **`97.38%`**
  - *Consists of `recovered` + `skip` + `external_dependency` ÷ exported*
  - *Total amount of completed tokens: `65,408`*

Progress DB status breakdown:

- `recovered`: `53,582` (81.92%)
...
- **`blocked`: `1,820` (2.71%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `1,825`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `1,825`*
...
### Recovered (53,582 functions) — binary caller context

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `15,778` | 29.45% |
...
| No recovered caller token yet (orphan risk) | `2,480` | 4.63% |
| No callsite evidence (no recorded code/data caller in the index) | `23,730` | 44.29% |
| Unclassified data xref (manual review) | `217` | 0.40% |
| RTTI-only | `4` | 0.01% |

### Not-yet-recovered (1,825 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `277` | 15.18% |
| Vtable-anchored (recover with the owning class) | `175` | 9.59% |
| Framework dispatch (wx/EH/Lua/reflection) | `0` | 0.00% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `926` | 50.74% |
| No indexed callsite evidence (needs investigation/evidence) | `445` | 24.38% |
