...
- Total FAF functions: `67,170`
  - *IDA index, exported*
- Progress coverage:  **`99.28%`**
  - *Consists of `recovered` + `skip` + `external_dependency` ÷ exported*
  - *Total amount of completed tokens: `66,685`*

Progress DB status breakdown:

- `recovered`: `52,744` (79.09%)
- `skip`: `8,199` (12.30%) — proven ICF aliases / thunks / EH or static-init glue with no distinct source body
- `external_dependency`: `5,742` (8.61%) — proven third-party/import-boundary code
...
- **`blocked`: `592` (0.88%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `595`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `595`*
...
### Not-yet-recovered (595 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `9` | 1.51% |
| Vtable-anchored (recover with the owning class) | `106` | 17.82% |
| Framework dispatch (wx/EH/Lua/reflection) | `0` | 0.00% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `174` | 29.24% |
| No indexed callsite evidence (needs investigation/evidence) | `305` | 51.26% |
| Unclassified data xref (manual review) | `1` | 0.17% |
| RTTI-only | `0` | 0.00% |
