...
- Total FAF functions: `67,167`
  - *IDA index, exported*
- Progress coverage:  **`98.50%`**
  - *Consists of `recovered` + `skip` + `external_dependency` ÷ exported*
  - *Total amount of completed tokens: `66,162`*

Progress DB status breakdown:

- `recovered`: `53,113` (80.28%)
- `skip`: `7,382` (11.16%) — proven ICF aliases / thunks / EH or static-init glue with no distinct source body
- `external_dependency`: `5,667` (8.57%) — proven third-party/import-boundary code
  - *libpng, zlib, wxWidgets, LuaPlus/Lua, external Boost internals, WildMagic/Wm3, CRI Sofdec/ADX, undname, bugsplat, CRT imports*
- `needs_evidence`: `20` (0.03%)
- `in_progress`: `9` (0.01%)
- **`blocked`: `1,049` (1.56%)**
  - *strict circular/dep-blocked (in-DB literal `status == "blocked"`)*  
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `1,069`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `1,069`*
...
### Recovered (53,113 functions) — binary caller context

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `16,810` | 31.65% |
...
| No recovered caller token yet (orphan risk) | `1,758` | 3.31% |
| No callsite evidence (no recorded code/data caller in the index) | `22,875` | 43.07% |
| Unclassified data xref (manual review) | `216` | 0.41% |
| RTTI-only | `4` | 0.01% |

### Not-yet-recovered (1,069 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `42` | 3.93% |
| Vtable-anchored (recover with the owning class) | `120` | 11.23% |
| Framework dispatch (wx/EH/Lua/reflection) | `8` | 0.75% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `532` | 49.77% |
| No indexed callsite evidence (needs investigation/evidence) | `366` | 34.24% |
