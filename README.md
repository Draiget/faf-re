...
- Progress coverage:  **`99.37%`**
  ...
  - *Total amount of completed tokens: `66,750`*
...
- **`blocked`: `527` (0.78%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `530`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `530`*
...
### Not-yet-recovered (530 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `10` | 1.89% |
| Vtable-anchored (recover with the owning class) | `89` | 16.79% |
| Framework dispatch (wx/EH/Lua/reflection) | `0` | 0.00% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `149` | 28.11% |
| No indexed callsite evidence (needs investigation/evidence) | `282` | 53.21% |
