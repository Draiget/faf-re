...
- **`blocked`: `1,660` (2.47%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `2,068`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `2,068`*
...
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `16,627` | 31.00% |
| Vtable-anchored (virtual override of a recovered class) | `5,912` | 11.02% |
| Framework dispatch (wx event, EH handler, Lua binding, reflection table, …) | `5,569` | 10.38% |
| No recovered caller token yet (orphan risk) | `2,061` | 3.84% |
| No callsite evidence (no recorded code/data caller in the index) | `23,242` | 43.34% |
| Unclassified data xref (manual review) | `217` | 0.40% |
| RTTI-only | `4` | 0.01% |

### Not-yet-recovered (2,068 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `296` | 14.31% |
| Vtable-anchored (recover with the owning class) | `111` | 5.37% |
| Framework dispatch (wx/EH/Lua/reflection) | `1` | 0.05% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `722` | 34.91% |
| No indexed callsite evidence (needs investigation/evidence) | `936` | 45.26% |
