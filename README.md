...
- Progress coverage:  **`96.46%`**
  ...
  - *Total amount of completed tokens: `64,789`*

Progress DB status breakdown:

- `recovered`: `52,712` (81.36%)
- `skip`: `6,591` (10.17%) — proven ICF aliases / thunks / EH or static-init glue with no distinct source body
...
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `245` | 10.80% |
| Vtable-anchored (recover with the owning class) | `136` | 6.00% |
| Framework dispatch (wx/EH/Lua/reflection) | `10` | 0.44% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `882` | 38.89% |
| No indexed callsite evidence (needs investigation/evidence) | `993` | 43.78% |
