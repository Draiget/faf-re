...
- Total FAF functions: `67,167`
  - *IDA index, exported*
- Progress coverage:  **`97.31%`**
  - *Consists of `recovered` + `skip` + `external_dependency` ÷ exported*
  - *Total amount of completed tokens: `65,357`*

Progress DB status breakdown:

- `recovered`: `53,752` (82.24%)
...
- **`blocked`: `1,802` (2.68%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `1,867`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `1,867`*
...
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `16,111` | 29.97% |
...
### Not-yet-recovered (1,867 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `308` | 16.50% |
| Vtable-anchored (recover with the owning class) | `174` | 9.32% |
| Framework dispatch (wx/EH/Lua/reflection) | `54` | 2.89% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `868` | 46.49% |
| No indexed callsite evidence (needs investigation/evidence) | `459` | 24.58% |
