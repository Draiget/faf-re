...
- Progress coverage:  **`98.29%`**
  ...
  - *Total amount of completed tokens: `66,016`*
...
- `external_dependency`: `5,639` (8.54%) — proven third-party/import-boundary code
...
- **`blocked`: `1,182` (1.76%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `1,215`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `1,215`*
...
### Not-yet-recovered (1,215 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `59` | 4.86% |
| Vtable-anchored (recover with the owning class) | `125` | 10.29% |
| Framework dispatch (wx/EH/Lua/reflection) | `8` | 0.66% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `632` | 52.02% |
| No indexed callsite evidence (needs investigation/evidence) | `390` | 32.10% |
