...
- `recovered`: `53,104` (79.81%)
- `skip`: `7,670` (11.53%) — proven ICF aliases / thunks / EH or static-init glue with no distinct source body
...
### Recovered (53,104 functions) — binary caller context

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `17,093` | 32.19% |
| Vtable-anchored (virtual override of a recovered class) | `5,882` | 11.08% |
| Framework dispatch (wx event, EH handler, Lua binding, reflection table, …) | `5,578` | 10.50% |
| No recovered caller token yet (orphan risk) | `1,697` | 3.20% |
| No callsite evidence (no recorded code/data caller in the index) | `22,634` | 42.62% |
