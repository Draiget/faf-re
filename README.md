...
- Progress coverage:  **`96.68%`**
  ...
  - *Total amount of completed tokens: `64,938`*

Progress DB status breakdown:

- `recovered`: `53,052` (81.70%)
- `skip`: `6,103` (9.40%) — CRT-internal / compiler-generated / orphan template instantiations / static-init glue
...
- **`blocked`: `2,256` (3.36%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `2,259`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `2,259`*
...
### Recovered (53,052 functions) — wiring health

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Confirmed caller** (recovered binary caller wired by name) | `14,950` | 28.18% |
| Vtable-anchored (virtual override of a recovered class) | `5,807` | 10.95% |
| Framework dispatch (wx event, EH handler, Lua binding, reflection table, …) | `5,494` | 10.36% |
| Caller still blocked (orphan-helper risk — caller awaits recovery) | `2,845` | 5.36% |
| No callsite evidence (no recorded code/data caller in the index) | `23,737` | 44.74% |
| Unclassified data xref (manual review) | `215` | 0.41% |
| RTTI-only | `4` | 0.01% |

### Not-yet-recovered (2,259 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Recoverable now** (`OK_RECOVERED_CALLER` — recovered caller exists; recover next) | `487` | 21.56% |
| Vtable-anchored (recover with the owning class) | `222` | 9.83% |
| Framework dispatch (wx/EH/Lua/reflection) | `35` | 1.55% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `1,068` | 47.28% |
| No callsite evidence (likely external/dead — candidates for `external_dependency`) | `445` | 19.70% |
