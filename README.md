...
- Progress coverage:  **`96.87%`**
  ...
  - *Total amount of completed tokens: `65,062`*

Progress DB status breakdown:

- `recovered`: `53,459` (82.17%)
- `skip`: `6,126` (9.42%) — proven ICF aliases / thunks / EH or static-init glue with no distinct source body
- `external_dependency`: `5,477` (8.42%) — proven third-party/import-boundary code
...
- **`blocked`: `1,766` (2.63%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `2,175`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `2,175`*
...
### Recovered (53,459 functions) — binary caller context
...
| Framework dispatch (wx event, EH handler, Lua binding, reflection table, …) | `5,557` | 10.39% |
| No recovered caller token yet (orphan risk) | `2,152` | 4.03% |
| No callsite evidence (no recorded code/data caller in the index) | `23,250` | 43.49% |
| Unclassified data xref (manual review) | `216` | 0.40% |
| RTTI-only | `4` | 0.01% |

### Not-yet-recovered (2,175 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `347` | 15.95% |
| Vtable-anchored (recover with the owning class) | `125` | 5.75% |
| Framework dispatch (wx/EH/Lua/reflection) | `10` | 0.46% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `771` | 35.45% |
| No indexed callsite evidence (needs investigation/evidence) | `919` | 42.25% |
