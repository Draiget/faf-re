...
- Progress coverage:  **`97.43%`**
  ...
  - *Total amount of completed tokens: `65,444`*

Progress DB status breakdown:

- `recovered`: `52,849` (80.75%)
...
- **`blocked`: `1,749` (2.60%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `1,783`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `1,783`*
...
### Recovered (52,849 functions) — binary caller context

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `16,458` | 31.14% |
...
| No callsite evidence (no recorded code/data caller in the index) | `22,864` | 43.26% |
| Unclassified data xref (manual review) | `216` | 0.41% |
| RTTI-only | `4` | 0.01% |

### Not-yet-recovered (1,783 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `204` | 11.44% |
| Vtable-anchored (recover with the owning class) | `131` | 7.35% |
| Framework dispatch (wx/EH/Lua/reflection) | `8` | 0.45% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `831` | 46.61% |
| No indexed callsite evidence (needs investigation/evidence) | `607` | 34.04% |
