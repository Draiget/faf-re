...
- Progress coverage:  **`95.91%`**
  ...
  - *Total amount of completed tokens: `64,417`*

Progress DB status breakdown:

- `recovered`: `52,744` (81.88%)
- `skip`: `6,249` (9.70%) — proven ICF aliases / thunks / EH or static-init glue with no distinct source body
- `external_dependency`: `5,424` (8.42%) — proven third-party/import-boundary code
...
- **`blocked`: `2,413` (3.59%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `2,821`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `2,821`*
...
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `16,314` | 30.93% |
...
### Not-yet-recovered (2,821 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `378` | 13.40% |
| Vtable-anchored (recover with the owning class) | `144` | 5.10% |
| Framework dispatch (wx/EH/Lua/reflection) | `13` | 0.46% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `1,035` | 36.69% |
| No indexed callsite evidence (needs investigation/evidence) | `1,248` | 44.24% |
