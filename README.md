...
- Progress coverage:  **`97.33%`**
  ...
  - *Total amount of completed tokens: `65,373`*

Progress DB status breakdown:

- `recovered`: `53,581` (81.96%)
- `skip`: `6,133` (9.38%) — proven ICF aliases / thunks / EH or static-init glue with no distinct source body
- `external_dependency`: `5,659` (8.66%) — proven third-party/import-boundary code
...
- **`blocked`: `1,792` (2.67%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `1,860`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `1,860`*
...
### Recovered (53,581 functions) — binary caller context

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `15,798` | 29.48% |
...
| No recovered caller token yet (orphan risk) | `2,470` | 4.61% |
| No callsite evidence (no recorded code/data caller in the index) | `23,714` | 44.26% |
| Unclassified data xref (manual review) | `216` | 0.40% |
| RTTI-only | `4` | 0.01% |

### Not-yet-recovered (1,860 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `279` | 15.00% |
| Vtable-anchored (recover with the owning class) | `179` | 9.62% |
| Framework dispatch (wx/EH/Lua/reflection) | `1` | 0.05% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `937` | 50.38% |
| No indexed callsite evidence (needs investigation/evidence) | `461` | 24.78% |
