...
- Progress coverage:  **`97.06%`**
  ...
  - *Total amount of completed tokens: `65,194`*

Progress DB status breakdown:

- `recovered`: `53,315` (81.78%)
...
- **`blocked`: `2,004` (2.98%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `2,008`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `2,008`*
...
### Recovered (53,315 functions) — wiring health

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Confirmed caller** (recovered binary caller wired by name) | `15,386` | 28.86% |
...
| Caller still blocked (orphan-helper risk — caller awaits recovery) | `2,647` | 4.96% |
| No callsite evidence (no recorded code/data caller in the index) | `23,750` | 44.55% |
| Unclassified data xref (manual review) | `215` | 0.40% |
| RTTI-only | `4` | 0.01% |

### Not-yet-recovered (2,008 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Recoverable now** (`OK_RECOVERED_CALLER` — recovered caller exists; recover next) | `314` | 15.64% |
| Vtable-anchored (recover with the owning class) | `209` | 10.41% |
| Framework dispatch (wx/EH/Lua/reflection) | `35` | 1.74% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `1,013` | 50.45% |
| No callsite evidence (likely external/dead — candidates for `external_dependency`) | `435` | 21.66% |
