...
- **`blocked`: `1,879` (2.80%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `1,882`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `1,882`*
...
|---|---:|---:|
| **Confirmed caller** (recovered binary caller wired by name) | `15,558` | 29.13% |
...
| Caller still blocked (orphan-helper risk — caller awaits recovery) | `2,572` | 4.82% |
| No callsite evidence (no recorded code/data caller in the index) | `23,710` | 44.40% |
| Unclassified data xref (manual review) | `215` | 0.40% |
| RTTI-only | `4` | 0.01% |

### Not-yet-recovered (1,882 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Recoverable now** (`OK_RECOVERED_CALLER` — recovered caller exists; recover next) | `282` | 14.98% |
| Vtable-anchored (recover with the owning class) | `178` | 9.46% |
| Framework dispatch (wx/EH/Lua/reflection) | `34` | 1.81% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `952` | 50.58% |
| No callsite evidence (likely external/dead — candidates for `external_dependency`) | `434` | 23.06% |
