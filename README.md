...
- Progress coverage:  **`96.76%`**
  ...
  - *Total amount of completed tokens: `64,991`*

Progress DB status breakdown:

- `recovered`: `53,110` (81.72%)
...
- **`blocked`: `2,205` (3.28%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `2,208`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `2,208`*
...
### Recovered (53,110 functions) — wiring health

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Confirmed caller** (recovered binary caller wired by name) | `14,998` | 28.24% |
...
### Not-yet-recovered (2,208 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Recoverable now** (`OK_RECOVERED_CALLER` — recovered caller exists; recover next) | `465` | 21.06% |
| Vtable-anchored (recover with the owning class) | `211` | 9.56% |
| Framework dispatch (wx/EH/Lua/reflection) | `35` | 1.59% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `1,055` | 47.78% |
| No callsite evidence (likely external/dead — candidates for `external_dependency`) | `440` | 19.93% |
