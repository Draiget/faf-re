...
- Total FAF functions: `67,167`
  - *IDA index, exported*
- Progress coverage:  **`95.87%`**
  - *Consists of `recovered` + `skip` + `external_dependency` ÷ exported*
  - *Total amount of completed tokens: `64,393`*

Progress DB status breakdown:

- `recovered`: `52,742` (81.91%)
...
- **`blocked`: `2,436` (3.63%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `2,844`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `2,844`*
...
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `16,277` | 30.86% |
...
### Not-yet-recovered (2,844 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `411` | 14.45% |
| Vtable-anchored (recover with the owning class) | `144` | 5.06% |
| Framework dispatch (wx/EH/Lua/reflection) | `13` | 0.46% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `1,035` | 36.39% |
| No indexed callsite evidence (needs investigation/evidence) | `1,238` | 43.53% |
