...
- Progress coverage:  **`96.75%`**
  ...
  - *Total amount of completed tokens: `64,983`*

Progress DB status breakdown:

- `recovered`: `53,102` (81.72%)
...
- **`blocked`: `2,211` (3.29%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `2,214`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `2,214`*
...
### Recovered (53,102 functions) — wiring health

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Confirmed caller** (recovered binary caller wired by name) | `14,987` | 28.22% |
...
| Caller still blocked (orphan-helper risk — caller awaits recovery) | `2,837` | 5.34% |
| No callsite evidence (no recorded code/data caller in the index) | `23,746` | 44.72% |
| Unclassified data xref (manual review) | `215` | 0.40% |
| RTTI-only | `4` | 0.01% |

### Not-yet-recovered (2,214 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Recoverable now** (`OK_RECOVERED_CALLER` — recovered caller exists; recover next) | `471` | 21.27% |
| Vtable-anchored (recover with the owning class) | `211` | 9.53% |
| Framework dispatch (wx/EH/Lua/reflection) | `35` | 1.58% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `1,055` | 47.65% |
| No callsite evidence (likely external/dead — candidates for `external_dependency`) | `440` | 19.87% |
