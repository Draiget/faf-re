...
- **`blocked`: `2,369` (3.53%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `2,372`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `2,372`*
...
### Recovered (52,945 functions) — wiring health

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Confirmed caller** (recovered binary caller wired by name) | `14,821` | 27.99% |
| Vtable-anchored (virtual override of a recovered class) | `5,792` | 10.94% |
| Framework dispatch (wx event, EH handler, Lua binding, reflection table, …) | `5,493` | 10.37% |
| Caller still blocked (orphan-helper risk — caller awaits recovery) | `2,889` | 5.46% |
| No callsite evidence (no recorded code/data caller in the index) | `23,731` | 44.82% |
| Unclassified data xref (manual review) | `215` | 0.41% |
| RTTI-only | `4` | 0.01% |

### Not-yet-recovered (2,372 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Recoverable now** (`OK_RECOVERED_CALLER` — recovered caller exists; recover next) | `558` | 23.52% |
| Vtable-anchored (recover with the owning class) | `238` | 10.03% |
| Framework dispatch (wx/EH/Lua/reflection) | `36` | 1.52% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `1,087` | 45.83% |
| No callsite evidence (likely external/dead — candidates for `external_dependency`) | `451` | 19.01% |
