...
- `recovered`: `52,746` (81.19%)
- `skip`: `6,714` (10.33%) — proven ICF aliases / thunks / EH or static-init glue with no distinct source body
- `external_dependency`: `5,506` (8.48%) — proven third-party/import-boundary code
  - *libpng, zlib, wxWidgets, LuaPlus/Lua, external Boost internals, WildMagic/Wm3, CRI Sofdec/ADX, undname, bugsplat, CRT imports*
- `needs_evidence`: `406` (0.60%)
- `in_progress`: `18` (0.03%)
- **`blocked`: `1,848` (2.75%)**
  - *strict circular/dep-blocked (in-DB literal `status == "blocked"`)*  
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `2,254`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `2,254`*
...
### Recovered (52,746 functions) — binary caller context

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `16,363` | 31.02% |
...
| No callsite evidence (no recorded code/data caller in the index) | `22,859` | 43.34% |
| Unclassified data xref (manual review) | `216` | 0.41% |
| RTTI-only | `4` | 0.01% |

### Not-yet-recovered (2,254 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `243` | 10.78% |
| Vtable-anchored (recover with the owning class) | `136` | 6.03% |
| Framework dispatch (wx/EH/Lua/reflection) | `10` | 0.44% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `878` | 38.95% |
| No indexed callsite evidence (needs investigation/evidence) | `985` | 43.70% |
