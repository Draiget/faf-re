...
- Total FAF functions: `67,167`
  - *IDA index, exported*
- Progress coverage:  **`96.68%`**
  - *Consists of `recovered` + `skip` + `external_dependency` ÷ exported*
  - *Total amount of completed tokens: `64,935`*

Progress DB status breakdown:

- `recovered`: `53,329` (82.13%)
...
- **`blocked`: `1,893` (2.82%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `2,302`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `2,302`*
...
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `16,339` | 30.64% |
| Vtable-anchored (virtual override of a recovered class) | `5,864` | 11.00% |
| Framework dispatch (wx event, EH handler, Lua binding, reflection table, …) | `5,515` | 10.34% |
| No recovered caller token yet (orphan risk) | `2,185` | 4.10% |
| No callsite evidence (no recorded code/data caller in the index) | `23,206` | 43.51% |
| Unclassified data xref (manual review) | `216` | 0.41% |
| RTTI-only | `4` | 0.01% |

### Not-yet-recovered (2,302 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `352` | 15.29% |
| Vtable-anchored (recover with the owning class) | `153` | 6.65% |
| Framework dispatch (wx/EH/Lua/reflection) | `45` | 1.95% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `785` | 34.10% |
| No indexed callsite evidence (needs investigation/evidence) | `964` | 41.88% |
