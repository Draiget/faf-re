...
- Progress coverage:  **`97.59%`**
  ...
  - *Total amount of completed tokens: `65,546`*
...
- **`blocked`: `1,649` (2.46%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `1,683`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `1,683`*
...
### Not-yet-recovered (1,683 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `170` | 10.10% |
| Vtable-anchored (recover with the owning class) | `129` | 7.66% |
| Framework dispatch (wx/EH/Lua/reflection) | `8` | 0.48% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `776` | 46.11% |
| No indexed callsite evidence (needs investigation/evidence) | `599` | 35.59% |
