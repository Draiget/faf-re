...
- Progress coverage:  **`97.73%`**
  ...
  - *Total amount of completed tokens: `65,642`*

Progress DB status breakdown:

- `recovered`: `52,980` (80.71%)
- `skip`: `7,115` (10.84%) — proven ICF aliases / thunks / EH or static-init glue with no distinct source body
...
- **`blocked`: `1,553` (2.31%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `1,587`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `1,587`*
...
### Recovered (52,980 functions) — binary caller context

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `16,629` | 31.39% |
...
### Not-yet-recovered (1,587 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `123` | 7.75% |
| Vtable-anchored (recover with the owning class) | `129` | 8.13% |
| Framework dispatch (wx/EH/Lua/reflection) | `8` | 0.50% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `728` | 45.87% |
| No indexed callsite evidence (needs investigation/evidence) | `598` | 37.68% |
