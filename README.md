...
- Total FAF functions: `67,167`
  - *IDA index, exported*
- Progress coverage:  **`97.67%`**
  - *Consists of `recovered` + `skip` + `external_dependency` ÷ exported*
  - *Total amount of completed tokens: `65,599`*

Progress DB status breakdown:

- `recovered`: `54,069` (82.42%)
...
- **`blocked`: `1,230` (1.83%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `1,638`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `1,638`*
...
### Recovered (54,069 functions) — binary caller context

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `16,685` | 30.86% |
...
### Not-yet-recovered (1,638 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `283` | 17.28% |
| Vtable-anchored (recover with the owning class) | `110` | 6.72% |
| Framework dispatch (wx/EH/Lua/reflection) | `1` | 0.06% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `705` | 43.04% |
| No indexed callsite evidence (needs investigation/evidence) | `537` | 32.78% |
