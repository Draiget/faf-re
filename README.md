...
### Recovered (53,121 functions) — wiring health

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Confirmed caller** (recovered binary caller wired by name) | `15,011` | 28.26% |
...
| Caller still blocked (orphan-helper risk — caller awaits recovery) | `2,832` | 5.33% |
...
|---|---:|---:|
| **Recoverable now** (`OK_RECOVERED_CALLER` — recovered caller exists; recover next) | `458` | 20.85% |
| Vtable-anchored (recover with the owning class) | `210` | 9.56% |
| Framework dispatch (wx/EH/Lua/reflection) | `35` | 1.59% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `1,053` | 47.93% |
| No callsite evidence (likely external/dead — candidates for `external_dependency`) | `439` | 19.98% |
