...
- Progress coverage:  **`97.20%`**
  ...
  - *Total amount of completed tokens: `65,284`*

Progress DB status breakdown:

- `recovered`: `53,670` (82.21%)
- `skip`: `6,115` (9.37%) — proven ICF aliases / thunks / EH or static-init glue with no distinct source body
- `external_dependency`: `5,499` (8.42%) — proven third-party/import-boundary code
...
### Recovered (53,670 functions) — binary caller context

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `15,995` | 29.80% |
...
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `315` | 16.24% |
| Vtable-anchored (recover with the owning class) | `193` | 9.95% |
| Framework dispatch (wx/EH/Lua/reflection) | `56` | 2.89% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `911` | 46.96% |
| No indexed callsite evidence (needs investigation/evidence) | `461` | 23.76% |
