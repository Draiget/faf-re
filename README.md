...
- Total FAF functions: `67,167`
  - *IDA index, exported*
- Progress coverage:  **`95.90%`**
  - *Consists of `recovered` + `skip` + `external_dependency` ÷ exported*
  - *Total amount of completed tokens: `64,414`*

Progress DB status breakdown:

- `recovered`: `52,761` (81.91%)
...
- **`blocked`: `2,415` (3.60%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `2,823`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `2,823`*
...
### Recovered (52,761 functions) — binary caller context

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `16,296` | 30.89% |
...
### Not-yet-recovered (2,823 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `390` | 13.82% |
| Vtable-anchored (recover with the owning class) | `144` | 5.10% |
| Framework dispatch (wx/EH/Lua/reflection) | `13` | 0.46% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `1,035` | 36.66% |
| No indexed callsite evidence (needs investigation/evidence) | `1,238` | 43.85% |
