...
- Total FAF functions: `67,167`
  - *IDA index, exported*
- Progress coverage:  **`96.36%`**
  - *Consists of `recovered` + `skip` + `external_dependency` ÷ exported*
  - *Total amount of completed tokens: `64,723`*

Progress DB status breakdown:

- `recovered`: `53,108` (82.05%)
...
- **`blocked`: `2,090` (3.11%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `2,514`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `2,514`*
...
### Recovered (53,108 functions) — binary caller context

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `16,129` | 30.37% |
| Vtable-anchored (virtual override of a recovered class) | `5,816` | 10.95% |
| Framework dispatch (wx event, EH handler, Lua binding, reflection table, …) | `5,491` | 10.34% |
| No recovered caller token yet (orphan risk) | `2,269` | 4.27% |
| No callsite evidence (no recorded code/data caller in the index) | `23,183` | 43.65% |
| Unclassified data xref (manual review) | `216` | 0.41% |
| RTTI-only | `4` | 0.01% |

### Not-yet-recovered (2,514 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `414` | 16.47% |
| Vtable-anchored (recover with the owning class) | `203` | 8.07% |
| Framework dispatch (wx/EH/Lua/reflection) | `68` | 2.70% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `840` | 33.41% |
| No indexed callsite evidence (needs investigation/evidence) | `985` | 39.18% |
