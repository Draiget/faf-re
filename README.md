...
- Progress coverage:  **`99.41%`**
  ...
  - *Total amount of completed tokens: `66,776`*

Progress DB status breakdown:

- `recovered`: `52,784` (79.05%)
...
- **`blocked`: `504` (0.75%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `507`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `507`*
...
### Recovered (52,784 functions) — binary caller context
...
### Not-yet-recovered (507 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `9` | 1.78% |
| Vtable-anchored (recover with the owning class) | `83` | 16.37% |
| Framework dispatch (wx/EH/Lua/reflection) | `0` | 0.00% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `137` | 27.02% |
| No indexed callsite evidence (needs investigation/evidence) | `278` | 54.83% |
