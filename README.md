...
- Total FAF functions: `67,167`
  - *IDA index, exported*
- Progress coverage:  **`96.53%`**
  - *Consists of `recovered` + `skip` + `external_dependency` ÷ exported*
  - *Total amount of completed tokens: `64,835`*

Progress DB status breakdown:

- `recovered`: `53,205` (82.06%)
...
- **`blocked`: `1,988` (2.96%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `2,402`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `2,402`*
...
### Recovered (53,205 functions) — binary caller context

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `16,223` | 30.49% |
...
| No recovered caller token yet (orphan risk) | `2,231` | 4.19% |
| No callsite evidence (no recorded code/data caller in the index) | `23,185` | 43.58% |
| Unclassified data xref (manual review) | `216` | 0.41% |
| RTTI-only | `4` | 0.01% |

### Not-yet-recovered (2,402 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `368` | 15.32% |
| Vtable-anchored (recover with the owning class) | `186` | 7.74% |
| Framework dispatch (wx/EH/Lua/reflection) | `45` | 1.87% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `816` | 33.97% |
| No indexed callsite evidence (needs investigation/evidence) | `983` | 40.92% |
