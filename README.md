...
- Progress coverage:  **`96.64%`**
  ...
  - *Total amount of completed tokens: `64,908`*

Progress DB status breakdown:

- `recovered`: `53,297` (82.11%)
- `skip`: `6,130` (9.44%) — proven ICF aliases / thunks / EH or static-init glue with no distinct source body
- `external_dependency`: `5,481` (8.44%) — proven third-party/import-boundary code
...
- **`blocked`: `1,917` (2.85%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `2,327`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `2,327`*
...
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `16,315` | 30.61% |
| Vtable-anchored (virtual override of a recovered class) | `5,855` | 10.99% |
| Framework dispatch (wx event, EH handler, Lua binding, reflection table, …) | `5,513` | 10.34% |
...
### Not-yet-recovered (2,327 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `350` | 15.04% |
| Vtable-anchored (recover with the owning class) | `162` | 6.96% |
| Framework dispatch (wx/EH/Lua/reflection) | `45` | 1.93% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `794` | 34.12% |
| No indexed callsite evidence (needs investigation/evidence) | `973` | 41.81% |
