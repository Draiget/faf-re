...
- Progress coverage:  **`97.79%`**
  ...
  - *Total amount of completed tokens: `65,682`*

Progress DB status breakdown:

- `recovered`: `53,020` (80.72%)
...
- **`blocked`: `1,513` (2.25%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `1,547`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `1,547`*
...
### Recovered (53,020 functions) — binary caller context

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `16,677` | 31.45% |
...
### Not-yet-recovered (1,547 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `93` | 6.01% |
| Vtable-anchored (recover with the owning class) | `129` | 8.34% |
| Framework dispatch (wx/EH/Lua/reflection) | `8` | 0.52% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `718` | 46.41% |
| No indexed callsite evidence (needs investigation/evidence) | `598` | 38.66% |
