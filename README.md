...
- Progress coverage:  **`96.70%`**
  ...
  - *Total amount of completed tokens: `64,948`*

Progress DB status breakdown:

- `recovered`: `53,345` (82.13%)
- `skip`: `6,128` (9.44%) — proven ICF aliases / thunks / EH or static-init glue with no distinct source body
- `external_dependency`: `5,475` (8.43%) — proven third-party/import-boundary code
...
- **`blocked`: `1,880` (2.80%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `2,289`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `2,289`*
...
### Recovered (53,345 functions) — binary caller context

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `16,356` | 30.66% |
| Vtable-anchored (virtual override of a recovered class) | `5,866` | 11.00% |
| Framework dispatch (wx event, EH handler, Lua binding, reflection table, …) | `5,518` | 10.34% |
| No recovered caller token yet (orphan risk) | `2,171` | 4.07% |
| No callsite evidence (no recorded code/data caller in the index) | `23,214` | 43.52% |
| Unclassified data xref (manual review) | `216` | 0.40% |
| RTTI-only | `4` | 0.01% |

### Not-yet-recovered (2,289 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `352` | 15.38% |
| Vtable-anchored (recover with the owning class) | `151` | 6.60% |
| Framework dispatch (wx/EH/Lua/reflection) | `44` | 1.92% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `783` | 34.21% |
| No indexed callsite evidence (needs investigation/evidence) | `956` | 41.76% |
