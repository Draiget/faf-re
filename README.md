...
- Total FAF functions: `67,167`
  - *IDA index, exported*
- Progress coverage:  **`98.42%`**
  - *Consists of `recovered` + `skip` + `external_dependency` ÷ exported*
  - *Total amount of completed tokens: `66,104`*

Progress DB status breakdown:

- `recovered`: `53,069` (80.28%)
- `skip`: `7,385` (11.17%) — proven ICF aliases / thunks / EH or static-init glue with no distinct source body
- `external_dependency`: `5,650` (8.55%) — proven third-party/import-boundary code
...
- **`blocked`: `1,094` (1.63%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `1,127`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `1,127`*
...
### Recovered (53,069 functions) — binary caller context

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `16,735` | 31.53% |
...
| No recovered caller token yet (orphan risk) | `1,789` | 3.37% |
| No callsite evidence (no recorded code/data caller in the index) | `22,875` | 43.10% |
| Unclassified data xref (manual review) | `216` | 0.41% |
| RTTI-only | `4` | 0.01% |

### Not-yet-recovered (1,127 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `52` | 4.61% |
| Vtable-anchored (recover with the owning class) | `124` | 11.00% |
| Framework dispatch (wx/EH/Lua/reflection) | `8` | 0.71% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `575` | 51.02% |
| No indexed callsite evidence (needs investigation/evidence) | `367` | 32.56% |
