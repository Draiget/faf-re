...
- Total FAF functions: `67,167`
  - *IDA index, exported*
- Progress coverage:  **`97.43%`**
  - *Consists of `recovered` + `skip` + `external_dependency` ÷ exported*
  - *Total amount of completed tokens: `65,438`*

Progress DB status breakdown:

- `recovered`: `52,843` (80.75%)
- `skip`: `7,078` (10.82%) — proven ICF aliases / thunks / EH or static-init glue with no distinct source body
- `external_dependency`: `5,517` (8.43%) — proven third-party/import-boundary code
...
- **`blocked`: `1,755` (2.61%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `1,789`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `1,789`*
...
### Recovered (52,843 functions) — binary caller context

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `16,452` | 31.13% |
| Vtable-anchored (virtual override of a recovered class) | `5,884` | 11.13% |
| Framework dispatch (wx event, EH handler, Lua binding, reflection table, …) | `5,563` | 10.53% |
| No recovered caller token yet (orphan risk) | `1,860` | 3.52% |
| No callsite evidence (no recorded code/data caller in the index) | `22,864` | 43.27% |
| Unclassified data xref (manual review) | `216` | 0.41% |
| RTTI-only | `4` | 0.01% |

### Not-yet-recovered (1,789 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `210` | 11.74% |
| Vtable-anchored (recover with the owning class) | `131` | 7.32% |
| Framework dispatch (wx/EH/Lua/reflection) | `8` | 0.45% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `831` | 46.45% |
| No indexed callsite evidence (needs investigation/evidence) | `607` | 33.93% |
