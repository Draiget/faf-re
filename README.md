...
- **`blocked`: `519` (0.77%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `522`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `522`*
...
### Not-yet-recovered (522 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `9` | 1.72% |
| Vtable-anchored (recover with the owning class) | `89` | 17.05% |
| Framework dispatch (wx/EH/Lua/reflection) | `0` | 0.00% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `143` | 27.39% |
| No indexed callsite evidence (needs investigation/evidence) | `281` | 53.83% |
