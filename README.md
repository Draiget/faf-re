...
- Progress coverage:  **`97.96%`**
  ...
  - *Total amount of completed tokens: `65,799`*

Progress DB status breakdown:

- `recovered`: `53,060` (80.64%)
- `skip`: `7,121` (10.82%) — proven ICF aliases / thunks / EH or static-init glue with no distinct source body
...
- **`blocked`: `1,399` (2.08%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `1,432`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `1,432`*
...
### Not-yet-recovered (1,432 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `59` | 4.12% |
| Vtable-anchored (recover with the owning class) | `126` | 8.80% |
| Framework dispatch (wx/EH/Lua/reflection) | `8` | 0.56% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `652` | 45.53% |
| No indexed callsite evidence (needs investigation/evidence) | `586` | 40.92% |
