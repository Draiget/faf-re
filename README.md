...
- Progress coverage:  **`95.92%`**
  ...
  - *Total amount of completed tokens: `64,428`*

Progress DB status breakdown:

- `recovered`: `52,775` (81.91%)
...
- **`blocked`: `2,401` (3.57%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `2,809`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `2,809`*
...
### Recovered (52,775 functions) — binary caller context

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `16,313` | 30.91% |
...
| No recovered caller token yet (orphan risk) | `1,862` | 3.53% |
| No callsite evidence (no recorded code/data caller in the index) | `22,943` | 43.47% |
| Unclassified data xref (manual review) | `216` | 0.41% |
| RTTI-only | `4` | 0.01% |

### Not-yet-recovered (2,809 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `376` | 13.39% |
| Vtable-anchored (recover with the owning class) | `144` | 5.13% |
| Framework dispatch (wx/EH/Lua/reflection) | `13` | 0.46% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `1,035` | 36.85% |
| No indexed callsite evidence (needs investigation/evidence) | `1,238` | 44.07% |
