...
- Progress coverage:  **`96.92%`**
  ...
  - *Total amount of completed tokens: `65,096`*

Progress DB status breakdown:

- `recovered`: `53,540` (82.25%)
...
- **`blocked`: `1,733` (2.58%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `2,141`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `2,141`*
...
### Recovered (53,540 functions) — binary caller context

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `16,478` | 30.78% |
...
### Not-yet-recovered (2,141 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `330` | 15.41% |
| Vtable-anchored (recover with the owning class) | `117` | 5.46% |
| Framework dispatch (wx/EH/Lua/reflection) | `1` | 0.05% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `779` | 36.38% |
| No indexed callsite evidence (needs investigation/evidence) | `912` | 42.60% |
