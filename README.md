...
- **`blocked`: `2,222` (3.31%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `2,225`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `2,225`*
...
### Recovered (53,091 functions) — wiring health

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Confirmed caller** (recovered binary caller wired by name) | `14,974` | 28.20% |
...
### Not-yet-recovered (2,225 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Recoverable now** (`OK_RECOVERED_CALLER` — recovered caller exists; recover next) | `478` | 21.48% |
| Vtable-anchored (recover with the owning class) | `211` | 9.48% |
| Framework dispatch (wx/EH/Lua/reflection) | `35` | 1.57% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `1,058` | 47.55% |
| No callsite evidence (likely external/dead — candidates for `external_dependency`) | `441` | 19.82% |
