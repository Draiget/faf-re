...
- Progress coverage:  **`97.84%`**
  ...
  - *Total amount of completed tokens: `65,719`*

Progress DB status breakdown:

- `recovered`: `54,065` (82.27%)
...
- **`blocked`: `1,110` (1.65%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `1,518`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `1,518`*
...
### Recovered (54,065 functions) — binary caller context

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `16,677` | 30.85% |
...
### Not-yet-recovered (1,518 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `164` | 10.80% |
| Vtable-anchored (recover with the owning class) | `110` | 7.25% |
| Framework dispatch (wx/EH/Lua/reflection) | `1` | 0.07% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `704` | 46.38% |
| No indexed callsite evidence (needs investigation/evidence) | `537` | 35.38% |
