...
- Progress coverage:  **`98.80%`**
  ...
  - *Total amount of completed tokens: `66,359`*

Progress DB status breakdown:

- `recovered`: `53,152` (80.10%)
...
- **`blocked`: `865` (1.29%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `872`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `872`*
...
### Recovered (53,152 functions) — binary caller context

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `16,867` | 31.73% |
...
| No recovered caller token yet (orphan risk) | `1,739` | 3.27% |
| No callsite evidence (no recorded code/data caller in the index) | `22,876` | 43.04% |
| Unclassified data xref (manual review) | `216` | 0.41% |
| RTTI-only | `4` | 0.01% |

### Not-yet-recovered (872 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `31` | 3.56% |
| Vtable-anchored (recover with the owning class) | `109` | 12.50% |
| Framework dispatch (wx/EH/Lua/reflection) | `0` | 0.00% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `371` | 42.55% |
| No indexed callsite evidence (needs investigation/evidence) | `360` | 41.28% |
