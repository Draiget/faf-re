...
- Progress coverage:  **`95.88%`**
  ...
  - *Total amount of completed tokens: `64,397`*

Progress DB status breakdown:

- `recovered`: `52,745` (81.91%)
...
- **`blocked`: `2,432` (3.62%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `2,840`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `2,840`*
...
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `16,280` | 30.87% |
...
### Not-yet-recovered (2,840 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `407` | 14.33% |
| Vtable-anchored (recover with the owning class) | `144` | 5.07% |
| Framework dispatch (wx/EH/Lua/reflection) | `13` | 0.46% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `1,035` | 36.44% |
| No indexed callsite evidence (needs investigation/evidence) | `1,238` | 43.59% |
