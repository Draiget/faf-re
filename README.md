...
- Progress coverage:  **`97.22%`**
  ...
  - *Total amount of completed tokens: `65,299`*

Progress DB status breakdown:

- `recovered`: `53,694` (82.23%)
...
- **`blocked`: `1,859` (2.77%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `1,925`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `1,925`*
...
### Recovered (53,694 functions) — binary caller context

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `16,018` | 29.83% |
| Vtable-anchored (virtual override of a recovered class) | `5,833` | 10.86% |
...
### Not-yet-recovered (1,925 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `309` | 16.05% |
| Vtable-anchored (recover with the owning class) | `188` | 9.77% |
| Framework dispatch (wx/EH/Lua/reflection) | `54` | 2.81% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `910` | 47.27% |
| No indexed callsite evidence (needs investigation/evidence) | `460` | 23.90% |
