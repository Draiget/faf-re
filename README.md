...
- Progress coverage:  **`97.36%`**
  ...
  - *Total amount of completed tokens: `65,394`*

Progress DB status breakdown:

- `recovered`: `52,803` (80.75%)
- `skip`: `7,084` (10.83%) — proven ICF aliases / thunks / EH or static-init glue with no distinct source body
...
- **`blocked`: `1,798` (2.68%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `1,832`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `1,832`*
...
### Recovered (52,803 functions) — binary caller context

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `16,416` | 31.09% |
...
| No callsite evidence (no recorded code/data caller in the index) | `22,861` | 43.29% |
| Unclassified data xref (manual review) | `216` | 0.41% |
| RTTI-only | `4` | 0.01% |

### Not-yet-recovered (1,832 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `235` | 12.83% |
| Vtable-anchored (recover with the owning class) | `131` | 7.15% |
| Framework dispatch (wx/EH/Lua/reflection) | `8` | 0.44% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `848` | 46.29% |
| No indexed callsite evidence (needs investigation/evidence) | `608` | 33.19% |
