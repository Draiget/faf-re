...
- Total FAF functions: `67,167`
  - *IDA index, exported*
- Progress coverage:  **`97.42%`**
  - *Consists of `recovered` + `skip` + `external_dependency` ÷ exported*
  - *Total amount of completed tokens: `65,433`*

Progress DB status breakdown:

- `recovered`: `52,836` (80.75%)
- `skip`: `7,080` (10.82%) — proven ICF aliases / thunks / EH or static-init glue with no distinct source body
...
- **`blocked`: `1,759` (2.62%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `1,793`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `1,793`*
...
### Recovered (52,836 functions) — binary caller context

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `16,447` | 31.13% |
| Vtable-anchored (virtual override of a recovered class) | `5,884` | 11.14% |
| Framework dispatch (wx event, EH handler, Lua binding, reflection table, …) | `5,562` | 10.53% |
| No recovered caller token yet (orphan risk) | `1,861` | 3.52% |
| No callsite evidence (no recorded code/data caller in the index) | `22,862` | 43.27% |
| Unclassified data xref (manual review) | `216` | 0.41% |
| RTTI-only | `4` | 0.01% |

### Not-yet-recovered (1,793 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `214` | 11.94% |
| Vtable-anchored (recover with the owning class) | `131` | 7.31% |
| Framework dispatch (wx/EH/Lua/reflection) | `8` | 0.45% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `831` | 46.35% |
| No indexed callsite evidence (needs investigation/evidence) | `607` | 33.85% |
