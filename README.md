...
- Progress coverage:  **`96.83%`**
  ...
  - *Total amount of completed tokens: `65,037`*

Progress DB status breakdown:

- `recovered`: `53,156` (81.73%)
...
- **`blocked`: `2,159` (3.21%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `2,162`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `2,162`*
...
### Recovered (53,156 functions) — wiring health

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Confirmed caller** (recovered binary caller wired by name) | `15,076` | 28.36% |
...
| Caller still blocked (orphan-helper risk — caller awaits recovery) | `2,802` | 5.27% |
| No callsite evidence (no recorded code/data caller in the index) | `23,747` | 44.67% |
| Unclassified data xref (manual review) | `215` | 0.40% |
| RTTI-only | `4` | 0.01% |

### Not-yet-recovered (2,162 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Recoverable now** (`OK_RECOVERED_CALLER` — recovered caller exists; recover next) | `423` | 19.57% |
| Vtable-anchored (recover with the owning class) | `210` | 9.71% |
| Framework dispatch (wx/EH/Lua/reflection) | `35` | 1.62% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `1,053` | 48.70% |
| No callsite evidence (likely external/dead — candidates for `external_dependency`) | `439` | 20.31% |
