...
- Total FAF functions: `67,167`
  - *IDA index, exported*
- Progress coverage:  **`97.17%`**
  - *Consists of `recovered` + `skip` + `external_dependency` ÷ exported*
  - *Total amount of completed tokens: `65,263`*

Progress DB status breakdown:

- `recovered`: `53,632` (82.18%)
...
- **`blocked`: `1,894` (2.82%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `1,961`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `1,961`*
...
### Recovered (53,632 functions) — binary caller context

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `15,962` | 29.76% |
...
| No callsite evidence (no recorded code/data caller in the index) | `23,717` | 44.22% |
| Unclassified data xref (manual review) | `216` | 0.40% |
| RTTI-only | `4` | 0.01% |

### Not-yet-recovered (1,961 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `326` | 16.62% |
| Vtable-anchored (recover with the owning class) | `193` | 9.84% |
| Framework dispatch (wx/EH/Lua/reflection) | `56` | 2.86% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `921` | 46.97% |
| No indexed callsite evidence (needs investigation/evidence) | `461` | 23.51% |
