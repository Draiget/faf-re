...
- Progress coverage:  **`96.59%`**
  ...
  - *Total amount of completed tokens: `64,877`*

Progress DB status breakdown:

- `recovered`: `53,241` (82.06%)
...
- **`blocked`: `1,933` (2.88%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `2,345`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `2,345`*
...
### Recovered (53,241 functions) — binary caller context

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `16,249` | 30.52% |
| Vtable-anchored (virtual override of a recovered class) | `5,839` | 10.97% |
| Framework dispatch (wx event, EH handler, Lua binding, reflection table, …) | `5,513` | 10.35% |
...
### Not-yet-recovered (2,345 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `351` | 14.97% |
| Vtable-anchored (recover with the owning class) | `169` | 7.21% |
| Framework dispatch (wx/EH/Lua/reflection) | `45` | 1.92% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `803` | 34.24% |
| No indexed callsite evidence (needs investigation/evidence) | `973` | 41.49% |
