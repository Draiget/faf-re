...
- Progress coverage:  **`96.85%`**
  ...
  - *Total amount of completed tokens: `65,052`*

Progress DB status breakdown:

- `recovered`: `53,445` (82.16%)
...
- **`blocked`: `1,777` (2.65%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `2,185`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `2,185`*
...
### Recovered (53,445 functions) — binary caller context
...
### Not-yet-recovered (2,185 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `347` | 15.88% |
| Vtable-anchored (recover with the owning class) | `127` | 5.81% |
| Framework dispatch (wx/EH/Lua/reflection) | `17` | 0.78% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `771` | 35.29% |
| No indexed callsite evidence (needs investigation/evidence) | `920` | 42.11% |
