...
- Progress coverage:  **`97.43%`**
  ...
  - *Total amount of completed tokens: `65,440`*

Progress DB status breakdown:

- `recovered`: `53,819` (82.24%)
...
- **`blocked`: `1,360` (2.02%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `1,784`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `1,784`*
...
### Recovered (53,819 functions) — binary caller context

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `16,233` | 30.16% |
...
| No recovered caller token yet (orphan risk) | `2,286` | 4.25% |
| No callsite evidence (no recorded code/data caller in the index) | `23,720` | 44.07% |
| Unclassified data xref (manual review) | `216` | 0.40% |
| RTTI-only | `4` | 0.01% |

### Not-yet-recovered (1,784 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `284` | 15.92% |
| Vtable-anchored (recover with the owning class) | `162` | 9.08% |
| Framework dispatch (wx/EH/Lua/reflection) | `54` | 3.03% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `836` | 46.86% |
| No indexed callsite evidence (needs investigation/evidence) | `444` | 24.89% |
