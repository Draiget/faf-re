...
- **`blocked`: `2,117` (3.15%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `2,120`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `2,120`*
...
### Recovered (53,200 functions) — wiring health

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Confirmed caller** (recovered binary caller wired by name) | `15,201` | 28.57% |
...
### Not-yet-recovered (2,120 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Recoverable now** (`OK_RECOVERED_CALLER` — recovered caller exists; recover next) | `388` | 18.30% |
| Vtable-anchored (recover with the owning class) | `210` | 9.91% |
| Framework dispatch (wx/EH/Lua/reflection) | `35` | 1.65% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `1,046` | 49.34% |
| No callsite evidence (likely external/dead — candidates for `external_dependency`) | `439` | 20.71% |
