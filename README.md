...
- Progress coverage:  **`96.62%`**
  ...
  - *Total amount of completed tokens: `64,898`*

Progress DB status breakdown:

- `recovered`: `53,011` (81.68%)
...
- **`blocked`: `2,301` (3.43%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `2,304`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `2,304`*
...
### Recovered (53,011 functions) — wiring health

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Confirmed caller** (recovered binary caller wired by name) | `14,923` | 28.15% |
| Vtable-anchored (virtual override of a recovered class) | `5,797` | 10.94% |
| Framework dispatch (wx event, EH handler, Lua binding, reflection table, …) | `5,494` | 10.36% |
| Caller still blocked (orphan-helper risk — caller awaits recovery) | `2,845` | 5.37% |
| No callsite evidence (no recorded code/data caller in the index) | `23,733` | 44.77% |
| Unclassified data xref (manual review) | `215` | 0.41% |
| RTTI-only | `4` | 0.01% |

### Not-yet-recovered (2,304 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Recoverable now** (`OK_RECOVERED_CALLER` — recovered caller exists; recover next) | `510` | 22.14% |
| Vtable-anchored (recover with the owning class) | `232` | 10.07% |
| Framework dispatch (wx/EH/Lua/reflection) | `35` | 1.52% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `1,076` | 46.70% |
| No callsite evidence (likely external/dead — candidates for `external_dependency`) | `449` | 19.49% |
