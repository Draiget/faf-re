...
- **`blocked`: `1,645` (2.45%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `2,053`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `2,053`*
...
### Recovered (53,638 functions) — binary caller context

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `16,567` | 30.89% |
...
### Not-yet-recovered (2,053 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `277` | 13.49% |
| Vtable-anchored (recover with the owning class) | `115` | 5.60% |
| Framework dispatch (wx/EH/Lua/reflection) | `1` | 0.05% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `747` | 36.39% |
| No indexed callsite evidence (needs investigation/evidence) | `911` | 44.37% |
