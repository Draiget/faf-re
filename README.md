...
### Recovered (53,497 functions) — binary caller context

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `16,405` | 30.67% |
...
| No callsite evidence (no recorded code/data caller in the index) | `23,262` | 43.48% |
...
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `345` | 16.06% |
| Vtable-anchored (recover with the owning class) | `118` | 5.49% |
| Framework dispatch (wx/EH/Lua/reflection) | `1` | 0.05% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `767` | 35.71% |
| No indexed callsite evidence (needs investigation/evidence) | `914` | 42.55% |
