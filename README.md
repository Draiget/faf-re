...
- **`blocked`: `1,747` (2.60%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `2,156`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `2,156`*
...
### Recovered (53,488 functions) — binary caller context
...
### Not-yet-recovered (2,156 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `346` | 16.05% |
| Vtable-anchored (recover with the owning class) | `122` | 5.66% |
| Framework dispatch (wx/EH/Lua/reflection) | `1` | 0.05% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `769` | 35.67% |
| No indexed callsite evidence (needs investigation/evidence) | `915` | 42.44% |
