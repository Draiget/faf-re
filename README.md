...
- Total FAF functions: `67,170`
  - *IDA index, exported*
- Progress coverage:  **`99.41%`**
  - *Consists of `recovered` + `skip` + `external_dependency` ÷ exported*
  - *Total amount of completed tokens: `66,773`*

Progress DB status breakdown:

- `recovered`: `52,781` (79.05%)
...
- **`blocked`: `507` (0.75%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `510`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `510`*
...
### Not-yet-recovered (510 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `9` | 1.76% |
| Vtable-anchored (recover with the owning class) | `83` | 16.27% |
| Framework dispatch (wx/EH/Lua/reflection) | `0` | 0.00% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `140` | 27.45% |
| No indexed callsite evidence (needs investigation/evidence) | `278` | 54.51% |
