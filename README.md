...
- Progress coverage:  **`96.38%`**
  ...
  - *Total amount of completed tokens: `64,735`*

Progress DB status breakdown:

- `recovered`: `53,120` (82.06%)
...
- **`blocked`: `2,080` (3.10%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `2,504`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `2,504`*
...
### Recovered (53,120 functions) — binary caller context
...
| Framework dispatch (wx event, EH handler, Lua binding, reflection table, …) | `5,498` | 10.35% |
...
### Not-yet-recovered (2,504 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `415` | 16.57% |
| Vtable-anchored (recover with the owning class) | `203` | 8.11% |
| Framework dispatch (wx/EH/Lua/reflection) | `61` | 2.44% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `834` | 33.31% |
| No indexed callsite evidence (needs investigation/evidence) | `987` | 39.42% |
