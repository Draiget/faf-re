...
- Progress coverage:  **`98.47%`**
  ...
  - *Total amount of completed tokens: `66,139`*
...
- **`blocked`: `1,059` (1.58%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `1,092`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `1,092`*
...
### Not-yet-recovered (1,092 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `46` | 4.21% |
| Vtable-anchored (recover with the owning class) | `124` | 11.36% |
| Framework dispatch (wx/EH/Lua/reflection) | `8` | 0.73% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `547` | 50.09% |
| No indexed callsite evidence (needs investigation/evidence) | `366` | 33.52% |
