...
- Progress coverage:  **`97.33%`**
  ...
  - *Total amount of completed tokens: `65,372`*

Progress DB status breakdown:

- `recovered`: `53,766` (82.25%)
...
### Recovered (53,766 functions) — binary caller context

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `16,134` | 30.01% |
| Vtable-anchored (virtual override of a recovered class) | `5,849` | 10.88% |
| Framework dispatch (wx event, EH handler, Lua binding, reflection table, …) | `5,504` | 10.24% |
| No recovered caller token yet (orphan risk) | `2,339` | 4.35% |
| No callsite evidence (no recorded code/data caller in the index) | `23,720` | 44.12% |
...
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `308` | 16.63% |
| Vtable-anchored (recover with the owning class) | `169` | 9.13% |
| Framework dispatch (wx/EH/Lua/reflection) | `54` | 2.92% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `858` | 46.33% |
| No indexed callsite evidence (needs investigation/evidence) | `459` | 24.78% |
