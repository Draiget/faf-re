...
- Progress coverage:  **`98.52%`**
  ...
  - *Total amount of completed tokens: `66,172`*
...
- **`blocked`: `1,039` (1.55%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `1,059`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `1,059`*
...
### Not-yet-recovered (1,059 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `40` | 3.78% |
| Vtable-anchored (recover with the owning class) | `120` | 11.33% |
| Framework dispatch (wx/EH/Lua/reflection) | `8` | 0.76% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `524` | 49.48% |
| No indexed callsite evidence (needs investigation/evidence) | `366` | 34.56% |
