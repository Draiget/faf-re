...
### Recovered (53,538 functions) — binary caller context

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `16,559` | 30.93% |
...
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `301` | 13.95% |
| Vtable-anchored (recover with the owning class) | `112` | 5.19% |
| Framework dispatch (wx/EH/Lua/reflection) | `1` | 0.05% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `757` | 35.08% |
| No indexed callsite evidence (needs investigation/evidence) | `985` | 45.64% |
