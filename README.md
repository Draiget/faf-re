...
- Total FAF functions: `67,167`
  - *IDA index, exported*
- Progress coverage:  **`97.06%`**
  - *Consists of `recovered` + `skip` + `external_dependency` ÷ exported*
  - *Total amount of completed tokens: `65,192`*

Progress DB status breakdown:

- `recovered`: `53,657` (82.31%)
- `skip`: `6,114` (9.38%) — proven ICF aliases / thunks / EH or static-init glue with no distinct source body
- `external_dependency`: `5,421` (8.32%) — proven third-party/import-boundary code
...
- **`blocked`: `1,637` (2.44%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `2,045`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `2,045`*
...
### Recovered (53,657 functions) — binary caller context

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `16,593` | 30.92% |
| Vtable-anchored (virtual override of a recovered class) | `5,910` | 11.01% |
| Framework dispatch (wx event, EH handler, Lua binding, reflection table, …) | `5,568` | 10.38% |
| No recovered caller token yet (orphan risk) | `2,098` | 3.91% |
| No callsite evidence (no recorded code/data caller in the index) | `23,267` | 43.36% |
| Unclassified data xref (manual review) | `217` | 0.40% |
| RTTI-only | `4` | 0.01% |

### Not-yet-recovered (2,045 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `279` | 13.64% |
| Vtable-anchored (recover with the owning class) | `112` | 5.48% |
| Framework dispatch (wx/EH/Lua/reflection) | `1` | 0.05% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `740` | 36.19% |
| No indexed callsite evidence (needs investigation/evidence) | `911` | 44.55% |
