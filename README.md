...
- Progress coverage:  **`96.52%`**
  ...
  - *Total amount of completed tokens: `64,830`*

Progress DB status breakdown:

- `recovered`: `52,943` (81.66%)
...
- **`blocked`: `2,371` (3.53%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `2,374`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `2,374`*
...
### Recovered (52,943 functions) — wiring health
...
### Not-yet-recovered (2,374 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Recoverable now** (`OK_RECOVERED_CALLER` — recovered caller exists; recover next) | `558` | 23.50% |
| Vtable-anchored (recover with the owning class) | `239` | 10.07% |
| Framework dispatch (wx/EH/Lua/reflection) | `36` | 1.52% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `1,088` | 45.83% |
| No callsite evidence (likely external/dead — candidates for `external_dependency`) | `451` | 19.00% |
