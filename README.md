...
- **`blocked`: `1,745` (2.60%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `2,154`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `2,154`*
...
### Recovered (53,491 functions) — binary caller context

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `16,401` | 30.66% |
| Vtable-anchored (virtual override of a recovered class) | `5,895` | 11.02% |
...
### Not-yet-recovered (2,154 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `346` | 16.06% |
| Vtable-anchored (recover with the owning class) | `121` | 5.62% |
| Framework dispatch (wx/EH/Lua/reflection) | `1` | 0.05% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `769` | 35.70% |
| No indexed callsite evidence (needs investigation/evidence) | `914` | 42.43% |
