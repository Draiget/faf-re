...
- Progress coverage:  **`96.59%`**
  ...
  - *Total amount of completed tokens: `64,876`*

Progress DB status breakdown:

- `recovered`: `52,989` (81.68%)
...
- **`blocked`: `2,323` (3.46%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `2,326`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `2,326`*
...
### Recovered (52,989 functions) — wiring health

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Confirmed caller** (recovered binary caller wired by name) | `14,890` | 28.10% |
...
### Not-yet-recovered (2,326 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Recoverable now** (`OK_RECOVERED_CALLER` — recovered caller exists; recover next) | `525` | 22.57% |
| Vtable-anchored (recover with the owning class) | `233` | 10.02% |
| Framework dispatch (wx/EH/Lua/reflection) | `35` | 1.50% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `1,082` | 46.52% |
