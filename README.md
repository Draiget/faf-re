...
- Progress coverage:  **`99.43%`**
  ...
  - *Total amount of completed tokens: `66,786`*

Progress DB status breakdown:

- `recovered`: `52,836` (79.11%)
- `skip`: `8,212` (12.30%) — proven ICF aliases / thunks / EH or static-init glue with no distinct source body
- `external_dependency`: `5,738` (8.59%) — proven third-party/import-boundary code
...
- **`blocked`: `494` (0.74%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `497`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `497`*
...
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `17,184` | 32.52% |
...
| No recovered caller token yet (orphan risk) | `1,645` | 3.11% |
| No callsite evidence (no recorded code/data caller in the index) | `22,262` | 42.13% |
| Unclassified data xref (manual review) | `216` | 0.41% |
| RTTI-only | `4` | 0.01% |

### Not-yet-recovered (497 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `7` | 1.41% |
| Vtable-anchored (recover with the owning class) | `82` | 16.50% |
| Framework dispatch (wx/EH/Lua/reflection) | `0` | 0.00% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `130` | 26.16% |
| No indexed callsite evidence (needs investigation/evidence) | `278` | 55.94% |
