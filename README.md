...
- Progress coverage:  **`97.02%`**
  ...
  - *Total amount of completed tokens: `65,166`*

Progress DB status breakdown:

- `recovered`: `53,284` (81.77%)
...
- **`blocked`: `2,032` (3.03%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `2,036`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `2,036`*
...
### Recovered (53,284 functions) — wiring health

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Confirmed caller** (recovered binary caller wired by name) | `15,345` | 28.80% |
...
| Caller still blocked (orphan-helper risk — caller awaits recovery) | `2,656` | 4.98% |
| No callsite evidence (no recorded code/data caller in the index) | `23,750` | 44.57% |
| Unclassified data xref (manual review) | `215` | 0.40% |
| RTTI-only | `4` | 0.01% |

### Not-yet-recovered (2,036 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Recoverable now** (`OK_RECOVERED_CALLER` — recovered caller exists; recover next) | `333` | 16.36% |
| Vtable-anchored (recover with the owning class) | `209` | 10.27% |
| Framework dispatch (wx/EH/Lua/reflection) | `35` | 1.72% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `1,022` | 50.20% |
| No callsite evidence (likely external/dead — candidates for `external_dependency`) | `435` | 21.37% |
