...
## Recovery Coverage (`18/08/2026`, `fa_full_2026_03_26`)

Progress snapshot:

- Total FAF functions: `67,167`
  - *IDA index, exported*
- Progress coverage:  **`97.28%`**
  - *Consists of `recovered` + `skip` + `external_dependency` ÷ exported*
  - *Total amount of completed tokens: `65,338`*

Progress DB status breakdown:

- `recovered`: `53,743` (82.25%)
- `skip`: `6,108` (9.35%) — proven ICF aliases / thunks / EH or static-init glue with no distinct source body
- `external_dependency`: `5,487` (8.40%) — proven third-party/import-boundary code
  - *libpng, zlib, wxWidgets, LuaPlus/Lua, external Boost internals, WildMagic/Wm3, CRI Sofdec/ADX, undname, bugsplat, CRT imports*
- `needs_evidence`: `66` (0.10%)
- `in_progress`: `14` (0.02%)
- **`blocked`: `1,820` (2.71%)**
  - *strict circular/dep-blocked (in-DB literal `status == "blocked"`)*  
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `1,886`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `1,886`*
...
### Recovered (53,743 functions) — binary caller context

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `16,098` | 29.95% |
| Vtable-anchored (virtual override of a recovered class) | `5,844` | 10.87% |
| Framework dispatch (wx event, EH handler, Lua binding, reflection table, …) | `5,504` | 10.24% |
| No recovered caller token yet (orphan risk) | `2,358` | 4.39% |
| No callsite evidence (no recorded code/data caller in the index) | `23,719` | 44.13% |
| Unclassified data xref (manual review) | `216` | 0.40% |
| RTTI-only | `4` | 0.01% |

### Not-yet-recovered (1,886 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `318` | 16.86% |
| Vtable-anchored (recover with the owning class) | `178` | 9.44% |
| Framework dispatch (wx/EH/Lua/reflection) | `54` | 2.86% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `872` | 46.24% |
| No indexed callsite evidence (needs investigation/evidence) | `460` | 24.39% |
