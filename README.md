...
- Progress coverage:  **`99.30%`**
  ...
  - *Total amount of completed tokens: `66,700`*

Progress DB status breakdown:

- `recovered`: `52,750` (79.09%)
- `skip`: `8,206` (12.30%) — proven ICF aliases / thunks / EH or static-init glue with no distinct source body
...
- **`blocked`: `577` (0.86%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `580`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `580`*
...
### Not-yet-recovered (580 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `9` | 1.55% |
| Vtable-anchored (recover with the owning class) | `106` | 18.28% |
| Framework dispatch (wx/EH/Lua/reflection) | `0` | 0.00% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `159` | 27.41% |
| No indexed callsite evidence (needs investigation/evidence) | `305` | 52.59% |
