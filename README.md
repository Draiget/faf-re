...
- Progress coverage:  **`96.72%`**
  ...
  - *Total amount of completed tokens: `64,964`*

Progress DB status breakdown:

- `recovered`: `53,083` (81.71%)
...
### Recovered (53,083 functions) — wiring health

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Confirmed caller** (recovered binary caller wired by name) | `14,966` | 28.19% |
...
|---|---:|---:|
| **Recoverable now** (`OK_RECOVERED_CALLER` — recovered caller exists; recover next) | `482` | 21.59% |
| Vtable-anchored (recover with the owning class) | `211` | 9.45% |
| Framework dispatch (wx/EH/Lua/reflection) | `35` | 1.57% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `1,062` | 47.56% |
| No callsite evidence (likely external/dead — candidates for `external_dependency`) | `441` | 19.75% |
