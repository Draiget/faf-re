...
- Total FAF functions: `67,167`
  - *IDA index, exported*
- Progress coverage:  **`97.59%`**
  - *Consists of `recovered` + `skip` + `external_dependency` ÷ exported*
  - *Total amount of completed tokens: `65,545`*

Progress DB status breakdown:

- `recovered`: `52,930` (80.75%)
- `skip`: `7,092` (10.82%) — proven ICF aliases / thunks / EH or static-init glue with no distinct source body
- `external_dependency`: `5,523` (8.43%) — proven third-party/import-boundary code
...
- **`blocked`: `1,650` (2.46%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `1,684`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `1,684`*
...
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `16,551` | 31.27% |
...
| No recovered caller token yet (orphan risk) | `1,837` | 3.47% |
| No callsite evidence (no recorded code/data caller in the index) | `22,873` | 43.21% |
| Unclassified data xref (manual review) | `216` | 0.41% |
| RTTI-only | `4` | 0.01% |

### Not-yet-recovered (1,684 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `171` | 10.15% |
| Vtable-anchored (recover with the owning class) | `129` | 7.66% |
| Framework dispatch (wx/EH/Lua/reflection) | `8` | 0.48% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `776` | 46.08% |
| No indexed callsite evidence (needs investigation/evidence) | `599` | 35.57% |
