...
- **`blocked`: `823` (1.23%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `826`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `826`*
...
### Recovered (53,246 functions) — binary caller context

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `16,974` | 31.88% |
...
### Not-yet-recovered (826 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `27` | 3.27% |
| Vtable-anchored (recover with the owning class) | `109` | 13.20% |
| Framework dispatch (wx/EH/Lua/reflection) | `0` | 0.00% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `353` | 42.74% |
| No indexed callsite evidence (needs investigation/evidence) | `336` | 40.68% |
