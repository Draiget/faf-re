...
- Progress coverage:  **`97.31%`**
  ...
  - *Total amount of completed tokens: `65,358`*
...
- **`blocked`: `1,801` (2.68%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `1,866`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `1,866`*
...
### Not-yet-recovered (1,866 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `308` | 16.51% |
| Vtable-anchored (recover with the owning class) | `174` | 9.32% |
| Framework dispatch (wx/EH/Lua/reflection) | `54` | 2.89% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `867` | 46.46% |
| No indexed callsite evidence (needs investigation/evidence) | `459` | 24.60% |
