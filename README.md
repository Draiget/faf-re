...
- Progress coverage:  **`99.38%`**
  ...
  - *Total amount of completed tokens: `66,752`*

Progress DB status breakdown:

- `recovered`: `52,751` (79.03%)
- `skip`: `8,252` (12.36%) — proven ICF aliases / thunks / EH or static-init glue with no distinct source body
- `external_dependency`: `5,749` (8.61%) — proven third-party/import-boundary code
...
- **`blocked`: `525` (0.78%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `528`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `528`*
...
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `17,173` | 32.55% |
...
### Not-yet-recovered (528 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `9` | 1.70% |
| Vtable-anchored (recover with the owning class) | `89` | 16.86% |
| Framework dispatch (wx/EH/Lua/reflection) | `0` | 0.00% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `148` | 28.03% |
| No indexed callsite evidence (needs investigation/evidence) | `282` | 53.41% |
