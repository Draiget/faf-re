...
- Total FAF functions: `67,167`
  - *IDA index, exported*
- Progress coverage:  **`97.52%`**
  - *Consists of `recovered` + `skip` + `external_dependency` ÷ exported*
  - *Total amount of completed tokens: `65,503`*

Progress DB status breakdown:

- `recovered`: `52,899` (80.76%)
- `skip`: `7,084` (10.81%) — proven ICF aliases / thunks / EH or static-init glue with no distinct source body
- `external_dependency`: `5,520` (8.43%) — proven third-party/import-boundary code
...
- **`blocked`: `1,691` (2.52%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `1,725`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `1,725`*
...
### Recovered (52,899 functions) — binary caller context

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `16,511` | 31.21% |
...
### Not-yet-recovered (1,725 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `188` | 10.90% |
| Vtable-anchored (recover with the owning class) | `130` | 7.54% |
| Framework dispatch (wx/EH/Lua/reflection) | `8` | 0.46% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `797` | 46.20% |
| No indexed callsite evidence (needs investigation/evidence) | `601` | 34.84% |
