...
- Progress coverage:  **`96.58%`**
  ...
  - *Total amount of completed tokens: `64,869`*

Progress DB status breakdown:

- `recovered`: `52,982` (81.68%)
...
- **`blocked`: `2,331` (3.47%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `2,334`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `2,334`*
...
### Recovered (52,982 functions) — wiring health

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Confirmed caller** (recovered binary caller wired by name) | `14,881` | 28.09% |
...
### Not-yet-recovered (2,334 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Recoverable now** (`OK_RECOVERED_CALLER` — recovered caller exists; recover next) | `533` | 22.84% |
| Vtable-anchored (recover with the owning class) | `233` | 9.98% |
| Framework dispatch (wx/EH/Lua/reflection) | `35` | 1.50% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `1,082` | 46.36% |
| No callsite evidence (likely external/dead — candidates for `external_dependency`) | `449` | 19.24% |
