...
- **`blocked`: `1,540` (2.29%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `1,964`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `1,964`*
...
### Recovered (53,644 functions) — binary caller context

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `16,127` | 30.06% |
...
### Not-yet-recovered (1,964 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `401` | 20.42% |
| Vtable-anchored (recover with the owning class) | `179` | 9.11% |
| Framework dispatch (wx/EH/Lua/reflection) | `64` | 3.26% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `851` | 43.33% |
| No indexed callsite evidence (needs investigation/evidence) | `465` | 23.68% |
