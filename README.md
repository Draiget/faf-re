...
- Progress coverage:  **`98.72%`**
  ...
  - *Total amount of completed tokens: `66,306`*

Progress DB status breakdown:

- `recovered`: `53,117` (80.11%)
...
- **`blocked`: `912` (1.36%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `925`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `925`*
...
### Recovered (53,117 functions) — binary caller context

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `16,822` | 31.67% |
...
### Not-yet-recovered (925 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `32` | 3.46% |
| Vtable-anchored (recover with the owning class) | `118` | 12.76% |
| Framework dispatch (wx/EH/Lua/reflection) | `8` | 0.86% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `402` | 43.46% |
| No indexed callsite evidence (needs investigation/evidence) | `364` | 39.35% |
