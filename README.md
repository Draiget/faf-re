...
- Progress coverage:  **`95.92%`**
  ...
  - *Total amount of completed tokens: `64,427`*

Progress DB status breakdown:

- `recovered`: `52,752` (81.88%)
...
- **`blocked`: `2,403` (3.58%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `2,811`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `2,811`*
...
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `16,324` | 30.94% |
...
### Not-yet-recovered (2,811 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `369` | 13.13% |
| Vtable-anchored (recover with the owning class) | `144` | 5.12% |
| Framework dispatch (wx/EH/Lua/reflection) | `13` | 0.46% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `1,034` | 36.78% |
| No indexed callsite evidence (needs investigation/evidence) | `1,248` | 44.40% |
