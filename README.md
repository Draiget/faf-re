...
- Progress coverage:  **`96.91%`**
  ...
  - *Total amount of completed tokens: `65,089`*

Progress DB status breakdown:

- `recovered`: `53,548` (82.27%)
- `skip`: `6,113` (9.39%) — proven ICF aliases / thunks / EH or static-init glue with no distinct source body
- `external_dependency`: `5,428` (8.34%) — proven third-party/import-boundary code
...
### Recovered (53,548 functions) — binary caller context

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `16,571` | 30.95% |
...
| No recovered caller token yet (orphan risk) | `2,084` | 3.89% |
...
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `303` | 14.11% |
| Vtable-anchored (recover with the owning class) | `112` | 5.21% |
| Framework dispatch (wx/EH/Lua/reflection) | `1` | 0.05% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `745` | 34.68% |
| No indexed callsite evidence (needs investigation/evidence) | `985` | 45.86% |
