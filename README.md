...
### Recovered (53,522 functions) — binary caller context

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `16,440` | 30.72% |
...
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `343` | 15.89% |
| Vtable-anchored (recover with the owning class) | `117` | 5.42% |
| Framework dispatch (wx/EH/Lua/reflection) | `1` | 0.05% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `783` | 36.28% |
| No indexed callsite evidence (needs investigation/evidence) | `912` | 42.26% |
