...
### Recovered (53,523 functions) — binary caller context

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `16,441` | 30.72% |
...
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `342` | 15.86% |
| Vtable-anchored (recover with the owning class) | `117` | 5.42% |
| Framework dispatch (wx/EH/Lua/reflection) | `1` | 0.05% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `783` | 36.30% |
| No indexed callsite evidence (needs investigation/evidence) | `912` | 42.28% |
