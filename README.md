...
- Total FAF functions: `67,167`
  - *IDA index, exported*
- Progress coverage:  **`97.70%`**
  - *Consists of `recovered` + `skip` + `external_dependency` ÷ exported*
  - *Total amount of completed tokens: `65,625`*

Progress DB status breakdown:

- `recovered`: `52,967` (80.71%)
- `skip`: `7,113` (10.84%) — proven ICF aliases / thunks / EH or static-init glue with no distinct source body
- `external_dependency`: `5,545` (8.45%) — proven third-party/import-boundary code
...
- **`blocked`: `1,570` (2.34%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `1,604`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `1,604`*
...
### Recovered (52,967 functions) — binary caller context

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `16,616` | 31.37% |
...
| No recovered caller token yet (orphan risk) | `1,809` | 3.42% |
| No callsite evidence (no recorded code/data caller in the index) | `22,873` | 43.18% |
| Unclassified data xref (manual review) | `216` | 0.41% |
| RTTI-only | `4` | 0.01% |

### Not-yet-recovered (1,604 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `137` | 8.54% |
| Vtable-anchored (recover with the owning class) | `129` | 8.04% |
| Framework dispatch (wx/EH/Lua/reflection) | `8` | 0.50% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `730` | 45.51% |
| No indexed callsite evidence (needs investigation/evidence) | `599` | 37.34% |
