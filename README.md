...
- Progress coverage:  **`96.46%`**
  ...
  - *Total amount of completed tokens: `64,787`*

Progress DB status breakdown:

- `recovered`: `53,163` (82.06%)
...
- **`blocked`: `2,032` (3.03%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `2,450`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `2,450`*
...
### Recovered (53,163 functions) — binary caller context

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `16,198` | 30.47% |
...
### Not-yet-recovered (2,450 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `379` | 15.47% |
| Vtable-anchored (recover with the owning class) | `203` | 8.29% |
| Framework dispatch (wx/EH/Lua/reflection) | `55` | 2.24% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `826` | 33.71% |
| No indexed callsite evidence (needs investigation/evidence) | `983` | 40.12% |
