...
- Total FAF functions: `67,167`
  - *IDA index, exported*
- Progress coverage:  **`95.98%`**
  - *Consists of `recovered` + `skip` + `external_dependency` ÷ exported*
  - *Total amount of completed tokens: `64,464`*

Progress DB status breakdown:

- `recovered`: `52,723` (81.79%)
- `skip`: `6,304` (9.78%) — proven ICF aliases / thunks / EH or static-init glue with no distinct source body
- `external_dependency`: `5,437` (8.43%) — proven third-party/import-boundary code
...
- **`blocked`: `2,366` (3.52%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `2,774`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `2,774`*
...
### Recovered (52,723 functions) — binary caller context

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `16,328` | 30.97% |
...
| No recovered caller token yet (orphan risk) | `1,847` | 3.50% |
| No callsite evidence (no recorded code/data caller in the index) | `22,893` | 43.42% |
| Unclassified data xref (manual review) | `216` | 0.41% |
| RTTI-only | `4` | 0.01% |

### Not-yet-recovered (2,774 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `351` | 12.65% |
| Vtable-anchored (recover with the owning class) | `144` | 5.19% |
| Framework dispatch (wx/EH/Lua/reflection) | `13` | 0.47% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `1,021` | 36.81% |
| No indexed callsite evidence (needs investigation/evidence) | `1,242` | 44.77% |
