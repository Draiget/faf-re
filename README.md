...
- Total FAF functions: `67,167`
  - *IDA index, exported*
- Progress coverage:  **`96.73%`**
  - *Consists of `recovered` + `skip` + `external_dependency` ÷ exported*
  - *Total amount of completed tokens: `64,971`*

Progress DB status breakdown:

- `recovered`: `53,366` (82.14%)
...
- **`blocked`: `1,858` (2.77%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `2,266`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `2,266`*
...
### Recovered (53,366 functions) — binary caller context
...
| Framework dispatch (wx event, EH handler, Lua binding, reflection table, …) | `5,522` | 10.35% |
| No recovered caller token yet (orphan risk) | `2,163` | 4.05% |
| No callsite evidence (no recorded code/data caller in the index) | `23,219` | 43.51% |
| Unclassified data xref (manual review) | `216` | 0.40% |
| RTTI-only | `4` | 0.01% |

### Not-yet-recovered (2,266 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `350` | 15.45% |
| Vtable-anchored (recover with the owning class) | `150` | 6.62% |
| Framework dispatch (wx/EH/Lua/reflection) | `40` | 1.77% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `773` | 34.11% |
| No indexed callsite evidence (needs investigation/evidence) | `950` | 41.92% |
