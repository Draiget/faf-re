...
- **`blocked`: `2,366` (3.52%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `2,369`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `2,369`*
...
### Recovered (52,948 functions) — wiring health

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Confirmed caller** (recovered binary caller wired by name) | `14,825` | 28.00% |
...
| Caller still blocked (orphan-helper risk — caller awaits recovery) | `2,888` | 5.45% |
| No callsite evidence (no recorded code/data caller in the index) | `23,731` | 44.82% |
| Unclassified data xref (manual review) | `215` | 0.41% |
| RTTI-only | `4` | 0.01% |

### Not-yet-recovered (2,369 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Recoverable now** (`OK_RECOVERED_CALLER` — recovered caller exists; recover next) | `557` | 23.51% |
| Vtable-anchored (recover with the owning class) | `238` | 10.05% |
| Framework dispatch (wx/EH/Lua/reflection) | `36` | 1.52% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `1,085` | 45.80% |
| No callsite evidence (likely external/dead — candidates for `external_dependency`) | `451` | 19.04% |
