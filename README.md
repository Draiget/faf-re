...
- Progress coverage:  **`97.38%`**
  ...
  - *Total amount of completed tokens: `65,405`*

Progress DB status breakdown:

- `recovered`: `53,772` (82.21%)
...
- **`blocked`: `1,751` (2.61%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `1,819`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `1,819`*
...
### Recovered (53,772 functions) — binary caller context

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `16,019` | 29.79% |
| Vtable-anchored (virtual override of a recovered class) | `5,845` | 10.87% |
...
| No callsite evidence (no recorded code/data caller in the index) | `23,720` | 44.11% |
| Unclassified data xref (manual review) | `217` | 0.40% |
| RTTI-only | `4` | 0.01% |

### Not-yet-recovered (1,819 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `275` | 15.12% |
| Vtable-anchored (recover with the owning class) | `173` | 9.51% |
| Framework dispatch (wx/EH/Lua/reflection) | `1` | 0.05% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `909` | 49.97% |
| No indexed callsite evidence (needs investigation/evidence) | `458` | 25.18% |
