...
- Progress coverage:  **`97.34%`**
  ...
  - *Total amount of completed tokens: `65,377`*

Progress DB status breakdown:

- `recovered`: `53,770` (82.25%)
- `skip`: `6,118` (9.36%) — proven ICF aliases / thunks / EH or static-init glue with no distinct source body
...
- **`blocked`: `1,782` (2.65%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `1,847`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `1,847`*
...
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `16,135` | 30.01% |
| Vtable-anchored (virtual override of a recovered class) | `5,852` | 10.88% |
...
### Not-yet-recovered (1,847 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `308` | 16.68% |
| Vtable-anchored (recover with the owning class) | `166` | 8.99% |
| Framework dispatch (wx/EH/Lua/reflection) | `54` | 2.92% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `857` | 46.40% |
| No indexed callsite evidence (needs investigation/evidence) | `458` | 24.80% |
