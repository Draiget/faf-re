...
- **`blocked`: `2,155` (3.21%)**
  ...
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `2,562`*
...
### Recovered (52,719 functions) — binary caller context

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `16,353` | 31.02% |
...
| No callsite evidence (no recorded code/data caller in the index) | `22,857` | 43.36% |
| Unclassified data xref (manual review) | `216` | 0.41% |
| RTTI-only | `4` | 0.01% |

### Not-yet-recovered (2,562 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `301` | 11.75% |
| Vtable-anchored (recover with the owning class) | `140` | 5.46% |
| Framework dispatch (wx/EH/Lua/reflection) | `11` | 0.43% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `966` | 37.70% |
| No indexed callsite evidence (needs investigation/evidence) | `1,142` | 44.57% |
