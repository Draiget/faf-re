...
### Recovered (53,673 functions) — binary caller context

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `16,167` | 30.12% |
...
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `382` | 19.73% |
| Vtable-anchored (recover with the owning class) | `177` | 9.14% |
| Framework dispatch (wx/EH/Lua/reflection) | `64` | 3.31% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `843` | 43.54% |
| No indexed callsite evidence (needs investigation/evidence) | `466` | 24.07% |
