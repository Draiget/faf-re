...
- Total FAF functions: `67,167`
  - *IDA index, exported*
- Progress coverage:  **`98.84%`**
  - *Consists of `recovered` + `skip` + `external_dependency` ÷ exported*
  - *Total amount of completed tokens: `66,388`*

Progress DB status breakdown:

- `recovered`: `53,195` (80.13%)
- `skip`: `7,387` (11.13%) — proven ICF aliases / thunks / EH or static-init glue with no distinct source body
...
- **`blocked`: `836` (1.24%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `843`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `843`*
...
### Recovered (53,195 functions) — binary caller context
...
| No recovered caller token yet (orphan risk) | `1,728` | 3.25% |
| No callsite evidence (no recorded code/data caller in the index) | `22,885` | 43.02% |
| Unclassified data xref (manual review) | `216` | 0.41% |
| RTTI-only | `4` | 0.01% |

### Not-yet-recovered (843 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `26` | 3.08% |
| Vtable-anchored (recover with the owning class) | `109` | 12.93% |
| Framework dispatch (wx/EH/Lua/reflection) | `0` | 0.00% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `364` | 43.18% |
| No indexed callsite evidence (needs investigation/evidence) | `343` | 40.69% |
