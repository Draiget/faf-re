...
- Total FAF functions: `67,167`
  - *IDA index, exported*
- Progress coverage:  **`96.51%`**
  - *Consists of `recovered` + `skip` + `external_dependency` ÷ exported*
  - *Total amount of completed tokens: `64,820`*

Progress DB status breakdown:

- `recovered`: `52,933` (81.66%)
...
### Recovered (52,933 functions) — wiring health

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Confirmed caller** (recovered binary caller wired by name) | `14,801` | 27.96% |
...
| Caller still blocked (orphan-helper risk — caller awaits recovery) | `2,898` | 5.47% |
...
|---|---:|---:|
| **Recoverable now** (`OK_RECOVERED_CALLER` — recovered caller exists; recover next) | `543` | 22.93% |
| Vtable-anchored (recover with the owning class) | `239` | 10.09% |
| Framework dispatch (wx/EH/Lua/reflection) | `36` | 1.52% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `1,097` | 46.33% |
