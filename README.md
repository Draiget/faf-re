...
- **`blocked`: `1,242` (1.85%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `1,650`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `1,650`*
...
### Recovered (54,051 functions) — binary caller context

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `16,655` | 30.81% |
...
| No recovered caller token yet (orphan risk) | `2,050` | 3.79% |
| No callsite evidence (no recorded code/data caller in the index) | `23,643` | 43.74% |
| Unclassified data xref (manual review) | `217` | 0.40% |
| RTTI-only | `4` | 0.01% |

### Not-yet-recovered (1,650 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `289` | 17.52% |
| Vtable-anchored (recover with the owning class) | `110` | 6.67% |
| Framework dispatch (wx/EH/Lua/reflection) | `1` | 0.06% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `713` | 43.21% |
| No indexed callsite evidence (needs investigation/evidence) | `535` | 32.42% |
