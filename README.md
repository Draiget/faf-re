...
- Progress coverage:  **`97.65%`**
  ...
  - *Total amount of completed tokens: `65,590`*

Progress DB status breakdown:

- `recovered`: `54,054` (82.41%)
...
- **`blocked`: `1,239` (1.84%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `1,647`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `1,647`*
...
### Recovered (54,054 functions) — binary caller context

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `16,657` | 30.82% |
...
| No recovered caller token yet (orphan risk) | `2,051` | 3.79% |
| No callsite evidence (no recorded code/data caller in the index) | `23,643` | 43.74% |
| Unclassified data xref (manual review) | `217` | 0.40% |
| RTTI-only | `4` | 0.01% |

### Not-yet-recovered (1,647 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `287` | 17.43% |
| Vtable-anchored (recover with the owning class) | `110` | 6.68% |
| Framework dispatch (wx/EH/Lua/reflection) | `1` | 0.06% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `712` | 43.23% |
| No indexed callsite evidence (needs investigation/evidence) | `535` | 32.48% |
