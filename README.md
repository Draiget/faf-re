...
- Progress coverage:  **`96.76%`**
  ...
  - *Total amount of completed tokens: `64,988`*

Progress DB status breakdown:

- `recovered`: `52,762` (81.19%)
...
- **`blocked`: `1,833` (2.73%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `2,238`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `2,238`*
...
### Recovered (52,762 functions) — binary caller context
...
### Not-yet-recovered (2,238 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `239` | 10.68% |
| Vtable-anchored (recover with the owning class) | `132` | 5.90% |
| Framework dispatch (wx/EH/Lua/reflection) | `10` | 0.45% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `874` | 39.05% |
| No indexed callsite evidence (needs investigation/evidence) | `981` | 43.83% |
