...
- Progress coverage:  **`97.15%`**
  ...
  - *Total amount of completed tokens: `65,256`*

Progress DB status breakdown:

- `recovered`: `53,378` (81.80%)
...
- **`blocked`: `1,943` (2.89%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `1,947`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `1,947`*
...
### Recovered (53,378 functions) — wiring health

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Confirmed caller** (recovered binary caller wired by name) | `15,460` | 28.96% |
...
### Not-yet-recovered (1,947 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Recoverable now** (`OK_RECOVERED_CALLER` — recovered caller exists; recover next) | `301` | 15.46% |
| Vtable-anchored (recover with the owning class) | `186` | 9.55% |
| Framework dispatch (wx/EH/Lua/reflection) | `34` | 1.75% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `990` | 50.85% |
| No callsite evidence (likely external/dead — candidates for `external_dependency`) | `434` | 22.29% |
