...
- **`blocked`: `1,180` (1.76%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `1,588`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `1,588`*
...
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `16,687` | 30.86% |
...
| No recovered caller token yet (orphan risk) | `2,034` | 3.76% |
| No callsite evidence (no recorded code/data caller in the index) | `23,643` | 43.73% |
| Unclassified data xref (manual review) | `217` | 0.40% |
| RTTI-only | `4` | 0.01% |

### Not-yet-recovered (1,588 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `230` | 14.48% |
| Vtable-anchored (recover with the owning class) | `110` | 6.93% |
| Framework dispatch (wx/EH/Lua/reflection) | `1` | 0.06% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `708` | 44.58% |
| No indexed callsite evidence (needs investigation/evidence) | `537` | 33.82% |
