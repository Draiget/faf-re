...
- Total FAF functions: `67,167`
  - *IDA index, exported*
- Progress coverage:  **`98.47%`**
  - *Consists of `recovered` + `skip` + `external_dependency` ÷ exported*
  - *Total amount of completed tokens: `66,140`*
...
- **`blocked`: `1,058` (1.58%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `1,091`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `1,091`*
...
### Not-yet-recovered (1,091 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `45` | 4.12% |
| Vtable-anchored (recover with the owning class) | `124` | 11.37% |
| Framework dispatch (wx/EH/Lua/reflection) | `8` | 0.73% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `547` | 50.14% |
| No indexed callsite evidence (needs investigation/evidence) | `366` | 33.55% |
