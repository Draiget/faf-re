...
- **`blocked`: `700` (1.04%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `703`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `703`*
...
### Not-yet-recovered (703 blocked + needs_evidence) — backlog readiness
