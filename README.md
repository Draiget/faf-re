...
- Progress coverage:  **`95.86%`**
  ...
  - *Total amount of completed tokens: `64,384`*

Progress DB status breakdown:

- `recovered`: `52,733` (81.90%)
...
- **`blocked`: `2,445` (3.64%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `2,853`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `2,853`*
...
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `16,259` | 30.83% |
...
### Not-yet-recovered (2,853 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `420` | 14.72% |
| Vtable-anchored (recover with the owning class) | `144` | 5.05% |
| Framework dispatch (wx/EH/Lua/reflection) | `13` | 0.46% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `1,035` | 36.28% |
| No indexed callsite evidence (needs investigation/evidence) | `1,238` | 43.39% |
