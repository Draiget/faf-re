...
- Total FAF functions: `67,167`
  - *IDA index, exported*
- Progress coverage:  **`95.83%`**
  - *Consists of `recovered` + `skip` + `external_dependency` ÷ exported*
  - *Total amount of completed tokens: `64,366`*

Progress DB status breakdown:

- `recovered`: `52,715` (81.90%)
...
- **`blocked`: `2,463` (3.67%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `2,871`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `2,871`*
...
### Recovered (52,715 functions) — binary caller context

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `16,192` | 30.72% |
| Vtable-anchored (virtual override of a recovered class) | `5,896` | 11.18% |
| Framework dispatch (wx event, EH handler, Lua binding, reflection table, …) | `5,557` | 10.54% |
| No recovered caller token yet (orphan risk) | `1,871` | 3.55% |
| No callsite evidence (no recorded code/data caller in the index) | `22,979` | 43.59% |
| Unclassified data xref (manual review) | `216` | 0.41% |
| RTTI-only | `4` | 0.01% |

### Not-yet-recovered (2,871 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `489` | 17.03% |
| Vtable-anchored (recover with the owning class) | `127` | 4.42% |
| Framework dispatch (wx/EH/Lua/reflection) | `13` | 0.45% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `1,037` | 36.12% |
| No indexed callsite evidence (needs investigation/evidence) | `1,202` | 41.87% |
