...
### Recovered (52,971 functions) — wiring health

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Confirmed caller** (recovered binary caller wired by name) | `14,864` | 28.06% |
...
| Caller still blocked (orphan-helper risk — caller awaits recovery) | `2,865` | 5.41% |
...
|---|---:|---:|
| **Recoverable now** (`OK_RECOVERED_CALLER` — recovered caller exists; recover next) | `544` | 23.20% |
| Vtable-anchored (recover with the owning class) | `233` | 9.94% |
| Framework dispatch (wx/EH/Lua/reflection) | `35` | 1.49% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `1,082` | 46.14% |
| No callsite evidence (likely external/dead — candidates for `external_dependency`) | `449` | 19.15% |
