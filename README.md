...
- Progress coverage:  **`98.81%`**
  ...
  - *Total amount of completed tokens: `66,366`*

Progress DB status breakdown:

- `recovered`: `53,164` (80.11%)
- `skip`: `7,382` (11.12%) — proven ICF aliases / thunks / EH or static-init glue with no distinct source body
- `external_dependency`: `5,820` (8.77%) — proven third-party/import-boundary code
...
- **`blocked`: `858` (1.28%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `865`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `865`*
...
### Recovered (53,164 functions) — binary caller context

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `16,885` | 31.76% |
...
| No recovered caller token yet (orphan risk) | `1,733` | 3.26% |
| No callsite evidence (no recorded code/data caller in the index) | `22,876` | 43.03% |
| Unclassified data xref (manual review) | `216` | 0.41% |
| RTTI-only | `4` | 0.01% |

### Not-yet-recovered (865 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `28` | 3.24% |
| Vtable-anchored (recover with the owning class) | `109` | 12.60% |
| Framework dispatch (wx/EH/Lua/reflection) | `0` | 0.00% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `367` | 42.43% |
| No indexed callsite evidence (needs investigation/evidence) | `360` | 41.62% |
