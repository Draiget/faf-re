...
## Recovery Coverage (`14/09/2026`, `fa_full_2026_03_26`)

Progress snapshot:

- Total FAF functions: `67,170`
  - *IDA index, exported*
- Progress coverage:  **`99.72%`**
  - *Consists of `recovered` + `skip` + `external_dependency` ÷ exported*
  - *Total amount of completed tokens: `66,985`*

Progress DB status breakdown:

- `recovered`: `47,467` (70.86%)
- `skip`: `13,747` (20.52%) — proven ICF aliases / thunks / EH or static-init glue with no distinct source body
- `external_dependency`: `5,771` (8.62%) — proven third-party/import-boundary code
...
- **`blocked`: `301` (0.45%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `304`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `304`*
...
### Recovered (47,467 functions) — binary caller context

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `17,322` | 36.49% |
| Vtable-anchored (virtual override of a recovered class) | `5,897` | 12.42% |
| Framework dispatch (wx event, EH handler, Lua binding, reflection table, …) | `5,618` | 11.84% |
| No recovered caller token yet (orphan risk) | `1,575` | 3.32% |
| No callsite evidence (no recorded code/data caller in the index) | `16,835` | 35.47% |
| Unclassified data xref (manual review) | `216` | 0.46% |
| RTTI-only | `4` | 0.01% |

### Not-yet-recovered (304 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `3` | 0.99% |
| Vtable-anchored (recover with the owning class) | `78` | 25.66% |
| Framework dispatch (wx/EH/Lua/reflection) | `0` | 0.00% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `114` | 37.50% |
| No indexed callsite evidence (needs investigation/evidence) | `109` | 35.86% |
