...
- Total FAF functions: `67,167`
  - *IDA index, exported*
- Progress coverage:  **`95.86%`**
  - *Consists of `recovered` + `skip` + `external_dependency` ÷ exported*
  - *Total amount of completed tokens: `64,389`*

Progress DB status breakdown:

- `recovered`: `52,738` (81.91%)
...
- **`blocked`: `2,440` (3.63%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `2,848`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `2,848`*
...
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `16,265` | 30.84% |
...
| No recovered caller token yet (orphan risk) | `1,873` | 3.55% |
| No callsite evidence (no recorded code/data caller in the index) | `22,943` | 43.50% |
| Unclassified data xref (manual review) | `216` | 0.41% |
| RTTI-only | `4` | 0.01% |

### Not-yet-recovered (2,848 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `415` | 14.57% |
| Vtable-anchored (recover with the owning class) | `144` | 5.06% |
| Framework dispatch (wx/EH/Lua/reflection) | `13` | 0.46% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `1,035` | 36.34% |
| No indexed callsite evidence (needs investigation/evidence) | `1,238` | 43.47% |
