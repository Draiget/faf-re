...
- Total FAF functions: `67,167`
  - *IDA index, exported*
- Progress coverage:  **`96.93%`**
  - *Consists of `recovered` + `skip` + `external_dependency` ÷ exported*
  - *Total amount of completed tokens: `65,106`*

Progress DB status breakdown:

- `recovered`: `53,568` (82.28%)
...
- **`blocked`: `1,723` (2.57%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `2,131`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `2,131`*
...
### Recovered (53,568 functions) — binary caller context

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `16,612` | 31.01% |
| Vtable-anchored (virtual override of a recovered class) | `5,912` | 11.04% |
| Framework dispatch (wx event, EH handler, Lua binding, reflection table, …) | `5,568` | 10.39% |
| No recovered caller token yet (orphan risk) | `2,063` | 3.85% |
| No callsite evidence (no recorded code/data caller in the index) | `23,192` | 43.29% |
| Unclassified data xref (manual review) | `217` | 0.41% |
| RTTI-only | `4` | 0.01% |

### Not-yet-recovered (2,131 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `301` | 14.12% |
| Vtable-anchored (recover with the owning class) | `111` | 5.21% |
| Framework dispatch (wx/EH/Lua/reflection) | `1` | 0.05% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `730` | 34.26% |
| No indexed callsite evidence (needs investigation/evidence) | `986` | 46.27% |
