...
## Recovery Coverage (`31/07/2026`, `fa_full_2026_03_26`)
...
- Progress coverage:  **`97.33%`**
  ...
  - *Total amount of completed tokens: `65,375`*

Progress DB status breakdown:

- `recovered`: `53,741` (82.20%)
- `skip`: `6,130` (9.38%) — proven ICF aliases / thunks / EH or static-init glue with no distinct source body
- `external_dependency`: `5,504` (8.42%) — proven third-party/import-boundary code
...
### Recovered (53,741 functions) — binary caller context

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `15,980` | 29.74% |
| Vtable-anchored (virtual override of a recovered class) | `5,843` | 10.87% |
| Framework dispatch (wx event, EH handler, Lua binding, reflection table, …) | `5,542` | 10.31% |
| No recovered caller token yet (orphan risk) | `2,438` | 4.54% |
...
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `279` | 15.02% |
| Vtable-anchored (recover with the owning class) | `179` | 9.63% |
| Framework dispatch (wx/EH/Lua/reflection) | `1` | 0.05% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `935` | 50.32% |
