...
- Progress coverage:  **`97.15%`**
  ...
  - *Total amount of completed tokens: `65,253`*

Progress DB status breakdown:

- `recovered`: `53,375` (81.80%)
...
### Recovered (53,375 functions) — wiring health

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Confirmed caller** (recovered binary caller wired by name) | `15,455` | 28.96% |
...
|---|---:|---:|
| **Recoverable now** (`OK_RECOVERED_CALLER` — recovered caller exists; recover next) | `304` | 15.59% |
| Vtable-anchored (recover with the owning class) | `186` | 9.54% |
| Framework dispatch (wx/EH/Lua/reflection) | `34` | 1.74% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `990` | 50.77% |
| No callsite evidence (likely external/dead — candidates for `external_dependency`) | `434` | 22.26% |
