...
- **`blocked`: `2,006` (2.99%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `2,010`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `2,010`*
...
### Recovered (53,313 functions) — wiring health

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Confirmed caller** (recovered binary caller wired by name) | `15,383` | 28.85% |
...
| Caller still blocked (orphan-helper risk — caller awaits recovery) | `2,648` | 4.97% |
| No callsite evidence (no recorded code/data caller in the index) | `23,750` | 44.55% |
| Unclassified data xref (manual review) | `215` | 0.40% |
| RTTI-only | `4` | 0.01% |

### Not-yet-recovered (2,010 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Recoverable now** (`OK_RECOVERED_CALLER` — recovered caller exists; recover next) | `315` | 15.67% |
| Vtable-anchored (recover with the owning class) | `209` | 10.40% |
| Framework dispatch (wx/EH/Lua/reflection) | `35` | 1.74% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `1,014` | 50.45% |
| No callsite evidence (likely external/dead — candidates for `external_dependency`) | `435` | 21.64% |
