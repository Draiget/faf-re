...
- **`blocked`: `1,663` (2.48%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `2,071`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `2,071`*
...
### Recovered (53,620 functions) — binary caller context

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `16,553` | 30.87% |
| Vtable-anchored (virtual override of a recovered class) | `5,895` | 10.99% |
...
| No callsite evidence (no recorded code/data caller in the index) | `23,265` | 43.39% |
| Unclassified data xref (manual review) | `217` | 0.40% |
| RTTI-only | `4` | 0.01% |

### Not-yet-recovered (2,071 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `277` | 13.38% |
| Vtable-anchored (recover with the owning class) | `117` | 5.65% |
| Framework dispatch (wx/EH/Lua/reflection) | `1` | 0.05% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `762` | 36.79% |
| No indexed callsite evidence (needs investigation/evidence) | `912` | 44.04% |
