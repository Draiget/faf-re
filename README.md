...
- Progress coverage:  **`99.06%`**
  ...
  - *Total amount of completed tokens: `66,534`*

Progress DB status breakdown:

- `recovered`: `53,334` (80.16%)
- `skip`: `7,424` (11.16%) — proven ICF aliases / thunks / EH or static-init glue with no distinct source body
- `external_dependency`: `5,776` (8.68%) — proven third-party/import-boundary code
...
- **`blocked`: `698` (1.04%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `701`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `701`*
...
### Recovered (53,334 functions) — binary caller context

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `17,081` | 32.03% |
...
| No recovered caller token yet (orphan risk) | `1,703` | 3.19% |
| No callsite evidence (no recorded code/data caller in the index) | `22,881` | 42.90% |
| Unclassified data xref (manual review) | `216` | 0.40% |
| RTTI-only | `4` | 0.01% |

### Not-yet-recovered (701 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `12` | 1.71% |
| Vtable-anchored (recover with the owning class) | `109` | 15.55% |
| Framework dispatch (wx/EH/Lua/reflection) | `0` | 0.00% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `266` | 37.95% |
| No indexed callsite evidence (needs investigation/evidence) | `313` | 44.65% |
