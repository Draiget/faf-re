...
- Progress coverage:  **`97.20%`**
  ...
  - *Total amount of completed tokens: `65,285`*

Progress DB status breakdown:

- `recovered`: `53,407` (81.81%)
...
### Recovered (53,407 functions) — wiring health

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Confirmed caller** (recovered binary caller wired by name) | `15,503` | 29.03% |
| Vtable-anchored (virtual override of a recovered class) | `5,840` | 10.93% |
| Framework dispatch (wx event, EH handler, Lua binding, reflection table, …) | `5,499` | 10.30% |
| Caller still blocked (orphan-helper risk — caller awaits recovery) | `2,594` | 4.86% |
| No callsite evidence (no recorded code/data caller in the index) | `23,752` | 44.47% |
...
|---|---:|---:|
| **Recoverable now** (`OK_RECOVERED_CALLER` — recovered caller exists; recover next) | `294` | 15.33% |
| Vtable-anchored (recover with the owning class) | `183` | 9.54% |
| Framework dispatch (wx/EH/Lua/reflection) | `34` | 1.77% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `971` | 50.63% |
| No callsite evidence (likely external/dead — candidates for `external_dependency`) | `434` | 22.63% |
