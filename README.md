...
- Total FAF functions: `67,167`
  - *IDA index, exported*
- Progress coverage:  **`96.82%`**
  - *Consists of `recovered` + `skip` + `external_dependency` ÷ exported*
  - *Total amount of completed tokens: `65,028`*

Progress DB status breakdown:

- `recovered`: `53,423` (82.15%)
...
- **`blocked`: `1,801` (2.68%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `2,209`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `2,209`*
...
### Recovered (53,423 functions) — binary caller context
...
| Framework dispatch (wx event, EH handler, Lua binding, reflection table, …) | `5,531` | 10.35% |
...
### Not-yet-recovered (2,209 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `350` | 15.84% |
| Vtable-anchored (recover with the owning class) | `132` | 5.98% |
| Framework dispatch (wx/EH/Lua/reflection) | `31` | 1.40% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `773` | 34.99% |
| No indexed callsite evidence (needs investigation/evidence) | `920` | 41.65% |
