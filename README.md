...
- **`blocked`: `2,012` (3.00%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `2,016`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `2,016`*
...
### Recovered (53,307 functions) — wiring health

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Confirmed caller** (recovered binary caller wired by name) | `15,372` | 28.84% |
...
### Not-yet-recovered (2,016 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Recoverable now** (`OK_RECOVERED_CALLER` — recovered caller exists; recover next) | `320` | 15.87% |
| Vtable-anchored (recover with the owning class) | `209` | 10.37% |
| Framework dispatch (wx/EH/Lua/reflection) | `35` | 1.74% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `1,015` | 50.35% |
| No callsite evidence (likely external/dead — candidates for `external_dependency`) | `435` | 21.58% |
