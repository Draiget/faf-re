...
- Progress coverage:  **`96.92%`**
  ...
  - *Total amount of completed tokens: `65,100`*

Progress DB status breakdown:

- `recovered`: `53,547` (82.25%)
...
- **`blocked`: `1,729` (2.57%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `2,137`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `2,137`*
...
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `16,485` | 30.79% |
...
### Not-yet-recovered (2,137 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `327` | 15.30% |
| Vtable-anchored (recover with the owning class) | `117` | 5.47% |
| Framework dispatch (wx/EH/Lua/reflection) | `1` | 0.05% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `778` | 36.41% |
| No indexed callsite evidence (needs investigation/evidence) | `912` | 42.68% |
