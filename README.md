...
- Progress coverage:  **`97.16%`**
  ...
  - *Total amount of completed tokens: `65,257`*

Progress DB status breakdown:

- `recovered`: `53,379` (81.80%)
...
- **`blocked`: `1,942` (2.89%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `1,946`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `1,946`*
...
### Recovered (53,379 functions) — wiring health

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Confirmed caller** (recovered binary caller wired by name) | `15,461` | 28.96% |
| Vtable-anchored (virtual override of a recovered class) | `5,837` | 10.94% |
| Framework dispatch (wx event, EH handler, Lua binding, reflection table, …) | `5,499` | 10.30% |
| Caller still blocked (orphan-helper risk — caller awaits recovery) | `2,611` | 4.89% |
| No callsite evidence (no recorded code/data caller in the index) | `23,752` | 44.50% |
| Unclassified data xref (manual review) | `215` | 0.40% |
| RTTI-only | `4` | 0.01% |

### Not-yet-recovered (1,946 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Recoverable now** (`OK_RECOVERED_CALLER` — recovered caller exists; recover next) | `300` | 15.42% |
| Vtable-anchored (recover with the owning class) | `186` | 9.56% |
| Framework dispatch (wx/EH/Lua/reflection) | `34` | 1.75% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `990` | 50.87% |
| No callsite evidence (likely external/dead — candidates for `external_dependency`) | `434` | 22.30% |
