...
- Total FAF functions: `67,167`
  - *IDA index, exported*
- Progress coverage:  **`98.93%`**
  - *Consists of `recovered` + `skip` + `external_dependency` ÷ exported*
  - *Total amount of completed tokens: `66,446`*

Progress DB status breakdown:

- `recovered`: `53,284` (80.19%)
...
- **`blocked`: `790` (1.18%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `793`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `793`*
...
### Recovered (53,284 functions) — binary caller context

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `16,997` | 31.90% |
...
| No callsite evidence (no recorded code/data caller in the index) | `22,908` | 42.99% |
| Unclassified data xref (manual review) | `216` | 0.41% |
| RTTI-only | `4` | 0.01% |

### Not-yet-recovered (793 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `25` | 3.15% |
| Vtable-anchored (recover with the owning class) | `109` | 13.75% |
| Framework dispatch (wx/EH/Lua/reflection) | `0` | 0.00% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `339` | 42.75% |
| No indexed callsite evidence (needs investigation/evidence) | `319` | 40.23% |
