...
- Progress coverage:  **`96.78%`**
  ...
  - *Total amount of completed tokens: `65,005`*

Progress DB status breakdown:

- `recovered`: `53,124` (81.72%)
...
### Recovered (53,124 functions) — wiring health

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Confirmed caller** (recovered binary caller wired by name) | `15,015` | 28.26% |
...
| Caller still blocked (orphan-helper risk — caller awaits recovery) | `2,831` | 5.33% |
...
|---|---:|---:|
| **Recoverable now** (`OK_RECOVERED_CALLER` — recovered caller exists; recover next) | `455` | 20.74% |
| Vtable-anchored (recover with the owning class) | `210` | 9.57% |
| Framework dispatch (wx/EH/Lua/reflection) | `35` | 1.60% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `1,053` | 47.99% |
| No callsite evidence (likely external/dead — candidates for `external_dependency`) | `439` | 20.01% |
