...
- Progress coverage:  **`98.43%`**
  ...
  - *Total amount of completed tokens: `66,115`*
...
- **`blocked`: `1,083` (1.61%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `1,116`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `1,116`*
...
### Not-yet-recovered (1,116 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `53` | 4.75% |
| Vtable-anchored (recover with the owning class) | `124` | 11.11% |
| Framework dispatch (wx/EH/Lua/reflection) | `8` | 0.72% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `564` | 50.54% |
| No indexed callsite evidence (needs investigation/evidence) | `366` | 32.80% |
