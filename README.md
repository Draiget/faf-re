...
- `recovered`: `52,978` (79.62%)
- `skip`: `7,798` (11.72%) — proven ICF aliases / thunks / EH or static-init glue with no distinct source body
...
### Recovered (52,978 functions) — binary caller context

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `17,093` | 32.26% |
| Vtable-anchored (virtual override of a recovered class) | `5,886` | 11.11% |
| Framework dispatch (wx event, EH handler, Lua binding, reflection table, …) | `5,580` | 10.53% |
| No recovered caller token yet (orphan risk) | `1,697` | 3.20% |
| No callsite evidence (no recorded code/data caller in the index) | `22,502` | 42.47% |
