...
- Progress coverage:  **`97.91%`**
  ...
  - *Total amount of completed tokens: `65,763`*
...
- **`blocked`: `1,434` (2.13%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `1,467`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `1,467`*
...
### Not-yet-recovered (1,467 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `63` | 4.29% |
| Vtable-anchored (recover with the owning class) | `126` | 8.59% |
| Framework dispatch (wx/EH/Lua/reflection) | `8` | 0.55% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `675` | 46.01% |
| No indexed callsite evidence (needs investigation/evidence) | `594` | 40.49% |
