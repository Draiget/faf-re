...
- Progress coverage:  **`96.59%`**
  ...
  - *Total amount of completed tokens: `64,875`*

Progress DB status breakdown:

- `recovered`: `52,988` (81.68%)
...
### Recovered (52,988 functions) — wiring health

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Confirmed caller** (recovered binary caller wired by name) | `14,889` | 28.10% |
...
| Caller still blocked (orphan-helper risk — caller awaits recovery) | `2,857` | 5.39% |
...
|---|---:|---:|
| **Recoverable now** (`OK_RECOVERED_CALLER` — recovered caller exists; recover next) | `526` | 22.60% |
| Vtable-anchored (recover with the owning class) | `233` | 10.01% |
| Framework dispatch (wx/EH/Lua/reflection) | `35` | 1.50% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `1,082` | 46.50% |
| No callsite evidence (likely external/dead — candidates for `external_dependency`) | `449` | 19.30% |
