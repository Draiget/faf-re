...
- Total FAF functions: `67,170`
  - *IDA index, exported*
- Progress coverage:  **`99.33%`**
  - *Consists of `recovered` + `skip` + `external_dependency` ÷ exported*
  - *Total amount of completed tokens: `66,717`*

Progress DB status breakdown:

- `recovered`: `52,749` (79.06%)
- `skip`: `8,223` (12.33%) — proven ICF aliases / thunks / EH or static-init glue with no distinct source body
...
- **`blocked`: `560` (0.83%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `563`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `563`*
...
### Not-yet-recovered (563 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `9` | 1.60% |
| Vtable-anchored (recover with the owning class) | `105` | 18.65% |
| Framework dispatch (wx/EH/Lua/reflection) | `0` | 0.00% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `155` | 27.53% |
| No indexed callsite evidence (needs investigation/evidence) | `293` | 52.04% |
