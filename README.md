...
- Progress coverage:  **`96.78%`**
  ...
  - *Total amount of completed tokens: `65,001`*

Progress DB status breakdown:

- `recovered`: `53,120` (81.72%)
...
### Recovered (53,120 functions) — wiring health
...
| No callsite evidence (no recorded code/data caller in the index) | `23,746` | 44.70% |
...
|---|---:|---:|
| **Recoverable now** (`OK_RECOVERED_CALLER` — recovered caller exists; recover next) | `458` | 20.84% |
| Vtable-anchored (recover with the owning class) | `210` | 9.55% |
| Framework dispatch (wx/EH/Lua/reflection) | `35` | 1.59% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `1,053` | 47.91% |
| No callsite evidence (likely external/dead — candidates for `external_dependency`) | `440` | 20.02% |
