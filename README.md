...
- `recovered`: `53,332` (80.15%)
- `skip`: `7,427` (11.16%) — proven ICF aliases / thunks / EH or static-init glue with no distinct source body
...
- **`blocked`: `699` (1.04%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `702`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `702`*
...
### Recovered (53,332 functions) — binary caller context

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `17,073` | 32.01% |
...
### Not-yet-recovered (702 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `12` | 1.71% |
| Vtable-anchored (recover with the owning class) | `109` | 15.53% |
| Framework dispatch (wx/EH/Lua/reflection) | `0` | 0.00% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `267` | 38.03% |
| No indexed callsite evidence (needs investigation/evidence) | `313` | 44.59% |
