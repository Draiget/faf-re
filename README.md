...
- Progress coverage:  **`97.31%`**
  ...
  - *Total amount of completed tokens: `65,359`*

Progress DB status breakdown:

- `recovered`: `53,753` (82.24%)
...
- **`blocked`: `1,800` (2.68%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `1,865`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `1,865`*
...
### Recovered (53,753 functions) — binary caller context

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `16,112` | 29.97% |
...
### Not-yet-recovered (1,865 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `307` | 16.46% |
| Vtable-anchored (recover with the owning class) | `174` | 9.33% |
| Framework dispatch (wx/EH/Lua/reflection) | `54` | 2.90% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `867` | 46.49% |
| No indexed callsite evidence (needs investigation/evidence) | `459` | 24.61% |
