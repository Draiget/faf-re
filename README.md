...
- Progress coverage:  **`98.79%`**
  ...
  - *Total amount of completed tokens: `66,353`*
...
- `external_dependency`: `5,824` (8.78%) — proven third-party/import-boundary code
...
- **`blocked`: `871` (1.30%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `878`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `878`*
...
### Not-yet-recovered (878 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `31` | 3.53% |
| Vtable-anchored (recover with the owning class) | `109` | 12.41% |
| Framework dispatch (wx/EH/Lua/reflection) | `0` | 0.00% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `377` | 42.94% |
| No indexed callsite evidence (needs investigation/evidence) | `360` | 41.00% |
