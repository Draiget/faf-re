...
- Progress coverage:  **`98.94%`**
  ...
  - *Total amount of completed tokens: `66,458`*
...
- `external_dependency`: `5,771` (8.68%) — proven third-party/import-boundary code
...
- **`blocked`: `778` (1.16%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `781`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `781`*
...
### Not-yet-recovered (781 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `24` | 3.07% |
| Vtable-anchored (recover with the owning class) | `109` | 13.96% |
| Framework dispatch (wx/EH/Lua/reflection) | `0` | 0.00% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `329` | 42.13% |
| No indexed callsite evidence (needs investigation/evidence) | `318` | 40.72% |
