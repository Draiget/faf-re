...
### Recovered (53,119 functions) — wiring health

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Confirmed caller** (recovered binary caller wired by name) | `15,010` | 28.26% |
...
|---|---:|---:|
| **Recoverable now** (`OK_RECOVERED_CALLER` — recovered caller exists; recover next) | `459` | 20.87% |
| Vtable-anchored (recover with the owning class) | `210` | 9.55% |
| Framework dispatch (wx/EH/Lua/reflection) | `35` | 1.59% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `1,053` | 47.89% |
| No callsite evidence (likely external/dead — candidates for `external_dependency`) | `440` | 20.01% |
