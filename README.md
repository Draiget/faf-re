...
- Total FAF functions: `67,167`
  - *IDA index, exported*
- Progress coverage:  **`96.09%`**
  - *Consists of `recovered` + `skip` + `external_dependency` ÷ exported*
  - *Total amount of completed tokens: `64,543`*

Progress DB status breakdown:

- `recovered`: `52,716` (81.68%)
- `skip`: `6,375` (9.88%) — proven ICF aliases / thunks / EH or static-init glue with no distinct source body
- `external_dependency`: `5,452` (8.45%) — proven third-party/import-boundary code
...
- `in_progress`: `119` (0.18%)
- **`blocked`: `2,169` (3.23%)**
  - *strict circular/dep-blocked (in-DB literal `status == "blocked"`)*  
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `2,576`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `2,576`*
...
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `16,333` | 30.98% |
...
| No callsite evidence (no recorded code/data caller in the index) | `22,884` | 43.41% |
| Unclassified data xref (manual review) | `216` | 0.41% |
| RTTI-only | `4` | 0.01% |

### Not-yet-recovered (2,576 blocked + needs_evidence) — backlog readiness
...
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `972` | 37.73% |
| No indexed callsite evidence (needs investigation/evidence) | `1,143` | 44.37% |
