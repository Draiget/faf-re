...
- Progress coverage:  **`97.24%`**
  ...
  - *Total amount of completed tokens: `65,312`*

Progress DB status breakdown:

- `recovered`: `53,713` (82.24%)
- `skip`: `6,111` (9.36%) — proven ICF aliases / thunks / EH or static-init glue with no distinct source body
- `external_dependency`: `5,488` (8.40%) — proven third-party/import-boundary code
...
- **`blocked`: `1,846` (2.75%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `1,912`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `1,912`*
...
### Recovered (53,713 functions) — binary caller context

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `16,032` | 29.85% |
| Vtable-anchored (virtual override of a recovered class) | `5,839` | 10.87% |
| Framework dispatch (wx event, EH handler, Lua binding, reflection table, …) | `5,504` | 10.25% |
| No recovered caller token yet (orphan risk) | `2,399` | 4.47% |
| No callsite evidence (no recorded code/data caller in the index) | `23,719` | 44.16% |
| Unclassified data xref (manual review) | `216` | 0.40% |
| RTTI-only | `4` | 0.01% |

### Not-yet-recovered (1,912 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `308` | 16.11% |
| Vtable-anchored (recover with the owning class) | `183` | 9.57% |
| Framework dispatch (wx/EH/Lua/reflection) | `54` | 2.82% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `903` | 47.23% |
| No indexed callsite evidence (needs investigation/evidence) | `460` | 24.06% |
