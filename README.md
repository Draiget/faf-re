...
- Progress coverage:  **`95.85%`**
  ...
  - *Total amount of completed tokens: `64,377`*

Progress DB status breakdown:

- `recovered`: `52,726` (81.90%)
...
- **`blocked`: `2,452` (3.65%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `2,860`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `2,860`*
...
### Recovered (52,726 functions) — binary caller context

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `16,251` | 30.82% |
...
### Not-yet-recovered (2,860 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `427` | 14.93% |
| Vtable-anchored (recover with the owning class) | `144` | 5.03% |
| Framework dispatch (wx/EH/Lua/reflection) | `13` | 0.45% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `1,035` | 36.19% |
| No indexed callsite evidence (needs investigation/evidence) | `1,238` | 43.29% |
