...
- Total FAF functions: `67,167`
  - *IDA index, exported*
- Progress coverage:  **`95.94%`**
  - *Consists of `recovered` + `skip` + `external_dependency` ÷ exported*
  - *Total amount of completed tokens: `64,439`*

Progress DB status breakdown:

- `recovered`: `52,749` (81.86%)
- `skip`: `6,260` (9.71%) — proven ICF aliases / thunks / EH or static-init glue with no distinct source body
...
- **`blocked`: `2,391` (3.56%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `2,799`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `2,799`*
...
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `16,333` | 30.96% |
...
| No callsite evidence (no recorded code/data caller in the index) | `22,906` | 43.42% |
| Unclassified data xref (manual review) | `216` | 0.41% |
| RTTI-only | `4` | 0.01% |

### Not-yet-recovered (2,799 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `361` | 12.90% |
| Vtable-anchored (recover with the owning class) | `144` | 5.14% |
| Framework dispatch (wx/EH/Lua/reflection) | `13` | 0.46% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `1,033` | 36.91% |
| No indexed callsite evidence (needs investigation/evidence) | `1,245` | 44.48% |
