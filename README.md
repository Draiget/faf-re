...
- Progress coverage:  **`97.56%`**
  ...
  - *Total amount of completed tokens: `65,527`*

Progress DB status breakdown:

- `recovered`: `52,912` (80.75%)
...
- **`blocked`: `1,667` (2.48%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `1,701`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `1,701`*
...
### Recovered (52,912 functions) — binary caller context

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `16,528` | 31.24% |
...
### Not-yet-recovered (1,701 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `184` | 10.82% |
| Vtable-anchored (recover with the owning class) | `129` | 7.58% |
| Framework dispatch (wx/EH/Lua/reflection) | `8` | 0.47% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `780` | 45.86% |
| No indexed callsite evidence (needs investigation/evidence) | `599` | 35.21% |
