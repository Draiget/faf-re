...
- Total FAF functions: `67,167`
  - *IDA index, exported*
- Progress coverage:  **`97.82%`**
  - *Consists of `recovered` + `skip` + `external_dependency` ÷ exported*
  - *Total amount of completed tokens: `65,703`*

Progress DB status breakdown:

- `recovered`: `53,037` (80.72%)
- `skip`: `7,118` (10.83%) — proven ICF aliases / thunks / EH or static-init glue with no distinct source body
...
- **`blocked`: `1,493` (2.22%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `1,527`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `1,527`*
...
### Recovered (53,037 functions) — binary caller context

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `16,691` | 31.47% |
...
### Not-yet-recovered (1,527 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `76` | 4.98% |
| Vtable-anchored (recover with the owning class) | `129` | 8.45% |
| Framework dispatch (wx/EH/Lua/reflection) | `8` | 0.52% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `716` | 46.89% |
| No indexed callsite evidence (needs investigation/evidence) | `597` | 39.10% |
