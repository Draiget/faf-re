...
- Total FAF functions: `67,167`
  - *IDA index, exported*
- Progress coverage:  **`96.57%`**
  - *Consists of `recovered` + `skip` + `external_dependency` ÷ exported*
  - *Total amount of completed tokens: `64,866`*

Progress DB status breakdown:

- `recovered`: `53,230` (82.06%)
...
- **`blocked`: `1,950` (2.90%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `2,362`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `2,362`*
...
### Recovered (53,230 functions) — binary caller context

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `16,245` | 30.52% |
| Vtable-anchored (virtual override of a recovered class) | `5,836` | 10.96% |
| Framework dispatch (wx event, EH handler, Lua binding, reflection table, …) | `5,513` | 10.36% |
| No recovered caller token yet (orphan risk) | `2,226` | 4.18% |
| No callsite evidence (no recorded code/data caller in the index) | `23,190` | 43.57% |
| Unclassified data xref (manual review) | `216` | 0.41% |
| RTTI-only | `4` | 0.01% |

### Not-yet-recovered (2,362 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `350` | 14.82% |
| Vtable-anchored (recover with the owning class) | `176` | 7.45% |
| Framework dispatch (wx/EH/Lua/reflection) | `45` | 1.91% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `809` | 34.25% |
| No indexed callsite evidence (needs investigation/evidence) | `978` | 41.41% |
