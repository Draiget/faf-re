...
- Progress coverage:  **`97.23%`**
  ...
  - *Total amount of completed tokens: `65,309`*

Progress DB status breakdown:

- `recovered`: `53,431` (81.81%)
...
- **`blocked`: `1,892` (2.82%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `1,895`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `1,895`*
...
### Recovered (53,431 functions) — wiring health

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Confirmed caller** (recovered binary caller wired by name) | `15,533` | 29.07% |
...
### Not-yet-recovered (1,895 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Recoverable now** (`OK_RECOVERED_CALLER` — recovered caller exists; recover next) | `286` | 15.09% |
| Vtable-anchored (recover with the owning class) | `183` | 9.66% |
| Framework dispatch (wx/EH/Lua/reflection) | `34` | 1.79% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `956` | 50.45% |
| No callsite evidence (likely external/dead — candidates for `external_dependency`) | `434` | 22.90% |
