...
- Total FAF functions: `67,167`
  - *IDA index, exported*
- Progress coverage:  **`97.90%`**
  - *Consists of `recovered` + `skip` + `external_dependency` ÷ exported*
  - *Total amount of completed tokens: `65,759`*

Progress DB status breakdown:

- `recovered`: `53,056` (80.68%)
- `skip`: `7,121` (10.83%) — proven ICF aliases / thunks / EH or static-init glue with no distinct source body
- `external_dependency`: `5,582` (8.49%) — proven third-party/import-boundary code
...
- **`blocked`: `1,438` (2.14%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `1,471`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `1,471`*
...
### Recovered (53,056 functions) — binary caller context

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `16,716` | 31.51% |
| Vtable-anchored (virtual override of a recovered class) | `5,885` | 11.09% |
| Framework dispatch (wx event, EH handler, Lua binding, reflection table, …) | `5,565` | 10.49% |
| No recovered caller token yet (orphan risk) | `1,795` | 3.38% |
| No callsite evidence (no recorded code/data caller in the index) | `22,875` | 43.11% |
| Unclassified data xref (manual review) | `216` | 0.41% |
| RTTI-only | `4` | 0.01% |

### Not-yet-recovered (1,471 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `63` | 4.28% |
| Vtable-anchored (recover with the owning class) | `126` | 8.57% |
| Framework dispatch (wx/EH/Lua/reflection) | `8` | 0.54% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `679` | 46.16% |
| No indexed callsite evidence (needs investigation/evidence) | `594` | 40.38% |
