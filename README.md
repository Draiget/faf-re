...
- Progress coverage:  **`99.39%`**
  ...
  - *Total amount of completed tokens: `66,757`*

Progress DB status breakdown:

- `recovered`: `52,773` (79.05%)
...
- **`blocked`: `520` (0.77%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `523`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `523`*
...
### Not-yet-recovered (523 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `9` | 1.72% |
| Vtable-anchored (recover with the owning class) | `89` | 17.02% |
| Framework dispatch (wx/EH/Lua/reflection) | `0` | 0.00% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `143` | 27.34% |
| No indexed callsite evidence (needs investigation/evidence) | `282` | 53.92% |
