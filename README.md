...
- Total FAF functions: `67,167`
  - *IDA index, exported*
- Progress coverage:  **`97.03%`**
  - *Consists of `recovered` + `skip` + `external_dependency` ÷ exported*
  - *Total amount of completed tokens: `65,169`*

Progress DB status breakdown:

- `recovered`: `53,287` (81.77%)
...
- **`blocked`: `2,029` (3.02%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `2,033`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `2,033`*
...
### Recovered (53,287 functions) — wiring health

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Confirmed caller** (recovered binary caller wired by name) | `15,350` | 28.81% |
| Vtable-anchored (virtual override of a recovered class) | `5,816` | 10.91% |
| Framework dispatch (wx event, EH handler, Lua binding, reflection table, …) | `5,498` | 10.32% |
| Caller still blocked (orphan-helper risk — caller awaits recovery) | `2,654` | 4.98% |
| No callsite evidence (no recorded code/data caller in the index) | `23,750` | 44.57% |
| Unclassified data xref (manual review) | `215` | 0.40% |
| RTTI-only | `4` | 0.01% |

### Not-yet-recovered (2,033 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Recoverable now** (`OK_RECOVERED_CALLER` — recovered caller exists; recover next) | `331` | 16.28% |
| Vtable-anchored (recover with the owning class) | `209` | 10.28% |
| Framework dispatch (wx/EH/Lua/reflection) | `35` | 1.72% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `1,021` | 50.22% |
| No callsite evidence (likely external/dead — candidates for `external_dependency`) | `435` | 21.40% |
