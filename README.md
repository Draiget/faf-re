...
- Progress coverage:  **`96.83%`**
  ...
  - *Total amount of completed tokens: `65,039`*

Progress DB status breakdown:

- `recovered`: `53,433` (82.16%)
...
- **`blocked`: `1,790` (2.66%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `2,198`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `2,198`*
...
### Not-yet-recovered (2,198 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `349` | 15.88% |
| Vtable-anchored (recover with the owning class) | `131` | 5.96% |
| Framework dispatch (wx/EH/Lua/reflection) | `23` | 1.05% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `772` | 35.12% |
| No indexed callsite evidence (needs investigation/evidence) | `920` | 41.86% |
