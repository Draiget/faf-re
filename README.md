...
- Progress coverage:  **`97.38%`**
  ...
  - *Total amount of completed tokens: `65,409`*

Progress DB status breakdown:

- `recovered`: `53,776` (82.21%)
- `skip`: `6,129` (9.37%) — proven ICF aliases / thunks / EH or static-init glue with no distinct source body
- `external_dependency`: `5,504` (8.41%) — proven third-party/import-boundary code
...
- **`blocked`: `1,747` (2.60%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `1,815`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `1,815`*
...
### Recovered (53,776 functions) — binary caller context

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `16,022` | 29.79% |
...
| No recovered caller token yet (orphan risk) | `2,426` | 4.51% |
| No callsite evidence (no recorded code/data caller in the index) | `23,720` | 44.11% |
| Unclassified data xref (manual review) | `217` | 0.40% |
| RTTI-only | `4` | 0.01% |

### Not-yet-recovered (1,815 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `275` | 15.15% |
| Vtable-anchored (recover with the owning class) | `173` | 9.53% |
| Framework dispatch (wx/EH/Lua/reflection) | `1` | 0.06% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `905` | 49.86% |
| No indexed callsite evidence (needs investigation/evidence) | `458` | 25.23% |
