...
- Progress coverage:  **`96.85%`**
  ...
  - *Total amount of completed tokens: `65,050`*

Progress DB status breakdown:

- `recovered`: `53,169` (81.74%)
...
- **`blocked`: `2,146` (3.20%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `2,149`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `2,149`*
...
### Recovered (53,169 functions) — wiring health

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Confirmed caller** (recovered binary caller wired by name) | `15,137` | 28.47% |
...
### Not-yet-recovered (2,149 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Recoverable now** (`OK_RECOVERED_CALLER` — recovered caller exists; recover next) | `411` | 19.13% |
| Vtable-anchored (recover with the owning class) | `210` | 9.77% |
| Framework dispatch (wx/EH/Lua/reflection) | `35` | 1.63% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `1,052` | 48.95% |
| No callsite evidence (likely external/dead — candidates for `external_dependency`) | `439` | 20.43% |
