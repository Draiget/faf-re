...
- Progress coverage:  **`97.08%`**
  ...
  - *Total amount of completed tokens: `65,203`*

Progress DB status breakdown:

- `recovered`: `53,325` (81.78%)
...
- **`blocked`: `1,995` (2.97%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `1,999`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `1,999`*
...
### Recovered (53,325 functions) — wiring health

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Confirmed caller** (recovered binary caller wired by name) | `15,413` | 28.90% |
...
### Not-yet-recovered (1,999 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Recoverable now** (`OK_RECOVERED_CALLER` — recovered caller exists; recover next) | `306` | 15.31% |
| Vtable-anchored (recover with the owning class) | `209` | 10.46% |
| Framework dispatch (wx/EH/Lua/reflection) | `35` | 1.75% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `1,012` | 50.63% |
| No callsite evidence (likely external/dead — candidates for `external_dependency`) | `435` | 21.76% |
