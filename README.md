...
- Progress coverage:  **`97.37%`**
  ...
  - *Total amount of completed tokens: `65,400`*

Progress DB status breakdown:

- `recovered`: `52,814` (80.76%)
- `skip`: `7,079` (10.82%) — proven ICF aliases / thunks / EH or static-init glue with no distinct source body
...
- **`blocked`: `1,792` (2.67%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `1,826`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `1,826`*
...
### Recovered (52,814 functions) — binary caller context

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `16,426` | 31.10% |
| Vtable-anchored (virtual override of a recovered class) | `5,884` | 11.14% |
| Framework dispatch (wx event, EH handler, Lua binding, reflection table, …) | `5,561` | 10.53% |
| No recovered caller token yet (orphan risk) | `1,861` | 3.52% |
| No callsite evidence (no recorded code/data caller in the index) | `22,862` | 43.29% |
| Unclassified data xref (manual review) | `216` | 0.41% |
| RTTI-only | `4` | 0.01% |

### Not-yet-recovered (1,826 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `233` | 12.76% |
| Vtable-anchored (recover with the owning class) | `131` | 7.17% |
| Framework dispatch (wx/EH/Lua/reflection) | `8` | 0.44% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `844` | 46.22% |
| No indexed callsite evidence (needs investigation/evidence) | `608` | 33.30% |
