...
- Progress coverage:  **`97.91%`**
  ...
  - *Total amount of completed tokens: `65,765`*

Progress DB status breakdown:

- `recovered`: `53,058` (80.68%)
...
- **`blocked`: `1,432` (2.13%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `1,465`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `1,465`*
...
### Recovered (53,058 functions) — binary caller context

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `16,721` | 31.51% |
...
| No recovered caller token yet (orphan risk) | `1,792` | 3.38% |
| No callsite evidence (no recorded code/data caller in the index) | `22,875` | 43.11% |
| Unclassified data xref (manual review) | `216` | 0.41% |
| RTTI-only | `4` | 0.01% |

### Not-yet-recovered (1,465 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `62` | 4.23% |
| Vtable-anchored (recover with the owning class) | `126` | 8.60% |
| Framework dispatch (wx/EH/Lua/reflection) | `8` | 0.55% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `674` | 46.01% |
| No indexed callsite evidence (needs investigation/evidence) | `594` | 40.55% |
