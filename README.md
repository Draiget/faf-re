...
- Total FAF functions: `67,167`
  - *IDA index, exported*
- Progress coverage:  **`98.89%`**
  - *Consists of `recovered` + `skip` + `external_dependency` ÷ exported*
  - *Total amount of completed tokens: `66,421`*
...
- **`blocked`: `810` (1.21%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `813`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `813`*
...
### Not-yet-recovered (813 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `27` | 3.32% |
| Vtable-anchored (recover with the owning class) | `109` | 13.41% |
| Framework dispatch (wx/EH/Lua/reflection) | `0` | 0.00% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `348` | 42.80% |
| No indexed callsite evidence (needs investigation/evidence) | `328` | 40.34% |
