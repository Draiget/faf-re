...
- Total FAF functions: `67,167`
  - *IDA index, exported*
- Progress coverage:  **`96.12%`**
  - *Consists of `recovered` + `skip` + `external_dependency` ÷ exported*
  - *Total amount of completed tokens: `64,564`*

Progress DB status breakdown:

- `recovered`: `52,691` (81.61%)
- `skip`: `6,412` (9.93%) — proven ICF aliases / thunks / EH or static-init glue with no distinct source body
- `external_dependency`: `5,461` (8.46%) — proven third-party/import-boundary code
...
- `in_progress`: `101` (0.15%)
- **`blocked`: `2,166` (3.22%)**
  - *strict circular/dep-blocked (in-DB literal `status == "blocked"`)*  
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `2,573`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `2,573`*
...
### Recovered (52,691 functions) — binary caller context

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `16,337` | 31.01% |
...
### Not-yet-recovered (2,573 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `307` | 11.93% |
| Vtable-anchored (recover with the owning class) | `140` | 5.44% |
| Framework dispatch (wx/EH/Lua/reflection) | `11` | 0.43% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `971` | 37.74% |
| No indexed callsite evidence (needs investigation/evidence) | `1,142` | 44.38% |
