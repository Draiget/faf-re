...
### Recovered (53,548 functions) — wiring health

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Confirmed caller** (recovered binary caller wired by name) | `15,703` | 29.33% |
| Vtable-anchored (virtual override of a recovered class) | `5,847` | 10.92% |
| Framework dispatch (wx event, EH handler, Lua binding, reflection table, …) | `5,522` | 10.31% |
| Caller still blocked (orphan-helper risk — caller awaits recovery) | `2,532` | 4.73% |
...
|---|---:|---:|
| **Recoverable now** (`OK_RECOVERED_CALLER` — recovered caller exists; recover next) | `280` | 15.36% |
| Vtable-anchored (recover with the owning class) | `174` | 9.54% |
| Framework dispatch (wx/EH/Lua/reflection) | `4` | 0.22% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `929` | 50.96% |
| No callsite evidence (likely external/dead — candidates for `external_dependency`) | `434` | 23.81% |
