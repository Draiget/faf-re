...
- Progress coverage:  **`98.94%`**
  ...
  - *Total amount of completed tokens: `66,454`*
...
- **`blocked`: `782` (1.16%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `785`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `785`*
...
### Not-yet-recovered (785 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `24` | 3.06% |
| Vtable-anchored (recover with the owning class) | `109` | 13.89% |
| Framework dispatch (wx/EH/Lua/reflection) | `0` | 0.00% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `333` | 42.42% |
| No indexed callsite evidence (needs investigation/evidence) | `318` | 40.51% |
