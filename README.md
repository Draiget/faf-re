...
- **`blocked`: `1,860` (2.77%)**
  ...
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `2,267`*
...
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `16,351` | 31.02% |
...
### Not-yet-recovered (2,267 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `244` | 10.76% |
| Vtable-anchored (recover with the owning class) | `136` | 6.00% |
| Framework dispatch (wx/EH/Lua/reflection) | `10` | 0.44% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `882` | 38.91% |
| No indexed callsite evidence (needs investigation/evidence) | `993` | 43.80% |
