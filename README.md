...
- Progress coverage:  **`96.43%`**
  ...
  - *Total amount of completed tokens: `64,771`*

Progress DB status breakdown:

- `recovered`: `52,884` (81.65%)
...
### Recovered (52,884 functions) — wiring health

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Confirmed caller** (recovered binary caller wired by name) | `14,728` | 27.85% |
...
| Caller still blocked (orphan-helper risk — caller awaits recovery) | `2,923` | 5.53% |
| No callsite evidence (no recorded code/data caller in the index) | `23,730` | 44.87% |
...
|---|---:|---:|
| **Recoverable now** (`OK_RECOVERED_CALLER` — recovered caller exists; recover next) | `538` | 22.67% |
| Vtable-anchored (recover with the owning class) | `239` | 10.07% |
| Framework dispatch (wx/EH/Lua/reflection) | `36` | 1.52% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `1,106` | 46.61% |
