...
- Progress coverage:  **`97.74%`**
  ...
  - *Total amount of completed tokens: `65,648`*

Progress DB status breakdown:

- `recovered`: `54,066` (82.36%)
- `skip`: `6,161` (9.38%) — proven ICF aliases / thunks / EH or static-init glue with no distinct source body
...
- **`blocked`: `1,181` (1.76%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `1,589`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `1,589`*
...
### Recovered (54,066 functions) — binary caller context
...
### Not-yet-recovered (1,589 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `231` | 14.54% |
| Vtable-anchored (recover with the owning class) | `110` | 6.92% |
| Framework dispatch (wx/EH/Lua/reflection) | `1` | 0.06% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `708` | 44.56% |
| No indexed callsite evidence (needs investigation/evidence) | `537` | 33.79% |
