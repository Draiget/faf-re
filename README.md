...
- **`blocked`: `1,725` (2.57%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `1,759`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `1,759`*
...
### Recovered (52,884 functions) — binary caller context

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `16,488` | 31.18% |
...
| No callsite evidence (no recorded code/data caller in the index) | `22,873` | 43.25% |
| Unclassified data xref (manual review) | `216` | 0.41% |
| RTTI-only | `4` | 0.01% |

### Not-yet-recovered (1,759 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `199` | 11.31% |
| Vtable-anchored (recover with the owning class) | `130` | 7.39% |
| Framework dispatch (wx/EH/Lua/reflection) | `8` | 0.45% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `820` | 46.62% |
| No indexed callsite evidence (needs investigation/evidence) | `601` | 34.17% |
