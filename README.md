...
- Progress coverage:  **`97.02%`**
  ...
  - *Total amount of completed tokens: `65,168`*

Progress DB status breakdown:

- `recovered`: `53,631` (82.30%)
...
- **`blocked`: `1,661` (2.47%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `2,069`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `2,069`*
...
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `16,626` | 31.00% |
...
### Not-yet-recovered (2,069 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `297` | 14.35% |
| Vtable-anchored (recover with the owning class) | `111` | 5.36% |
| Framework dispatch (wx/EH/Lua/reflection) | `1` | 0.05% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `722` | 34.90% |
| No indexed callsite evidence (needs investigation/evidence) | `936` | 45.24% |
