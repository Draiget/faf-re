...
- `recovered`: `53,137` (79.86%)
- `skip`: `7,636` (11.48%) — proven ICF aliases / thunks / EH or static-init glue with no distinct source body
...
### Recovered (53,137 functions) — binary caller context

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `17,093` | 32.17% |
| Vtable-anchored (virtual override of a recovered class) | `5,882` | 11.07% |
| Framework dispatch (wx event, EH handler, Lua binding, reflection table, …) | `5,577` | 10.50% |
| No recovered caller token yet (orphan risk) | `1,697` | 3.19% |
| No callsite evidence (no recorded code/data caller in the index) | `22,668` | 42.66% |
