...
- Progress coverage:  **`98.46%`**
  ...
  - *Total amount of completed tokens: `66,134`*
...
- **`blocked`: `1,064` (1.58%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `1,097`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `1,097`*
...
### Not-yet-recovered (1,097 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `50` | 4.56% |
| Vtable-anchored (recover with the owning class) | `124` | 11.30% |
| Framework dispatch (wx/EH/Lua/reflection) | `8` | 0.73% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `548` | 49.95% |
| No indexed callsite evidence (needs investigation/evidence) | `366` | 33.36% |
