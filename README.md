...
- Progress coverage:  **`97.09%`**
  ...
  - *Total amount of completed tokens: `65,215`*

Progress DB status breakdown:

- `recovered`: `53,337` (81.79%)
...
- **`blocked`: `1,984` (2.95%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `1,988`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `1,988`*
...
### Recovered (53,337 functions) — wiring health

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Confirmed caller** (recovered binary caller wired by name) | `15,421` | 28.91% |
| Vtable-anchored (virtual override of a recovered class) | `5,821` | 10.91% |
...
### Not-yet-recovered (1,988 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Recoverable now** (`OK_RECOVERED_CALLER` — recovered caller exists; recover next) | `303` | 15.24% |
| Vtable-anchored (recover with the owning class) | `203` | 10.21% |
| Framework dispatch (wx/EH/Lua/reflection) | `35` | 1.76% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `1,010` | 50.80% |
| No callsite evidence (likely external/dead — candidates for `external_dependency`) | `435` | 21.88% |
