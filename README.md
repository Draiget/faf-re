...
- Progress coverage:  **`99.15%`**
  ...
  - *Total amount of completed tokens: `66,599`*

Progress DB status breakdown:

- `recovered`: `52,874` (79.39%)
- `skip`: `7,982` (11.99%) — proven ICF aliases / thunks / EH or static-init glue with no distinct source body
- `external_dependency`: `5,743` (8.62%) — proven third-party/import-boundary code
...
- **`blocked`: `673` (1.00%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `676`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `676`*
...
### Recovered (52,874 functions) — binary caller context
...
| No recovered caller token yet (orphan risk) | `1,674` | 3.17% |
| No callsite evidence (no recorded code/data caller in the index) | `22,353` | 42.28% |
| Unclassified data xref (manual review) | `216` | 0.41% |
| RTTI-only | `4` | 0.01% |

### Not-yet-recovered (676 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `9` | 1.33% |
| Vtable-anchored (recover with the owning class) | `108` | 15.98% |
| Framework dispatch (wx/EH/Lua/reflection) | `0` | 0.00% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `251` | 37.13% |
| No indexed callsite evidence (needs investigation/evidence) | `307` | 45.41% |
