...
- Progress coverage:  **`97.39%`**
  ...
  - *Total amount of completed tokens: `65,417`*

Progress DB status breakdown:

- `recovered`: `53,786` (82.22%)
- `skip`: `6,129` (9.37%) — proven ICF aliases / thunks / EH or static-init glue with no distinct source body
- `external_dependency`: `5,502` (8.41%) — proven third-party/import-boundary code
...
- **`blocked`: `1,740` (2.59%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `1,807`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `1,807`*
...
### Recovered (53,786 functions) — binary caller context

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `16,048` | 29.84% |
| Vtable-anchored (virtual override of a recovered class) | `5,846` | 10.87% |
| Framework dispatch (wx event, EH handler, Lua binding, reflection table, …) | `5,542` | 10.30% |
| No recovered caller token yet (orphan risk) | `2,409` | 4.48% |
| No callsite evidence (no recorded code/data caller in the index) | `23,720` | 44.10% |
| Unclassified data xref (manual review) | `217` | 0.40% |
| RTTI-only | `4` | 0.01% |

### Not-yet-recovered (1,807 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `275` | 15.22% |
| Vtable-anchored (recover with the owning class) | `172` | 9.52% |
| Framework dispatch (wx/EH/Lua/reflection) | `1` | 0.06% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `898` | 49.70% |
| No indexed callsite evidence (needs investigation/evidence) | `458` | 25.35% |
