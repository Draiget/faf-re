...
- Total FAF functions: `67,167`
  - *IDA index, exported*
- Progress coverage:  **`97.78%`**
  - *Consists of `recovered` + `skip` + `external_dependency` ÷ exported*
  - *Total amount of completed tokens: `65,674`*

Progress DB status breakdown:

- `recovered`: `53,011` (80.72%)
...
- **`blocked`: `1,521` (2.26%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `1,555`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `1,555`*
...
### Recovered (53,011 functions) — binary caller context

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `16,666` | 31.44% |
...
### Not-yet-recovered (1,555 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `96` | 6.17% |
| Vtable-anchored (recover with the owning class) | `129` | 8.30% |
| Framework dispatch (wx/EH/Lua/reflection) | `8` | 0.51% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `723` | 46.50% |
| No indexed callsite evidence (needs investigation/evidence) | `598` | 38.46% |
