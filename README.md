...
- Progress coverage:  **`99.18%`**
  ...
  - *Total amount of completed tokens: `66,614`*

Progress DB status breakdown:

- `recovered`: `52,821` (79.29%)
- `skip`: `8,038` (12.07%) — proven ICF aliases / thunks / EH or static-init glue with no distinct source body
- `external_dependency`: `5,755` (8.64%) — proven third-party/import-boundary code
...
- **`blocked`: `658` (0.98%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `661`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `661`*
...
### Recovered (52,821 functions) — binary caller context
...
| No callsite evidence (no recorded code/data caller in the index) | `22,295` | 42.21% |
| Unclassified data xref (manual review) | `216` | 0.41% |
| RTTI-only | `4` | 0.01% |

### Not-yet-recovered (661 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `9` | 1.36% |
| Vtable-anchored (recover with the owning class) | `108` | 16.34% |
| Framework dispatch (wx/EH/Lua/reflection) | `0` | 0.00% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `236` | 35.70% |
| No indexed callsite evidence (needs investigation/evidence) | `307` | 46.44% |
