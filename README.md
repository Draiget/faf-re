...
- Progress coverage:  **`98.46%`**
  ...
  - *Total amount of completed tokens: `66,131`*
...
- **`blocked`: `1,067` (1.59%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `1,100`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `1,100`*
...
### Not-yet-recovered (1,100 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `50` | 4.55% |
| Vtable-anchored (recover with the owning class) | `124` | 11.27% |
| Framework dispatch (wx/EH/Lua/reflection) | `8` | 0.73% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `551` | 50.09% |
| No indexed callsite evidence (needs investigation/evidence) | `366` | 33.27% |
