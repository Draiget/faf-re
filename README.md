...
- `recovered`: `53,139` (80.09%)
- `skip`: `7,386` (11.13%) — proven ICF aliases / thunks / EH or static-init glue with no distinct source body
...
### Recovered (53,139 functions) — binary caller context

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `16,844` | 31.70% |
...
| No recovered caller token yet (orphan risk) | `1,749` | 3.29% |
...
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `30` | 3.40% |
| Vtable-anchored (recover with the owning class) | `109` | 12.36% |
| Framework dispatch (wx/EH/Lua/reflection) | `0` | 0.00% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `382` | 43.31% |
