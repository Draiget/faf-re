...
- Progress coverage:  **`97.21%`**
  ...
  - *Total amount of completed tokens: `65,293`*

Progress DB status breakdown:

- `recovered`: `53,686` (82.22%)
...
- **`blocked`: `1,865` (2.78%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `1,931`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `1,931`*
...
### Recovered (53,686 functions) — binary caller context

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `16,011` | 29.82% |
...
| No callsite evidence (no recorded code/data caller in the index) | `23,718` | 44.18% |
| Unclassified data xref (manual review) | `216` | 0.40% |
| RTTI-only | `4` | 0.01% |

### Not-yet-recovered (1,931 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `311` | 16.11% |
| Vtable-anchored (recover with the owning class) | `189` | 9.79% |
| Framework dispatch (wx/EH/Lua/reflection) | `55` | 2.85% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `911` | 47.18% |
| No indexed callsite evidence (needs investigation/evidence) | `461` | 23.87% |
