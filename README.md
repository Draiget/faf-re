...
- Progress coverage:  **`97.57%`**
  ...
  - *Total amount of completed tokens: `65,534`*

Progress DB status breakdown:

- `recovered`: `52,919` (80.75%)
...
- **`blocked`: `1,661` (2.47%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `1,695`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `1,695`*
...
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `16,538` | 31.25% |
...
### Not-yet-recovered (1,695 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `179` | 10.56% |
| Vtable-anchored (recover with the owning class) | `129` | 7.61% |
| Framework dispatch (wx/EH/Lua/reflection) | `8` | 0.47% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `779` | 45.96% |
| No indexed callsite evidence (needs investigation/evidence) | `599` | 35.34% |
