...
- Total FAF functions: `67,167`
  - *IDA index, exported*
- Progress coverage:  **`97.37%`**
  - *Consists of `recovered` + `skip` + `external_dependency` ÷ exported*
  - *Total amount of completed tokens: `65,403`*

Progress DB status breakdown:

- `recovered`: `53,770` (82.21%)
...
- **`blocked`: `1,746` (2.60%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `1,814`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `1,814`*
...
### Recovered (53,770 functions) — binary caller context

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `16,014` | 29.78% |
...
| No recovered caller token yet (orphan risk) | `2,429` | 4.52% |
| No callsite evidence (no recorded code/data caller in the index) | `23,719` | 44.11% |
| Unclassified data xref (manual review) | `217` | 0.40% |
| RTTI-only | `4` | 0.01% |

### Not-yet-recovered (1,814 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `276` | 15.21% |
| Vtable-anchored (recover with the owning class) | `172` | 9.48% |
| Framework dispatch (wx/EH/Lua/reflection) | `1` | 0.06% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `903` | 49.78% |
| No indexed callsite evidence (needs investigation/evidence) | `459` | 25.30% |
