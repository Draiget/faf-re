...
- Progress coverage:  **`99.33%`**
  ...
  - *Total amount of completed tokens: `66,721`*

Progress DB status breakdown:

- `recovered`: `52,749` (79.06%)
- `skip`: `8,227` (12.33%) — proven ICF aliases / thunks / EH or static-init glue with no distinct source body
...
- **`blocked`: `556` (0.83%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `559`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `559`*
...
### Not-yet-recovered (559 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `9` | 1.61% |
| Vtable-anchored (recover with the owning class) | `105` | 18.78% |
| Framework dispatch (wx/EH/Lua/reflection) | `0` | 0.00% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `154` | 27.55% |
| No indexed callsite evidence (needs investigation/evidence) | `290` | 51.88% |
