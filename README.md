...
- Progress coverage:  **`97.64%`**
  ...
  - *Total amount of completed tokens: `65,584`*

Progress DB status breakdown:

- `recovered`: `54,048` (82.41%)
...
- **`blocked`: `1,245` (1.85%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `1,653`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `1,653`*
...
### Recovered (54,048 functions) — binary caller context

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `16,649` | 30.80% |
...
### Not-yet-recovered (1,653 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `290` | 17.54% |
| Vtable-anchored (recover with the owning class) | `110` | 6.65% |
| Framework dispatch (wx/EH/Lua/reflection) | `1` | 0.06% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `715` | 43.25% |
| No indexed callsite evidence (needs investigation/evidence) | `535` | 32.37% |
