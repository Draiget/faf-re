...
- Total FAF functions: `67,167`
  - *IDA index, exported*
- Progress coverage:  **`97.51%`**
  - *Consists of `recovered` + `skip` + `external_dependency` ÷ exported*
  - *Total amount of completed tokens: `65,494`*

Progress DB status breakdown:

- `recovered`: `52,898` (80.77%)
- `skip`: `7,084` (10.82%) — proven ICF aliases / thunks / EH or static-init glue with no distinct source body
- `external_dependency`: `5,512` (8.42%) — proven third-party/import-boundary code
...
- **`blocked`: `1,700` (2.53%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `1,734`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `1,734`*
...
### Recovered (52,898 functions) — binary caller context

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `16,510` | 31.21% |
...
| No recovered caller token yet (orphan risk) | `1,846` | 3.49% |
| No callsite evidence (no recorded code/data caller in the index) | `22,873` | 43.24% |
| Unclassified data xref (manual review) | `216` | 0.41% |
| RTTI-only | `4` | 0.01% |

### Not-yet-recovered (1,734 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `188` | 10.84% |
| Vtable-anchored (recover with the owning class) | `130` | 7.50% |
| Framework dispatch (wx/EH/Lua/reflection) | `8` | 0.46% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `806` | 46.48% |
| No indexed callsite evidence (needs investigation/evidence) | `601` | 34.66% |
