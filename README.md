...
- Progress coverage:  **`96.82%`**
  ...
  - *Total amount of completed tokens: `65,030`*

Progress DB status breakdown:

- `recovered`: `53,425` (82.15%)
...
### Recovered (53,425 functions) — binary caller context
...
| Framework dispatch (wx event, EH handler, Lua binding, reflection table, …) | `5,533` | 10.36% |
...
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `350` | 15.86% |
| Vtable-anchored (recover with the owning class) | `132` | 5.98% |
| Framework dispatch (wx/EH/Lua/reflection) | `29` | 1.31% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `773` | 35.02% |
| No indexed callsite evidence (needs investigation/evidence) | `920` | 41.69% |
