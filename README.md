...
- **`blocked`: `756` (1.13%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `759`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `759`*
...
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `17,007` | 31.91% |
...
| No recovered caller token yet (orphan risk) | `1,708` | 3.20% |
| No callsite evidence (no recorded code/data caller in the index) | `22,907` | 42.98% |
| Unclassified data xref (manual review) | `216` | 0.41% |
| RTTI-only | `4` | 0.01% |

### Not-yet-recovered (759 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `24` | 3.16% |
| Vtable-anchored (recover with the owning class) | `109` | 14.36% |
| Framework dispatch (wx/EH/Lua/reflection) | `0` | 0.00% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `307` | 40.45% |
| No indexed callsite evidence (needs investigation/evidence) | `318` | 41.90% |
