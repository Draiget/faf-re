...
- Total FAF functions: `67,167`
  - *IDA index, exported*
- Progress coverage:  **`97.22%`**
  - *Consists of `recovered` + `skip` + `external_dependency` ÷ exported*
  - *Total amount of completed tokens: `65,298`*

Progress DB status breakdown:

- `recovered`: `53,691` (82.22%)
- `skip`: `6,115` (9.36%) — proven ICF aliases / thunks / EH or static-init glue with no distinct source body
...
- **`blocked`: `1,860` (2.77%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `1,926`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `1,926`*
...
### Recovered (53,691 functions) — binary caller context

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `16,016` | 29.83% |
| Vtable-anchored (virtual override of a recovered class) | `5,832` | 10.86% |
| Framework dispatch (wx event, EH handler, Lua binding, reflection table, …) | `5,504` | 10.25% |
| No recovered caller token yet (orphan risk) | `2,400` | 4.47% |
| No callsite evidence (no recorded code/data caller in the index) | `23,719` | 44.18% |
| Unclassified data xref (manual review) | `216` | 0.40% |
| RTTI-only | `4` | 0.01% |

### Not-yet-recovered (1,926 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `309` | 16.04% |
| Vtable-anchored (recover with the owning class) | `189` | 9.81% |
| Framework dispatch (wx/EH/Lua/reflection) | `54` | 2.80% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `910` | 47.25% |
| No indexed callsite evidence (needs investigation/evidence) | `460` | 23.88% |
