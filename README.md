...
- Progress coverage:  **`97.04%`**
  ...
  - *Total amount of completed tokens: `65,178`*

Progress DB status breakdown:

- `recovered`: `53,632` (82.29%)
...
- **`blocked`: `1,651` (2.46%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `2,059`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `2,059`*
...
| Vtable-anchored (virtual override of a recovered class) | `5,897` | 11.00% |
...
### Not-yet-recovered (2,059 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `275` | 13.36% |
| Vtable-anchored (recover with the owning class) | `115` | 5.59% |
| Framework dispatch (wx/EH/Lua/reflection) | `1` | 0.05% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `754` | 36.62% |
| No indexed callsite evidence (needs investigation/evidence) | `912` | 44.29% |
