...
- Progress coverage:  **`99.28%`**
  ...
  - *Total amount of completed tokens: `66,689`*
...
- **`blocked`: `588` (0.88%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `591`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `591`*
...
### Not-yet-recovered (591 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `9` | 1.52% |
| Vtable-anchored (recover with the owning class) | `106` | 17.94% |
| Framework dispatch (wx/EH/Lua/reflection) | `0` | 0.00% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `170` | 28.76% |
| No indexed callsite evidence (needs investigation/evidence) | `305` | 51.61% |
