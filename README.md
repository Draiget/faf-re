...
### Recovered (53,533 functions) — binary caller context
...
| No callsite evidence (no recorded code/data caller in the index) | `23,194` | 43.33% |
...
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `306` | 14.10% |
| Vtable-anchored (recover with the owning class) | `113` | 5.21% |
| Framework dispatch (wx/EH/Lua/reflection) | `1` | 0.05% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `764` | 35.21% |
| No indexed callsite evidence (needs investigation/evidence) | `984` | 45.35% |
