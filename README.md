...
- Progress coverage:  **`96.40%`**
  ...
  - *Total amount of completed tokens: `64,752`*

Progress DB status breakdown:

- `recovered`: `53,132` (82.05%)
...
- **`blocked`: `2,068` (3.08%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `2,486`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `2,486`*
...
### Recovered (53,132 functions) — binary caller context

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `16,146` | 30.39% |
...
### Not-yet-recovered (2,486 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `406` | 16.33% |
| Vtable-anchored (recover with the owning class) | `203` | 8.17% |
| Framework dispatch (wx/EH/Lua/reflection) | `55` | 2.21% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `831` | 33.43% |
| No indexed callsite evidence (needs investigation/evidence) | `987` | 39.70% |
