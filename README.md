...
- Total FAF functions: `67,167`
  - *IDA index, exported*
- Progress coverage:  **`97.18%`**
  - *Consists of `recovered` + `skip` + `external_dependency` ÷ exported*
  - *Total amount of completed tokens: `65,274`*

Progress DB status breakdown:

- `recovered`: `53,643` (82.18%)
- `skip`: `6,129` (9.39%) — proven ICF aliases / thunks / EH or static-init glue with no distinct source body
- `external_dependency`: `5,502` (8.43%) — proven third-party/import-boundary code
...
- **`blocked`: `1,883` (2.80%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `1,950`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `1,950`*
...
### Recovered (53,643 functions) — binary caller context

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `15,973` | 29.78% |
| Vtable-anchored (virtual override of a recovered class) | `5,829` | 10.87% |
| Framework dispatch (wx event, EH handler, Lua binding, reflection table, …) | `5,487` | 10.23% |
| No recovered caller token yet (orphan risk) | `2,417` | 4.51% |
| No callsite evidence (no recorded code/data caller in the index) | `23,717` | 44.21% |
| Unclassified data xref (manual review) | `216` | 0.40% |
| RTTI-only | `4` | 0.01% |

### Not-yet-recovered (1,950 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `315` | 16.15% |
| Vtable-anchored (recover with the owning class) | `193` | 9.90% |
| Framework dispatch (wx/EH/Lua/reflection) | `56` | 2.87% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `921` | 47.23% |
| No indexed callsite evidence (needs investigation/evidence) | `461` | 23.64% |
