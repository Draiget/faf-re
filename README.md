...
- Progress coverage:  **`97.43%`**
  ...
  - *Total amount of completed tokens: `65,441`*

Progress DB status breakdown:

- `recovered`: `53,820` (82.24%)
...
### Recovered (53,820 functions) — binary caller context

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `16,234` | 30.16% |
...
| No recovered caller token yet (orphan risk) | `2,287` | 4.25% |
...
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `282` | 15.82% |
| Vtable-anchored (recover with the owning class) | `162` | 9.09% |
| Framework dispatch (wx/EH/Lua/reflection) | `54` | 3.03% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `836` | 46.89% |
| No indexed callsite evidence (needs investigation/evidence) | `445` | 24.96% |
