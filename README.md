...
- Progress coverage:  **`96.84%`**
  ...
  - *Total amount of completed tokens: `65,045`*

Progress DB status breakdown:

- `recovered`: `53,438` (82.16%)
...
- **`blocked`: `1,784` (2.66%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `2,192`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `2,192`*
...
### Recovered (53,438 functions) — binary caller context
...
| Framework dispatch (wx event, EH handler, Lua binding, reflection table, …) | `5,543` | 10.37% |
...
### Not-yet-recovered (2,192 blocked + needs_evidence) — backlog readiness
...
| Framework dispatch (wx/EH/Lua/reflection) | `19` | 0.87% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `772` | 35.22% |
| No indexed callsite evidence (needs investigation/evidence) | `920` | 41.97% |
