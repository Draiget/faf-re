...
- Total FAF functions: `67,167`
  - *IDA index, exported*
- Progress coverage:  **`96.56%`**
  - *Consists of `recovered` + `skip` + `external_dependency` ÷ exported*
  - *Total amount of completed tokens: `64,858`*

Progress DB status breakdown:

- `recovered`: `53,222` (82.06%)
...
- **`blocked`: `1,967` (2.93%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `2,379`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `2,379`*
...
### Recovered (53,222 functions) — binary caller context

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `16,244` | 30.52% |
| Vtable-anchored (virtual override of a recovered class) | `5,833` | 10.96% |
| Framework dispatch (wx event, EH handler, Lua binding, reflection table, …) | `5,513` | 10.36% |
| No recovered caller token yet (orphan risk) | `2,227` | 4.18% |
| No callsite evidence (no recorded code/data caller in the index) | `23,185` | 43.56% |
| Unclassified data xref (manual review) | `216` | 0.41% |
| RTTI-only | `4` | 0.01% |

### Not-yet-recovered (2,379 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `350` | 14.71% |
| Vtable-anchored (recover with the owning class) | `186` | 7.82% |
| Framework dispatch (wx/EH/Lua/reflection) | `45` | 1.89% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `811` | 34.09% |
| No indexed callsite evidence (needs investigation/evidence) | `983` | 41.32% |
