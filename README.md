...
- Progress coverage:  **`96.84%`**
  ...
  - *Total amount of completed tokens: `65,046`*

Progress DB status breakdown:

- `recovered`: `53,439` (82.16%)
...
- **`blocked`: `1,783` (2.65%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `2,191`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `2,191`*
...
### Recovered (53,439 functions) — binary caller context
...
| Framework dispatch (wx event, EH handler, Lua binding, reflection table, …) | `5,544` | 10.37% |
...
### Not-yet-recovered (2,191 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `348` | 15.88% |
| Vtable-anchored (recover with the owning class) | `130` | 5.93% |
| Framework dispatch (wx/EH/Lua/reflection) | `18` | 0.82% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `772` | 35.24% |
| No indexed callsite evidence (needs investigation/evidence) | `920` | 41.99% |
