...
- Progress coverage:  **`97.09%`**
  ...
  - *Total amount of completed tokens: `65,214`*

Progress DB status breakdown:

- `recovered`: `53,336` (81.79%)
...
- **`blocked`: `1,985` (2.96%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `1,989`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `1,989`*
...
### Recovered (53,336 functions) — wiring health

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Confirmed caller** (recovered binary caller wired by name) | `15,421` | 28.91% |
| Vtable-anchored (virtual override of a recovered class) | `5,820` | 10.91% |
...
### Not-yet-recovered (1,989 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Recoverable now** (`OK_RECOVERED_CALLER` — recovered caller exists; recover next) | `303` | 15.23% |
| Vtable-anchored (recover with the owning class) | `204` | 10.26% |
| Framework dispatch (wx/EH/Lua/reflection) | `35` | 1.76% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `1,010` | 50.78% |
| No callsite evidence (likely external/dead — candidates for `external_dependency`) | `435` | 21.87% |
