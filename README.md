...
- Total FAF functions: `67,167`
  - *IDA index, exported*
- Progress coverage:  **`96.61%`**
  - *Consists of `recovered` + `skip` + `external_dependency` ÷ exported*
  - *Total amount of completed tokens: `64,889`*

Progress DB status breakdown:

- `recovered`: `53,258` (82.08%)
...
- **`blocked`: `1,931` (2.87%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `2,341`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `2,341`*
...
### Recovered (53,258 functions) — binary caller context

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `16,272` | 30.55% |
| Vtable-anchored (virtual override of a recovered class) | `5,844` | 10.97% |
| Framework dispatch (wx event, EH handler, Lua binding, reflection table, …) | `5,513` | 10.35% |
| No recovered caller token yet (orphan risk) | `2,214` | 4.16% |
| No callsite evidence (no recorded code/data caller in the index) | `23,195` | 43.55% |
| Unclassified data xref (manual review) | `216` | 0.41% |
| RTTI-only | `4` | 0.01% |

### Not-yet-recovered (2,341 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `350` | 14.95% |
| Vtable-anchored (recover with the owning class) | `168` | 7.18% |
| Framework dispatch (wx/EH/Lua/reflection) | `45` | 1.92% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `802` | 34.26% |
| No indexed callsite evidence (needs investigation/evidence) | `973` | 41.56% |
