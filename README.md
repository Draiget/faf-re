...
- Total FAF functions: `67,167`
  - *IDA index, exported*
- Progress coverage:  **`95.85%`**
  - *Consists of `recovered` + `skip` + `external_dependency` ÷ exported*
  - *Total amount of completed tokens: `64,380`*

Progress DB status breakdown:

- `recovered`: `52,729` (81.90%)
...
- **`blocked`: `2,449` (3.65%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `2,857`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `2,857`*
...
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `16,255` | 30.83% |
...
### Not-yet-recovered (2,857 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `424` | 14.84% |
| Vtable-anchored (recover with the owning class) | `144` | 5.04% |
| Framework dispatch (wx/EH/Lua/reflection) | `13` | 0.46% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `1,035` | 36.23% |
| No indexed callsite evidence (needs investigation/evidence) | `1,238` | 43.33% |
