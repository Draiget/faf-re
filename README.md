...
### Recovered (53,540 functions) — wiring health
...
| Framework dispatch (wx event, EH handler, Lua binding, reflection table, …) | `5,518` | 10.31% |
...
|---|---:|---:|
| **Recoverable now** (`OK_RECOVERED_CALLER` — recovered caller exists; recover next) | `280` | 15.32% |
| Vtable-anchored (recover with the owning class) | `174` | 9.52% |
| Framework dispatch (wx/EH/Lua/reflection) | `8` | 0.44% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `930` | 50.88% |
| No callsite evidence (likely external/dead — candidates for `external_dependency`) | `434` | 23.74% |
