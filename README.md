...
- Progress coverage:  **`96.86%`**
  ...
  - *Total amount of completed tokens: `65,059`*

Progress DB status breakdown:

- `recovered`: `53,525` (82.27%)
...
- **`blocked`: `1,770` (2.64%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `2,178`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `2,178`*
...
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `16,553` | 30.93% |
...
| No recovered caller token yet (orphan risk) | `2,081` | 3.89% |
| No callsite evidence (no recorded code/data caller in the index) | `23,193` | 43.33% |
| Unclassified data xref (manual review) | `217` | 0.41% |
| RTTI-only | `4` | 0.01% |

### Not-yet-recovered (2,178 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `310` | 14.23% |
| Vtable-anchored (recover with the owning class) | `113` | 5.19% |
| Framework dispatch (wx/EH/Lua/reflection) | `1` | 0.05% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `767` | 35.22% |
| No indexed callsite evidence (needs investigation/evidence) | `985` | 45.22% |
