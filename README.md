...
- Total FAF functions: `67,167`
  - *IDA index, exported*
- Progress coverage:  **`98.80%`**
  - *Consists of `recovered` + `skip` + `external_dependency` ÷ exported*
  - *Total amount of completed tokens: `66,363`*

Progress DB status breakdown:

- `recovered`: `53,156` (80.10%)
- `skip`: `7,382` (11.12%) — proven ICF aliases / thunks / EH or static-init glue with no distinct source body
- `external_dependency`: `5,825` (8.78%) — proven third-party/import-boundary code
  - *libpng, zlib, wxWidgets, LuaPlus/Lua, external Boost internals, WildMagic/Wm3, CRI Sofdec/ADX, undname, bugsplat, CRT imports*
- `needs_evidence`: `7` (0.01%)
- `in_progress`: `9` (0.01%)
- **`blocked`: `861` (1.28%)**
  - *strict circular/dep-blocked (in-DB literal `status == "blocked"`)*  
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `868`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `868`*
...
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `16,872` | 31.74% |
| Vtable-anchored (virtual override of a recovered class) | `5,885` | 11.07% |
| Framework dispatch (wx event, EH handler, Lua binding, reflection table, …) | `5,565` | 10.47% |
| No recovered caller token yet (orphan risk) | `1,738` | 3.27% |
| No callsite evidence (no recorded code/data caller in the index) | `22,876` | 43.04% |
| Unclassified data xref (manual review) | `216` | 0.41% |
| RTTI-only | `4` | 0.01% |

### Not-yet-recovered (868 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `28` | 3.23% |
| Vtable-anchored (recover with the owning class) | `109` | 12.56% |
| Framework dispatch (wx/EH/Lua/reflection) | `0` | 0.00% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `370` | 42.63% |
| No indexed callsite evidence (needs investigation/evidence) | `360` | 41.47% |
| Unclassified data xref (manual review) | `1` | 0.12% |
| RTTI-only | `0` | 0.00% |
