...
- Progress coverage:  **`97.68%`**
  ...
  - *Total amount of completed tokens: `65,609`*

Progress DB status breakdown:

- `recovered`: `52,962` (80.72%)
...
- **`blocked`: `1,586` (2.36%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `1,620`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `1,620`*
...
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `16,601` | 31.35% |
...
### Not-yet-recovered (1,620 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `145` | 8.95% |
| Vtable-anchored (recover with the owning class) | `129` | 7.96% |
| Framework dispatch (wx/EH/Lua/reflection) | `8` | 0.49% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `738` | 45.56% |
| No indexed callsite evidence (needs investigation/evidence) | `599` | 36.98% |
