...
- Total FAF functions: `67,170`
  - *IDA index, exported*
- Progress coverage:  **`99.25%`**
  - *Consists of `recovered` + `skip` + `external_dependency` ÷ exported*
  - *Total amount of completed tokens: `66,666`*

Progress DB status breakdown:

- `recovered`: `52,744` (79.12%)
- `skip`: `8,190` (12.29%) — proven ICF aliases / thunks / EH or static-init glue with no distinct source body
...
- **`blocked`: `611` (0.91%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `614`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `614`*
...
### Not-yet-recovered (614 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `9` | 1.47% |
| Vtable-anchored (recover with the owning class) | `106` | 17.26% |
| Framework dispatch (wx/EH/Lua/reflection) | `0` | 0.00% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `193` | 31.43% |
| No indexed callsite evidence (needs investigation/evidence) | `305` | 49.67% |
| Unclassified data xref (manual review) | `1` | 0.16% |
| RTTI-only | `0` | 0.00% |
