...
- Progress coverage:  **`95.85%`**
  ...
  - *Total amount of completed tokens: `64,379`*

Progress DB status breakdown:

- `recovered`: `52,728` (81.90%)
...
- **`blocked`: `2,450` (3.65%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `2,858`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `2,858`*
...
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `16,253` | 30.82% |
...
### Not-yet-recovered (2,858 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `425` | 14.87% |
| Vtable-anchored (recover with the owning class) | `144` | 5.04% |
| Framework dispatch (wx/EH/Lua/reflection) | `13` | 0.45% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `1,035` | 36.21% |
| No indexed callsite evidence (needs investigation/evidence) | `1,238` | 43.32% |
