...
- Progress coverage:  **`97.60%`**
  ...
  - *Total amount of completed tokens: `65,556`*

Progress DB status breakdown:

- `recovered`: `52,940` (80.76%)
...
- **`blocked`: `1,639` (2.44%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `1,673`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `1,673`*
...
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `16,562` | 31.28% |
...
### Not-yet-recovered (1,673 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `162` | 9.68% |
| Vtable-anchored (recover with the owning class) | `129` | 7.71% |
| Framework dispatch (wx/EH/Lua/reflection) | `8` | 0.48% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `774` | 46.26% |
| No indexed callsite evidence (needs investigation/evidence) | `599` | 35.80% |
