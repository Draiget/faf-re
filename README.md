...
- Progress coverage:  **`96.90%`**
  ...
  - *Total amount of completed tokens: `65,086`*

Progress DB status breakdown:

- `recovered`: `53,494` (82.19%)
...
### Recovered (53,494 functions) — binary caller context

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `16,403` | 30.66% |
...
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `346` | 16.09% |
| Vtable-anchored (recover with the owning class) | `118` | 5.49% |
| Framework dispatch (wx/EH/Lua/reflection) | `1` | 0.05% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `769` | 35.75% |
| No indexed callsite evidence (needs investigation/evidence) | `914` | 42.49% |
