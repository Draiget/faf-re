...
- Progress coverage:  **`96.73%`**
  ...
  - *Total amount of completed tokens: `64,969`*

Progress DB status breakdown:

- `recovered`: `53,364` (82.14%)
...
- **`blocked`: `1,860` (2.77%)**
  ...
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `2,268`*
...
### Recovered (53,364 functions) — binary caller context

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `16,374` | 30.68% |
...
| No recovered caller token yet (orphan risk) | `2,164` | 4.06% |
| No callsite evidence (no recorded code/data caller in the index) | `23,217` | 43.51% |
| Unclassified data xref (manual review) | `216` | 0.40% |
| RTTI-only | `4` | 0.01% |

### Not-yet-recovered (2,268 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `350` | 15.43% |
| Vtable-anchored (recover with the owning class) | `150` | 6.61% |
| Framework dispatch (wx/EH/Lua/reflection) | `40` | 1.76% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `773` | 34.08% |
| No indexed callsite evidence (needs investigation/evidence) | `952` | 41.98% |
