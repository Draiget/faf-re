...
- Progress coverage:  **`96.39%`**
  ...
  - *Total amount of completed tokens: `64,740`*

Progress DB status breakdown:

- `recovered`: `53,125` (82.06%)
...
- **`blocked`: `2,075` (3.09%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `2,499`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `2,499`*
...
### Recovered (53,125 functions) — binary caller context
...
| Framework dispatch (wx event, EH handler, Lua binding, reflection table, …) | `5,503` | 10.36% |
...
### Not-yet-recovered (2,499 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `415` | 16.61% |
| Vtable-anchored (recover with the owning class) | `203` | 8.12% |
| Framework dispatch (wx/EH/Lua/reflection) | `56` | 2.24% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `834` | 33.37% |
| No indexed callsite evidence (needs investigation/evidence) | `987` | 39.50% |
