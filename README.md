...
- Progress coverage:  **`96.63%`**
  ...
  - *Total amount of completed tokens: `64,904`*

Progress DB status breakdown:

- `recovered`: `53,292` (82.11%)
...
- **`blocked`: `1,921` (2.86%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `2,331`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `2,331`*
...
### Recovered (53,292 functions) — binary caller context

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `16,314` | 30.61% |
...
### Not-yet-recovered (2,331 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `350` | 15.02% |
| Vtable-anchored (recover with the owning class) | `165` | 7.08% |
| Framework dispatch (wx/EH/Lua/reflection) | `45` | 1.93% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `795` | 34.11% |
| No indexed callsite evidence (needs investigation/evidence) | `973` | 41.74% |
