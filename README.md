...
- Progress coverage:  **`97.49%`**
  ...
  - *Total amount of completed tokens: `65,484`*

Progress DB status breakdown:

- `recovered`: `52,896` (80.78%)
...
- **`blocked`: `1,709` (2.54%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `1,743`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `1,743`*
...
### Recovered (52,896 functions) — binary caller context

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `16,500` | 31.19% |
...
| No recovered caller token yet (orphan risk) | `1,854` | 3.50% |
| No callsite evidence (no recorded code/data caller in the index) | `22,873` | 43.24% |
| Unclassified data xref (manual review) | `216` | 0.41% |
| RTTI-only | `4` | 0.01% |

### Not-yet-recovered (1,743 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `189` | 10.84% |
| Vtable-anchored (recover with the owning class) | `130` | 7.46% |
| Framework dispatch (wx/EH/Lua/reflection) | `8` | 0.46% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `814` | 46.70% |
| No indexed callsite evidence (needs investigation/evidence) | `601` | 34.48% |
