...
- Progress coverage:  **`98.93%`**
  ...
  - *Total amount of completed tokens: `66,451`*

Progress DB status breakdown:

- `recovered`: `53,289` (80.19%)
...
- **`blocked`: `785` (1.17%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `788`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `788`*
...
### Recovered (53,289 functions) — binary caller context
...
### Not-yet-recovered (788 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `24` | 3.05% |
| Vtable-anchored (recover with the owning class) | `109` | 13.83% |
| Framework dispatch (wx/EH/Lua/reflection) | `0` | 0.00% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `335` | 42.51% |
| No indexed callsite evidence (needs investigation/evidence) | `319` | 40.48% |
