...
- Progress coverage:  **`96.37%`**
  ...
  - *Total amount of completed tokens: `64,732`*

Progress DB status breakdown:

- `recovered`: `53,117` (82.06%)
...
- **`blocked`: `2,083` (3.10%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `2,507`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `2,507`*
...
| Framework dispatch (wx event, EH handler, Lua binding, reflection table, …) | `5,495` | 10.35% |
...
### Not-yet-recovered (2,507 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `415` | 16.55% |
| Vtable-anchored (recover with the owning class) | `203` | 8.10% |
| Framework dispatch (wx/EH/Lua/reflection) | `64` | 2.55% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `834` | 33.27% |
| No indexed callsite evidence (needs investigation/evidence) | `987` | 39.37% |
