...
- Progress coverage:  **`96.53%`**
  ...
  - *Total amount of completed tokens: `64,836`*

Progress DB status breakdown:

- `recovered`: `53,206` (82.06%)
...
- **`blocked`: `1,987` (2.96%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `2,401`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `2,401`*
...
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `16,226` | 30.50% |
...
### Not-yet-recovered (2,401 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `367` | 15.29% |
| Vtable-anchored (recover with the owning class) | `186` | 7.75% |
| Framework dispatch (wx/EH/Lua/reflection) | `45` | 1.87% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `816` | 33.99% |
| No indexed callsite evidence (needs investigation/evidence) | `983` | 40.94% |
