...
- Progress coverage:  **`98.38%`**
  ...
  - *Total amount of completed tokens: `66,082`*

Progress DB status breakdown:

- `recovered`: `53,064` (80.30%)
- `skip`: `7,370` (11.15%) — proven ICF aliases / thunks / EH or static-init glue with no distinct source body
...
- **`blocked`: `1,116` (1.66%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `1,149`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `1,149`*
...
### Recovered (53,064 functions) — binary caller context

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `16,730` | 31.53% |
...
### Not-yet-recovered (1,149 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `54` | 4.70% |
| Vtable-anchored (recover with the owning class) | `124` | 10.79% |
| Framework dispatch (wx/EH/Lua/reflection) | `8` | 0.70% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `593` | 51.61% |
| No indexed callsite evidence (needs investigation/evidence) | `369` | 32.11% |
