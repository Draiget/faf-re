...
- Total FAF functions: `67,167`
  - *IDA index, exported*
- Progress coverage:  **`96.77%`**
  - *Consists of `recovered` + `skip` + `external_dependency` ÷ exported*
  - *Total amount of completed tokens: `65,000`*

Progress DB status breakdown:

- `recovered`: `52,774` (81.19%)
- `skip`: `6,719` (10.34%) — proven ICF aliases / thunks / EH or static-init glue with no distinct source body
- `external_dependency`: `5,507` (8.47%) — proven third-party/import-boundary code
...
- **`blocked`: `1,821` (2.71%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `2,226`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `2,226`*
...
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `16,393` | 31.06% |
| Vtable-anchored (virtual override of a recovered class) | `5,883` | 11.15% |
| Framework dispatch (wx event, EH handler, Lua binding, reflection table, …) | `5,557` | 10.53% |
| No recovered caller token yet (orphan risk) | `1,864` | 3.53% |
| No callsite evidence (no recorded code/data caller in the index) | `22,857` | 43.31% |
| Unclassified data xref (manual review) | `216` | 0.41% |
| RTTI-only | `4` | 0.01% |

### Not-yet-recovered (2,226 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `237` | 10.65% |
| Vtable-anchored (recover with the owning class) | `132` | 5.93% |
| Framework dispatch (wx/EH/Lua/reflection) | `8` | 0.36% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `866` | 38.90% |
| No indexed callsite evidence (needs investigation/evidence) | `981` | 44.07% |
