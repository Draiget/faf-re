...
- Progress coverage:  **`96.84%`**
  ...
  - *Total amount of completed tokens: `65,047`*

Progress DB status breakdown:

- `recovered`: `53,440` (82.16%)
...
- **`blocked`: `1,782` (2.65%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `2,190`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `2,190`*
...
### Recovered (53,440 functions) — binary caller context
...
| Framework dispatch (wx event, EH handler, Lua binding, reflection table, …) | `5,545` | 10.38% |
| No recovered caller token yet (orphan risk) | `2,156` | 4.03% |
| No callsite evidence (no recorded code/data caller in the index) | `23,249` | 43.50% |
| Unclassified data xref (manual review) | `216` | 0.40% |
| RTTI-only | `4` | 0.01% |

### Not-yet-recovered (2,190 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `348` | 15.89% |
| Vtable-anchored (recover with the owning class) | `130` | 5.94% |
| Framework dispatch (wx/EH/Lua/reflection) | `17` | 0.78% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `772` | 35.25% |
| No indexed callsite evidence (needs investigation/evidence) | `920` | 42.01% |
