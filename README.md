...
- Progress coverage:  **`99.72%`**
  ...
  - *Total amount of completed tokens: `66,981`*

Progress DB status breakdown:

- `recovered`: `47,463` (70.86%)
...
- **`blocked`: `305` (0.45%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `308`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `308`*
...
### Recovered (47,463 functions) — binary caller context

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `17,320` | 36.49% |
...
| No recovered caller token yet (orphan risk) | `1,576` | 3.32% |
| No callsite evidence (no recorded code/data caller in the index) | `16,834` | 35.47% |
| Unclassified data xref (manual review) | `216` | 0.46% |
| RTTI-only | `4` | 0.01% |

### Not-yet-recovered (308 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `3` | 0.97% |
| Vtable-anchored (recover with the owning class) | `80` | 25.97% |
| Framework dispatch (wx/EH/Lua/reflection) | `0` | 0.00% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `115` | 37.34% |
| No indexed callsite evidence (needs investigation/evidence) | `110` | 35.71% |
