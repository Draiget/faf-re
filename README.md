...
- Progress coverage:  **`97.12%`**
  ...
  - *Total amount of completed tokens: `65,230`*

Progress DB status breakdown:

- `recovered`: `53,352` (81.79%)
...
- **`blocked`: `1,969` (2.93%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `1,973`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `1,973`*
...
### Recovered (53,352 functions) — wiring health
...
### Not-yet-recovered (1,973 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Recoverable now** (`OK_RECOVERED_CALLER` — recovered caller exists; recover next) | `307` | 15.56% |
| Vtable-anchored (recover with the owning class) | `193` | 9.78% |
| Framework dispatch (wx/EH/Lua/reflection) | `35` | 1.77% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `1,002` | 50.79% |
| No callsite evidence (likely external/dead — candidates for `external_dependency`) | `434` | 22.00% |
