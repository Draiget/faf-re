...
- Progress coverage:  **`97.20%`**
  ...
  - *Total amount of completed tokens: `65,287`*
...
- **`blocked`: `1,513` (2.25%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `1,937`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `1,937`*
...
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `16,166` | 30.12% |
...
### Not-yet-recovered (1,937 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `383` | 19.77% |
| Vtable-anchored (recover with the owning class) | `177` | 9.14% |
| Framework dispatch (wx/EH/Lua/reflection) | `64` | 3.30% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `843` | 43.52% |
| No indexed callsite evidence (needs investigation/evidence) | `466` | 24.06% |
