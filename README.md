...
- **`blocked`: `2,023` (3.01%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `2,027`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `2,027`*
...
### Recovered (53,293 functions) — wiring health

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Confirmed caller** (recovered binary caller wired by name) | `15,356` | 28.81% |
...
### Not-yet-recovered (2,027 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Recoverable now** (`OK_RECOVERED_CALLER` — recovered caller exists; recover next) | `328` | 16.18% |
| Vtable-anchored (recover with the owning class) | `209` | 10.31% |
| Framework dispatch (wx/EH/Lua/reflection) | `35` | 1.73% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `1,018` | 50.22% |
| No callsite evidence (likely external/dead — candidates for `external_dependency`) | `435` | 21.46% |
