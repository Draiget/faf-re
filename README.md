...
- Total FAF functions: `67,167`
  - *IDA index, exported*
- Progress coverage:  **`97.68%`**
  - *Consists of `recovered` + `skip` + `external_dependency` ÷ exported*
  - *Total amount of completed tokens: `65,606`*

Progress DB status breakdown:

- `recovered`: `52,960` (80.72%)
- `skip`: `7,099` (10.82%) — proven ICF aliases / thunks / EH or static-init glue with no distinct source body
- `external_dependency`: `5,547` (8.46%) — proven third-party/import-boundary code
...
- **`blocked`: `1,589` (2.37%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `1,623`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `1,623`*
...
### Recovered (52,960 functions) — binary caller context

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `16,594` | 31.33% |
...
| No recovered caller token yet (orphan risk) | `1,824` | 3.44% |
| No callsite evidence (no recorded code/data caller in the index) | `22,873` | 43.19% |
| Unclassified data xref (manual review) | `216` | 0.41% |
| RTTI-only | `4` | 0.01% |

### Not-yet-recovered (1,623 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `147` | 9.06% |
| Vtable-anchored (recover with the owning class) | `129` | 7.95% |
| Framework dispatch (wx/EH/Lua/reflection) | `8` | 0.49% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `739` | 45.53% |
| No indexed callsite evidence (needs investigation/evidence) | `599` | 36.91% |
