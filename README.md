...
- Progress coverage:  **`97.59%`**
  ...
  - *Total amount of completed tokens: `65,549`*

Progress DB status breakdown:

- `recovered`: `52,933` (80.75%)
...
- **`blocked`: `1,646` (2.45%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `1,680`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `1,680`*
...
### Recovered (52,933 functions) — binary caller context

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `16,554` | 31.27% |
...
### Not-yet-recovered (1,680 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `167` | 9.94% |
| Vtable-anchored (recover with the owning class) | `129` | 7.68% |
| Framework dispatch (wx/EH/Lua/reflection) | `8` | 0.48% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `776` | 46.19% |
| No indexed callsite evidence (needs investigation/evidence) | `599` | 35.65% |
