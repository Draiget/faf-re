...
- Progress coverage:  **`96.55%`**
  ...
  - *Total amount of completed tokens: `64,849`*

Progress DB status breakdown:

- `recovered`: `53,212` (82.06%)
...
- **`blocked`: `1,976` (2.94%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `2,388`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `2,388`*
...
### Recovered (53,212 functions) — binary caller context

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `16,232` | 30.50% |
...
### Not-yet-recovered (2,388 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `358` | 14.99% |
| Vtable-anchored (recover with the owning class) | `186` | 7.79% |
| Framework dispatch (wx/EH/Lua/reflection) | `45` | 1.88% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `812` | 34.00% |
| No indexed callsite evidence (needs investigation/evidence) | `983` | 41.16% |
