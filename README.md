...
- Total FAF functions: `67,170`
  - *IDA index, exported*
- Progress coverage:  **`99.42%`**
  - *Consists of `recovered` + `skip` + `external_dependency` ÷ exported*
  - *Total amount of completed tokens: `66,783`*

Progress DB status breakdown:

- `recovered`: `52,718` (78.94%)
- `skip`: `8,325` (12.47%) — proven ICF aliases / thunks / EH or static-init glue with no distinct source body
- `external_dependency`: `5,740` (8.60%) — proven third-party/import-boundary code
...
- **`blocked`: `497` (0.74%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `500`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `500`*
...
### Recovered (52,718 functions) — binary caller context

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `17,172` | 32.57% |
...
| No callsite evidence (no recorded code/data caller in the index) | `22,168` | 42.05% |
| Unclassified data xref (manual review) | `216` | 0.41% |
| RTTI-only | `4` | 0.01% |

### Not-yet-recovered (500 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `7` | 1.40% |
| Vtable-anchored (recover with the owning class) | `82` | 16.40% |
| Framework dispatch (wx/EH/Lua/reflection) | `0` | 0.00% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `133` | 26.60% |
| No indexed callsite evidence (needs investigation/evidence) | `278` | 55.60% |
