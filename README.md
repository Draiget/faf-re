...
- Total FAF functions: `67,167`
  - *IDA index, exported*
- Progress coverage:  **`97.07%`**
  - *Consists of `recovered` + `skip` + `external_dependency` ÷ exported*
  - *Total amount of completed tokens: `65,201`*

Progress DB status breakdown:

- `recovered`: `53,323` (81.78%)
...
- **`blocked`: `1,997` (2.97%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `2,001`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `2,001`*
...
### Recovered (53,323 functions) — wiring health

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Confirmed caller** (recovered binary caller wired by name) | `15,408` | 28.90% |
...
| Caller still blocked (orphan-helper risk — caller awaits recovery) | `2,633` | 4.94% |
| No callsite evidence (no recorded code/data caller in the index) | `23,750` | 44.54% |
| Unclassified data xref (manual review) | `215` | 0.40% |
| RTTI-only | `4` | 0.01% |

### Not-yet-recovered (2,001 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Recoverable now** (`OK_RECOVERED_CALLER` — recovered caller exists; recover next) | `307` | 15.34% |
| Vtable-anchored (recover with the owning class) | `209` | 10.44% |
| Framework dispatch (wx/EH/Lua/reflection) | `35` | 1.75% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `1,013` | 50.62% |
| No callsite evidence (likely external/dead — candidates for `external_dependency`) | `435` | 21.74% |
