...
- **`blocked`: `2,203` (3.28%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `2,206`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `2,206`*
...
### Recovered (53,112 functions) — wiring health

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Confirmed caller** (recovered binary caller wired by name) | `15,000` | 28.24% |
...
### Not-yet-recovered (2,206 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Recoverable now** (`OK_RECOVERED_CALLER` — recovered caller exists; recover next) | `463` | 20.99% |
| Vtable-anchored (recover with the owning class) | `211` | 9.56% |
| Framework dispatch (wx/EH/Lua/reflection) | `35` | 1.59% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `1,055` | 47.82% |
