...
- Progress coverage:  **`96.72%`**
  ...
  - *Total amount of completed tokens: `64,962`*

Progress DB status breakdown:

- `recovered`: `53,358` (82.14%)
...
- **`blocked`: `1,867` (2.78%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `2,275`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `2,275`*
...
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `16,366` | 30.67% |
| Vtable-anchored (virtual override of a recovered class) | `5,867` | 11.00% |
| Framework dispatch (wx event, EH handler, Lua binding, reflection table, …) | `5,520` | 10.35% |
| No recovered caller token yet (orphan risk) | `2,168` | 4.06% |
| No callsite evidence (no recorded code/data caller in the index) | `23,217` | 43.51% |
| Unclassified data xref (manual review) | `216` | 0.40% |
| RTTI-only | `4` | 0.01% |

### Not-yet-recovered (2,275 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `351` | 15.43% |
| Vtable-anchored (recover with the owning class) | `150` | 6.59% |
| Framework dispatch (wx/EH/Lua/reflection) | `42` | 1.85% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `777` | 34.15% |
| No indexed callsite evidence (needs investigation/evidence) | `952` | 41.85% |
