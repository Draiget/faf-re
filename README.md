...
- **`blocked`: `1,906` (2.84%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `1,909`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `1,909`*
...
### Recovered (53,416 functions) — wiring health

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Confirmed caller** (recovered binary caller wired by name) | `15,516` | 29.05% |
...
| Caller still blocked (orphan-helper risk — caller awaits recovery) | `2,590` | 4.85% |
| No callsite evidence (no recorded code/data caller in the index) | `23,752` | 44.47% |
| Unclassified data xref (manual review) | `215` | 0.40% |
| RTTI-only | `4` | 0.01% |

### Not-yet-recovered (1,909 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Recoverable now** (`OK_RECOVERED_CALLER` — recovered caller exists; recover next) | `291` | 15.24% |
| Vtable-anchored (recover with the owning class) | `183` | 9.59% |
| Framework dispatch (wx/EH/Lua/reflection) | `34` | 1.78% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `965` | 50.55% |
| No callsite evidence (likely external/dead — candidates for `external_dependency`) | `434` | 22.73% |
