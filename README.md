...
- Progress coverage:  **`96.38%`**
  ...
  - *Total amount of completed tokens: `64,733`*

Progress DB status breakdown:

- `recovered`: `53,118` (82.06%)
...
- **`blocked`: `2,082` (3.10%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `2,506`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `2,506`*
...
### Not-yet-recovered (2,506 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `415` | 16.56% |
| Vtable-anchored (recover with the owning class) | `203` | 8.10% |
| Framework dispatch (wx/EH/Lua/reflection) | `63` | 2.51% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `834` | 33.28% |
| No indexed callsite evidence (needs investigation/evidence) | `987` | 39.39% |
