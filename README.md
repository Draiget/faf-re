...
- Progress coverage:  **`96.74%`**
  ...
  - *Total amount of completed tokens: `64,974`*

Progress DB status breakdown:

- `recovered`: `53,093` (81.71%)
...
- **`blocked`: `2,220` (3.31%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `2,223`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `2,223`*
...
### Recovered (53,093 functions) — wiring health

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Confirmed caller** (recovered binary caller wired by name) | `14,976` | 28.21% |
...
### Not-yet-recovered (2,223 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Recoverable now** (`OK_RECOVERED_CALLER` — recovered caller exists; recover next) | `477` | 21.46% |
| Vtable-anchored (recover with the owning class) | `211` | 9.49% |
| Framework dispatch (wx/EH/Lua/reflection) | `35` | 1.57% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `1,057` | 47.55% |
| No callsite evidence (likely external/dead — candidates for `external_dependency`) | `441` | 19.84% |
