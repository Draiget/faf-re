...
- Progress coverage:  **`97.19%`**
  ...
  - *Total amount of completed tokens: `65,281`*

Progress DB status breakdown:

- `recovered`: `53,403` (81.80%)
...
### Recovered (53,403 functions) — wiring health

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Confirmed caller** (recovered binary caller wired by name) | `15,493` | 29.01% |
...
|---|---:|---:|
| **Recoverable now** (`OK_RECOVERED_CALLER` — recovered caller exists; recover next) | `295` | 15.35% |
| Vtable-anchored (recover with the owning class) | `184` | 9.57% |
| Framework dispatch (wx/EH/Lua/reflection) | `34` | 1.77% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `973` | 50.62% |
| No callsite evidence (likely external/dead — candidates for `external_dependency`) | `434` | 22.58% |
