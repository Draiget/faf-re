...
- Total FAF functions: `67,167`
  - *IDA index, exported*
- Progress coverage:  **`96.94%`**
  - *Consists of `recovered` + `skip` + `external_dependency` ÷ exported*
  - *Total amount of completed tokens: `65,115`*

Progress DB status breakdown:

- `recovered`: `53,577` (82.28%)
- `skip`: `6,112` (9.39%) — proven ICF aliases / thunks / EH or static-init glue with no distinct source body
...
### Recovered (53,577 functions) — binary caller context

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `16,623` | 31.03% |
| Vtable-anchored (virtual override of a recovered class) | `5,912` | 11.03% |
| Framework dispatch (wx event, EH handler, Lua binding, reflection table, …) | `5,568` | 10.39% |
| No recovered caller token yet (orphan risk) | `2,061` | 3.85% |
| No callsite evidence (no recorded code/data caller in the index) | `23,192` | 43.29% |
...
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `299` | 14.09% |
| Vtable-anchored (recover with the owning class) | `111` | 5.23% |
| Framework dispatch (wx/EH/Lua/reflection) | `1` | 0.05% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `723` | 34.07% |
| No indexed callsite evidence (needs investigation/evidence) | `986` | 46.47% |
