...
- **`blocked`: `2,156` (3.21%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `2,159`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `2,159`*
...
### Recovered (53,159 functions) — wiring health

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Confirmed caller** (recovered binary caller wired by name) | `15,092` | 28.39% |
...
| Caller still blocked (orphan-helper risk — caller awaits recovery) | `2,789` | 5.25% |
| No callsite evidence (no recorded code/data caller in the index) | `23,747` | 44.67% |
| Unclassified data xref (manual review) | `215` | 0.40% |
| RTTI-only | `4` | 0.01% |

### Not-yet-recovered (2,159 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Recoverable now** (`OK_RECOVERED_CALLER` — recovered caller exists; recover next) | `420` | 19.45% |
| Vtable-anchored (recover with the owning class) | `210` | 9.73% |
| Framework dispatch (wx/EH/Lua/reflection) | `35` | 1.62% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `1,053` | 48.77% |
| No callsite evidence (likely external/dead — candidates for `external_dependency`) | `439` | 20.33% |
