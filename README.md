...
- Progress coverage:  **`97.87%`**
  ...
  - *Total amount of completed tokens: `65,737`*

Progress DB status breakdown:

- `recovered`: `53,052` (80.70%)
...
- **`blocked`: `1,460` (2.17%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `1,493`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `1,493`*
...
### Recovered (53,052 functions) — binary caller context

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `16,708` | 31.49% |
...
### Not-yet-recovered (1,493 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `65` | 4.35% |
| Vtable-anchored (recover with the owning class) | `126` | 8.44% |
| Framework dispatch (wx/EH/Lua/reflection) | `8` | 0.54% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `698` | 46.75% |
| No indexed callsite evidence (needs investigation/evidence) | `595` | 39.85% |
