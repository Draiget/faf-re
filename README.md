...
- Progress coverage:  **`97.67%`**
  ...
  - *Total amount of completed tokens: `65,604`*

Progress DB status breakdown:

- `recovered`: `52,960` (80.73%)
...
- **`blocked`: `1,591` (2.37%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `1,625`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `1,625`*
...
### Not-yet-recovered (1,625 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `149` | 9.17% |
| Vtable-anchored (recover with the owning class) | `129` | 7.94% |
| Framework dispatch (wx/EH/Lua/reflection) | `8` | 0.49% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `739` | 45.48% |
| No indexed callsite evidence (needs investigation/evidence) | `599` | 36.86% |
