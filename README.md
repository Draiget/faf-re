...
- Progress coverage:  **`96.85%`**
  ...
  - *Total amount of completed tokens: `65,053`*

Progress DB status breakdown:

- `recovered`: `53,518` (82.27%)
...
- **`blocked`: `1,776` (2.64%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `2,184`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `2,184`*
...
### Recovered (53,518 functions) — binary caller context

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `16,548` | 30.92% |
...
### Not-yet-recovered (2,184 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `315` | 14.42% |
| Vtable-anchored (recover with the owning class) | `113` | 5.17% |
| Framework dispatch (wx/EH/Lua/reflection) | `1` | 0.05% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `768` | 35.16% |
| No indexed callsite evidence (needs investigation/evidence) | `985` | 45.10% |
