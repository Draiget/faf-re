...
- Progress coverage:  **`97.37%`**
  ...
  - *Total amount of completed tokens: `65,399`*

Progress DB status breakdown:

- `recovered`: `52,812` (80.75%)
- `skip`: `7,080` (10.83%) — proven ICF aliases / thunks / EH or static-init glue with no distinct source body
...
- **`blocked`: `1,793` (2.67%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `1,827`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `1,827`*
...
### Recovered (52,812 functions) — binary caller context

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `16,423` | 31.10% |
| Vtable-anchored (virtual override of a recovered class) | `5,884` | 11.14% |
| Framework dispatch (wx event, EH handler, Lua binding, reflection table, …) | `5,560` | 10.53% |
...
### Not-yet-recovered (1,827 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `234` | 12.81% |
| Vtable-anchored (recover with the owning class) | `131` | 7.17% |
| Framework dispatch (wx/EH/Lua/reflection) | `8` | 0.44% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `844` | 46.20% |
| No indexed callsite evidence (needs investigation/evidence) | `608` | 33.28% |
