...
- Progress coverage:  **`96.83%`**
  ...
  - *Total amount of completed tokens: `65,035`*

Progress DB status breakdown:

- `recovered`: `53,430` (82.16%)
...
- **`blocked`: `1,794` (2.67%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `2,202`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `2,202`*
...
### Recovered (53,430 functions) — binary caller context
...
| Framework dispatch (wx event, EH handler, Lua binding, reflection table, …) | `5,537` | 10.36% |
...
### Not-yet-recovered (2,202 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `350` | 15.89% |
| Vtable-anchored (recover with the owning class) | `131` | 5.95% |
| Framework dispatch (wx/EH/Lua/reflection) | `25` | 1.14% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `773` | 35.10% |
| No indexed callsite evidence (needs investigation/evidence) | `920` | 41.78% |
