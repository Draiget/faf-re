...
- `recovered`: `53,388` (81.73%)
- `skip`: `6,168` (9.44%) — CRT-internal / compiler-generated / orphan template instantiations / static-init glue
- `external_dependency`: `5,764` (8.82%) — third-party libs
...
- **`blocked`: `1,881` (2.80%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `1,884`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `1,884`*
...
### Recovered (53,388 functions) — wiring health

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Confirmed caller** (recovered binary caller wired by name) | `15,542` | 29.11% |
...
| Caller still blocked (orphan-helper risk — caller awaits recovery) | `2,575` | 4.82% |
| No callsite evidence (no recorded code/data caller in the index) | `23,710` | 44.41% |
| Unclassified data xref (manual review) | `215` | 0.40% |
| RTTI-only | `4` | 0.01% |

### Not-yet-recovered (1,884 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Recoverable now** (`OK_RECOVERED_CALLER` — recovered caller exists; recover next) | `283` | 15.02% |
| Vtable-anchored (recover with the owning class) | `178` | 9.45% |
| Framework dispatch (wx/EH/Lua/reflection) | `34` | 1.80% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `953` | 50.58% |
| No callsite evidence (likely external/dead — candidates for `external_dependency`) | `434` | 23.04% |
