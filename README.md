...
## Recovery Coverage (`18/06/2026`, `fa_full_2026_03_26`)
...
- Progress coverage:  **`96.65%`**
  ...
  - *Total amount of completed tokens: `64,916`*

Progress DB status breakdown:

- `recovered`: `53,029` (81.69%)
...
- **`blocked`: `2,283` (3.40%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `2,286`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `2,286`*
...
### Recovered (53,029 functions) — wiring health

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Confirmed caller** (recovered binary caller wired by name) | `14,941` | 28.18% |
...
### Not-yet-recovered (2,286 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Recoverable now** (`OK_RECOVERED_CALLER` — recovered caller exists; recover next) | `497` | 21.74% |
| Vtable-anchored (recover with the owning class) | `232` | 10.15% |
| Framework dispatch (wx/EH/Lua/reflection) | `35` | 1.53% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `1,071` | 46.85% |
| No callsite evidence (likely external/dead — candidates for `external_dependency`) | `449` | 19.64% |
