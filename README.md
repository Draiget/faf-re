...
- Total FAF functions: `67,167`
  - *IDA index, exported*
- Progress coverage:  **`97.60%`**
  - *Consists of `recovered` + `skip` + `external_dependency` ÷ exported*
  - *Total amount of completed tokens: `65,558`*

Progress DB status breakdown:

- `recovered`: `52,942` (80.76%)
...
- **`blocked`: `1,637` (2.44%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `1,671`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `1,671`*
...
### Not-yet-recovered (1,671 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `161` | 9.63% |
| Vtable-anchored (recover with the owning class) | `129` | 7.72% |
| Framework dispatch (wx/EH/Lua/reflection) | `8` | 0.48% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `773` | 46.26% |
| No indexed callsite evidence (needs investigation/evidence) | `599` | 35.85% |
