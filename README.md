...
- Progress coverage:  **`96.72%`**
  ...
  - *Total amount of completed tokens: `64,965`*

Progress DB status breakdown:

- `recovered`: `53,360` (82.14%)
...
- **`blocked`: `1,864` (2.78%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `2,272`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `2,272`*
...
### Recovered (53,360 functions) — binary caller context
...
| Framework dispatch (wx event, EH handler, Lua binding, reflection table, …) | `5,520` | 10.34% |
...
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `351` | 15.45% |
| Vtable-anchored (recover with the owning class) | `150` | 6.60% |
| Framework dispatch (wx/EH/Lua/reflection) | `42` | 1.85% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `774` | 34.07% |
| No indexed callsite evidence (needs investigation/evidence) | `952` | 41.90% |
