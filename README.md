...
- Progress coverage:  **`99.06%`**
  ...
  - *Total amount of completed tokens: `66,535`*

Progress DB status breakdown:

- `recovered`: `53,335` (80.16%)
...
- **`blocked`: `697` (1.04%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `700`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `700`*
...
### Recovered (53,335 functions) — binary caller context

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `17,082` | 32.03% |
...
### Not-yet-recovered (700 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `12` | 1.71% |
| Vtable-anchored (recover with the owning class) | `109` | 15.57% |
| Framework dispatch (wx/EH/Lua/reflection) | `0` | 0.00% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `265` | 37.86% |
| No indexed callsite evidence (needs investigation/evidence) | `313` | 44.71% |
