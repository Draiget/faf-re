...
- `recovered`: `53,322` (80.14%)
- `skip`: `7,447` (11.19%) — proven ICF aliases / thunks / EH or static-init glue with no distinct source body
- `external_dependency`: `5,771` (8.67%) — proven third-party/import-boundary code
...
### Recovered (53,322 functions) — binary caller context

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `17,093` | 32.06% |
| Vtable-anchored (virtual override of a recovered class) | `5,882` | 11.03% |
| Framework dispatch (wx event, EH handler, Lua binding, reflection table, …) | `5,571` | 10.45% |
| No recovered caller token yet (orphan risk) | `1,697` | 3.18% |
| No callsite evidence (no recorded code/data caller in the index) | `22,859` | 42.87% |
