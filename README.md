...
- **`blocked`: `2,210` (3.29%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `2,213`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `2,213`*
...
### Recovered (53,103 functions) — wiring health

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Confirmed caller** (recovered binary caller wired by name) | `14,989` | 28.23% |
...
### Not-yet-recovered (2,213 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Recoverable now** (`OK_RECOVERED_CALLER` — recovered caller exists; recover next) | `470` | 21.24% |
| Vtable-anchored (recover with the owning class) | `211` | 9.53% |
| Framework dispatch (wx/EH/Lua/reflection) | `35` | 1.58% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `1,055` | 47.67% |
| No callsite evidence (likely external/dead — candidates for `external_dependency`) | `440` | 19.88% |
