...
- Progress coverage:  **`96.92%`**
  ...
  - *Total amount of completed tokens: `65,097`*

Progress DB status breakdown:

- `recovered`: `53,511` (82.20%)
...
- **`blocked`: `1,731` (2.58%)**
  ...
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `2,140`*
...
### Recovered (53,511 functions) — binary caller context

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `16,418` | 30.68% |
...
| No recovered caller token yet (orphan risk) | `2,143` | 4.00% |
| No callsite evidence (no recorded code/data caller in the index) | `23,265` | 43.48% |
| Unclassified data xref (manual review) | `217` | 0.41% |
| RTTI-only | `4` | 0.01% |

### Not-yet-recovered (2,140 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `342` | 15.98% |
| Vtable-anchored (recover with the owning class) | `117` | 5.47% |
| Framework dispatch (wx/EH/Lua/reflection) | `1` | 0.05% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `766` | 35.79% |
| No indexed callsite evidence (needs investigation/evidence) | `912` | 42.62% |
