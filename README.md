...
- Progress coverage:  **`96.76%`**
  ...
  - *Total amount of completed tokens: `64,994`*

Progress DB status breakdown:

- `recovered`: `52,768` (81.19%)
...
- **`blocked`: `1,827` (2.72%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `2,232`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `2,232`*
...
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `16,389` | 31.06% |
...
| No callsite evidence (no recorded code/data caller in the index) | `22,856` | 43.31% |
| Unclassified data xref (manual review) | `216` | 0.41% |
| RTTI-only | `4` | 0.01% |

### Not-yet-recovered (2,232 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `239` | 10.71% |
| Vtable-anchored (recover with the owning class) | `132` | 5.91% |
| Framework dispatch (wx/EH/Lua/reflection) | `10` | 0.45% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `868` | 38.89% |
| No indexed callsite evidence (needs investigation/evidence) | `981` | 43.95% |
