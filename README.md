...
- Progress coverage:  **`97.83%`**
  ...
  - *Total amount of completed tokens: `65,707`*

Progress DB status breakdown:

- `recovered`: `53,042` (80.73%)
...
- **`blocked`: `1,489` (2.22%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `1,523`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `1,523`*
...
### Recovered (53,042 functions) — binary caller context

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `16,696` | 31.48% |
...
### Not-yet-recovered (1,523 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `72` | 4.73% |
| Vtable-anchored (recover with the owning class) | `129` | 8.47% |
| Framework dispatch (wx/EH/Lua/reflection) | `8` | 0.53% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `716` | 47.01% |
| No indexed callsite evidence (needs investigation/evidence) | `597` | 39.20% |
