...
- Progress coverage:  **`98.92%`**
  ...
  - *Total amount of completed tokens: `66,444`*

Progress DB status breakdown:

- `recovered`: `53,282` (80.19%)
...
- **`blocked`: `792` (1.18%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `795`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `795`*
...
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `16,996` | 31.90% |
...
| No recovered caller token yet (orphan risk) | `1,709` | 3.21% |
| No callsite evidence (no recorded code/data caller in the index) | `22,907` | 42.99% |
| Unclassified data xref (manual review) | `216` | 0.41% |
| RTTI-only | `4` | 0.01% |

### Not-yet-recovered (795 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `25` | 3.14% |
| Vtable-anchored (recover with the owning class) | `109` | 13.71% |
| Framework dispatch (wx/EH/Lua/reflection) | `0` | 0.00% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `340` | 42.77% |
| No indexed callsite evidence (needs investigation/evidence) | `320` | 40.25% |
