...
- Total FAF functions: `67,167`
  - *IDA index, exported*
- Progress coverage:  **`97.91%`**
  - *Consists of `recovered` + `skip` + `external_dependency` ÷ exported*
  - *Total amount of completed tokens: `65,762`*
...
- `external_dependency`: `5,585` (8.49%) — proven third-party/import-boundary code
...
- **`blocked`: `1,435` (2.14%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `1,468`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `1,468`*
...
### Not-yet-recovered (1,468 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `63` | 4.29% |
| Vtable-anchored (recover with the owning class) | `126` | 8.58% |
| Framework dispatch (wx/EH/Lua/reflection) | `8` | 0.54% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `676` | 46.05% |
| No indexed callsite evidence (needs investigation/evidence) | `594` | 40.46% |
