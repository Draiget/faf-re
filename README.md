...
- Progress coverage:  **`97.63%`**
  ...
  - *Total amount of completed tokens: `65,575`*

Progress DB status breakdown:

- `recovered`: `52,944` (80.74%)
- `skip`: `7,092` (10.82%) — proven ICF aliases / thunks / EH or static-init glue with no distinct source body
- `external_dependency`: `5,539` (8.45%) — proven third-party/import-boundary code
...
- **`blocked`: `1,620` (2.41%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `1,654`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `1,654`*
...
### Not-yet-recovered (1,654 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `156` | 9.43% |
| Vtable-anchored (recover with the owning class) | `129` | 7.80% |
| Framework dispatch (wx/EH/Lua/reflection) | `8` | 0.48% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `761` | 46.01% |
| No indexed callsite evidence (needs investigation/evidence) | `599` | 36.22% |
