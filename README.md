...
- Progress coverage:  **`96.39%`**
  ...
  - *Total amount of completed tokens: `64,745`*

Progress DB status breakdown:

- `recovered`: `53,130` (82.06%)
...
- **`blocked`: `2,069` (3.08%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `2,493`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `2,493`*
...
### Not-yet-recovered (2,493 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `413` | 16.57% |
| Vtable-anchored (recover with the owning class) | `203` | 8.14% |
| Framework dispatch (wx/EH/Lua/reflection) | `55` | 2.21% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `831` | 33.33% |
| No indexed callsite evidence (needs investigation/evidence) | `987` | 39.59% |
