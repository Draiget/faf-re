...
- Progress coverage:  **`98.80%`**
  ...
  - *Total amount of completed tokens: `66,358`*

Progress DB status breakdown:

- `recovered`: `53,149` (80.09%)
...
- **`blocked`: `866` (1.29%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `873`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `873`*
...
### Recovered (53,149 functions) — binary caller context
...
| No recovered caller token yet (orphan risk) | `1,743` | 3.28% |
| No callsite evidence (no recorded code/data caller in the index) | `22,876` | 43.04% |
| Unclassified data xref (manual review) | `216` | 0.41% |
| RTTI-only | `4` | 0.01% |

### Not-yet-recovered (873 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `31` | 3.55% |
| Vtable-anchored (recover with the owning class) | `109` | 12.49% |
| Framework dispatch (wx/EH/Lua/reflection) | `0` | 0.00% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `372` | 42.61% |
| No indexed callsite evidence (needs investigation/evidence) | `360` | 41.24% |
