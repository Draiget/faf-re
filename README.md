...
- Progress coverage:  **`97.09%`**
  ...
  - *Total amount of completed tokens: `65,210`*

Progress DB status breakdown:

- `recovered`: `53,332` (81.79%)
...
- **`blocked`: `1,989` (2.96%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `1,993`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `1,993`*
...
### Recovered (53,332 functions) — wiring health
...
### Not-yet-recovered (1,993 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Recoverable now** (`OK_RECOVERED_CALLER` — recovered caller exists; recover next) | `303` | 15.20% |
| Vtable-anchored (recover with the owning class) | `208` | 10.44% |
| Framework dispatch (wx/EH/Lua/reflection) | `35` | 1.76% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `1,010` | 50.68% |
| No callsite evidence (likely external/dead — candidates for `external_dependency`) | `435` | 21.83% |
