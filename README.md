...
- Progress coverage:  **`97.22%`**
  ...
  - *Total amount of completed tokens: `65,302`*

Progress DB status breakdown:

- `recovered`: `53,697` (82.23%)
...
- **`blocked`: `1,856` (2.76%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `1,922`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `1,922`*
...
### Recovered (53,697 functions) — binary caller context

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `16,022` | 29.84% |
| Vtable-anchored (virtual override of a recovered class) | `5,834` | 10.86% |
...
### Not-yet-recovered (1,922 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `309` | 16.08% |
| Vtable-anchored (recover with the owning class) | `187` | 9.73% |
| Framework dispatch (wx/EH/Lua/reflection) | `54` | 2.81% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `908` | 47.24% |
| No indexed callsite evidence (needs investigation/evidence) | `460` | 23.93% |
