...
- Progress coverage:  **`98.86%`**
  ...
  - *Total amount of completed tokens: `66,398`*

Progress DB status breakdown:

- `recovered`: `53,218` (80.15%)
- `skip`: `7,395` (11.14%) — proven ICF aliases / thunks / EH or static-init glue with no distinct source body
- `external_dependency`: `5,785` (8.71%) — proven third-party/import-boundary code
...
- **`blocked`: `828` (1.23%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `835`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `835`*
...
### Recovered (53,218 functions) — binary caller context

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `16,933` | 31.82% |
...
| No callsite evidence (no recorded code/data caller in the index) | `22,888` | 43.01% |
| Unclassified data xref (manual review) | `216` | 0.41% |
| RTTI-only | `4` | 0.01% |

### Not-yet-recovered (835 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `27` | 3.23% |
| Vtable-anchored (recover with the owning class) | `109` | 13.05% |
| Framework dispatch (wx/EH/Lua/reflection) | `0` | 0.00% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `361` | 43.23% |
| No indexed callsite evidence (needs investigation/evidence) | `337` | 40.36% |
