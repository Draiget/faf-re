...
- **`blocked`: `2,209` (3.29%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `2,212`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `2,212`*
...
### Recovered (53,105 functions) — wiring health

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Confirmed caller** (recovered binary caller wired by name) | `14,991` | 28.23% |
...
| Caller still blocked (orphan-helper risk — caller awaits recovery) | `2,836` | 5.34% |
| No callsite evidence (no recorded code/data caller in the index) | `23,746` | 44.72% |
| Unclassified data xref (manual review) | `215` | 0.40% |
| RTTI-only | `4` | 0.01% |

### Not-yet-recovered (2,212 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Recoverable now** (`OK_RECOVERED_CALLER` — recovered caller exists; recover next) | `469` | 21.20% |
| Vtable-anchored (recover with the owning class) | `211` | 9.54% |
| Framework dispatch (wx/EH/Lua/reflection) | `35` | 1.58% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `1,055` | 47.69% |
| No callsite evidence (likely external/dead — candidates for `external_dependency`) | `440` | 19.89% |
