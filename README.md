...
- Progress coverage:  **`97.05%`**
  ...
  - *Total amount of completed tokens: `65,183`*

Progress DB status breakdown:

- `recovered`: `53,303` (81.77%)
...
- **`blocked`: `2,015` (3.00%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `2,019`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `2,019`*
...
### Recovered (53,303 functions) — wiring health

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Confirmed caller** (recovered binary caller wired by name) | `15,368` | 28.83% |
...
### Not-yet-recovered (2,019 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Recoverable now** (`OK_RECOVERED_CALLER` — recovered caller exists; recover next) | `323` | 16.00% |
| Vtable-anchored (recover with the owning class) | `209` | 10.35% |
| Framework dispatch (wx/EH/Lua/reflection) | `35` | 1.73% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `1,015` | 50.27% |
| No callsite evidence (likely external/dead — candidates for `external_dependency`) | `435` | 21.55% |
