...
- Progress coverage:  **`97.83%`**
  ...
  - *Total amount of completed tokens: `65,708`*

Progress DB status breakdown:

- `recovered`: `53,043` (80.73%)
- `skip`: `7,117` (10.83%) — proven ICF aliases / thunks / EH or static-init glue with no distinct source body
- `external_dependency`: `5,548` (8.44%) — proven third-party/import-boundary code
...
- **`blocked`: `1,488` (2.22%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `1,522`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `1,522`*
...
### Recovered (53,043 functions) — binary caller context

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `16,697` | 31.48% |
...
| No callsite evidence (no recorded code/data caller in the index) | `22,875` | 43.13% |
| Unclassified data xref (manual review) | `216` | 0.41% |
| RTTI-only | `4` | 0.01% |

### Not-yet-recovered (1,522 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `71` | 4.66% |
| Vtable-anchored (recover with the owning class) | `129` | 8.48% |
| Framework dispatch (wx/EH/Lua/reflection) | `8` | 0.53% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `716` | 47.04% |
| No indexed callsite evidence (needs investigation/evidence) | `597` | 39.22% |
