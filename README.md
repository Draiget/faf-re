...
- **`blocked`: `2,148` (3.20%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `2,151`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `2,151`*
...
### Recovered (53,167 functions) — wiring health

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Confirmed caller** (recovered binary caller wired by name) | `15,125` | 28.45% |
...
| Caller still blocked (orphan-helper risk — caller awaits recovery) | `2,764` | 5.20% |
| No callsite evidence (no recorded code/data caller in the index) | `23,747` | 44.66% |
| Unclassified data xref (manual review) | `215` | 0.40% |
| RTTI-only | `4` | 0.01% |

### Not-yet-recovered (2,151 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Recoverable now** (`OK_RECOVERED_CALLER` — recovered caller exists; recover next) | `413` | 19.20% |
| Vtable-anchored (recover with the owning class) | `210` | 9.76% |
| Framework dispatch (wx/EH/Lua/reflection) | `35` | 1.63% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `1,052` | 48.91% |
| No callsite evidence (likely external/dead — candidates for `external_dependency`) | `439` | 20.41% |
