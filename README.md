...
### Recovered (53,062 functions) — wiring health

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Confirmed caller** (recovered binary caller wired by name) | `14,953` | 28.18% |
| Vtable-anchored (virtual override of a recovered class) | `5,811` | 10.95% |
| Framework dispatch (wx event, EH handler, Lua binding, reflection table, …) | `5,495` | 10.36% |
| Caller still blocked (orphan-helper risk — caller awaits recovery) | `2,844` | 5.36% |
| No callsite evidence (no recorded code/data caller in the index) | `23,740` | 44.74% |
...
|---|---:|---:|
| **Recoverable now** (`OK_RECOVERED_CALLER` — recovered caller exists; recover next) | `487` | 21.61% |
| Vtable-anchored (recover with the owning class) | `219` | 9.72% |
| Framework dispatch (wx/EH/Lua/reflection) | `35` | 1.55% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `1,067` | 47.34% |
| No callsite evidence (likely external/dead — candidates for `external_dependency`) | `444` | 19.70% |
