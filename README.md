...
## Recovery Coverage (`23/06/2026`, `fa_full_2026_03_26`)

Progress snapshot:

- Total FAF functions: `67,167`
  - *IDA index, exported*
- Progress coverage:  **`96.66%`**
  - *Consists of `recovered` + `skip` + `external_dependency` ÷ exported*
  - *Total amount of completed tokens: `64,921`*

Progress DB status breakdown:

- `recovered`: `53,034` (81.69%)
...
- **`blocked`: `2,277` (3.39%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `2,280`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `2,280`*
...
### Not-yet-recovered (2,280 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Recoverable now** (`OK_RECOVERED_CALLER` — recovered caller exists; recover next) | `491` | 21.54% |
| Vtable-anchored (recover with the owning class) | `232` | 10.18% |
| Framework dispatch (wx/EH/Lua/reflection) | `35` | 1.54% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `1,071` | 46.97% |
| No callsite evidence (likely external/dead — candidates for `external_dependency`) | `449` | 19.69% |
