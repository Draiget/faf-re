...
- Total FAF functions: `67,167`
  - *IDA index, exported*
- Progress coverage:  **`97.16%`**
  - *Consists of `recovered` + `skip` + `external_dependency` ÷ exported*
  - *Total amount of completed tokens: `65,260`*

Progress DB status breakdown:

- `recovered`: `53,382` (81.80%)
...
- **`blocked`: `1,939` (2.89%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `1,943`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `1,943`*
...
### Recovered (53,382 functions) — wiring health

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Confirmed caller** (recovered binary caller wired by name) | `15,467` | 28.97% |
| Vtable-anchored (virtual override of a recovered class) | `5,837` | 10.93% |
| Framework dispatch (wx event, EH handler, Lua binding, reflection table, …) | `5,499` | 10.30% |
| Caller still blocked (orphan-helper risk — caller awaits recovery) | `2,608` | 4.89% |
| No callsite evidence (no recorded code/data caller in the index) | `23,752` | 44.49% |
| Unclassified data xref (manual review) | `215` | 0.40% |
| RTTI-only | `4` | 0.01% |

### Not-yet-recovered (1,943 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Recoverable now** (`OK_RECOVERED_CALLER` — recovered caller exists; recover next) | `302` | 15.54% |
| Vtable-anchored (recover with the owning class) | `186` | 9.57% |
| Framework dispatch (wx/EH/Lua/reflection) | `34` | 1.75% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `985` | 50.69% |
| No callsite evidence (likely external/dead — candidates for `external_dependency`) | `434` | 22.34% |
