...
- Total FAF functions: `67,167`
  - *IDA index, exported*
- Progress coverage:  **`98.95%`**
  - *Consists of `recovered` + `skip` + `external_dependency` ÷ exported*
  - *Total amount of completed tokens: `66,459`*

Progress DB status breakdown:

- `recovered`: `53,288` (80.18%)
...
- **`blocked`: `777` (1.16%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `780`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `780`*
...
### Not-yet-recovered (780 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `24` | 3.08% |
| Vtable-anchored (recover with the owning class) | `109` | 13.97% |
| Framework dispatch (wx/EH/Lua/reflection) | `0` | 0.00% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `328` | 42.05% |
| No indexed callsite evidence (needs investigation/evidence) | `318` | 40.77% |
