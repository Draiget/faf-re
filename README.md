...
### Recovered (53,529 functions) — binary caller context

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `16,454` | 30.74% |
...
| No recovered caller token yet (orphan risk) | `2,125` | 3.97% |
...
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `339` | 15.76% |
| Vtable-anchored (recover with the owning class) | `117` | 5.44% |
| Framework dispatch (wx/EH/Lua/reflection) | `1` | 0.05% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `780` | 36.26% |
| No indexed callsite evidence (needs investigation/evidence) | `912` | 42.40% |
