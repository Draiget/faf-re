...
- Total FAF functions: `67,170`
  - *IDA index, exported*
- Progress coverage:  **`99.26%`**
  - *Consists of `recovered` + `skip` + `external_dependency` ÷ exported*
  - *Total amount of completed tokens: `66,675`*

Progress DB status breakdown:

- `recovered`: `52,744` (79.11%)
...
- **`blocked`: `602` (0.90%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `605`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `605`*
...
### Not-yet-recovered (605 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `9` | 1.49% |
| Vtable-anchored (recover with the owning class) | `106` | 17.52% |
| Framework dispatch (wx/EH/Lua/reflection) | `0` | 0.00% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `184` | 30.41% |
| No indexed callsite evidence (needs investigation/evidence) | `305` | 50.41% |
