...
- Total FAF functions: `67,167`
  - *IDA index, exported*
- Progress coverage:  **`98.86%`**
  - *Consists of `recovered` + `skip` + `external_dependency` ÷ exported*
  - *Total amount of completed tokens: `66,404`*

Progress DB status breakdown:

- `recovered`: `53,241` (80.18%)
- `skip`: `7,393` (11.13%) — proven ICF aliases / thunks / EH or static-init glue with no distinct source body
- `external_dependency`: `5,770` (8.69%) — proven third-party/import-boundary code
...
- **`blocked`: `823` (1.23%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `830`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `830`*
...
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `16,969` | 31.87% |
| Vtable-anchored (virtual override of a recovered class) | `5,885` | 11.05% |
| Framework dispatch (wx event, EH handler, Lua binding, reflection table, …) | `5,565` | 10.45% |
| No recovered caller token yet (orphan risk) | `1,713` | 3.22% |
| No callsite evidence (no recorded code/data caller in the index) | `22,889` | 42.99% |
| Unclassified data xref (manual review) | `216` | 0.41% |
| RTTI-only | `4` | 0.01% |

### Not-yet-recovered (830 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `27` | 3.25% |
| Vtable-anchored (recover with the owning class) | `109` | 13.13% |
| Framework dispatch (wx/EH/Lua/reflection) | `0` | 0.00% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `356` | 42.89% |
| No indexed callsite evidence (needs investigation/evidence) | `337` | 40.60% |
