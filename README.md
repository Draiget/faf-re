...
- Progress coverage:  **`96.89%`**
  ...
  - *Total amount of completed tokens: `65,078`*

Progress DB status breakdown:

- `recovered`: `53,199` (81.75%)
...
- **`blocked`: `2,118` (3.15%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `2,121`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `2,121`*
...
### Recovered (53,199 functions) — wiring health

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Confirmed caller** (recovered binary caller wired by name) | `15,200` | 28.57% |
...
### Not-yet-recovered (2,121 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Recoverable now** (`OK_RECOVERED_CALLER` — recovered caller exists; recover next) | `389` | 18.34% |
| Vtable-anchored (recover with the owning class) | `210` | 9.90% |
| Framework dispatch (wx/EH/Lua/reflection) | `35` | 1.65% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `1,046` | 49.32% |
| No callsite evidence (likely external/dead — candidates for `external_dependency`) | `439` | 20.70% |
