...
- Progress coverage:  **`98.87%`**
  ...
  - *Total amount of completed tokens: `66,410`*

Progress DB status breakdown:

- `recovered`: `53,247` (80.18%)
- `skip`: `7,391` (11.13%) — proven ICF aliases / thunks / EH or static-init glue with no distinct source body
- `external_dependency`: `5,772` (8.69%) — proven third-party/import-boundary code
...
- **`blocked`: `821` (1.22%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `824`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `824`*
...
### Recovered (53,247 functions) — binary caller context

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `16,976` | 31.88% |
...
| No recovered caller token yet (orphan risk) | `1,712` | 3.22% |
| No callsite evidence (no recorded code/data caller in the index) | `22,889` | 42.99% |
| Unclassified data xref (manual review) | `216` | 0.41% |
| RTTI-only | `4` | 0.01% |

### Not-yet-recovered (824 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `29` | 3.52% |
| Vtable-anchored (recover with the owning class) | `109` | 13.23% |
| Framework dispatch (wx/EH/Lua/reflection) | `0` | 0.00% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `349` | 42.35% |
| No indexed callsite evidence (needs investigation/evidence) | `336` | 40.78% |
