...
- Progress coverage:  **`96.73%`**
  ...
  - *Total amount of completed tokens: `64,973`*

Progress DB status breakdown:

- `recovered`: `52,744` (81.18%)
- `skip`: `6,722` (10.35%) — proven ICF aliases / thunks / EH or static-init glue with no distinct source body
- `external_dependency`: `5,507` (8.48%) — proven third-party/import-boundary code
...
- **`blocked`: `1,844` (2.75%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `2,249`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `2,249`*
...
### Recovered (52,744 functions) — binary caller context

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `16,369` | 31.03% |
...
| No callsite evidence (no recorded code/data caller in the index) | `22,852` | 43.33% |
| Unclassified data xref (manual review) | `216` | 0.41% |
| RTTI-only | `4` | 0.01% |

### Not-yet-recovered (2,249 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `239` | 10.63% |
| Vtable-anchored (recover with the owning class) | `136` | 6.05% |
| Framework dispatch (wx/EH/Lua/reflection) | `10` | 0.44% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `877` | 39.00% |
| No indexed callsite evidence (needs investigation/evidence) | `985` | 43.80% |
