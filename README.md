...
- Progress coverage:  **`95.86%`**
  ...
  - *Total amount of completed tokens: `64,387`*

Progress DB status breakdown:

- `recovered`: `52,736` (81.90%)
...
- **`blocked`: `2,442` (3.64%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `2,850`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `2,850`*
...
### Recovered (52,736 functions) — binary caller context

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `16,262` | 30.84% |
...
| No recovered caller token yet (orphan risk) | `1,874` | 3.55% |
| No callsite evidence (no recorded code/data caller in the index) | `22,943` | 43.51% |
| Unclassified data xref (manual review) | `216` | 0.41% |
| RTTI-only | `4` | 0.01% |

### Not-yet-recovered (2,850 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `417` | 14.63% |
| Vtable-anchored (recover with the owning class) | `144` | 5.05% |
| Framework dispatch (wx/EH/Lua/reflection) | `13` | 0.46% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `1,035` | 36.32% |
| No indexed callsite evidence (needs investigation/evidence) | `1,238` | 43.44% |
