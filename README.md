...
- Progress coverage:  **`99.73%`**
  ...
  - *Total amount of completed tokens: `66,987`*

Progress DB status breakdown:

- `recovered`: `47,469` (70.86%)
...
- **`blocked`: `299` (0.45%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `302`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `302`*
...
### Recovered (47,469 functions) — binary caller context

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `17,324` | 36.50% |
| Vtable-anchored (virtual override of a recovered class) | `5,899` | 12.43% |
...
### Not-yet-recovered (302 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `3` | 0.99% |
| Vtable-anchored (recover with the owning class) | `76` | 25.17% |
| Framework dispatch (wx/EH/Lua/reflection) | `0` | 0.00% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `114` | 37.75% |
| No indexed callsite evidence (needs investigation/evidence) | `109` | 36.09% |
