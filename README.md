...
- Progress coverage:  **`96.36%`**
  ...
  - *Total amount of completed tokens: `64,722`*

Progress DB status breakdown:

- `recovered`: `53,107` (82.05%)
...
- **`blocked`: `2,091` (3.11%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `2,515`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `2,515`*
...
### Recovered (53,107 functions) — binary caller context
...
| Framework dispatch (wx event, EH handler, Lua binding, reflection table, …) | `5,490` | 10.34% |
...
### Not-yet-recovered (2,515 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `414` | 16.46% |
| Vtable-anchored (recover with the owning class) | `203` | 8.07% |
| Framework dispatch (wx/EH/Lua/reflection) | `69` | 2.74% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `840` | 33.40% |
| No indexed callsite evidence (needs investigation/evidence) | `985` | 39.17% |
