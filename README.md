...
- Progress coverage:  **`95.77%`**
  ...
  - *Total amount of completed tokens: `64,327`*

Progress DB status breakdown:

- `recovered`: `52,676` (81.89%)
...
- **`blocked`: `2,502` (3.73%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `2,910`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `2,910`*
...
### Recovered (52,676 functions) — binary caller context

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `16,202` | 30.76% |
...
### Not-yet-recovered (2,910 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `471` | 16.19% |
| Vtable-anchored (recover with the owning class) | `144` | 4.95% |
| Framework dispatch (wx/EH/Lua/reflection) | `13` | 0.45% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `1,039` | 35.70% |
| No indexed callsite evidence (needs investigation/evidence) | `1,240` | 42.61% |
