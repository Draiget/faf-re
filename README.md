...
- Total FAF functions: `67,167`
  - *IDA index, exported*
- Progress coverage:  **`96.99%`**
  - *Consists of `recovered` + `skip` + `external_dependency` ÷ exported*
  - *Total amount of completed tokens: `65,146`*

Progress DB status breakdown:

- `recovered`: `53,591` (82.26%)
- `skip`: `6,118` (9.39%) — proven ICF aliases / thunks / EH or static-init glue with no distinct source body
- `external_dependency`: `5,437` (8.35%) — proven third-party/import-boundary code
...
- **`blocked`: `1,683` (2.51%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `2,091`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `2,091`*
...
### Recovered (53,591 functions) — binary caller context

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `16,530` | 30.84% |
| Vtable-anchored (virtual override of a recovered class) | `5,896` | 11.00% |
| Framework dispatch (wx event, EH handler, Lua binding, reflection table, …) | `5,568` | 10.39% |
| No recovered caller token yet (orphan risk) | `2,111` | 3.94% |
| No callsite evidence (no recorded code/data caller in the index) | `23,265` | 43.41% |
| Unclassified data xref (manual review) | `217` | 0.40% |
| RTTI-only | `4` | 0.01% |

### Not-yet-recovered (2,091 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `286` | 13.68% |
| Vtable-anchored (recover with the owning class) | `117` | 5.60% |
| Framework dispatch (wx/EH/Lua/reflection) | `1` | 0.05% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `773` | 36.97% |
| No indexed callsite evidence (needs investigation/evidence) | `912` | 43.62% |
