...
- Progress coverage:  **`97.45%`**
  ...
  - *Total amount of completed tokens: `65,457`*

Progress DB status breakdown:

- `recovered`: `52,870` (80.77%)
- `skip`: `7,074` (10.81%) — proven ICF aliases / thunks / EH or static-init glue with no distinct source body
- `external_dependency`: `5,513` (8.42%) — proven third-party/import-boundary code
...
- **`blocked`: `1,736` (2.58%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `1,770`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `1,770`*
...
### Recovered (52,870 functions) — binary caller context

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `16,479` | 31.17% |
...
| No callsite evidence (no recorded code/data caller in the index) | `22,868` | 43.25% |
| Unclassified data xref (manual review) | `216` | 0.41% |
| RTTI-only | `4` | 0.01% |

### Not-yet-recovered (1,770 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `202` | 11.41% |
| Vtable-anchored (recover with the owning class) | `130` | 7.34% |
| Framework dispatch (wx/EH/Lua/reflection) | `8` | 0.45% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `824` | 46.55% |
| No indexed callsite evidence (needs investigation/evidence) | `605` | 34.18% |
