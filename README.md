...
- Progress coverage:  **`96.55%`**
  ...
  - *Total amount of completed tokens: `64,851`*

Progress DB status breakdown:

- `recovered`: `53,214` (82.06%)
...
- **`blocked`: `1,974` (2.94%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `2,386`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `2,386`*
...
### Recovered (53,214 functions) — binary caller context

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `16,234` | 30.51% |
...
### Not-yet-recovered (2,386 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `356` | 14.92% |
| Vtable-anchored (recover with the owning class) | `186` | 7.80% |
| Framework dispatch (wx/EH/Lua/reflection) | `45` | 1.89% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `812` | 34.03% |
| No indexed callsite evidence (needs investigation/evidence) | `983` | 41.20% |
