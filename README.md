...
- Progress coverage:  **`97.64%`**
  ...
  - *Total amount of completed tokens: `65,582`*

Progress DB status breakdown:

- `recovered`: `54,046` (82.41%)
...
- **`blocked`: `1,247` (1.86%)**
  ...
  - *combined with `needs_evidence`, the "not-yet-recovered non-engine-external" bucket is `1,655`*
  - *the `stats` tool's `blocked_count` aggregates the same two buckets and reports `1,655`*
...
### Recovered (54,046 functions) — binary caller context

| Bucket | Count | % of recovered |
|---|---:|---:|
| **Recovered caller token exists** (source edge still requires verification) | `16,647` | 30.80% |
...
| No recovered caller token yet (orphan risk) | `2,053` | 3.80% |
| No callsite evidence (no recorded code/data caller in the index) | `23,643` | 43.75% |
| Unclassified data xref (manual review) | `217` | 0.40% |
| RTTI-only | `4` | 0.01% |

### Not-yet-recovered (1,655 blocked + needs_evidence) — backlog readiness

| Bucket | Count | % of backlog |
|---|---:|---:|
| **Candidate** (`OK_RECOVERED_CALLER` — caller token recovered; inspect its body) | `292` | 17.64% |
| Vtable-anchored (recover with the owning class) | `110` | 6.65% |
| Framework dispatch (wx/EH/Lua/reflection) | `1` | 0.06% |
| **Caller functions unrecovered** (`NEEDS_RECOVERED_CALLER` — recover the parent first) | `715` | 43.20% |
| No indexed callsite evidence (needs investigation/evidence) | `535` | 32.33% |
